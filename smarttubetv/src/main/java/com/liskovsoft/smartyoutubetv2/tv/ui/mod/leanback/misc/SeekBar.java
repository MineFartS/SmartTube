...
/**
 * Replacement of SeekBar, has two bar heights and two thumb size when focused/not_focused. The
 * widget does not deal with KeyEvent, it's client's responsibility to set a key listener.
 *
 * @hide
 */
...
public final class SeekBar extends View {
  ...
  private void calculate() {
    ...
    mBackgroundRect.set(
        mBarHeight / 2, verticalPadding, width - mBarHeight / 2, height - verticalPadding);

    final int radius = isFocused() ? mActiveRadius : mBarHeight / 2;
    final int progressWidth = width - radius * 2;
    final float progressPixels = mProgress / (float) mMax * progressWidth;
    mProgressRect.set(
        mBarHeight / 2, verticalPadding, mBarHeight / 2 + progressPixels, height - verticalPadding);

    final float secondProgressPixels = mSecondProgress / (float) mMax * progressWidth;
    mSecondProgressRect.set(
        mProgressRect.right,
        verticalPadding,
        mBarHeight / 2 + secondProgressPixels,
        height - verticalPadding);

    mKnobx = radius + (int) progressPixels;

    for (SeekBarRectangle seekBarRectangle : mSeekBarRectangles) {
      seekBarRectangle.rect.set(
          seekBarRectangle.rect.left,
          verticalPadding,
          seekBarRectangle.rect.right,
          height - verticalPadding);
    }
