 ...
 * https://stackoverflow.com/questions/38169378/use-multiple-orb-buttons-or-other-buttons-in-the-leanbacks-title-view
 * <br>
...
public class NavigateTitleView extends TitleView implements OnDataChange, AccountChangeListener {
  ...
  public View focusSearch(View focused, int direction) {
    ...
    if (nextFoundFocusableViewInLayout != null
        && nextFoundFocusableViewInLayout != focused
        && nextFoundFocusableViewInLayout.isFocusable()) {
      ...
      // No focusable view found in layout...propagate to super (should invoke the
  ...
  protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
    ...
    return getSearchAffordanceView().requestFocus()
  ...
  public void updateComponentsVisibility(int flags) {
    ...
    mSearchVisibility =
        (flags & SEARCH_VIEW_VISIBLE) == SEARCH_VIEW_VISIBLE ? View.VISIBLE : View.INVISIBLE;

    mBrandingVisibility =
        (flags & BRANDING_VIEW_VISIBLE) == BRANDING_VIEW_VISIBLE ? View.VISIBLE : View.INVISIBLE;
    ...
    if (mExitPip != null
        && (PlaybackPresenter.instance(getContext()).isRunningInBackground()
            || mSearchVisibility != View.VISIBLE)) {
  ...
  private void setupButtons() {
    ...
    mAccountView.setOnOrbClickedListener(
        v -> AccountSelectionPresenter.instance(getContext()).nextAccountOrDialog());
    mAccountView.setOnOrbLongClickedListener(
        v -> {
          AccountSettingsPresenter.instance(getContext()).show();
          return true;
        });
    TooltipCompatHandler.setTooltipText(
        mAccountView, getContext().getString(R.string.settings_accounts));

    mExitPip = findViewById(R.id.exit_pip);
    mPipTitle = findViewById(R.id.pip_title);
    mExitPip.setOnOrbClickedListener(
        v -> ViewManager.instance(getContext()).startView(PlaybackView.class));
    ViewUtil.enableMarquee(mPipTitle);
    ViewUtil.setTextScrollSpeed(mPipTitle, 2.5f);
    TooltipCompatHandler.setTooltipText(
        mExitPip, getContext().getString(R.string.return_to_background_video));
  ...
  private void updateButtonsVisibility() {
    ...
    Utils.postDelayed(
        this::updateAccountIcon, 1_000); // give a time to engine to fetch an updated icon url
  }
  ...
  private void applyPipParameters() {
    if (mExitPip != null) {
      ...
      if (newVisibility == View.VISIBLE) {
        ...
        mPipTitle.setText(
            video != null ? String.format("%s - %s", video.getTitle(), video.getAuthor()) : "");
      }
    }
  }
  ...
  private void updateAccountIcon() {
    ...
    if (current != null && current.getAvatarImageUrl() != null) {
      ...
      // TooltipCompatHandler.setTooltipText(mAccountView, Utils.updateTooltip(getContext(),
      // accountName));
      TooltipCompatHandler.setTooltipText(mAccountView, accountName);
    } else {
      Colors orbColors = mAccountView.getOrbColors();
      mAccountView.setOrbColors(
          new Colors(
              orbColors.color,
              orbColors.brightColor,
              ContextCompat.getColor(getContext(), R.color.orb_icon_color)));
      mAccountView.setOrbIcon(
          ContextCompat.getDrawable(getContext(), R.drawable.browse_title_account));
      TooltipCompatHandler.setTooltipText(
          mAccountView, getContext().getString(R.string.dialog_account_none));
    }
  }
  ...
  private void loadIcon(SearchOrbView view, String url, boolean useCache) {
    ...
    // Size of the view might increase after icon change (bug on some firmwares). So, it's better to
  ...
  private static void loadIcon(
    ...
    Glide.with(context)
        ...
        .into(
            new SimpleTarget<Drawable>(iconWidth, iconHeight) {
              @Override
              public void onResourceReady(
                  @NonNull Drawable resource, @Nullable Transition<? super Drawable> transition) {
                Colors orbColors = view.getOrbColors();
                view.setOrbColors(
                    new Colors(orbColors.color, orbColors.brightColor, Color.TRANSPARENT));
