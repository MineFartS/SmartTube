...
public class BrowseFragment extends BrowseSupportFragment implements BrowseView {
  ...
  public void onSaveInstanceState(Bundle outState) {
    ...
    if (mBrowsePresenter.getCurrentVideo() != null) {
      ...
      outState.putBoolean(
          IS_PLAYER_IN_FOREGROUND, ViewManager.instance(getContext()).isPlayerInForeground());
    }
  }
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
    ...
    if (mSelectedHeaderIndex != -1) {
      ...
      if (PlaybackPresenter.instance(getContext()).getPlayer() == null && mIsPlayerInForeground) {
        ...
        boolean isVideoStateSynced =
            mSelectedVideo == null || stateService.getByVideoId(mSelectedVideo.videoId) != null;
  ...
  private void setupEventListeners() {
    ...
    ((ExtendedHeadersSupportFragment) getHeadersSupportFragment())
        .setOnHeaderLongPressedListener(
            (viewHolder, row) -> {
              long headerId = row.getHeaderItem().getId();
  ...
  private void setupFragmentFactory() {
    mSectionFragmentFactory =
        new BrowseSectionFragmentFactory(
            (row) -> {
  ...
  private void setupUi() {
    ...
    Drawable bridgeIcon =
        Utils.getDrawable(
            getActivity(),
            SplashPresenter.instance(getActivity()).getBridgePackageName(),
            "app_icon");
  ...
  public void updateErrorIfEmpty(ErrorFragmentData data) {
    mHandler.postDelayed(
        () -> showErrorIfEmpty(data), 500); // need delay because header may be not updated
  }
  ...
  private void replaceMainFragment(Fragment fragment) {
    ...
    if (mainFragment != null && fragment != null && mainFragment != fragment) {
      ...
      // mFocusOnContent = !isShowingHeaders(); // Fix focus lost when error fragment shown and
      // sidebar is hidden
  ...
  public void selectSection(int index, boolean focusOnContent) {
    if (index >= 0 && mSectionRowAdapter.size() > 0) {
      ...
      setSelectedPosition(
          index < mSectionRowAdapter.size() ? index : mSectionRowAdapter.size() - 1, false);
    }
  }
