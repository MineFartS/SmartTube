...
public final class LinkifyCompat {
  ...
  public static final int ALL =
      WEB_URLS | EMAIL_ADDRESSES | PHONE_NUMBERS | MAP_ADDRESSES | TIME_CODES;
  ...
  public static boolean addLinks(@NonNull Spannable text, @LinkifyMask int mask) {
    ...
    if ((mask & Linkify.EMAIL_ADDRESSES) != 0) {
      gatherLinks(
          links, text, PatternsCompat.AUTOLINK_EMAIL_ADDRESS, new String[] {"mailto:"}, null, null);
    }

    if ((mask & TIME_CODES) != 0) {
      gatherLinks(timeLinks, text, PatternsCompat.AUTOLINK_TIME_CODE, null, null, null);
    }
  ...
  public static boolean addLinks(
      @NonNull TextView text, @LinkifyMask int mask, LinkifyClickHandler onClick) {
    if (onClick != null) {
   ...
   * in the mask into clickable links. If matches are found the movement method for the TextView is
  ...
  /**
   * Applies a regex to the text of a TextView turning the matches into links. If links are found
   * then UrlSpans are applied to the link text match areas, and the movement method for the text is
   * changed to LinkMovementMethod.
   *
   * @param text TextView whose text is to be marked-up with links
   * @param pattern Regex pattern to be used for finding links
   * @param scheme URL scheme string (eg <code>http://</code>) to be prepended to the links that do
   *     not start with this scheme.
   */
  public static void addLinks(
      @NonNull TextView text, @NonNull Pattern pattern, @Nullable String scheme) {
    if (shouldAddLinksFallbackToFramework()) {
   ...
   * @param scheme URL scheme string (eg <code>http://</code>) to be prepended to the links that do
   *     not start with this scheme.
   * @param matchFilter The filter that is used to allow the client code additional control over
   *     which pattern matches are to be converted into links.
   */
  public static void addLinks(
      @NonNull TextView text,
      @NonNull Pattern pattern,
      @Nullable String scheme,
      @Nullable MatchFilter matchFilter,
   ...
   * @param text TextView whose text is to be marked-up with links.
   * @param pattern Regex pattern to be used for finding links.
   * @param defaultScheme The default scheme to be prepended to links if the link does not start
   *     with one of the <code>schemes</code> given.
   * @param schemes Array of schemes (eg <code>http://</code>) to check if the link found contains a
   *     scheme. Passing a null or empty value means prepend defaultScheme to all links.
   * @param matchFilter The filter that is used to allow the client code additional control over
   *     which pattern matches are to be converted into links.
   ...
   */
  public static boolean addLinks(
      @NonNull Spannable text, @NonNull Pattern pattern, @Nullable String scheme) {
    if (shouldAddLinksFallbackToFramework()) {
  ...
  public static boolean addLinks(
      ...
      @Nullable TransformFilter transformFilter) {
    if (shouldAddLinksFallbackToFramework()) {
      return Linkify.addLinks(
          spannable, pattern, defaultScheme, schemes, matchFilter, transformFilter);
    }
  ...
  private static void gatherLinks(
    ...
    while (m.find()) {
      ...
      if (matchFilter == null || matchFilter.acceptMatch(s, start, end)) {
        ...
        String url =
            schemes != null ? makeUrl(m.group(0), schemes, m, transformFilter) : m.group(0);
