...
/**
 * A presenter that assumes a LinearLayout container for a series of control buttons backed by
 * objects of type {@link Action}.
 *
 * <p>Different layouts may be passed to the presenter constructor. The layout must contain a view
 * with id control_bar.
 */
class ControlBarPresenter extends Presenter {
  ...
  interface OnControlLongClickedListener {
    boolean onControlLongClicked(
        Presenter.ViewHolder controlViewHolder, Object item, BoundData data);
  }
  ...
  class ViewHolder extends Presenter.ViewHolder {
    ...
    SparseArray<Presenter.ViewHolder> mViewHolders = new SparseArray<Presenter.ViewHolder>();
    ...
    ViewHolder(View rootView) {
      ...
      mControlBar.setOnChildFocusedListener(
          new ControlBar.OnChildFocusedListener() {
            @Override
              ...
              for (int position = 0; position < mViewHolders.size(); position++) {
                ...
                if (vh != null && vh.view == child) {
                  mOnControlSelectedListener.onControlSelected(
                      vh, getDisplayedAdapter().get(position), mData);
                  break;
                }
              }
            }
    ...
    void showControls(Presenter presenter) {
      ...
      for (int position = 0; position < adapterSize && position < MAX_CONTROLS; position++) {
    ...
    private void bindControlToAction(
        final int position, ObjectAdapter adapter, Presenter presenter) {
      Presenter.ViewHolder vh = mViewHolders.get(position);
      ...
      if (vh == null) {
        ...
        ((ControlButtonPresenterSelector.ControlButtonPresenter) presenter)
                ...
                new View.OnLongClickListener() {
                  ...
                  public boolean onLongClick(View v) {
                    ...
                    if (mOnControlLongClickedListener != null) {
                      return mOnControlLongClickedListener.onControlLongClicked(
                          itemViewHolder, item, mData);
                    }
    ...
    /**
     * Returns the adapter currently bound to the displayed controls. May be overridden in a
     * subclass.
     */
  ...
  int getChildMarginDefault(Context context) {
    if (sChildMarginDefault == 0) {
      sChildMarginDefault =
          context
              .getResources()
              .getDimensionPixelSize(R.dimen.lb_playback_controls_child_margin_default);
    }
  ...
  int getControlIconWidth(Context context) {
    if (sControlIconWidth == 0) {
      sControlIconWidth =
          context.getResources().getDimensionPixelSize(R.dimen.lb_control_icon_width);
    }
