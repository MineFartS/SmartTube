...
public class ComplexImageView extends RelativeLayout {
  ...
  public void startPlayback() {
    ...
    if (getVideo().previewUrl != null && mPreferSimplePreview) {
      if (mPreviewImage == null) {
        ...
        mPreviewContainer.addView(
            mPreviewImage, new FrameLayout.LayoutParams(mPreviewWidth, mPreviewHeight));
        mPreviewContainer.setVisibility(View.VISIBLE);
      }

      Glide.with(
              getContext()
                  .getApplicationContext()) // FIX: "You cannot start a load for a destroyed
                                            // activity"
  ...
  private void createAndStartPlayer() {
    ...
    if (mPreviewPlayer == null) {
      ...
      mPreviewPlayer.setQuality(
          Math.min(mPreviewWidth, mPreviewHeight) < 300
              ? EmbedPlayerView.QUALITY_LOW
              : EmbedPlayerView.QUALITY_NORMAL);
      mPreviewPlayer.setUseController(false);
      mPreviewPlayer.setMute(mMute);
      mPreviewPlayer.setBackgroundColor(Color.BLACK);
      mPreviewContainer.addView(
          mPreviewPlayer, new FrameLayout.LayoutParams(mPreviewWidth, mPreviewHeight));
      mPreviewContainer.setVisibility(View.VISIBLE);
    }
