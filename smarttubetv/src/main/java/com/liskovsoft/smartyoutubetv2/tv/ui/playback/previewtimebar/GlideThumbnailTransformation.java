...
public class GlideThumbnailTransformation extends BitmapTransformation {
  ...
  public GlideThumbnailTransformation(
      long position, int width, int height, int rowCount, int colCount, int durationMS) {
    mWidth = width;
    mHeight = height;
    mMaxLines = rowCount;
    ...
    if (BuildConfig.DEBUG) {
      Log.d(
          TAG,
          "Thumbnail coordinates (%sx%s): pos: %s, line: %s, col: %s",
          mMaxColumns,
          mMaxLines,
          thumbPos + 1,
          mY + 1,
  ...
  protected Bitmap transform(
      @NonNull BitmapPool pool, @NonNull Bitmap groupBitmap, int outWidth, int outHeight) {
