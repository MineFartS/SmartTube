...
public class PlaybackFragment extends SeekModePlaybackFragment implements PlaybackView {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mSelectedVideoId =
        savedInstanceState != null ? savedInstanceState.getString(SELECTED_VIDEO_ID, null) : null;
    ...
    mBackgroundManager.setBackgroundColor(
        ContextCompat.getColor(getContext(), R.color.player_background));
  ...
  // NOTE: depending of SDK version Start/Stop may be called with delay (SDK_INT > 23) or not called
  // at all (PIP/Dialogs)!
  ...
  private void applyTickle(MotionEvent event) {
    ...
    if (getActivity() != null) {
      ...
      if (event.getAxisValue(MotionEvent.AXIS_X)
          > (displayMetrics.widthPixels - gestureAreaWidthPx)) {
        return;
      }
    }
  ...
  private void createPlayerGlue() {
    PlayerAdapter playerAdapter =
        new LeanbackPlayerAdapter(
            getContext(), mPlayer, UPDATE_DELAY_MS); // NOTE: possible context memory leak
    ...
    mPlayerGlue =
        new VideoPlayerGlue(
            getContext(),
            playerAdapter,
            playerActionListener); // NOTE: possible context memory leak
    mPlayerGlue.setHost(new SurfacePlaybackFragmentGlueHost(this));
    mPlayerGlue.setSeekEnabled(true);
    mPlayerGlue.setControlsOverlayAutoHideEnabled(
        false); // don't show controls on some player events like play/pause/end
  ...
  private void createMediaSession() {
    if (VERSION.SDK_INT <= 19 || getContext() == null) {
      // Fix Android 4.4 bug: java.lang.IllegalArgumentException: MediaButtonReceiver component may
      // not be null
      return;
    }
    ...
    mMediaSessionConnector.setMediaMetadataProvider(
        player -> {
          if (getVideo() == null) {
            return null;
          }
          ...
          metadataBuilder.putString(
              MediaMetadataCompat.METADATA_KEY_TITLE, getVideo().getTitleFull());
          metadataBuilder.putString(
              MediaMetadataCompat.METADATA_KEY_DISPLAY_TITLE, getVideo().getTitleFull());
          metadataBuilder.putString(
              MediaMetadataCompat.METADATA_KEY_ARTIST, getVideo().getAuthor());
          metadataBuilder.putString(
              MediaMetadataCompat.METADATA_KEY_DISPLAY_SUBTITLE,
              Helpers.toString(getVideo().getSecondTitleFull()));
          metadataBuilder.putString(
              MediaMetadataCompat.METADATA_KEY_ALBUM_ART_URI, getVideo().getCardImageUrl());
    ...
    mMediaSessionConnector.setControlDispatcher(
        new DefaultControlDispatcher() {
          ...
          public boolean dispatchSetPlayWhenReady(Player player, boolean playWhenReady) {
            ...
            if (System.currentTimeMillis() - PlayerData.instance(getContext()).getAfrSwitchTimeMs()
                < 5_000) {
              return false;
            }
    ...
    if (getContext() != null && video.likeCount != null) {
      result =
          TextUtils.concat(
              ...
              Helpers.THUMB_UP); // color of thumb cannot be changed
    }
    ...
    if (getContext() != null && video.dislikeCount != null) {
      result =
          TextUtils.concat(
              ...
              video.dislikeCount,
              Helpers.NON_BREAKING_SPACE,
              Helpers.THUMB_DOWN); // color of thumb cannot be changed
    }
  ...
  public void loadStoryboard() {
    if (mPlayerGlue.getSeekProvider() instanceof StoryboardSeekDataProvider) {
      ((StoryboardSeekDataProvider) mPlayerGlue.getSeekProvider())
          .init(getVideo(), mExoPlayerController.getDurationMs());
    }
  }
  ...
  public void setSpeed(float speed) {
    ...
    setButtonState(
        R.id.action_video_speed, speed != 1.0f ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  }
  ...
  public void updateSuggestions(VideoGroup group) {
    ...
    VideoGroupObjectAdapter existingAdapter =
        GridFragmentHelper.findRelatedAdapter(mVideoGroupAdapters, group, this::freeze);
    ...
    if (existingAdapter == null) {
      ...
      VideoGroupObjectAdapter videoGroupAdapter =
          new VideoGroupObjectAdapter(group, group.isShorts() ? mShortsPresenter : mCardPresenter);
  ...
  public VideoGroup getSuggestionsByIndex(int rowIndex) {
    ...
    Object row =
        mRowsAdapter != null && mRowsAdapter.size() > realIndex
  ...
  private void focusPendingSuggestedItem(ViewHolder holder) {
    ...
    VideoGroupObjectAdapter existingAdapter =
        mVideoGroupAdapters.get(mPendingFocus.getGroup().getId());
    ...
    if (holder != null
        ...
        && ((ListRow) holder.getRow()).getAdapter() == existingAdapter) {
  ...
  private void freeze(boolean freeze) {
    ...
    if (mRowPresenter != null && mRowsSupportFragment != null) {
      ViewHolder vh =
          mRowsSupportFragment.getRowViewHolder(mRowsSupportFragment.getSelectedPosition());
      // Skip PlaybackRowPresenter.ViewHolder
      if (vh instanceof ListRowPresenter.ViewHolder) {
  ...
  /**
   * Simply recreates exoplayer objects (silently) if prev track (current from this perspective)
   * isn't empty<br>
