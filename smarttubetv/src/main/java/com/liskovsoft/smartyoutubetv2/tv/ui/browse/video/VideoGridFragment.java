...
public class VideoGridFragment extends GridFragment implements VideoSection {
  ...
  private void setupAdapter() {
    ...
    presenter.setNumberOfColumns(
        GridFragmentHelper.getMaxColsNum(
            getContext(), isShorts() ? R.dimen.shorts_card_width : R.dimen.card_width, 1.0f // Scale
            ));
  ...
  public void update(VideoGroup group) {
    ...
    if ((action == VideoGroup.ACTION_SYNC || action == VideoGroup.ACTION_REPLACE)
        && getBrowseGrid() != null
        && getBrowseGrid().isComputingLayout()) {
      return;
    }
  ...
  private void restorePosition() {
    ...
    if ((mSelectedItemIndex == -1 && mSelectedItem == null)
        || mGridAdapter == null
        || mGridAdapter.size() > RESTORE_MAX_SIZE) {
  ...
  /**
   * Disable scrolling on partially updated grid. This shouldn't fix card position bug on Android
   * 4.4.
   */
  ...
  private final class ItemViewSelectedListener implements OnItemViewSelectedListener {
    ...
    private void checkScrollEnd(Video item) {
      ...
      if (index
          > (size
              - (isShorts()
                  ? ViewUtil.GRID_SCROLL_CONTINUE_NUM * 2
                  : ViewUtil.GRID_SCROLL_CONTINUE_NUM))) {
