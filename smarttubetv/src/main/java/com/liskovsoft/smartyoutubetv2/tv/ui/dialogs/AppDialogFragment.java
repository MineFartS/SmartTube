...
public class AppDialogFragment extends LeanbackSettingsFragment implements AppDialogView {
  ...
  public boolean onPreferenceStartScreen(
      PreferenceFragment preferenceFragment, PreferenceScreen preferenceScreen) {
    // Contains only child fragments.
    return false;
  }

  private AppPreferenceFragment buildPreferenceFragment(
      List<OptionCategory> categories, String title) {
  ...
  public boolean onPreferenceDisplayDialog(
      @Nullable PreferenceFragment caller, @NonNull Preference pref) {
    ...
    if (pref instanceof StringListPreference) {
      ...
      StringListPreferenceDialogFragment f =
          StringListPreferenceDialogFragment.newInstanceStringList(listPreference.getKey());
      ...
      RadioListPreferenceDialogFragment f =
          RadioListPreferenceDialogFragment.newInstanceSingle(listPreference.getKey());
      f.enableTransparent(mIsTransparent);
      f.setTargetFragment(caller, 0);
      f.setPreference(pref);
      startPreferenceFragment(f);
      ...
      ChatPreferenceDialogFragment f =
          ChatPreferenceDialogFragment.newInstance(
              chatPreference.getChatReceiver(), chatPreference.getKey());
      f.enableTransparent(mIsTransparent);
      f.setTargetFragment(caller, 0);
      f.setPreference(pref);
      startPreferenceFragment(f);
      ...
      CommentsPreferenceDialogFragment f =
          CommentsPreferenceDialogFragment.newInstance(
              commentsPreference.getCommentsReceiver(), commentsPreference.getKey());
      f.enableTransparent(mIsTransparent);
      f.setTargetFragment(caller, 0);
      f.setPreference(pref);
      startPreferenceFragment(f);
      ...
      LeanbackListPreferenceDialogFragment f =
          LeanbackListPreferenceDialogFragment.newInstanceMulti(listPreference.getKey());
      f.setTargetFragment(caller, 0);
      f.setPreference(pref);
      startPreferenceFragment(f);
    } else {
  ...
  public void startPreferenceFragment(@NonNull Fragment fragment) {
    ...
    if (prevFragment != null) {
      ...
      transaction.add(
          R.id.settings_preference_fragment_container, fragment, PREFERENCE_FRAGMENT_TAG);
    }
    // Fix possible state loss!!!
