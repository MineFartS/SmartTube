...
public class GridFragmentHelper {
  ...
  public static Pair<Integer, Integer> getCardDimensPx(
      Context context, int cardWidthResId, int cardHeightResId, float cardScale) {
  ...
  public static Pair<Integer, Integer> getCardDimensPx(
    ...
    cardDimensPx =
        getCardDimensPxInt(context, cardWidthResId, cardHeightResId, cardScale, isSingleColumn);
   ...
   */
  private static Pair<Integer, Integer> getCardDimensPxInt(
      Context context,
      int cardWidthResId,
      int cardHeightResId,
      float cardScale,
  ...
  public static VideoGroupObjectAdapter findRelatedAdapter(
      Map<Integer, VideoGroupObjectAdapter> mediaGroupAdapters,
      VideoGroup group,
      RowFreezer freezer) {
    ...
    if (existingAdapter == null) {
      Float value =
          sMaxColsNum.get(group.isShorts() ? R.dimen.shorts_card_width : R.dimen.card_width);
      ...
      for (VideoGroupObjectAdapter adapter : mediaGroupAdapters.values()) {
        if (canMergeWithGroup(adapter, group)) {
          // Remain other rows of the same type untitled (usually the such rows share the same
          // titles)
          group.setTitle(null);
    ...
    boolean matchedRowFound =
        lastGroup != null
            ...
            && (Helpers.equals(lastGroup.getTitle(), group.getTitle())
                || lastGroup.getTitle()
                    == null); // we could set title to null in the previous iteration
