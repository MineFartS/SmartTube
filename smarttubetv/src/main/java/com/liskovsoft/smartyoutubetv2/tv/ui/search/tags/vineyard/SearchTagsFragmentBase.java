...
public abstract class SearchTagsFragmentBase extends SearchSupportFragment
  ...
  private ArrayObjectAdapter
      mResultsAdapter; // contains tags adapter and results adapter (see attachAdapter method)
  ...
  private void setupListenersAndPermissions() {
    ...
    setOnItemViewClickedListener(
        (itemViewHolder, item, rowViewHolder, row) -> onItemViewClicked(item));

    setOnItemViewSelectedListener(
        (itemViewHolder, item, rowViewHolder, row) -> onItemViewSelected(item));
  }
  ...
  protected void freeze(boolean freeze) {
    ...
    if (mResultsPresenter != null && rowsSupportFragment != null) {
      ViewHolder vh =
          rowsSupportFragment.getRowViewHolder(rowsSupportFragment.getSelectedPosition());
      if (vh != null) {
