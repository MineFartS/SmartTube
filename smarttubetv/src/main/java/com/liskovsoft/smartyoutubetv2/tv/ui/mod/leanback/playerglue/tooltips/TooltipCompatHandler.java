...
public class TooltipCompatHandler
  ...
  private TooltipCompatHandler(View anchor, CharSequence tooltipText) {
    ...
    mHoverSlop =
        ViewConfigurationCompat.getScaledHoverSlop(ViewConfiguration.get(mAnchor.getContext()));
    clearAnchorPos();
  ...
  public boolean onHover(View v, MotionEvent event) {
    ...
    AccessibilityManager manager =
        (AccessibilityManager) mAnchor.getContext().getSystemService(Context.ACCESSIBILITY_SERVICE);
  ...
  void show(boolean fromTouch) {
    ...
    } else if ((ViewCompat.getWindowSystemUiVisibility(mAnchor) & SYSTEM_UI_FLAG_LOW_PROFILE)
  ...
  /**
   * Update the anchor position if it significantly (that is by at least mHoverSlope) different from
   * the previously stored position. Ignoring insignificant changes filters out the jitter which is
   * typical for such input sources as stylus.
   *
