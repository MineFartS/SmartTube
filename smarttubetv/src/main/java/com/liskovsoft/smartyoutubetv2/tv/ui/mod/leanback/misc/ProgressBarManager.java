 ...
 * after which the progress bar will be shown. This is currently being used in {@link
 * BrowseFragment} & {@link VerticalGridFragment} to show {@link ProgressBar} while the data is
 * being loaded.
 */
public final class ProgressBarManager {
  ...
  private static View createProgressBar(ViewGroup rootView, int position) {
    ProgressBar progressBarView =
        new ProgressBar(rootView.getContext(), null, android.R.attr.progressBarStyleLarge);
  ...
  /**
   * Sets the root view on which the progress bar will be attached. This class assumes the root view
   * to be {@link FrameLayout} in order to position the progress bar widget in the center of the
   * screen.
   *
  ...
  public void show() {
    if (mEnableProgressBar) {
      ...
      Utils.postDelayed(
          showRunnable,
          System.currentTimeMillis() - mHideTimeMs > HIDE_DELAY_MS ? SHOW_DELAY_MS : 0);
    }
  }
  ...
  /**
   * Sets a custom view to be shown in place of the default {@link ProgressBar}. This view must have
   * a parent. Once set, we maintain the visibility property of this view.
   *
