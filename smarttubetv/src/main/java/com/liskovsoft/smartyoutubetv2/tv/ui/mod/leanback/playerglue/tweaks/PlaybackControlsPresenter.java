...
/**
 * A presenter for a control bar that supports "more actions", and toggling the set of controls
 * between primary and secondary sets of {@link Actions}.
 */
class PlaybackControlsPresenter extends ControlBarPresenter {
  ...
  class ViewHolder extends ControlBarPresenter.ViewHolder {
    ...
    ViewHolder(View rootView) {
      ...
      mTotalTimeMarginEnd = ((MarginLayoutParams) mTotalTime.getLayoutParams()).getMarginEnd();
    }
  ...
  /**
   * Enables the display of secondary actions. A "more actions" button will be displayed. When "more
   * actions" is selected, the primary actions are replaced with the secondary actions.
   */
  ...
  public void setProgressColor(ViewHolder vh, @ColorInt int color) {
    Drawable drawable =
        new ClipDrawable(new ColorDrawable(color), Gravity.LEFT, ClipDrawable.HORIZONTAL);
  ...
  int getChildMarginBigger(Context context) {
    if (sChildMarginBigger == 0) {
      sChildMarginBigger =
          context
              .getResources()
              .getDimensionPixelSize(R.dimen.lb_playback_controls_child_margin_bigger);
    }
  ...
  int getChildMarginBiggest(Context context) {
    if (sChildMarginBiggest == 0) {
      sChildMarginBiggest =
          context
              .getResources()
              .getDimensionPixelSize(R.dimen.lb_playback_controls_child_margin_biggest);
    }
