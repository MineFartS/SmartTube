...
public class StoryboardManager {
  ...
  private long getFrameDurationMsAlt() {
    ...
    return mStoryboard.getGroupDurationMS()
        / ((long) groupSize.getRowCount() * groupSize.getColCount());
  }
  ...
  private void loadPreview(long currentPosition, Callback callback) {
    ...
    GlideThumbnailTransformation transformation =
        new GlideThumbnailTransformation(
            ...
            size.getRowCount(),
            size.getColCount(),
            size.getDurationEachMS());

    // Log.d(TAG, "Loading preview. Position: %s, groupNum: %s, groupDurationMS: %s, groupSize",
    // currentPosition, groupNum, mStoryboard.getGroupDurationMS(), size);
  ...
  private void preloadNextImage() {
    ...
    for (int i = 1; i <= MAX_PRELOADED_IMAGES; i++) {
      int imgNum =
          mSeekDirection == DIRECTION_RIGHT
              ? mCurrentImgNum + i
              : mCurrentImgNum - i; // get next image
      preloadImage(imgNum);
    }
  }
