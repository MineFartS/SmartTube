...
public class VideoGroupObjectAdapter extends ObjectAdapter {
  ...
  private final List<VideoGroup> mVideoGroups =
      new ArrayList<>(); // keep groups from being garbage collected
  ...
  public void removeAuthor(VideoGroup group) {
    ...
    List<Video> result =
        Helpers.filter(mVideoItems, video -> Helpers.equals(author, video.getAuthor()));
    if (result != null) {
  ...
  private void notifyOtherAdapters(VideoGroup group, int type) {
    sAdapterRegistry.forEach(
        adapter -> {
          if (adapter == this || adapter.isEmpty()) return;
