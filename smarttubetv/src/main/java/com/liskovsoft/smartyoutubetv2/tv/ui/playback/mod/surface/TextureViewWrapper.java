...
public class TextureViewWrapper implements SurfaceWrapper {
  ...
  public TextureViewWrapper(Context context, ViewGroup root) {
    mVideoSurface =
        (TextureView) LayoutInflater.from(context).inflate(R.layout.lb_video_texture, root, false);
    mVideoSurface.setSurfaceTextureListener(
        new SurfaceTextureListener() {
          @Override
          public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
            if (mMediaPlaybackCallback != null) {
              mMediaPlaybackCallback.surfaceCreated(
                  new TextureViewSurfaceHolder(new Surface(surface)));
            }
          ...
          public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) {
            if (mMediaPlaybackCallback != null) {
              mMediaPlaybackCallback.surfaceChanged(
                  new TextureViewSurfaceHolder(new Surface(surface)), 4, width, height);
            }
          }
          ...
          public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
            if (mMediaPlaybackCallback != null) {
              mMediaPlaybackCallback.surfaceDestroyed(
                  new TextureViewSurfaceHolder(new Surface(surface)));
            }
  ...
  public void setSurfaceHolderCallback(SurfaceHolder.Callback callback) {
    ...
    if (callback != null) {
      if (mState == SURFACE_CREATED) {
        mMediaPlaybackCallback.surfaceCreated(
            new TextureViewSurfaceHolder(new Surface(mVideoSurface.getSurfaceTexture())));
      }
    }
  }
