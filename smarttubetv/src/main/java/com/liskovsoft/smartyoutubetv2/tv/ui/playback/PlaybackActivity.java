...
/**
 * Loads PlaybackFragment and delegates input from a game controller. <br>
 * For more information on game controller capabilities with leanback, review the <a
 * href="https://developer.android.com/training/game-controllers/controller-input.html">docs</href>.
 */
public class PlaybackActivity extends LeanbackActivity {
  ...
  private void enterPipMode() {
    ...
    // More info:
    // https://developer.android.com/guide/topics/ui/picture-in-picture#continuing_playback
  ...
  public void finish() {
    ...
    if (doNotDestroy() && !skipPip()) {
      ...
      if (getPlayerTweaksData().isKeepFinishedActivityEnabled()) {
        // moveTaskToBack(true); // Don't do this or you'll have problems when player overlaps other
        // apps (e.g. casting)
        getViewManager().startParentView(this);

        // Player with TextureView keeps running in background because onStop() fired with huge
        // delay (~5sec).
  ...
  private boolean doNotDestroy() {
    ...
    boolean isBackground =
        getPlayerData().getBackgroundMode() == PlayerEngine.BACKGROUND_MODE_SOUND
            || isEngineBlocked();
