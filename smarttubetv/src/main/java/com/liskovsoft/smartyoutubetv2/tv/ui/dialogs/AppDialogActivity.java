...
public class AppDialogActivity extends MotherActivity {
  ...
  private boolean handleNavigation(KeyEvent event) {
    ...
    // Toggle dialog
    if (!mFragment.isOverlay()
        && (KeyHelpers.isLeftRightKey(event.getKeyCode())
            || KeyHelpers.isMenuKey(event.getKeyCode()))) {
    ...
    if (mFragment.isOverlay()
        && (KeyHelpers.isNavigationKey(event.getKeyCode())
            || KeyHelpers.isMenuKey(event.getKeyCode()))) {
      if (event.getAction() == KeyEvent.ACTION_DOWN) {
        finish();
      }
  ...
  protected void onUserLeaveHint() {
    ...
    if (ViewManager.instance(this).getTopView() == PlaybackView.class
        && PlaybackPresenter.instance(this).getContext() instanceof PlaybackActivity) {
