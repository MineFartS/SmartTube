...
public class MultiVideoGridFragment extends MultiGridFragment implements VideoSection {
  ...
  public void update(VideoGroup group) {
    if (group.getPosition() == 0) {
      ...
      // Attempt to fix: IllegalStateException: Cannot call this method while RecyclerView is
      // computing a layout or scrolling
      if ((action == VideoGroup.ACTION_SYNC || action == VideoGroup.ACTION_REPLACE)
          && getBrowseGrid2() != null
          && getBrowseGrid2().isComputingLayout()) {
        return;
      }
  ...
  private void removePadding() {
    ...
    if (browseGrid != null) {
      ...
      browseGrid.setPadding(
          browseGrid.getPaddingLeft(),
          browseGrid.getPaddingTop(),
          browseGrid.getPaddingRight() / 3,
          browseGrid.getPaddingBottom());
    }
    ...
    if (browseGrid != null) {
      // Don't remove padding at all. This could cause weird card zooming.
      browseGrid.setPadding(
          browseGrid.getPaddingLeft() / 3,
          browseGrid.getPaddingTop(),
          browseGrid.getPaddingRight(),
          browseGrid.getPaddingBottom());
    }
  }
  ...
  private void setupAdapter() {
    ...
    VerticalGridPresenter presenter1 =
        new CustomVerticalGridPresenter(R.layout.lb_vertical_grid1, R.id.browse_grid1);
    presenter1.setNumberOfColumns(1);

    // Right grid of channel's content
    VerticalGridPresenter presenter2 =
        new CustomVerticalGridPresenter(R.layout.lb_vertical_grid2, R.id.browse_grid2);

    int maxColsNum =
        GridFragmentHelper.getMaxColsNum(
            getContext(), R.dimen.card_width, 1.0f // Scale
            );
  ...
  private void addSearchHeader() {
    ...
    mGridAdapter1.setHeader(
        new SearchFieldCallback() {
          @Override
  ...
  private final class ItemViewClickedListener2
