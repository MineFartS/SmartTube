...
public class DialogsListAdapter<DIALOG extends IDialog>
  ...
  public BaseDialogViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    try {
      Constructor<? extends BaseDialogViewHolder> constructor =
    ...
    Collections.sort(
        ...
        (o1, o2) -> {
          if (o1.getLastMessage().getCreatedAt().after(o2.getLastMessage().getCreatedAt())) {
            return -1;
          } else if (o1.getLastMessage()
              .getCreatedAt()
              .before(o2.getLastMessage().getCreatedAt())) {
            return 1;
          } else return 0;
        });
    notifyDataSetChanged();
   ...
   */
  public void setOnDialogLongClickListener(
      OnDialogLongClickListener<DIALOG> onLongItemClickListener) {
  ...
  public void setOnDialogViewLongClickListener(
      OnDialogViewLongClickListener<DIALOG> clickListener) {
  ...
  public abstract static class BaseDialogViewHolder<DIALOG extends IDialog>
    ...
    protected void setOnDialogViewClickListener(
        OnDialogViewClickListener<DIALOG> onDialogViewClickListener) {
      this.onDialogViewClickListener = onDialogViewClickListener;
    }

    protected void setOnLongItemClickListener(
        OnDialogLongClickListener<DIALOG> onLongItemClickListener) {
      this.onLongItemClickListener = onLongItemClickListener;
    }

    protected void setOnDialogViewLongClickListener(
        OnDialogViewLongClickListener<DIALOG> onDialogViewLongClickListener) {
  ...
  public static class DialogViewHolder<DIALOG extends IDialog>
    ...
    private void applyStyle() {
      if (dialogStyle != null) {
        ...
        if (tvLastMessage != null) {
          tvLastMessage.setTextSize(
              TypedValue.COMPLEX_UNIT_PX, dialogStyle.getDialogMessageTextSize());
        }
        ...
        // Divider
        if (divider != null) divider.setBackgroundColor(dialogStyle.getDialogDividerColor());
        if (dividerContainer != null)
          dividerContainer.setPadding(
              dialogStyle.getDialogDividerLeftPadding(),
              0,
              dialogStyle.getDialogDividerRightPadding(),
              0);
        // Avatar
        if (ivAvatar != null) {
        ...
        if (tvBubble != null) {
          ...
          tvBubble.setTextSize(
              TypedValue.COMPLEX_UNIT_PX, dialogStyle.getDialogUnreadBubbleTextSize());
          tvBubble.setTextColor(dialogStyle.getDialogUnreadBubbleTextColor());
          tvBubble.setTypeface(
              tvBubble.getTypeface(), dialogStyle.getDialogUnreadBubbleTextStyle());
        }
      }
    }
      ...
      if (dialogStyle != null) {
        ...
        if (tvLastMessage != null) {
          ...
          tvLastMessage.setTypeface(
              Typeface.DEFAULT, dialogStyle.getDialogUnreadMessageTextStyle());
        }
      }
    }
    ...
    public void onBind(final DIALOG dialog) {
      ...
      if (dialog.getLastMessage() != null) {
        ...
        tvDate.setText(formattedDate == null ? getDateString(lastMessageDate) : formattedDate);
      } else {
        tvDate.setText(null);
      }
      ...
      if (imageLoader != null && dialog.getLastMessage() != null) {
        imageLoader.loadImage(
            ivLastMessageUser, dialog.getLastMessage().getUser().getAvatar(), null);
      }
      ivLastMessageUser.setVisibility(
          dialogStyle.isDialogMessageAvatarEnabled()
                  && dialog.getUsers().size() > 1
                  && dialog.getLastMessage() != null
              ? VISIBLE
              : GONE);
      ...
      tvBubble.setVisibility(
          dialogStyle.isDialogUnreadBubbleEnabled() && dialog.getUnreadCount() > 0
              ? VISIBLE
              : GONE);
