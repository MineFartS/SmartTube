...
public class ChannelsProvider {
   ...
   * @param drawableId resource to write as the channel logo. This must be a bitmap and not, say a
   *     vector drawable.
   */
  @WorkerThread
  private static void writeChannelLogo(
      Context context, long channelId, @DrawableRes int drawableId) {
      ...
      if (cursor != null) {
        while (cursor.moveToNext()) {
          if (!cursor.isNull(COLUMN_WATCH_NEXT_INTERNAL_PROVIDER_ID_INDEX)
              && TextUtils.equals(
                  clipId, cursor.getString(COLUMN_WATCH_NEXT_INTERNAL_PROVIDER_ID_INDEX))) {
            ...
            if (cursor.getInt(COLUMN_WATCH_NEXT_COLUMN_BROWSABLE_INDEX) == 0) {
              ...
              WatchNextProgram.Builder builder =
                  new WatchNextProgram.Builder(existingProgram)
              ...
              int rowsUpdated =
                  context
                      .getContentResolver()
                      .update(watchNextProgramUri, contentValues, null, null);
              if (rowsUpdated < 1) {
  ...
  public static void createOrUpdateChannel(Context context, Playlist playlist) {
    ...
    if (publishedId != -1) {
      Log.d(
          TAG,
          "Oops: channel already published. Doing update instead... publishedId: " + publishedId);
      updateChannel(context, playlist);
      // addClipsToChannel(context, publishedId, Helpers.isGoogleTVLauncher(context) ||
      // channel.isBrowsable() ? playlist.getClips() : Collections.emptyList());
    ...
    if (channel != null) {
      Log.d(
          TAG,
          "Oops: channel already published but not memorized by the app. Doing update instead..."
              + " foundId: "
              + channel.getId());
      playlist.setPublishedId(channel.getId());
      updateChannel(context, playlist);
      // addClipsToChannel(context, channel.getId(), Helpers.isGoogleTVLauncher(context) ||
      // channel.isBrowsable() ? playlist.getClips() : Collections.emptyList());
  ...
  private static long createChannel(Context context, Playlist playlist) {
    ...
    try {
      channelUri =
          context
              .getContentResolver()
              .insert(Channels.CONTENT_URI, builder.build().toContentValues());
  ...
  private static void cleanupChannel(Context context, long channelId) {
    context
        .getContentResolver()
        .delete(TvContractCompat.buildPreviewProgramsUriForChannel(channelId), null, null);
  }
  ...
  private static void publishProgram(Context context, Clip clip, long channelId, int weight) {
    ...
    PreviewProgram.Builder builder =
        createProgramBuilder(context, clip).setWeight(weight).setChannelId(channelId);
    ...
    try {
      programUri =
          context
              .getContentResolver()
              .insert(PREVIEW_PROGRAMS_CONTENT_URI, builder.build().toContentValues());
    } catch (Exception e) {
      Log.e(TAG, e.getMessage());
      e.printStackTrace();
    }
  ...
  static void setProgramViewCount(Context context, long programId, int numberOfViews) {
    ...
    try (Cursor cursor = context.getContentResolver().query(programUri, null, null, null, null)) {
      ...
      PreviewProgram.Builder builder =
          new PreviewProgram.Builder(existingProgram)
  ...
  private static PreviewProgram.Builder createProgramBuilder(
      PreviewProgram.Builder baseBuilder, Context context, Clip clip) {
    Uri previewUri =
        clip.getPreviewVideoUrl() == null ? null : Uri.parse(clip.getPreviewVideoUrl());
  ...
  private static Channel.Builder createChannelBuilder(Context context, Playlist playlist) {
    Channel.Builder builder =
        new Channel.Builder()
            ...
            .setAppLinkIntent(
                AppUtil.getInstance(context).createAppIntent(playlist.getPlaylistUrl()))
  ...
  private static void visitChannels(Context context, ChannelVisitor visitor) {
    Cursor cursor =
        context
            .getContentResolver()
            .query(TvContractCompat.Channels.CONTENT_URI, CHANNEL_COLUMNS, null, null, null);
