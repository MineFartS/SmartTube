...
public abstract class VideoInfoServiceBase {
  ...
  private void decipherFormats(VideoInfo videoInfo) {
    ...
    Pair<List<String>, List<String>> result =
        mAppService.bulkSigExtract(extractNParams(urlHolders), extractSParams(urlHolders));
  ...
  private DashInfo getDashInfo(VideoInfo videoInfo) {
    if (videoInfo == null
        || videoInfo.getAdaptiveFormats() == null
        || videoInfo.getAdaptiveFormats().isEmpty()) {
      return null;
    }
    ...
    if (info == null
        || info.getSegmentDurationUs() <= 0
        || info.getStartTimeMs() <= 0
        || info.getStartSegmentNum() < 0) {
