...
public class AppServiceIntCached extends AppServiceInt {
  ...
  private static final long CACHE_REFRESH_PERIOD_MS =
      10 * 60 * 60 * 1_000; // check updated core files every 10 hours
  ...
  protected synchronized AppInfo getAppInfo(String userAgent) {
    if (mAppInfo != null
        && System.currentTimeMillis() - mAppInfoUpdateTimeMs < CACHE_REFRESH_PERIOD_MS) {
      return mAppInfo;
    }
  ...
  public PlayerDataExtractor getPlayerDataExtractor(String playerUrl) {
    synchronized (mPlayerSync) {
      if (mPlayerDataExtractor != null
          && Helpers.equalsAny(
              playerUrl, mPlayerDataExtractor.getPlayerUrl(), getFailedPlayerUrl())) {
  ...
  private String getFailedPlayerUrl() {
    return getData().getFailedAppInfo() != null
        ? getData().getFailedAppInfo().getPlayerUrl()
        : null;
  }
