...
public class PlaylistServiceWrapper extends PlaylistService {
  ...
  private static void createCachedPlaylist(String playlistName, @Nullable String videoId) {
    ...
    if (videoId == null) {
      ItemGroup playlist =
          PlaylistGroupServiceImpl.createPlaylistGroup(playlistName, null, Collections.emptyList());
      PlaylistGroupServiceImpl.addPlaylistGroup(playlist);
    } else if (cachedVideo != null && Helpers.equals(cachedVideo.getVideoId(), videoId)) {
      ItemGroup playlist =
          PlaylistGroupServiceImpl.createPlaylistGroup(
              playlistName,
              cachedVideo.getCardImageUrl(),
      ...
      ItemGroup playlist =
          PlaylistGroupServiceImpl.createPlaylistGroup(
              ...
              Collections.singletonList(
                  new ItemImpl(channelId, title, null, videoId, subtitle, badgeText, -1)));
      PlaylistGroupServiceImpl.addPlaylistGroup(playlist);
    }
  }
  ...
  private List<PlaylistInfo> getCachedPlaylistInfo(
      List<PlaylistInfo> playlistsInfos, String videoId) {
    ...
    if (!playlistGroups.isEmpty()) {
      ...
      for (ItemGroup itemGroup : playlistGroups) {
        ...
        if (playlistsInfos != null && !playlistsInfos.isEmpty()) {
          PlaylistInfo item =
              findFirst(playlistsInfos, itemGroup.getTitle()); // More robust to find by id?
          if (item != null) {
  ...
  private void addToCachedPlaylist(String playlistId, String videoId) {
    ...
    if (playlistGroup == null) {
      ...
      playlistGroup =
          PlaylistGroupServiceImpl.createPlaylistGroup(playlistId, title, (String) null);
    }
  ...
  private static void renameCachedPlaylist(String playlistId, String newName) {
    if (Helpers.equalsAny(
        playlistId, BrowseApiHelper.LIKED_PLAYLIST, BrowseApiHelper.WATCH_LATER_PLAYLIST)) {
  ...
  private String findTitle(String playlistId) {
    PlaylistInfo first =
        Helpers.findFirst(
            mCachedPlaylistInfos, item -> Helpers.equals(item.getPlaylistId(), playlistId));
