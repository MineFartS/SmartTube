...
public class YouTubeMediaGroup implements MediaGroup {
  ...
  public static MediaGroup from(GridTab browseResult, int type) {
    ...
    return create(
        new YouTubeMediaGroup(type), browseResult.getItemWrappers(), browseResult.getNextPageKey());
  }

  public static MediaGroup from(
      GridTabContinuation continuation, String reloadPageKey, String groupTitle, int groupType) {
  ...
  public static List<MediaGroup> from(SearchResult searchResult, int type) {
    if (searchResult == null
        || searchResult.getSections() == null
        || searchResult.getSections().size() == 0) {
      return null;
    }
  ...
  private static MediaGroup create(
      YouTubeMediaGroup baseGroup, List<ItemWrapper> items, String nextPageKey) {
    ArrayList<MediaItem> mediaItems = new ArrayList<>();
