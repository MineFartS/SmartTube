...
class YouTubeContentService implements ContentService {
  ...
  public MediaGroup getGroup(MediaItem mediaItem) {
    return mediaItem.getReloadPageKey() != null
        ? getBrowseService2()
            .getGroup(mediaItem.getReloadPageKey(), mediaItem.getType(), mediaItem.getTitle())
  ...
  private Observable<List<MediaGroup>> getChannelObserve(
      String channelId, String title, String params) {
    return RxHelper.create(
        emitter -> {
          checkSigned();
          ...
          // Special type of channel that could be found inside Music section (see Liked row More
          // button)
          if (YouTubeHelper.isGridChannel(canonicalId)) {
            ...
            if (gridChannel instanceof BaseMediaGroup && !gridChannel.isEmpty()) {
              ...
              kotlin.Pair<List<MediaGroup>, String> channel =
                  getBrowseService2().getChannel(canonicalId, params);
  ...
  public Observable<List<MediaGroup>> getChannelSortingObserve(MediaItem item) {
    return item != null && item.getChannelId() != null
        ? getChannelSortingObserve(item.getChannelId())
        : null;
  }
  ...
  private void emitGroups(
      ObservableEmitter<List<MediaGroup>> emitter,
      kotlin.Pair<List<MediaGroup>, String> groupsAndKey) {
    emitGroupsPartial(emitter, groupsAndKey);

    emitter.onComplete();
  }

  private void emitGroupsPartial(
      ObservableEmitter<List<MediaGroup>> emitter,
      kotlin.Pair<List<MediaGroup>, String> groupsAndKey) {
  ...
  private void emitGroupsPartial(
      ObservableEmitter<List<MediaGroup>> emitter, List<MediaGroup> groups) {
    ...
    Log.d(
        TAG,
        "emitGroupsPartial: begin emitting group of type %s...",
        firstGroup != null ? firstGroup.getType() : null);
  ...
  private MediaGroup continueGroupChecked(MediaGroup mediaGroup) {
    ...
    if (Helpers.equals(mediaGroup.getMediaItems(), result.getMediaItems())
      ...
      // Result group is duplicate of the original. Seems that we've reached the end before.
      // Skipping...
      return null;
    }
  ...
  private MediaGroup continueGroupInt(MediaGroup mediaGroup) {
    ...
    if (mediaGroup instanceof BaseMediaGroup) {
      ...
      // Fix channels with multiple empty groups (e.g.
      // https://www.youtube.com/@RuhiCenetMedya/videos)
      for (int i = 0; i < 3; i++) {
    ...
    switch (mediaGroup.getType()) {
      case MediaGroup.TYPE_SEARCH:
        return YouTubeMediaGroup.from(getSearchService().continueSearch(nextKey), mediaGroup);
      ...
      case MediaGroup.TYPE_UNDEFINED:
        return YouTubeMediaGroup.from(getBrowseService().continueGridTab(nextKey), mediaGroup);
      default:
        return YouTubeMediaGroup.from(getBrowseService().continueSection(nextKey), mediaGroup);
    }
  }
        ...
        emitter -> {
          ...
          if (playlists != null && playlists.getMediaItems() != null) {
            for (MediaItem playlist : playlists.getMediaItems()) {
              kotlin.Pair<List<MediaGroup>, String> content =
                  getBrowseService2().getChannel(playlist.getChannelId(), playlist.getParams());
