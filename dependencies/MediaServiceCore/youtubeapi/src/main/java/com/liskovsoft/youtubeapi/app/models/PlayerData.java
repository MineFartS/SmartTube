...
public class PlayerData {
  ...
  private static final String FUNCTION_RANDOM_BYTES =
      "var window={};window.crypto={getRandomValues:function(arr){for(var"
          + " i=0;i<arr.length;i++){arr[i]=Math.floor(Math.random()*Math.floor(Math.pow(2,8*arr.BYTES_PER_ELEMENT)))}}};";
  ...
  private static final Pattern SIGNATURE_CLIENT_PLAYBACK_NONCE_V2 =
      Pattern.compile(
          "(function [$\\w]+\\(([\\w])\\)[\\S\\s]*)(function [$\\w]+\\([\\w]\\))([\\S\\s]*)");
   ...
   * entity against attacks which fall under the very large umbrella of "replay".<br>
   * Used with History and other stuff.<br>
   * Note: [\S\s]* - match any char (including new lines) after getRandomValues<br>
   * Player url example:
   * <b>https://www.youtube.com/s/player/e49bfb00/tv-player-ias.vflset/tv-player-ias.js</b>
   */
  @FieldNullable
  @RegExp(
      ";function"
          + " [$\\w]+\\([\\w]?\\)\\{if\\(window\\.crypto&&window\\.crypto\\.getRandomValues[\\S\\s]*?function"
          + " [$\\w]+\\(\\)\\{for\\(var .*[\\w]\\.push\\(\".*\"\\.charAt\\(.*\\)\\);return"
          + " [\\w]\\.join\\(\"\"\\)\\}")
  ...
  @RegExp(
      "function"
          + " [$\\w]+\\([\\w]?\\)\\{if\\(window\\.crypto&&window\\.crypto\\.getRandomValues[\\S\\s]*?function"
          + " [$\\w]+\\([\\w]?\\)\\{.*for\\(\\w+ .*[\\w]\\.push\\(\".*\"\\.charAt\\(.*\\)\\);return"
          + " [\\w]\\.join\\(\"\"\\)\\}")
  ...
  private String getClientPlaybackNonceFunctionV1() {
    return Helpers.replace(
        mClientPlaybackNonceFunctionV1,
        SIGNATURE_CLIENT_PLAYBACK_NONCE_V1,
        "function getClientPlaybackNonce()");
  }

  private String getClientPlaybackNonceFunctionV2() {
    return Helpers.replace(
        mClientPlaybackNonceFunctionV2,
        SIGNATURE_CLIENT_PLAYBACK_NONCE_V2,
  ...
  private static final Pattern SIGNATURE_DECIPHER =
      Pattern.compile("function [$\\w]+\\(([\\w])\\)");
   ...
   */
  @RegExp({
    ";\\w+ [$\\w]+=\\{[\\S\\s]{10,200}?[\\w]\\.reverse\\(\\)[\\S\\s]*?function"
        + " [$\\w]+\\([\\w]\\)\\{.*[\\w]\\.split\\(\"\"\\).*;return [\\w]\\.join\\(\"\"\\)\\}",
  })
  ...
  public String getDecipherFunction() {
    ...
    if (mDecipherFunction != null) {
      deFunc =
          Helpers.replace(mDecipherFunction, SIGNATURE_DECIPHER, "function decipherSignature($1)");
    } else if (mDecipherFunctionPart1 != null && mDecipherFunctionPart2 != null) {
      deFunc =
          Helpers.replace(
                  mDecipherFunctionPart1, SIGNATURE_DECIPHER, "function decipherSignature($1)")
              + ";"
              + mDecipherFunctionPart2;
    }
