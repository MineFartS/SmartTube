...
public class LoungeService {
  ...
  private String getPairingCodeInt() {
    ...
    Call<PairingCodeV2> pairingCodeWrapper =
        mInfoManager.getPairingCodeV2(
            mLoungeToken,
            mScreenId,
            mScreenName,
  ...
  private void startListeningInt(OnCommand callback) throws IOException {
    ...
    if (sessionInfos == null) {
      Log.e(
          TAG,
          "Can't open a session because it's empty. Expired lounge token or too frequent request?");
      mLoungeToken = null;
      return;
    }
    ...
    String url =
        BindParams.createBindRpcUrl(mScreenName, mDeviceId, mLoungeToken, mSessionId, mGSessionId);
    ...
    // Skip initial commands: TYPE_SESSION_ID, TYPE_G_SESSION_ID, TYPE_LOUNGE_STATUS,
    // TYPE_GET_NOW_PLAYING
  ...
  public void postStartPlaying(
      String videoId, long positionMs, long durationMs, boolean isPlaying) {
    Log.d(TAG, "Post nowPlaying id: %s, pos: %s, dur: %s...", videoId, positionMs, durationMs);
    postCommand(
        CommandParams.getNowPlaying(
            videoId, positionMs, durationMs, mCtt, mPlaylistId, mPlaylistIndex));
  ...
  public void postStateChange(long positionMs, long durationMs, boolean isPlaying) {
    ...
    if (durationMs > 0 && positionMs <= durationMs) {
      Log.d(
          TAG,
          "Post onStateChange pos: %s, dur: %s, playing: %s...",
          positionMs,
          durationMs,
          isPlaying);

      Map<String, String> stateChange =
          CommandParams.getOnStateChange(
              positionMs,
              durationMs,
  ...
  private void updateData(CommandItem info) {
    if (info != null && info.getPlaylistParams() != null) {
      ...
      mPlaylistIndex =
          playlistData.getPlaylistIndex() != null
              ? playlistData.getPlaylistIndex()
              : mPlaylistIndex;
      mPlaylistId =
          playlistData.getPlaylistId() != null ? playlistData.getPlaylistId() : mPlaylistId;
    }
  }
  ...
  private CommandList getSessionBind() {
    Call<CommandList> bindDataWrapper =
        mCommandManager.getSessionData(mScreenName, mDeviceId, mLoungeToken, 0);

    return RetrofitHelper.get(bindDataWrapper);
  }

  private CommandList toCommandInfos(String result) {
    return mLineSkipAdapter.read(
  ...
  private void postCommand(Map<String, String> command) {
    ...
    Call<Void> wrapper =
        mCommandManager.postCommand(
            mScreenName, mDeviceId, mLoungeToken, mSessionId, mGSessionId, command);
