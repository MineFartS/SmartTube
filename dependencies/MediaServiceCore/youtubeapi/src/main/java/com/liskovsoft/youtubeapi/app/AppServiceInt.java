...
public class AppServiceInt {
  ...
  protected AppInfo getAppInfo(String userAgent) {
    ...
    if (response != null) {
      // String visitorInfoCookie = RetrofitHelper.getCookie(response,
      // AppConstants.VISITOR_INFO_COOKIE);
      // String visitorPrivacyCookie = RetrofitHelper.getCookie(response,
      // AppConstants.VISITOR_PRIVACY_COOKIE);
  ...
  private static String getLegacyClientUrl(String clientUrl) {
    ...
    return clientUrl.replace("/dg=0/", "/exm=base/ed=1/").replace("/m=base", "/m=main");
  }
  ...
  public String getPlayerUrl() {
    // NOTE: NPE 2.5K
    // return getData().getPlayerUrl() != null ? getData().getPlayerUrl() : mCachedAppInfo != null ?
    // mCachedAppInfo.getPlayerUrl() : null;
