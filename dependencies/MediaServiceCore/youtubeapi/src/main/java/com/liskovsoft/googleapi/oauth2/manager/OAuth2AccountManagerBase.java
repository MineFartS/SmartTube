...
public abstract class OAuth2AccountManagerBase {
  ...
  private static final long TOKEN_REFRESH_PERIOD_MS =
      60 * 60 * 1_000; // NOTE: auth token max lifetime is 60 min
  ...
  protected void updateAuthHeadersIfNeeded() {
    if (mCachedAuthorizationHeader != null
        && Helpers.equals(
            mCachedAuthorizationHeader, RetrofitOkHttpHelper.getAuthHeaders().get("Authorization"))
  ...
  private void syncWithRetrofit() {
    ...
    if (mCachedAuthorizationHeader != null && getSelectedAccount() != null) {
      ...
      if (pageIdToken != null) {
        // Apply branded account rights (restricted videos). Branded refresh token with current
        // account page id.
