...
public final class YouTubeHelper {
  ...
  /**
   * NOTE: Optimal thumbnail index is 3. Lower values cause black borders around images on
   * Chromecast and Sony.
   */
   ...
   * For Kotlin counterpart see: {@link
   * com.liskovsoft.youtubeapi.common.models.gen.CommonHelperKt#getOptimalResThumbnailUrl(ThumbnailItem)}
   */
  public static String findOptimalResThumbnailUrl(List<Thumbnail> thumbnails) {
    ...
    return thumbnails
        .get(size > OPTIMAL_RES_THUMBNAIL_INDEX ? OPTIMAL_RES_THUMBNAIL_INDEX : size - 1)
        .getUrl();
  }

  /**
   * For Kotlin counterpart see: {@link
   * com.liskovsoft.youtubeapi.common.models.gen.CommonHelperKt#getHighResThumbnailUrl(ThumbnailItem)}
   */
  ...
  public static void filterIfNeeded(MediaGroup mediaGroup) {
    ...
    if (mediaGroup.getType() == MediaGroup.TYPE_SUBSCRIPTIONS) {
      ...
      Collections.sort(
          mediaGroup.getMediaItems(),
          (o1, o2) ->
              o1.isLive() == o2.isLive() && o1.isUpcoming() == o2.isUpcoming()
                  ? 0
    ...
    if (GlobalPreferences.isInitialized()) {
      ...
      if (isHideShortsEnabled
          || isHideUpcomingEnabled
          || isHideStreamsEnabled
          || isHideMixesEnabled) {
        ...
        Helpers.removeIf(
            ...
            mediaItem -> {
              ...
              return (isHideShortsEnabled && isShorts(mediaItem))
                  || (isHideUpcomingEnabled && mediaItem.isUpcoming())
                  || (isHideStreamsEnabled && mediaItem.isLive())
                  || (isHideMixesEnabled && isMix(mediaItem));
            });
      }
    }
  }
  ...
  public static boolean isShortsLegacy(MediaItem mediaItem) {
    ...
    boolean isShortsMaxLength =
        lengthMs > 0 && lengthMs <= SHORTS_LEN_MAX_MS && title.contains("#");
    ...
    boolean isShortsUrl =
        imageUrl != null
            && (imageUrl.contains("-AG2CIACgA-")
                || imageUrl.contains("-oaymwEmCIAFEOAD8quKqQMa8AEB-AHOBYAC"));
  ...
  public static boolean isMix(MediaItem mediaItem) {
    ...
    return !mediaItem.isLive()
        && Helpers.hasWords(mediaItem.getBadgeText())
        && mediaItem.getDurationMs() <= 0
        && (mediaItem.getPlaylistId() != null
            || mediaItem.getChannelId() != null
            || mediaItem.hasUploads());
  }
  ...
  public static boolean isEmpty(MediaItem item) {
    ...
    return Helpers.allNulls(
        item.getVideoId(),
        item.getPlaylistId(),
        item.getReloadPageKey(),
        item.getParams(),
        item.getChannelId(),
        item.getSearchQuery());
  }
   ...
   *
   * <p>Some researches needs to be done to know how this parameter, unique at each request, is
   * generated.
   *
   ...
   * A nonce is a unique value chosen by an entity in a protocol, and it is used to protect that
   * entity against attacks which fall under the very large umbrella of "replay".
   */
  @NonNull
