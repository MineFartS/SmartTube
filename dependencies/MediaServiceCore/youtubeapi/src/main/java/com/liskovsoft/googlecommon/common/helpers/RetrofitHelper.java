...
public class RetrofitHelper {
  ...
  public static Retrofit buildRetrofit(Converter.Factory factory) {
    ...
    return builder.addConverterFactory(factory).build();
  }
  ...
  private static <T> void handleResponseErrors(Response<T> response) {
    ...
    if (response.code() == 400 || response.code() == 403 || response.code() == 428) {
      ...
      try (ResponseBody body = response.errorBody()) {
        ...
        try {
          ErrorResponse error =
              errorData != null ? gson.fromJson(errorData, ErrorResponse.class) : null;
          errorMsg =
              error != null && error.getError() != null
                  ? ErrorResponse.class.getSimpleName() + ": " + error.getError().getMessage()
                  : null;
