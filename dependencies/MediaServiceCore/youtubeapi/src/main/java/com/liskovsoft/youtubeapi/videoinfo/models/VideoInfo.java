...
public class VideoInfo {
  ...
  public boolean isUnplayable() {
    return isUnknownRestricted()
        || isVisibilityRestricted()
        || isAgeRestricted()
        || isAdaptiveFormatsBroken();
  }
  ...
  public boolean isStoryboardBroken() {
    return !isLive()
        && getStoryboardSpec() == null
        && (containsAdaptiveVideoInfo() || containsRegularVideoInfo());
  }
  ...
  private boolean isAdaptiveFullHD() {
    return getAdaptiveFormats() != null
        && !getAdaptiveFormats().isEmpty()
        && "1080p".equals(getAdaptiveFormats().get(0).getQualityLabel());
  }
