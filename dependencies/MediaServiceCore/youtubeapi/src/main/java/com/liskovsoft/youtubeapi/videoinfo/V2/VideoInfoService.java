...
public class VideoInfoService extends VideoInfoServiceBase {
  ...
  private static final AppClient[] VIDEO_INFO_TYPE_LIST = {
  ...
  @Nullable private AppClient mVideoInfoType = null;
  @Nullable private AppClient mRecentInfoType = null;
  ...
  private VideoInfo firstInfoWith(
      String videoId, String clickTrackingParams, InfoTester infoTester) {
  ...
  public void switchNextSubtitle() {
    CaptionTrack.sFormat =
        Helpers.getNextValue(CaptionTrack.CaptionFormat.values(), CaptionTrack.sFormat);
  }
  ...
  private VideoInfo getVideoInfoWithRentFix(
      AppClient client, String videoId, String clickTrackingParams) {
  ...
  private VideoInfo getVideoInfo(AppClient client, String videoId, String clickTrackingParams) {
    ...
    String videoInfoQuery =
        VideoInfoApiHelper.getVideoInfoQuery(client, videoId, clickTrackingParams);
  ...
  private VideoInfoHls getVideoInfoIOSHls(String videoId, String clickTrackingParams) {
    String videoInfoQuery =
        VideoInfoApiHelper.getVideoInfoQuery(AppClient.IOS, videoId, clickTrackingParams);
  ...
  private VideoInfoHls getVideoInfoHls(AppClient client, String videoInfoQuery) {
    Call<VideoInfoHls> wrapper =
        mVideoInfoApi.getVideoInfoHls(videoInfoQuery, mAppService.getVisitorData());
  ...
  private void restoreVideoInfoType() {
    ...
    if (videoInfoType != -1) {
      mVideoInfoType =
          videoInfoType < AppClient.values().length ? AppClient.values()[videoInfoType] : null;
  ...
  private void persistRecentTypeIfNeeded(VideoInfo videoInfo) {
    if (videoInfo == null
        || videoInfo.isUnplayable()
        || mRecentInfoType == null
        || mRecentInfoType == mVideoInfoType) {
      return;
    }
  ...
  private static boolean shouldObtainExtendedFormats(VideoInfo result) {
    return getData().isFormatEnabled(MediaServiceData.FORMATS_EXTENDED_HLS)
        && result.isExtendedHlsFormatsBroken();
  }

  private static boolean needMoreSubtitles(VideoInfo videoInfo) {
    return videoInfo != null
        && videoInfo.hasSubtitles()
        && (videoInfo.getTranslationLanguages() == null
            || videoInfo.getTranslationLanguages().size() < 100);
  }
  ...
  private AppClient getDefaultClient() {
    return mVideoInfoType != null && Arrays.asList(VIDEO_INFO_TYPE_LIST).contains(mVideoInfoType)
        ? mVideoInfoType
