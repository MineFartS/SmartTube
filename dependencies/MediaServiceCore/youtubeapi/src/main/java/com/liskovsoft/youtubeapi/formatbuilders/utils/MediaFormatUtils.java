...
public class MediaFormatUtils {
  ...
  public static String extractMimeType(MediaFormat format) {
    ...
    if (codecs.startsWith("vorbis") || codecs.startsWith("opus")) {
      return MIME_WEBM_AUDIO;
    }

    if (codecs.startsWith("vp9") || codecs.startsWith("vp09")) {
      return MIME_WEBM_VIDEO;
    }

    if (codecs.startsWith("mp4a") || codecs.startsWith("ec-3") || codecs.startsWith("ac-3")) {
      return MIME_MP4_AUDIO;
    }

    if (codecs.startsWith("avc") || codecs.startsWith("av01")) {
