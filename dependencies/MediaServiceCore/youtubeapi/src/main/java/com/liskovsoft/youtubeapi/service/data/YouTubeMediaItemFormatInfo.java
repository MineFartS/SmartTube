...
public class YouTubeMediaItemFormatInfo implements MediaItemFormatInfo {
  ...
  public boolean containsMedia() {
    return containsDashUrl()
        || containsHlsUrl()
        || containsAdaptiveVideoFormats()
        || containsUrlFormats();
  }
  ...
  public boolean isCacheActual() {
    ...
    return containsMedia()
        && AppService.instance().isPlayerCacheActual()
  ...
  /**
   * Extracts time from video url (if present). Url examples: <br>
   * "http://example.com?dur=544.99&key=val&key2=val2" <br>
  ...
  private String extractDurationFromTrack() {
    ...
    if (result == null) {
      // throw new IllegalStateException("Videos in the list doesn't have a duration. Content: " +
      // mMP4Videos);
