...
public class YouTubeMediaItemMetadata implements MediaItemMetadata {
  ...
  public static YouTubeMediaItemMetadata from(WatchNextResult watchNextResult) {
    ...
    if (videoOwner != null) {
      ...
      mediaItemMetadata.mAuthorImageUrl =
          YouTubeHelper.findOptimalResThumbnailUrl(videoOwner.getThumbnails());
    ...
    if (videoMetadata != null) {
      String author =
          mediaItemMetadata.mAuthor != null ? mediaItemMetadata.mAuthor : videoMetadata.getByLine();
      String publishedTime =
          videoMetadata.getPublishedTime() != null
              ? videoMetadata.getPublishedTime()
              : videoMetadata.getAlbumName();
      mediaItemMetadata.mTitle = videoMetadata.getTitle();
      mediaItemMetadata.mSecondTitle =
          YouTubeHelper.createInfo(author, publishedTime, videoMetadata.getShortViewCount());
      mediaItemMetadata.mSecondTitleAlt =
          YouTubeHelper.createInfo(
              author, videoMetadata.getPublishedDate(), videoMetadata.getShortViewCount());
    ...
    // Alt path to get like/subscribe status (when no such info in metadata section, e.g. YouTube
    // Music items)
