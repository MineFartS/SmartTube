...
public class YouTubeMPDBuilder implements MPDBuilder {
  ...
  private void writePrologue() {
    ...
    if (isLive()) {
      attribute(
          "",
          "profiles",
          "urn:mpeg:dash:profile:isoff-on-demand:2011"); // Better caching. Default is isoff-main.
  ...
  private void writeMediaTagsForGroup(Set<MediaFormat> items) {
    ...
    writeMediaListPrologue(
        String.valueOf(mId++),
        MediaFormatUtils.extractMimeType(firstItem),
        firstItem.getLanguage());

    // Representation
    for (MediaFormat item : filtered) {
      if (mLimitVideoCodec != null
          && isVideo(item)
          && !item.getMimeType().contains(mLimitVideoCodec)) {
        continue;
      }

      if (mLimitAudioCodec != null
          && isAudio(item)
          && !item.getMimeType().contains(mLimitAudioCodec)) {
        continue;
      }
  ...
  private void writeLiveMediaSegmentList(MediaFormat format) {
    ...
    if (mInfo.isLive() || lengthSeconds <= 0) {
      // For premiere streams (length > 0) or regular streams (length == 0) set window that exceeds
      // normal limits - 48hrs
    ...
    int segmentCount =
        (int) Math.ceil(lengthSeconds * (double) unitsPerSecond / segmentDurationUnits);
    ...
    attribute(
        "",
        "duration",
        segmentDurationUnitsStr); // segment duration in units (could be safely omitted)
  ...
  /**
   * Ensures that required fields are set. If no, initialize them or throw an exception. <br>
  ...
  private void writeOtfSegmentTemplate(
      String mediaUrl, String initUrl, String startNumber, List<OtfSegment> segments) {
  ...
  private static Set<MediaFormat> getFormats(
      Map<String, Set<MediaFormat>> formatMap, String language) {
    if (language == null) {
      language = "default";
    }
