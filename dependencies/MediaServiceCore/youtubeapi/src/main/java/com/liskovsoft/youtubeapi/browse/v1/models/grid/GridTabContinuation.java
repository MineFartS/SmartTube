...
public class GridTabContinuation {
  @JsonPath({
    "$.continuationContents.gridContinuation.items[*]", // other grid like history, subscriptions
    "$.continuationContents.tvSurfaceContentContinuation.content.gridRenderer.items[*]", // user
    // playlist
  ...
  @JsonPath({
    "$.continuationContents.gridContinuation.continuations[0].nextContinuationData.continuation", // other grid like history, subscriptions
   ...
   */
  @JsonPath(
      "$.continuationContents.tvSurfaceContentContinuation.header.tvSurfaceHeaderRenderer.buttons[*].buttonRenderer")
  private List<ChannelButton> mChannelButtons;

  /** Generic button when channel has no content. */
  @JsonPath(
      "$.continuationContents.tvSurfaceContentContinuation.content.genericPromoRenderer.actionButton.buttonRenderer")
