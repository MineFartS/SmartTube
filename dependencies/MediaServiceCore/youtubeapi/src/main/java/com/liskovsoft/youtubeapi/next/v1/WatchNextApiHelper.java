...
public class WatchNextApiHelper {
  ...
  public static String getWatchNextQuery(
      @Nullable String videoId, @Nullable String playlistId, int playlistIndex) {
    return getWatchNextQuery(videoId, playlistId, playlistIndex, null);
  }

  /**
   * Video query from playlist example:
   * "params":"OAI%3D","playlistId":"RDx7g_SWE90O8","playlistIndex": 2,"videoId":"x7g_SWE90O8"<br>
  ...
  public static String getWatchNextQuery(
    ...
    if (playlistId != null) {
      videoData +=
          String.format(
              ",\"playlistId\":\"%s\",\"playlistIndex\":%s",
              playlistId, Math.max(playlistIndex, 0));
    }
