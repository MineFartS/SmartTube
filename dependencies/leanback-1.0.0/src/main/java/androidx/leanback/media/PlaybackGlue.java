 ...
 * implementation of Media Player or a connection to playback Service. App initializes PlaybackGlue
 * subclass, associated it with a {@link PlaybackGlueHost}. {@link PlaybackGlueHost} is typically
 * implemented by a Fragment or an Activity, it provides the environment to render UI for
 * PlaybackGlue object, it optionally provides SurfaceHolder via {@link SurfaceHolderGlueHost} to
 * render video. A typical PlaybackGlue should release resources (e.g. MediaPlayer or connection to
 * playback Service) in {@link #onDetachedFromHost()}. {@link #onDetachedFromHost()} is called in
 * two cases:
 *
 * <ul>
 *   <li>app manually change it using {@link #setHost(PlaybackGlueHost)} call
 *   <li>When host (fragment or activity) is destroyed
 * </ul>
 *
...
public abstract class PlaybackGlue {
  ...
  /**
   * Starts the media player. Does nothing if {@link #isPrepared()} is false. To wait {@link
   * #isPrepared()} to be true before playing, use {@link #playWhenPrepared()}.
   */
  public void play() {}
  ...
  protected void onAttachedToHost(PlaybackGlueHost host) {
    mPlaybackGlueHost = host;
    mPlaybackGlueHost.setHostCallback(
        new PlaybackGlueHost.HostCallback() {
          @Override
   ...
   * and call super.onDetachedFromHost() at last. A typical PlaybackGlue will release resources
