...
/**
 * A helper class for managing a {@link PlaybackControlsRow} being displayed in {@link
 * PlaybackGlueHost}. It supports standard playback control actions play/pause and skip
 * next/previous. This helper class is a glue layer that manages interaction between the leanback UI
 * components {@link PlaybackControlsRow} {@link PlaybackControlsRowPresenter} and a functional
 * {@link PlayerAdapter} which represents the underlying media player.
 *
 * <p>Apps must pass a {@link PlayerAdapter} in the constructor for a specific implementation e.g. a
 * {@link MediaPlayerAdapter}.
 *
 * <p>The glue has two action bars: primary action bars and secondary action bars. Apps can provide
 * additional actions by overriding {@link #onCreatePrimaryActions} and / or {@link
 * #onCreateSecondaryActions} and respond to actions by overriding {@link #onActionClicked(Action)}.
 *
 * <p>The subclass is responsible for implementing the "repeat mode" in {@link #onPlayCompleted()}.
 * Sample Code:
 *
 * <pre><code>
...
public class PlaybackBannerControlGlue<T extends PlayerAdapter> extends PlaybackBaseControlGlue<T> {
  ...
  public static final int ACTION_SKIP_TO_PREVIOUS = PlaybackBaseControlGlue.ACTION_SKIP_TO_PREVIOUS;
  ...
  @SPEED private int mPlaybackSpeed = PLAYBACK_SPEED_PAUSED;
   ...
   *
   * @param context
   * @param fastForwardSpeeds The array of seek speeds for fast forward. The maximum length of the
   *     array is defined as NUMBER_OF_SEEK_SPEEDS.
   * @param rewindSpeeds The array of seek speeds for rewind. The maximum length of the array is
   *     defined as NUMBER_OF_SEEK_SPEEDS.
   * @param impl Implementation to underlying media player.
   */
  public PlaybackBannerControlGlue(
      Context context, int[] fastForwardSpeeds, int[] rewindSpeeds, T impl) {
    super(context, impl);
  ...
  public boolean onKey(View v, int keyCode, KeyEvent event) {
    ...
    if (action == null) {
      action = mControlsRow.getActionForKeyCode(mControlsRow.getSecondaryActionsAdapter(), keyCode);
    }
  ...
  /**
   * Gets current position of the player. If the player is playing/paused, this method returns
   * current position from {@link PlayerAdapter}. Otherwise, if the player is
   * fastforwarding/rewinding, the method fake-pauses the {@link PlayerAdapter} and returns its own
   * calculated position.
   *
