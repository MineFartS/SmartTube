...
/**
 * A helper class for managing a {@link PlaybackControlsRow} and {@link PlaybackGlueHost} that
 * implements a recommended approach to handling standard playback control actions such as
 * play/pause, fast forward/rewind at progressive speed levels, and skip to next/previous. This
 * helper class is a glue layer in that manages the configuration of and interaction between the
 * leanback UI components by defining a functional interface to the media player.
 *
 * <p>You can instantiate a concrete subclass such as MediaPlayerGlue or you must subclass this
 * abstract helper. To create a subclass you must implement all of the abstract methods and the
 * subclass must invoke {@link #onMetadataChanged()} and {@link #onStateChanged()} appropriately.
 *
 ...
 *
 * <p>You may override {@link #onCreateControlsRowAndPresenter()} which will create a {@link
 * PlaybackControlsRow} and a {@link PlaybackControlsRowPresenter}. You may call {@link
 * #setControlsRow(PlaybackControlsRow)} and {@link #setPlaybackRowPresenter(PlaybackRowPresenter)}
 * to customize your own row and presenter.
 *
 * <p>The helper sets a {@link SparseArrayObjectAdapter} on the controls row as the primary actions
 * adapter, and adds actions to it. You can provide additional actions by overriding {@link
 * #onCreatePrimaryActions}. This helper does not deal in secondary actions so those you may add
 * separately.
 *
 * <p>Provide a click listener on your fragment and if an action is clicked, call {@link
 * #onActionClicked}.
 *
 * <p>This helper implements a key event handler. If you pass a {@link PlaybackGlueHost}, it will
 * configure its fragment to intercept all key events. Otherwise, you should set the glue object as
 * key event handler to the ViewHolder when bound by your row presenter; see {@link
 ...
 * to manage the lifecycle of a periodic callback to {@link #updateProgress()}. {@link
...
public abstract class PlaybackControlGlue extends PlaybackGlue
  ...
  /**
   * The adapter key for the first custom control on the left side of the predefined primary
   * controls.
   */
  ...
  /**
   * The adapter key for the first custom control on the right side of the predefined primary
   * controls.
   */
  ...
  /**
   * The initial (level 0) fast forward playback speed. The negative of this value is for rewind at
   * the same speed.
   */
  public static final int PLAYBACK_SPEED_FAST_L0 = 10;

  /**
   * The level 1 fast forward playback speed. The negative of this value is for rewind at the same
   * speed.
   */
  public static final int PLAYBACK_SPEED_FAST_L1 = 11;

  /**
   * The level 2 fast forward playback speed. The negative of this value is for rewind at the same
   * speed.
   */
  public static final int PLAYBACK_SPEED_FAST_L2 = 12;

  /**
   * The level 3 fast forward playback speed. The negative of this value is for rewind at the same
   * speed.
   */
  public static final int PLAYBACK_SPEED_FAST_L3 = 13;

  /**
   * The level 4 fast forward playback speed. The negative of this value is for rewind at the same
   * speed.
   */
  ...
  private static final int NUMBER_OF_SEEK_SPEEDS =
      PLAYBACK_SPEED_FAST_L4 - PLAYBACK_SPEED_FAST_L0 + 1;
  ...
  /**
   * Instantiating a {@link PlaybackControlsRow} and corresponding {@link
  ...
  /**
   * Sets the controls row to be managed by the glue layer. The primary actions and playback state
   * related aspects of the row are updated by the glue.
   */
  public void setControlsRow(PlaybackControlsRow controlsRow) {
    ...
    ArrayObjectAdapter secondaryActions =
        new ArrayObjectAdapter(new ControlButtonPresenterSelector());
   ...
   *
   * @deprecated PlaybackControlGlue supports any PlaybackRowPresenter, use {@link
  ...
  public PlaybackControlsRowPresenter getControlsRowPresenter() {
    return mControlsRowPresenter instanceof PlaybackControlsRowPresenter
        ? (PlaybackControlsRowPresenter) mControlsRowPresenter
        : null;
  }

  /**
   * Sets the controls row Presenter to be passed to {@link PlaybackGlueHost} in {@link
  ...
  /**
   * Returns the playback row Presenter to be passed to {@link PlaybackGlueHost} in {@link
   * #onAttachedToHost(PlaybackGlueHost)}.
   */
  ...
  /**
   * Override this to start/stop a runnable to call {@link #updateProgress} at an interval such as
   * {@link #getUpdatePeriod}.
   */
  public void enableProgressUpdating(boolean enable) {}

  /**
   * Returns the time period in milliseconds that should be used to update the progress. See {@link
   * #updateProgress()}.
   */
  ...
  public boolean onKey(View v, int keyCode, KeyEvent event) {
    switch (keyCode) {
      ...
      case KeyEvent.KEYCODE_ESCAPE:
        boolean abortSeek =
            mPlaybackSpeed >= PLAYBACK_SPEED_FAST_L0 || mPlaybackSpeed <= -PLAYBACK_SPEED_FAST_L0;
        if (abortSeek) {
    ...
    final SparseArrayObjectAdapter primaryActionsAdapter =
        (SparseArrayObjectAdapter) mControlsRow.getPrimaryActionsAdapter();
   ...
   *
   * @param speed The desired playback speed. For normal playback this will be {@link
   *     #PLAYBACK_SPEED_NORMAL}; higher positive values for fast forward, and negative values for
   *     rewind.
   */
  ...
  void updateControlButtons() {
    final SparseArrayObjectAdapter primaryActionsAdapter =
        (SparseArrayObjectAdapter) getControlsRow().getPrimaryActionsAdapter();
    ...
    if ((actions & ACTION_REWIND) != 0 && mRewindAction == null) {
      mRewindAction = new PlaybackControlsRow.RewindAction(getContext(), mRewindSpeeds.length);
    ...
    if ((actions & ACTION_FAST_FORWARD) != 0 && mFastForwardAction == null) {
      mFastForwardAction =
          new PlaybackControlsRow.FastForwardAction(getContext(), mFastForwardSpeeds.length);
  ...
  /**
   * Returns the current playback speed. When playing normally, {@link #PLAYBACK_SPEED_NORMAL}
   * should be returned.
   */
  ...
  /**
   * Must be called appropriately by a subclass when the playback state has changed. It updates the
   * playback state displayed on the media player.
   */
