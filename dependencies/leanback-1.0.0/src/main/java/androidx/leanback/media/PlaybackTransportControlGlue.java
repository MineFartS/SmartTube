 ...
 * next/previous. This helper class is a glue layer in that manages interaction between the leanback
 * UI components {@link PlaybackControlsRow} {@link PlaybackTransportRowPresenter} and a functional
 * {@link PlayerAdapter} which represents the underlying media player.
 *
 * <p>App must pass a {@link PlayerAdapter} in constructor for a specific implementation e.g. a
 * {@link MediaPlayerAdapter}.
 *
 * <p>The glue has two actions bar: primary actions bar and secondary actions bar. App can provide
 * additional actions by overriding {@link #onCreatePrimaryActions} and / or {@link
 * #onCreateSecondaryActions} and respond to actions by override {@link #onActionClicked(Action)}.
 *
 * <p>It's also subclass's responsibility to implement the "repeat mode" in {@link
 * #onPlayCompleted()}.
 *
 * <p>Apps calls {@link #setSeekProvider(PlaybackSeekDataProvider)} to provide seek data. If the
 * {@link PlaybackGlueHost} is instance of {@link PlaybackSeekUi}, the provider will be passed to
 * PlaybackGlueHost to render thumb bitmaps. Sample Code:
 *
 * <pre><code>
...
public class PlaybackTransportControlGlue<T extends PlayerAdapter>
  ...
  private void updatePlaybackState(boolean isPlaying) {
    ...
    if (mPlayPauseAction != null) {
      ...
      if (mPlayPauseAction.getIndex() != index) {
        ...
        notifyItemChanged(
            (ArrayObjectAdapter) getControlsRow().getPrimaryActionsAdapter(), mPlayPauseAction);
      }
    }
  }
  ...
  /**
   * Enable or disable seek when {@link #getSeekProvider()} is null. When true, {@link
   * PlayerAdapter#seekTo(long)} will be called during user seeking.
   *
