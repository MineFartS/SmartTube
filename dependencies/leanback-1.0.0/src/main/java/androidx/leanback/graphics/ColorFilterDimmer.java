...
/**
 * Helper class for applying a dim level to a View. The ColorFilterDimmer uses a ColorFilter in a
 * Paint object to dim the view according to the currently active level.
 */
public final class ColorFilterDimmer {
  ...
  public static ColorFilterDimmer createDefault(Context context) {
    ...
    int dimColor =
        a.getColor(
            R.styleable.LeanbackTheme_overlayDimMaskColor,
    ...
    float dimmedLevel =
        a.getFraction(
            R.styleable.LeanbackTheme_overlayDimDimmedLevel,
            1,
            1,
            context.getResources().getFraction(R.fraction.lb_view_dimmed_level, 1, 1));
    a.recycle();
    return new ColorFilterDimmer(
        ColorFilterCache.getColorFilterCache(dimColor), activeLevel, dimmedLevel);
  }
   ...
   * @param activeLevel The level of dimming when the View is in its active state. Must be a float
   *     value between 0.0 and 1.0.
   * @param dimmedLevel The level of dimming when the View is in its dimmed state. Must be a float
   *     value between 0.0 and 1.0.
   */
  public static ColorFilterDimmer create(
      ColorFilterCache dimmer, float activeLevel, float dimmedLevel) {
  ...
  /**
   * Apply current the ColorFilter to a View. This method will set the hardware layer of the view
   * when applying a filter, and remove it when not applying a filter.
   *
