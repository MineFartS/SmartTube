...
/**
 * Helper class for assigning a dim color to Paint. It holds the alpha value for the current active
 * level.
 */
public final class ColorOverlayDimmer {
   ...
   *
   * @param dimColor The color for fully dimmed. Only the RGB values are used; the alpha channel is
   *     ignored.
  ...
  /**
   * Change the RGB of the color according to current dim level. Maintains the alpha value of the
   * color.
   *
   * @param color The color to apply the dim level to.
   * @return A color with the RGB values adjusted by the alpha of the current dim level.
   */
  ...
  public void drawColorOverlay(Canvas c, View v, boolean includePadding) {
    ...
    if (includePadding) {
      ...
      c.drawRect(
          v.getLeft() + v.getPaddingLeft(),
          v.getTop() + v.getPaddingTop(),
          v.getRight() - v.getPaddingRight(),
          v.getBottom() - v.getPaddingBottom(),
          mPaint);
    }
    c.restore();
  }
