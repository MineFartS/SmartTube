 ...
 * be the first row in a fragment such as the {@link androidx.leanback.app.DetailsFragment}. The
 * View created by the FullWidthDetailsOverviewRowPresenter is made in three parts: logo view on the
 * left, action list view on the top and a customizable detailed description view on the right.
 *
 * <p>The detailed description is rendered using a {@link Presenter} passed in {@link
 * #FullWidthDetailsOverviewRowPresenter(Presenter)}. Typically this will be an instance of {@link
 ...
 * {@link #FullWidthDetailsOverviewRowPresenter(Presenter, DetailsOverviewLogoPresenter)}. The
 * application can access the logo ViewHolder from {@link ViewHolder#getLogoViewHolder()}.
 *
 * <p>To support activity shared element transition, call {@link #setListener(Listener)} with {@link
 * FullWidthDetailsOverviewSharedElementHelper} during Activity's onCreate(). Application is free to
 * create its own "shared element helper" class using the Listener for image binding. Call {@link
 * #setParticipatingEntranceTransition(boolean)} with false
 *
 * <p>The view has three states: {@link #STATE_HALF} {@link #STATE_FULL} and {@link #STATE_SMALL}.
 * See {@link androidx.leanback.app.DetailsFragment} where it switches states based on selected row
 * position.
 */
public class FullWidthDetailsOverviewRowPresenter extends RowPresenter {
  ...
  /**
   * This is the default state corresponding to layout file. The view takes full width of screen and
   * covers bottom half of the screen.
   */
  ...
  public class ViewHolder extends RowPresenter.ViewHolder {
      ...
      if (ibvh == null) {
        ...
        if (getOnItemViewSelectedListener() != null) {
          getOnItemViewSelectedListener()
              .onItemSelected(ibvh.getViewHolder(), ibvh.getItem(), ViewHolder.this, getRow());
        }
      }
    }
    ;
    ...
    void checkFirstAndLastPosition(boolean fromScroll) {
      ...
      boolean showRight =
          (viewHolder == null || viewHolder.itemView.getRight() > mActionsRow.getWidth());
     ...
     */
    public ViewHolder(
        View rootView, Presenter detailsPresenter, DetailsOverviewLogoPresenter logoPresenter) {
      super(rootView);
      ...
      final int fadeLength =
          rootView
              .getResources()
              .getDimensionPixelSize(R.dimen.lb_details_overview_actions_fade_size);
      ...
      mDetailsLogoViewHolder =
          (DetailsOverviewLogoPresenter.ViewHolder) logoPresenter.onCreateViewHolder(mOverviewRoot);
    ...
    /**
     * Returns current state of the ViewHolder set by {@link
     * FullWidthDetailsOverviewRowPresenter#setState(ViewHolder, int)}.
     */
   ...
   *
   * @param detailsPresenter The {@link Presenter} used to render the detailed description of the
   *     row.
   */
  ...
  public FullWidthDetailsOverviewRowPresenter(
      Presenter detailsPresenter, DetailsOverviewLogoPresenter logoPresenter) {
  ...
  /** Returns the background color. If {@link #setBackgroundColor(int)}, transparent is returned. */
  ...
  /**
   * Returns the background color of actions. If {@link #setActionsBackgroundColor(int)} is not
   * called, transparent is returned.
   */
  ...
  protected RowPresenter.ViewHolder createRowViewHolder(ViewGroup parent) {
    ...
    vh.mActionsRow.setOnUnhandledKeyListener(
  ...
  /**
   * Called by {@link DetailsOverviewLogoPresenter} to notify logo was bound to view. Application
  ...
  /**
   * Layout logo position based on current state. Subclass may override. The method is called when a
   * logo is bound to view or state changes.
   *
  ...
  protected void onLayoutLogo(ViewHolder viewHolder, int oldState, boolean logoChanged) {
    ...
    switch (mAlignmentMode) {
      ...
      default:
        lp.setMarginStart(
            v.getResources().getDimensionPixelSize(R.dimen.lb_details_v2_logo_margin_start));
        break;
      case ALIGN_MODE_MIDDLE:
        lp.setMarginStart(
            v.getResources().getDimensionPixelSize(R.dimen.lb_details_v2_left) - lp.width);
        break;
    }
    ...
    switch (viewHolder.getState()) {
      ...
      case STATE_HALF:
        lp.topMargin =
            v.getResources().getDimensionPixelSize(R.dimen.lb_details_v2_blank_height)
                + v.getResources().getDimensionPixelSize(R.dimen.lb_details_v2_actions_height)
                + v.getResources()
                    .getDimensionPixelSize(R.dimen.lb_details_v2_description_margin_top);
        break;
      case STATE_SMALL:
        lp.topMargin = 0;
        break;
  ...
  /**
   * Layout overview frame based on current state. Subclass may override. The method is called when
   * a logo is bound to view or state changes.
   *
   * @param viewHolder The row ViewHolder that contains the logo.
   * @param oldState The old state, can be same as current viewHolder.getState()
   * @param logoChanged Whether logo was changed.
  ...
  protected void onLayoutOverviewFrame(ViewHolder viewHolder, int oldState, boolean logoChanged) {
    ...
    if (wasBanner != isBanner || logoChanged) {
      ...
      int logoWidth = 0;
      if (mDetailsOverviewLogoPresenter.isBoundToImage(
          viewHolder.getLogoViewHolder(), (DetailsOverviewRow) viewHolder.getRow())) {
      ...
      switch (mAlignmentMode) {
        ...
        default:
          if (isBanner) {
            ...
            frameMarginStart = 0;
            descriptionMarginStart =
                logoWidth + res.getDimensionPixelSize(R.dimen.lb_details_v2_logo_margin_start);
          }
          break;
        case ALIGN_MODE_MIDDLE:
          if (isBanner) {
      ...
      lpFrame.topMargin =
          isBanner ? 0 : res.getDimensionPixelSize(R.dimen.lb_details_v2_blank_height);
   ...
   * @param state New state, can be {@link #STATE_FULL}, {@link #STATE_HALF} or {@link
   *     #STATE_SMALL}.
   */
  ...
  /**
   * Called when {@link ViewHolder#getState()} changes. Subclass may override. The default
   * implementation calls {@link #onLayoutLogo(ViewHolder, int, boolean)} and {@link
