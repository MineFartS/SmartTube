...
/**
 * A widget that draws a search affordance, represented by a round background and an icon. The
...
public class SearchOrbView extends FrameLayout implements View.OnClickListener {
  ...
  public static class Colors {
    ...
    /**
     * Constructs a color set using the given color for the search orb. Other colors are provided by
     * the framework.
     *
    ...
    /**
     * Constructs a color set using the given colors for the search orb. Other colors are provided
     * by the framework.
     *
     * @param color The main search orb color.
    ...
    @ColorInt public int brightColor;

    /** A color used to tint the search orb icon. */
    @ColorInt public int iconColor;
  ...
  public SearchOrbView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.lbSearchOrbView, defStyleAttr, 0);
   ...
   *
   * <p>Orb color animation is handled automatically when the orb is focused/unfocused, however, an
   * app may choose to override the current animation state, for example when an activity is paused.
   */
    ...
    if (mColorAnimationEnabled && mAttachedToWindow) {
      ...
      mColorAnimator =
          ValueAnimator.ofObject(
              mColorEvaluator, mColors.color, mColors.brightColor, mColors.color);
