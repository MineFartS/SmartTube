...
/**
 * A dynamic data structure that caches staggered grid position information for each individual
 * child. The algorithm ensures that each row will be kept as balanced as possible when prepending
 * and appending a child.
 *
 * <p>You may keep view {@link StaggeredGrid.Location} inside StaggeredGrid as much as possible
 * since prepending and appending views is not symmetric: layout going from 0 to N will likely
 * produce a different result than layout going from N to 0 for the staggered cases. If a user
 * scrolls from 0 to N then scrolls back to 0 and we don't keep history location information, edges
 * of the very beginning of rows will not be aligned. It is recommended to keep a list of tens of
 * thousands of {@link StaggeredGrid.Location}s which will be big enough to remember a typical
 * user's scroll history.
 *
 * <p>This class is abstract and can be replaced with different implementations.
 */
abstract class StaggeredGrid extends Grid {
  ...
  public static class Location extends Grid.Location {
    /**
     * Offset to previous item location. min_edge(index) - min_edge(index - 1) for non reversed case
  ...
  /**
   * Returns index of first item (cached or visible) in the staggered grid. Returns negative value
   * if no item.
   */
  ...
  /**
   * Returns index of last item (cached or visible) in the staggered grid. Returns negative value if
   * no item.
   */
  ...
  /**
   * Prepends items using cached locations, returning true if toLimit is reached. This method should
  ...
  /**
   * Appends items using cached locations, returning true if at least one item is appended and
   * (oneColumnMode is true or reach limit and aboveIndex). This method should only be called by
   * appendVisibleItems()
   */
  ...
  /**
   * algorithm of layout staggered grid, this method should only be called by appendVisibleItems().
   */
