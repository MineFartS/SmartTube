...
/**
 * A {@link android.view.ViewGroup} that shows items in a horizontal scrolling list. The items come
 * from the {@link RecyclerView.Adapter} associated with this view.
 *
 * <p>{@link RecyclerView.Adapter} can optionally implement {@link FacetProviderAdapter} which
 * provides {@link FacetProvider} for a given view type; {@link RecyclerView.ViewHolder} can also
 * implement {@link FacetProvider}. Facet from ViewHolder has a higher priority than the one from
 * FacetProviderAdapter associated with viewType. Supported optional facets are:
 *
 * <ol>
 ...
 *       HorizontalGridView. This facet also allows multiple alignment positions within one
 *       ViewHolder.
 * </ol>
 */
public class HorizontalGridView extends BaseGridView {
   ...
   * Sets the row height.
   *
   * @param height May be {@link android.view.ViewGroup.LayoutParams#WRAP_CONTENT WRAP_CONTENT}, or
   *     a size in pixels. If zero, row height will be fixed based on number of rows and view
   *     height.
   */
  ...
  /**
   * Sets the fade out left edge to transparent. Note turn on fading edge is very expensive that you
   * should turn off when HorizontalGridView is scrolling.
   */
  ...
  /**
   * Sets the distance in pixels between fading start position and left padding edge. The fading
   * start position is positive when start position is inside left padding area. Default value is 0,
   * means that the fading starts from left padding edge.
   */
  ...
  /**
   * Returns the distance in pixels between fading start position and left padding edge. The fading
   * start position is positive when start position is inside left padding area. Default value is 0,
   * means that the fading starts from left padding edge.
   */
  ...
  /**
   * Sets the fade out right edge to transparent. Note turn on fading edge is very expensive that
   * you should turn off when HorizontalGridView is scrolling.
   */
  ...
  /**
   * Returns the distance in pixels between fading start position and right padding edge. The fading
   * start position is positive when start position is inside right padding area. Default value is
   * 0, means that the fading starts from right padding edge.
   */
  ...
  /**
   * Sets the distance in pixels between fading start position and right padding edge. The fading
   * start position is positive when start position is inside right padding area. Default value is
   * 0, means that the fading starts from right padding edge.
   */
  ...
  private boolean needsFadingHighEdge() {
    ...
    for (int i = c - 1; i >= 0; i--) {
      View view = getChildAt(i);
      if (mLayoutManager.getOpticalRight(view)
          > getWidth() - getPaddingRight() + mHighFadeShaderOffset) {
        return true;
      }
    }
    return false;
  ...
  private Bitmap getTempBitmapLow() {
    if (mTempBitmapLow == null
      ...
      mTempBitmapLow =
          Bitmap.createBitmap(mLowFadeShaderLength, getHeight(), Bitmap.Config.ARGB_8888);
    }
  ...
  private Bitmap getTempBitmapHigh() {
    if (mTempBitmapHigh == null
      ...
      if (false
        ...
        mTempBitmapHigh =
            Bitmap.createBitmap(mHighFadeShaderLength, getHeight(), Bitmap.Config.ARGB_8888);
      }
    }
  ...
  public void draw(Canvas canvas) {
    ...
    int highEdge =
        mFadingHighEdge
            ? getWidth() - getPaddingRight() + mHighFadeShaderOffset + mHighFadeShaderLength
            : getWidth();

    // draw not-fade content
    int save = canvas.save();
    canvas.clipRect(
        lowEdge + (mFadingLowEdge ? mLowFadeShaderLength : 0),
        0,
        highEdge - (mFadingHighEdge ? mHighFadeShaderLength : 0),
        getHeight());
    super.draw(canvas);
  ...
  /**
   * Updates the layer type for this view. If fading edges are needed, use a hardware layer. This
   * works around the problem that when a child invalidates itself (for example has an animated
   * background), the parent view must also be invalidated to refresh the display list which updates
