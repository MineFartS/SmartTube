...
/**
 * Parallax tracks a list of dynamic {@link Property}s typically representing foreground UI element
 * positions on screen. Parallax keeps a list of {@link ParallaxEffect} objects which define rules
 * to mapping property values to {@link ParallaxTarget}.
 *
 * <p>Example: <code>
 ...
 * </code>
 *
 * <p>To create a {@link ParallaxEffect}, user calls {@link #addEffect(PropertyMarkerValue[])} with
 * a list of {@link PropertyMarkerValue} which defines the range of {@link Parallax.IntProperty} or
 * {@link Parallax.FloatProperty}. Then user adds {@link ParallaxTarget} into {@link
 * ParallaxEffect}.
 *
 * <p>App may subclass {@link Parallax.IntProperty} or {@link Parallax.FloatProperty} to supply
 * additional information about how to retrieve Property value. {@link RecyclerViewParallax} is a
 * great example of Parallax implementation tracking child view positions on screen.
 *
 * <p>
 *
 * <ul>
 *   Restrictions of properties
 *   <li>FloatProperty and IntProperty cannot be mixed in one Parallax
 *   <li>Values must be in ascending order.
 *   <li>If the UI element is unknown above screen, use UNKNOWN_BEFORE.
 *   <li>if the UI element is unknown below screen, use UNKNOWN_AFTER.
 *   <li>UNKNOWN_BEFORE and UNKNOWN_AFTER are not allowed to be next to each other.
 * </ul>
 *
 * These rules will be verified at runtime.
 *
 * <p>Subclass must override {@link #updateValues()} to update property values and perform {@link
 * ParallaxEffect}s. Subclass may call {@link #updateValues()} automatically e.g. {@link
 * RecyclerViewParallax} calls {@link #updateValues()} in RecyclerView scrolling. App might call
 * {@link #updateValues()} manually when Parallax is unaware of the value change. For example, when
 * a slide transition is running, {@link RecyclerViewParallax} is unaware of translation value
...
public abstract class Parallax<PropertyT extends android.util.Property> {
  ...
  /**
   * IntProperty provide access to an index based integer type property inside {@link Parallax}. The
   * IntProperty typically represents UI element position inside {@link Parallax}.
   */
  public static class IntProperty extends Property<Parallax, Integer> {

    /**
     * Property value is unknown and it's smaller than minimal value of Parallax. For example if a
     * child is not created and before the first visible child of RecyclerView.
     */
    public static final int UNKNOWN_BEFORE = Integer.MIN_VALUE;

    /**
     * Property value is unknown and it's larger than {@link Parallax#getMaxValue()}. For example if
     * a child is not created and after the last visible child of RecyclerView.
     */
    ...
    /**
     * Creates an {@link PropertyMarkerValue} object for the marker value representing {@link
    ...
    /**
     * Creates an {@link PropertyMarkerValue} object for a fraction of {@link
     * Parallax#getMaxValue()}.
     *
     * @param fractionOfMaxValue 0 to 1 fraction to multiply with {@link Parallax#getMaxValue()} for
     *     the marker value.
     * @return A new {@link PropertyMarkerValue} object.
     */
    ...
    /**
     * Create an {@link PropertyMarkerValue} object by multiplying the fraction with {@link
     * Parallax#getMaxValue()} and adding offsetValue to it.
     *
     * @param offsetValue An offset integer value to be added to marker value.
     * @param fractionOfMaxParentVisibleSize 0 to 1 fraction to multiply with {@link
     *     Parallax#getMaxValue()} for the marker value.
     * @return A new {@link PropertyMarkerValue} object.
     */
  ...
  static class IntPropertyMarkerValue extends PropertyMarkerValue<IntProperty> {
    ...
    final int getMarkerValue(Parallax source) {
      return mFactionOfMax == 0
          ? mValue
          : mValue + Math.round(source.getMaxValue() * mFactionOfMax);
    }
  }

  /**
   * FloatProperty provide access to an index based integer type property inside {@link Parallax}.
   * The FloatProperty typically represents UI element position inside {@link Parallax}.
   */
  ...
  static class FloatPropertyMarkerValue extends PropertyMarkerValue<FloatProperty> {
    ...
    final float getMarkerValue(Parallax source) {
      return mFactionOfMax == 0 ? mValue : mValue + source.getMaxValue() * mFactionOfMax;
    }
  }
  ...
  /**
   * Return the max value which is typically size of parent visible area, e.g. RecyclerView's height
   * if we are tracking Y position of a child. The size can be used to calculate marker value using
   * the provided fraction of FloatPropertyMarkerValue.
   *
  ...
  /**
   * Add a new IntProperty in the Parallax object. App may override {@link #createProperty(String,
   * int)}.
   *
  ...
  /**
   * Verify sanity of property values, throws RuntimeException if fails. The property values must be
   * in ascending order. UNKNOW_BEFORE and UNKNOWN_AFTER are not allowed to be next to each other.
   */
    ...
    for (int i = 1; i < mProperties.size(); i++) {
      ...
      if (v < last) {
        throw new IllegalStateException(
            String.format(
                "Parallax Property[%d]\"%s\" is" + " smaller than Property[%d]\"%s\"",
                i, mProperties.get(i).getName(), i - 1, mProperties.get(i - 1).getName()));
      } else if (last == IntProperty.UNKNOWN_BEFORE && v == IntProperty.UNKNOWN_AFTER) {
        throw new IllegalStateException(
            String.format(
                "Parallax Property[%d]\"%s\" is"
                    + " UNKNOWN_BEFORE and Property[%d]\"%s\" is UNKNOWN_AFTER",
                i - 1, mProperties.get(i - 1).getName(), i, mProperties.get(i).getName()));
      }
      last = v;
    }
  }
      ...
      if (v < last) {
        throw new IllegalStateException(
            String.format(
                "Parallax Property[%d]\"%s\" is" + " smaller than Property[%d]\"%s\"",
                i, mProperties.get(i).getName(), i - 1, mProperties.get(i - 1).getName()));
      } else if (last == FloatProperty.UNKNOWN_BEFORE && v == FloatProperty.UNKNOWN_AFTER) {
        throw new IllegalStateException(
            String.format(
                "Parallax Property[%d]\"%s\" is"
                    + " UNKNOWN_BEFORE and Property[%d]\"%s\" is UNKNOWN_AFTER",
  ...
  /**
   * Create a new Property object. App does not directly call this method. See {@link
   * #addProperty(String)}.
   *
