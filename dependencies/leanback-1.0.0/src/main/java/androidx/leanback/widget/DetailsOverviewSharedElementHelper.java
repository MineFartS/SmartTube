...
final class DetailsOverviewSharedElementHelper extends SharedElementCallback {
  ...
  public void onSharedElementStart(
    ...
    if (mRightPanelWidth != 0 && mRightPanelHeight != 0) {
      rightPanel.measure(
          MeasureSpec.makeMeasureSpec(mRightPanelWidth, MeasureSpec.EXACTLY),
          MeasureSpec.makeMeasureSpec(mRightPanelHeight, MeasureSpec.EXACTLY));
      rightPanel.layout(
          width,
          rightPanel.getTop(),
          width + mRightPanelWidth,
  ...
  void onBindToDrawable(ViewHolder vh) {
    ...
    if (mViewHolder != null) {
      if (DEBUG) {
        Log.d(
            TAG,
            "rebind? clear transitionName on current viewHolder " + mViewHolder.mOverviewFrame);
      }
    ...
    mViewHolder = vh;
    mViewHolder.mRightPanel.addOnLayoutChangeListener(
        new View.OnLayoutChangeListener() {
          ...
          public void onLayoutChange(
            ...
            if (DEBUG) {
              Log.d(
                  TAG,
                  "onLayoutChange records size of right panel as "
                      + mRightPanelWidth
                      + ", "
                      + mRightPanelHeight);
            }
    ...
    mViewHolder.mRightPanel.postOnAnimation(
        new Runnable() {
          @Override
          public void run() {
            if (DEBUG) {
