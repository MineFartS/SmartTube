...
/**
 * Abstract {@link Presenter} class for rendering media items in a playlist format. Media item data
 * provided for this presenter can implement the interface {@link MultiActionsProvider}, if the
 * media rows wish to contain custom actions. Media items in the playlist are arranged as a vertical
 * list with each row holding each media item's details provided by the user of this class and a set
 * of optional custom actions. Each media item's details and actions are separately focusable. The
 * appearance of each one of the media row components can be controlled through setting theme's
 * attributes. Each media item row provides a view flipper for switching between different views
 * depending on the playback state. A default layout is provided by this presenter for rendering
 * different playback states, or a custom layout can be provided by the user by overriding the
 * playbackMediaItemNumberViewFlipperLayout attribute in the currently specified theme. Subclasses
 * should also override {@link #getMediaPlayState(Object)} to provide the current play state of
 * their media item model in case they wish to use different views depending on the playback state.
 * The presenter can optionally provide line separators between media rows by setting {@link
 * #setHasMediaRowSeparator(boolean)} to true.
 *
 * <p>Subclasses must override {@link #onBindMediaDetails} to implement their media item model data
 * binding to each row view.
 *
 * <p>The {@link OnItemViewClickedListener} and {@link OnItemViewSelectedListener} can be used in
 * the same fashion to handle selection or click events on either of media details or each
 * individual action views.
 *
 * <p>{@link AbstractMediaListHeaderPresenter} can be used in conjunction with this presenter in
...
public abstract class AbstractMediaItemPresenter extends RowPresenter {
   ...
   * Constructor used for creating an abstract media item presenter.
   *
   * @param themeId The resource id of the theme that defines attributes controlling the appearance
   *     of different widgets in a media item row.
   */
  ...
  /**
   * The ViewHolder for the {@link AbstractMediaItemPresenter}. It references different views that
   * place different meta-data corresponding to a media item details, actions, selector, listeners,
   * and presenters,
   */
  public static class ViewHolder extends RowPresenter.ViewHolder {
    ...
    public ViewHolder(View view) {
      ...
      mMediaItemActionsContainer = (ViewGroup) view.findViewById(R.id.mediaItemActionsContainer);
                ...
                public void onClick(View view) {
                  if (getOnItemViewClickedListener() != null) {
                    getOnItemViewClickedListener()
                        .onItemClicked(null, null, ViewHolder.this, getRowObject());
                  }
                }
              });
      getMediaItemDetailsView()
          .setOnFocusChangeListener(
              new View.OnFocusChangeListener() {
                ...
                public void onFocusChange(View view, boolean hasFocus) {
                  mFocusViewAnimator =
                      updateSelector(mSelectorView, view, mFocusViewAnimator, true);
                }
              });
      ...
      boolean found =
          view.getContext()
              .getTheme()
              .resolveAttribute(R.attr.playbackMediaItemNumberViewFlipperLayout, typedValue, true);
      View mergeView =
          LayoutInflater.from(view.getContext())
              .inflate(
                  found ? typedValue.resourceId : R.layout.lb_media_item_number_view_flipper,
                  mMediaItemNumberViewFlipper,
                  true);
    ...
    /**
     * Binds the actions in a media item row object to their views. This consists of creating (or
     * reusing the existing) action view holders, and populating them with the actions' icons.
     */
    public void onBindRowActions() {
      for (int i = getMediaItemActionsContainer().getChildCount() - 1;
          i >= mActionViewHolders.size();
          i--) {
      ...
      for (int i = mActionViewHolders.size(); i < actionList.length; i++) {
        ...
        actionViewHolder.view.setOnFocusChangeListener(
            new View.OnFocusChangeListener() {
              @Override
              public void onFocusChange(View view, boolean hasFocus) {
              ...
              public void onClick(View view) {
                if (getOnItemViewClickedListener() != null) {
                  getOnItemViewClickedListener()
                      .onItemClicked(
                          actionViewHolder,
                          mMediaItemRowActions[actionIndex],
                          ViewHolder.this,
                          getRowObject());
                }
              }
    ...
    /**
     * Notifies the content of the media item details in a row has changed and triggers updating the
     * UI. This causes {@link #onBindMediaDetails(ViewHolder, Object)} on the user's provided
     * presenter to be called back, allowing them to update UI accordingly.
     */
    ...
    /**
     * Notifies the playback state of the media item row has changed. This in turn triggers updating
     * of the UI for that media item row if corresponding views are specified for each playback
     * state. By default, 3 views are provided for each playback state, or these views can be
    ...
    /**
     * @return The SelectorView responsible for highlighting the in-focus view within each media
     *     item row
     */
    ...
    /**
     * @return The FlipperView responsible for flipping between different media item number views
    ...
    /**
     * @return The TextView responsible for rendering the media item number. This view is rendered
     *     when the media item row is neither playing nor paused.
     */
    ...
    /**
     * Flips to the view at index 'position'. This position corresponds to the index of a particular
     * view within the ViewFlipper layout specified for the MediaItemNumberView (see
     * playbackMediaItemNumberViewFlipperLayout attribute).
     *
    ...
    /**
     * Returns the view displayed when the media item is neither playing nor paused, corresponding
     * to the playback state of PLAY_STATE_INITIAL.
     *
  ...
  protected void onBindRowViewHolder(RowPresenter.ViewHolder vh, Object item) {
    ...
    mvh.getMediaItemRowSeparator().setVisibility(hasMediaRowSeparator() ? View.VISIBLE : View.GONE);
  ...
  /**
   * Sets the background color for the row views within the playlist. If this is not set, a default
   * color, defaultBrandColor, from theme is used. This defaultBrandColor defaults to
   * android:attr/colorPrimary on v21, if it's specified.
   *
  ...
  /**
   * Binds the media item details to their views provided by the {@link AbstractMediaItemPresenter}.
   * This method is to be overridden by the users of this presenter. The subclasses of this
   * presenter can access and bind individual views for either of the media item number, name, or
   * duration (depending on whichever views are visible according to the providing theme
   * attributes), by calling {@link ViewHolder#getMediaItemNumberView()}, {@link
   * ViewHolder#getMediaItemNameView()}, and {@link ViewHolder#getMediaItemDurationView()}, on the
   * {@link ViewHolder} provided as the argument {@code vh} of this presenter.
   *
  ...
  /**
   * Unbinds the media item details from their views provided by the {@link
   * AbstractMediaItemPresenter}. This method can be overridden by the subclasses of this presenter
   * if required.
   *
  ...
  /**
   * Binds the media item number view to the appropriate play state view of the media item. The play
   * state of the media item is extracted by calling {@link #getMediaPlayState(Object)} for the
   * media item embedded within this view. This method triggers updating of the playback state UI if
   * corresponding views are specified for the current playback state. By default, 3 views are
   * provided for each playback state, or these views can be provided by the user.
   */
   ...
   * PLAY_STATE_INITIAL which causes the media item number {@link ViewHolder#getMediaItemNameView()}
   * to be displayed for different playback states. Users of this class should override this method
   * in order to provide the play state of their custom media item data model.
   *
  ...
  /**
   * Each media item row can have multiple focusable elements; the details on the left and a set of
   * optional custom actions on the right. The selector is a highlight that moves to highlight to
   * cover whichever views is in focus.
   *
   ...
   * @param layoutAnimator the ValueAnimator producing animation frames for the selector's width and
   *     x-translation, generated by this method and stored for the each {@link ViewHolder}.
   * @param isDetails Whether the changed-focused view is for a media item details (true) or an
   *     action (false).
   */
  static ValueAnimator updateSelector(
      final View selectorView,
      View focusChangedView,
    ...
    if (!focusChangedView.hasFocus()) {
      ...
      final ViewGroup.MarginLayoutParams lp =
          (ViewGroup.MarginLayoutParams) selectorView.getLayoutParams();
