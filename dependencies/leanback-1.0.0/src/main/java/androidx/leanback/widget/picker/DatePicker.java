...
/**
 * {@link DatePicker} is a directly subclass of {@link Picker}. This class is a widget for selecting
 * a date. The date can be selected by a year, month, and day Columns. The "minDate" and "maxDate"
 * from which dates to be selected can be customized. The columns can be customized by attribute
 * "datePickerFormat" or {@link #setDatePickerFormat(String)}.
 *
...
public class DatePicker extends Picker {
  ...
  public DatePicker(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    final TypedArray attributesArray =
        context.obtainStyledAttributes(attrs, R.styleable.lbDatePicker);
  ...
  /**
   * Returns the best localized representation of the date for the given date format and the current
   * locale.
   *
   * @param datePickerFormat The date format skeleton (e.g. "dMy") used to gather the appropriate
   *     representation of the date in the current locale.
  ...
  String getBestYearMonthDayPattern(String datePickerFormat) {
    ...
    if (PickerUtility.SUPPORTS_BEST_DATE_TIME_PATTERN) {
      yearPattern =
          android.text.format.DateFormat.getBestDateTimePattern(mConstant.locale, datePickerFormat);
    } else {
  ...
  /**
   * Extracts the separators used to separate date fields (including before the first and after the
   * last date field). The separators can vary based on the individual locale date format, defined
   * in the Unicode CLDR and cannot be supposed to be "/".
   *
   * <p>See http://unicode.org/cldr/trac/browser/trunk/common/main
   *
   * <p>For example, for Croatian in dMy format, the best localized representation is "d. M. y".
   * This method returns {"", ".", ".", "."}, where the first separator indicates nothing needs to
   * be displayed to the left of the day field, "." needs to be displayed tos the right of the day
   * field, and so forth.
   *
    ...
    if (separators.size() != (datePickerFormat.length() + 1)) {
      throw new IllegalStateException(
          "Separators size: "
              + separators.size()
              + " must equal"
              + " the size of datePickerFormat: "
              + datePickerFormat.length()
              + " + 1");
    }
  ...
  /**
   * Get format of showing dates. For example "YMD". Default value is from {@link
  ...
  private void updateCurrentLocale() {
    mConstant =
        PickerUtility.getDateConstantInstance(Locale.getDefault(), getContext().getResources());
  ...
  /**
   * Sets the minimal date supported by this {@link DatePicker} in milliseconds since January 1,
  ...
  /**
   * Gets current date value in milliseconds since January 1, 1970 00:00:00 in {@link
  ...
  void updateSpinnersImpl(boolean animation) {
    ...
    for (int i = DATE_FIELDS.length - 1; i >= 0; i--) {
      ...
      if (dateFieldIndices[i] < 0) continue;
      ...
      if (allLargerDateFieldsHaveBeenEqualToMinDate) {
        dateFieldChanged |= updateMin(currPickerColumn, mMinDate.get(currField));
      } else {
        dateFieldChanged |= updateMin(currPickerColumn, mCurrentDate.getActualMinimum(currField));
      }

      if (allLargerDateFieldsHaveBeenEqualToMaxDate) {
        dateFieldChanged |= updateMax(currPickerColumn, mMaxDate.get(currField));
      } else {
        dateFieldChanged |= updateMax(currPickerColumn, mCurrentDate.getActualMaximum(currField));
      }
