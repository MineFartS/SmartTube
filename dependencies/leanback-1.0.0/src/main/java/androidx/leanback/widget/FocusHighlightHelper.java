...
public class FocusHighlightHelper {
  ...
  static class BrowseItemFocusHighlight implements FocusHighlightHandler {
    ...
    private float getScale(Resources res) {
      return mScaleIndex == ZOOM_FACTOR_NONE ? 1f : res.getFraction(getResId(mScaleIndex), 1, 1);
    }
    ...
    private FocusAnimator getOrCreateAnimator(View view) {
      ...
      if (animator == null) {
        animator = new FocusAnimator(view, getScale(view.getResources()), mUseDimmer, DURATION_MS);
  ...
  /**
   * Sets up the focus highlight behavior of a focused item in browse list row. App usually does not
   * call this method, it uses {@link ListRowPresenter#ListRowPresenter(int, boolean)}.
   *
   * @param zoomIndex One of {@link FocusHighlight#ZOOM_FACTOR_SMALL} {@link
   *     FocusHighlight#ZOOM_FACTOR_XSMALL} {@link FocusHighlight#ZOOM_FACTOR_MEDIUM} {@link
   *     FocusHighlight#ZOOM_FACTOR_LARGE} {@link FocusHighlight#ZOOM_FACTOR_NONE}.
  ...
  public static void setupBrowseItemFocusHighlight(
      ItemBridgeAdapter adapter, int zoomIndex, boolean useDimmer) {
    adapter.setFocusHighlight(new BrowseItemFocusHighlight(zoomIndex, useDimmer));
  }

  /**
   * Sets up default focus highlight behavior of a focused item in header list. It would scale the
   * focused item and update {@link
   * RowHeaderPresenter#onSelectLevelChanged(RowHeaderPresenter.ViewHolder)}. Equivalent to call
   ...
   *
   * @param gridView The header list.
   * @param scaleEnabled True if scale the item when focused, false otherwise. Note that {@link
   *     RowHeaderPresenter#onSelectLevelChanged(RowHeaderPresenter.ViewHolder)} will always be
   *     called regardless value of scaleEnabled.
  ...
  public static void setupHeaderItemFocusHighlight(
      VerticalGridView gridView, boolean scaleEnabled) {
   ...
   */
  public static void setupHeaderItemFocusHighlight(
      ItemBridgeAdapter adapter, boolean scaleEnabled) {
  ...
  static class HeaderItemFocusHighlight implements FocusHighlightHandler {
    ...
    static class HeaderFocusAnimator extends FocusAnimator {
      ...
      HeaderFocusAnimator(View view, float scale, int duration) {
        ...
        if (parent != null) {
          mViewHolder =
              (ItemBridgeAdapter.ViewHolder) ((RecyclerView) parent).getChildViewHolder(view);
        }
      }
      ...
      void setFocusLevel(float level) {
        ...
        if (presenter instanceof RowHeaderPresenter) {
          ((RowHeaderPresenter) presenter)
              .setSelectLevel(((RowHeaderPresenter.ViewHolder) mViewHolder.getViewHolder()), level);
        }
