...
public class ArrayObjectAdapter extends ObjectAdapter {
   ...
   * @return Index of the first occurrence of the item in the adapter, or -1 if not found.
   */
  ...
  /**
   * Notify that the content of a range of items changed. Note that this is not same as items being
   * added or removed.
   *
  ...
  /**
   * Inserts an item into this adapter at the specified index. If the index is > {@link #size} an
  ...
  /**
   * Adds the objects in the given collection to the adapter, starting at the given index. If the
   * index is >= {@link #size} an exception will be thrown.
   *
  ...
  /**
   * Replaces item at position with a new item and calls notifyItemRangeChanged() at the given
   * position. Note that this method does not compare new item to existing item.
   *
  ...
  /**
   * Removes a range of items from the adapter. The range is specified by giving the starting
   * position and the number of elements to remove.
   *
   ...
   * @param callback Optional DiffCallback Object to compute the difference between the old data set
   *     and new data set. When null, {@link #notifyChanged()} will be fired.
   */
            ...
            new DiffUtil.Callback() {
              ...
              public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                return callback.areItemsTheSame(
                    mOldItems.get(oldItemPosition), itemList.get(newItemPosition));
              }
