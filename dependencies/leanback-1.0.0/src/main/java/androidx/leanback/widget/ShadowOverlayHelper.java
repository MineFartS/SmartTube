...
/**
 * ShadowOverlayHelper is a helper class for shadow, overlay color and rounded corner. There are
 * many choices to implement Shadow, overlay color. Initialize it with ShadowOverlayHelper.Builder
 * and it decides the best strategy based on options user choose and current platform version.
 * <li>For shadow: it may use 9-patch with opticalBounds or Z-value based shadow for API >= 21. When
 *     9-patch is used, it requires a ShadowOverlayContainer to include 9-patch views.
 * <li>For overlay: it may use ShadowOverlayContainer which overrides draw() or it may use
 *     setForeground(new ColorDrawable()) for API>=23. The foreground support might be disabled if
 *     rounded corner is applied due to performance reason.
 * <li>For rounded-corner: it uses a ViewOutlineProvider for API>=21.
 *
 *     <p>There are two different strategies: use Wrapper with a ShadowOverlayContainer; or apply
 *     rounded corner, overlay and rounded-corner to the view itself. Below is an example of how
 *     helper is used. <code>
...
public final class ShadowOverlayHelper {
  ...
  public static final class Builder {
    ...
    /**
     * Set if prefer z-order shadow. On old devices, z-order shadow might be slow, set to false to
     * fall back to static 9-patch shadow. Recommend to read from system wide Setting value: see
     * {@link Settings}.
     *
    ...
    /**
     * Set if not using foreground drawable for overlay color. For example if the view has already
     * assigned a foreground drawable for other purposes. When it's true, helper will use a
     * ShadowOverlayContainer for overlay color.
     *
    ...
    public ShadowOverlayHelper build(Context context) {
      ...
      if (helper.mNeedsShadow) {
        ...
        if (!preferZOrder || !supportsDynamicShadow()) {
          ...
          helper.mNeedsWrapper =
              ((!supportsForeground() || keepForegroundDrawable) && helper.mNeedsOverlay);
        }
      } else {
  ...
  public static final class Options {
     ...
     *
     * @param roundedCornerRadius Number of pixels of rounded corner radius. Set to 0 to use default
     *     settings.
   ...
   * shadow. Depending on Shadow type, optical bounds might be applied.
   */
  ...
  /**
   * Returns true if a "wrapper" ShadowOverlayContainer is needed. When needsWrapper() is true, call
   * {@link #createShadowOverlayContainer(Context)} to create the wrapper.
   */
  ...
  public ShadowOverlayContainer createShadowOverlayContainer(Context context) {
    ...
    return new ShadowOverlayContainer(
        context, mShadowType, mNeedsOverlay, mUnfocusedZ, mFocusedZ, mRoundedCornerRadius);
  }

  /**
   * Set overlay color for view other than ShadowOverlayContainer. See also {@link
  ...
  /**
   * Set overlay color for view, it can be a ShadowOverlayContainer if needsWrapper() is true, or
   * other view type.
   */
    ...
    if (!needsWrapper()) {
      if (!mNeedsShadow) {
        ...
        if (mShadowType == SHADOW_DYNAMIC) {
          Object tag =
              ShadowHelper.addDynamicShadow(view, mUnfocusedZ, mFocusedZ, mRoundedCornerRadius);
  ...
  /**
   * Set shadow focus level (0 to 1). 0 for unfocused, 1 for fully focused. This is for view other
   * than ShadowOverlayContainer. See also {@link
  ...
  void setupRoundedCornerRadius(Options options, Context context) {
    if (options.getRoundedCornerRadius() == 0) {
      Resources res = context.getResources();
      mRoundedCornerRadius = res.getDimensionPixelSize(R.dimen.lb_rounded_rect_corner_radius);
    } else {
