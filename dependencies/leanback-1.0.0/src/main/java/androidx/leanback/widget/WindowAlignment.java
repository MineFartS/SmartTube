...
class WindowAlignment {
  ...
  public static class Axis {
    ...
    public final void setWindowAlignmentOffsetPercent(float percent) {
      if ((percent < 0 || percent > 100) && percent != WINDOW_ALIGN_OFFSET_PERCENT_DISABLED) {
    ...
    public final void updateMinMax(
        int minEdge, int maxEdge, int minChildViewCenter, int maxChildViewCenter) {
      mMinEdge = minEdge;
      mMaxEdge = maxEdge;
        ...
        if (!mReversedFlow) {
          if ((mWindowAlignment & WINDOW_ALIGN_LOW_EDGE) != 0) {
            if (isPreferKeylineOverLowEdge()) {
              ...
              // minScroll
              mMinScroll =
                  Math.min(mMinScroll, calculateScrollToKeyLine(maxChildViewCenter, keyLine));
            }
            ...
            if (isPreferKeylineOverHighEdge()) {
              ...
              // maxScroll
              mMaxScroll =
                  Math.max(mMaxScroll, calculateScrollToKeyLine(minChildViewCenter, keyLine));
            }
    ...
    /**
     * Get scroll distance of align an item (depends on ALIGN_LOW_EDGE, ALIGN_HIGH_EDGE or the item
     * should be aligned to key line). The scroll distance will be capped by {@link #getMinScroll()}
     * and {@link #getMaxScroll()}.
     */
