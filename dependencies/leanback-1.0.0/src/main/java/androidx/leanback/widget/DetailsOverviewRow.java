...
/**
 * An overview {@link Row} for a details fragment. This row consists of an image, a description
 * view, and optionally a series of {@link Action}s that can be taken for the item.
 *
 * <h3>Actions</h3>
 *
 * Application uses {@link #setActionsAdapter(ObjectAdapter)} to set actions on the overview row.
 * {@link SparseArrayObjectAdapter} is recommended for easily updating actions while maintaining the
 * order. The application can add or remove actions on the UI thread after the row is bound to a
 * view.
 *
 * <h3>Updating main item</h3>
 *
 * After the row is bound to a view, the application may call {@link #setItem(Object)} on UI thread
 ...
 * #setImageBitmap(Context, Bitmap)} or {@link #setImageDrawable(Drawable)} on the UI thread, and
...
public class DetailsOverviewRow extends Row {
  ...
  /**
   * Sets the main item for the details page. Must be called on UI thread after row is bound to
   * view.
   */
  ...
  /**
   * Sets a drawable as the image of this details overview. Must be called on UI thread after row is
   * bound to view.
   *
  ...
  /**
   * Sets a Bitmap as the image of this details overview. Must be called on UI thread after row is
   * bound to view.
   *
  ...
  /**
   * Allows or disallows scaling up of images. Images will always be scaled down if necessary. Must
   * be called on UI thread after row is bound to view.
   */
  ...
  /**
   * Returns the actions adapter. Throws ClassCastException if the current actions adapter is not an
   * instance of {@link ArrayObjectAdapter}.
   */
  ...
  /**
   * Adds an Action to the overview. It will throw ClassCastException if the current actions adapter
   * is not an instance of {@link ArrayObjectAdapter}. Must be called on the UI thread.
   *
   ...
   * current actions adapter is not an instance of f{@link ArrayObjectAdapter}. Must be called on
   * the UI thread.
   *
  ...
  /**
   * Removes the given Action from the overview. It will throw ClassCastException if current actions
   * adapter is not {@link ArrayObjectAdapter}. Must be called on UI thread.
   *
   ...
   * ClassCastException if current actions adapter is not {@link ArrayObjectAdapter}. Must be called
   * on UI thread.
   *
