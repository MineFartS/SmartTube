...
/**
 * An abstract {@link Presenter} that renders an Object in RowsFragment, the object can be subclass
 * {@link Row} or a generic one. When the object is not {@link Row} class, {@link
 * ViewHolder#getRow()} returns null.
 *
 * <h3>Customize UI widgets</h3>
 *
 * When a subclass of RowPresenter adds UI widgets, it should subclass {@link
 * RowPresenter.ViewHolder} and override {@link #createRowViewHolder(ViewGroup)} and {@link
 * #initializeRowViewHolder(ViewHolder)}. The subclass must use layout id "row_content" for the
 * widget that will be aligned to the title of any {@link HeadersFragment} that may exist in the
 * parent fragment. RowPresenter contains an optional and replaceable {@link RowHeaderPresenter}
 * that renders the header. You can disable the default rendering or replace the Presenter with a
 * new header presenter by calling {@link #setHeaderPresenter(RowHeaderPresenter)}.
 *
 ...
 *
 * <ul>
 *   <li>A row is selected via {@link #setRowViewSelected(Presenter.ViewHolder, boolean)}. The event
 ...
 *       "Activated status" below). Subclasses of RowPresenter may override {@link
 *       #onRowViewSelected(ViewHolder, boolean)}.
 *   <li>A row is expanded to full height via {@link #setRowViewExpanded(Presenter.ViewHolder,
 *       boolean)} when BrowseFragment hides fast lane on the left. The event is triggered
 *       immediately before the expand animation is started. Row title is shown when row is
 *       expanded. Expanded status may control activated status of the row (see "Activated status"
 *       below). Subclasses of RowPresenter may override {@link #onRowViewExpanded(ViewHolder,
 *       boolean)}.
 * </ul>
 *
 * <h3>Activated status</h3>
 *
 * The activated status of a row is applied to the row view and its children via {@link
 * View#setActivated(boolean)}. The activated status is typically used to control {@link
 * BaseCardView} info region visibility. The row's activated status can be controlled by selected
 * status and/or expanded status. Call {@link #setSyncActivatePolicy(int)} and choose one of the
 * four policies:
 *
 * <ul>
 *   <li>{@link #SYNC_ACTIVATED_TO_EXPANDED} Activated status is synced with row expanded status
 *   <li>{@link #SYNC_ACTIVATED_TO_SELECTED} Activated status is synced with row selected status
 *   <li>{@link #SYNC_ACTIVATED_TO_EXPANDED_AND_SELECTED} Activated status is set to true when both
 *       expanded and selected status are true
 *   <li>{@link #SYNC_ACTIVATED_CUSTOM} Activated status is not controlled by selected status or
 *       expanded status, application can control activated status by its own. Application should
 *       call {@link RowPresenter.ViewHolder#setActivated(boolean)} to change activated status of
 *       row view.
 * </ul>
 *
 * <h3>User events</h3>
 ...
 * If a subclass wants to add its own {@link View.OnFocusChangeListener} or {@link
 * View.OnClickListener}, it must do that in {@link #createRowViewHolder(ViewGroup)} to be properly
 * chained by the library. Adding View listeners after {@link #createRowViewHolder(ViewGroup)} is
 * undefined and may result in incorrect behavior by the library's listeners.
 *
 * <h3>Selection animation</h3>
 *
 * <p>When a user scrolls through rows, a fragment will initiate animation and call {@link
 * #setSelectLevel(Presenter.ViewHolder, float)} with float value between 0 and 1. By default, the
 * RowPresenter draws a dim overlay on top of the row view for views that are not selected.
 * Subclasses may override this default effect by having {@link #isUsingDefaultSelectEffect()}
 * return false and overriding {@link #onSelectLevelChanged(ViewHolder)} to apply a different
 * selection effect.
 *
 * <p>Call {@link #setSelectEffectEnabled(boolean)} to enable/disable the select effect, This will
 * not only enable/disable the default dim effect but also subclasses must respect this flag as
 * well.
 */
public abstract class RowPresenter extends Presenter {
  ...
  public static class ViewHolder extends Presenter.ViewHolder {
    ...
    /**
     * Sets the row view's activated status. The status will be applied to children through {@link
     * #syncActivatedStatus(View)}. Application should only call this function when {@link
     * RowPresenter#getSyncActivatePolicy()} is {@link RowPresenter#SYNC_ACTIVATED_CUSTOM};
     * otherwise the value will be overwritten when expanded or selected status changes.
     */
    ...
    /**
     * Synchronizes the activated status of view to the last value passed through {@link
     * RowPresenter.ViewHolder#setActivated(boolean)}. No operation if {@link
     * RowPresenter.ViewHolder#setActivated(boolean)} is never called. Normally application does not
     * need to call this method, {@link ListRowPresenter} automatically calls this method when a
     * child is attached to list row. However if application writes its own custom RowPresenter, it
     * should call this method when attaches a child to the row view.
     */
    ...
    /**
     * Sets the listener for item or row selection. RowPresenter fires row selection event with null
     * item. A subclass of RowPresenter e.g. {@link ListRowPresenter} may fire a selection event
     * with selected item.
     */
     ...
     * OnItemViewClickedListener will override {@link View.OnClickListener} that item presenter sets
  ...
  /**
   * Called to create a ViewHolder object for a Row. Subclasses will override this method to return
   * a different concrete ViewHolder object.
   *
  ...
  /**
   * Returns true if the Row view should clip its children. The clipChildren flag is set on view in
   * {@link #initializeRowViewHolder(ViewHolder)}. Note that Slide transition or explode transition
   * need turn off clipChildren. Default value is false.
   */
  ...
  /**
   * Called after a {@link RowPresenter.ViewHolder} is created for a Row. Subclasses may override
   * this method and start by calling super.initializeRowViewHolder(ViewHolder).
   *
  ...
  /**
   * Sets the Presenter used for rendering the header. Can be null to disable header rendering. The
   * method must be called before creating any Row Views.
   */
  ...
  /** Returns the {@link RowPresenter.ViewHolder} from the given RowPresenter ViewHolder. */
   ...
   * respond to expanded state changes of a Row. The default implementation will hide/show the
   * header view. Subclasses may make visual changes to the Row View but must not create animation
   * on the Row view.
   */
  ...
  /**
   * This method is only called from {@link #onRowViewSelected(ViewHolder, boolean)}
   * onRowViewSelected. The default behavior is to signal row selected events with a null item
   * parameter. A Subclass of RowPresenter having child items should override this method and
   * dispatch events with item information.
   */
  ...
  /**
   * Called when the given row view changes selection state. A subclass may override this to respond
   * to selected state changes of a Row. A subclass may make visual changes to Row view but must not
   * create animation on the Row view.
   */
  ...
  /**
   * Sets the current select level to a value between 0 (unselected) and 1 (selected). Subclasses
   * may override {@link #onSelectLevelChanged(ViewHolder)} to respond to changes in the selected
   * level.
   */
  ...
  /**
   * Callback when the select level changes. The default implementation applies the select level to
   * {@link RowHeaderPresenter#setSelectLevel(RowHeaderPresenter.ViewHolder, float)} when {@link
   * #getSelectEffectEnabled()} is true. Subclasses may override this function and implement a
   * different select effect. In this case, the method {@link #isUsingDefaultSelectEffect()} should
   * also be overridden to disable the default dimming effect.
   */
  protected void onSelectLevelChanged(ViewHolder vh) {
    if (getSelectEffectEnabled()) {
      ...
      if (isUsingDefaultSelectEffect()) {
        ((RowContainerView) vh.mContainerViewHolder.view)
            .setForegroundColor(vh.mColorDimmer.getPaint().getColor());
      }
    }
  }

  /**
   * Enables or disables the row selection effect. This will not only affect the default dim effect,
   * but subclasses must respect this flag as well.
   */
  ...
  /**
   * Returns true if the row selection effect is enabled. This value not only determines whether the
   * default dim implementation is used, but subclasses must also respect this flag.
   */
  ...
  /**
   * Returns true if this RowPresenter is using the default dimming effect. A subclass may (most
   * likely) return false and override {@link #onSelectLevelChanged(ViewHolder)}.
   */
  ...
  /**
   * Binds the given row object to the given ViewHolder. Derived classes of {@link RowPresenter}
   * overriding {@link #onBindRowViewHolder(ViewHolder, Object)} must call through the super class's
  ...
  /**
   * Unbinds the given ViewHolder. Derived classes of {@link RowPresenter} overriding {@link
   * #onUnbindRowViewHolder(ViewHolder)} must call through the super class's implementation of this
   * method.
   */
  ...
  /**
   * Freezes/unfreezes the row, typically used when a transition starts/ends. This method is called
   * by the fragment, it should not call it directly by the application.
   */
   ...
   * super.setEntranceTransitionState(). This method is called by the fragment, it should not be
   * called directly by the application.
   *
   * @param holder The ViewHolder of the row.
   * @param afterEntrance true if children of row participating in entrance transition should be set
   *     to visible, false otherwise.
   */
