...
/**
 * A grid is representation of single or multiple rows layout data structure and algorithm. Grid is
 * the base class for single row, non-staggered grid and staggered grid.
 *
 * <p>To use the Grid, user must implement a Provider to create or remove visible item. Grid
 * maintains a list of visible items. Visible items are created when user calls appendVisibleItems()
 * or prependVisibleItems() with certain limitation (e.g. a max edge that append up to). Visible
 * items are deleted when user calls removeInvisibleItemsAtEnd() or removeInvisibleItemsAtFront().
 * Grid's algorithm uses size of visible item returned from Provider.createItem() to decide which
 * row to add a new visible item and may cache the algorithm results. User must call
 * invalidateItemsAfter() when it detects item size changed to ask Grid to remove cached results.
 */
abstract class Grid {

  /**
   * A constant representing a default starting index, indicating that the developer did not provide
   * a start index.
   */
  ...
  /**
   * When user uses Grid, he should provide count of items and the method to create item and remove
   * item.
   */
  public static interface Provider {
    ...
    /**
     * @return Min index to prepend, usually it's 0; but in the preLayout case, when grid was
     *     showing 5,6,7. Removing 3,4,5 will make the layoutPosition to be 3(deleted),4,5 in
     *     prelayout pass; Grid's index is still 5,6,7 in prelayout. When we prepend in prelayout,
     *     we can call createItem(4), createItem(3), createItem(2), the minimal index is 2, which is
     *     also the delta to mapping to layoutPosition in prelayout pass.
     */
    int getMinIndex();

    /**
     * Create visible item and where the provider should measure it. The call is always followed by
     * addItem().
     *
     * @param index 0-based index of the item in provider
     * @param append True if new item is after last visible item, false if new item is before first
     *     visible item.
     * @param item item[0] returns created item that will be passed in addItem() call.
     * @param disappearingItem The item is a disappearing item added by {@link
     *     Grid#fillDisappearingItems(int[], int, SparseIntArray)}.
    ...
    /**
     * Get edge of an existing visible item. edge will be the min_edge if not reversed or the
     * max_edge if reversed.
     *
     * @param index 0-based index of the item in provider
     */
  ...
  /**
   * Sets number of rows to fill into. For views that represent a horizontal list, this will be the
   * rows of the view. For views that represent a vertical list, this will be the columns.
   *
  ...
  /**
   * Returns index of first visible item in the staggered grid. Returns negative value if no visible
   * item.
   */
  ...
  /**
   * Returns index of last visible item in the staggered grid. Returns negative value if no visible
   * item.
   */
  ...
  /**
   * Invalidate items after or equal to index. This will remove visible items after that and
   * invalidate cache of layout results after that. Note that it's client's responsibility to
   * perform removing child action, {@link Provider#removeItem(int)} will not be called because the
  ...
  /**
   * Finds the largest or smallest row min edge of visible items, the row index is returned in
   * indices[0], the item index is returned in indices[1].
   */
  public final int findRowMin(boolean findLarge, @Nullable int[] indices) {
    return findRowMin(findLarge, mReversedFlow ? mLastVisibleIndex : mFirstVisibleIndex, indices);
  }

  /**
   * Finds the largest or smallest row min edge of visible items, starts searching from indexLimit,
  ...
  public final int findRowMax(boolean findLarge, @Nullable int[] indices) {
    return findRowMax(findLarge, mReversedFlow ? mFirstVisibleIndex : mLastVisibleIndex, indices);
  }

  /**
   * Find largest or smallest row max edge of visible items, starts searching from indexLimit, the
   * row index is returned in indices[0], the item index is returned in indices[1].
   */
  ...
  /**
   * Return array of int array for all rows, each int array contains visible item positions in pair
   * on that row between startPos(included) and endPositions(included). Returned value is read only,
   * do not change it.
   *
   * <p>E.g. First row has 3,7,8, second row has 4,5,6. getItemPositionsInRows(3, 8) returns {
   * {3,3,7,8}, {4,6} }
   */
  ...
  /**
   * Return array of int array for all rows, each int array contains visible item positions in pair
   * on that row. Returned value is read only, do not change it.
   *
   * <p>E.g. First row has 3,7,8, second row has 4,5,6 { {3,3,7,8}, {4,6} }
   */
  ...
  /**
   * Prepends items and stops after one column is filled. (i.e. filled items from row 0 to row
   * mNumRows - 1)
   *
  ...
  /**
   * Prepends items until first item or reaches toLimit (min edge when not reversed or max edge when
   * reversed)
   */
   ...
   * reversed).
   *
   * @param oneColumnMode true when fills one column and stops, false when checks if condition
   *     matches before filling first column.
   * @return true if at least one item is filled.
   */
  protected abstract boolean prependVisibleItems(int toLimit, boolean oneColumnMode);

  /**
   * Appends items and stops after one column is filled. (i.e. filled items from row 0 to row
   * mNumRows - 1)
   *
   * @return true if at least one item is filled.
   */
  ...
  /**
   * Append items until last item or reaches toLimit (max edge when not reversed or min edge when
   * reversed)
   */
  ...
  /**
   * Appends items until last or reaches toLimit (high edge when not reversed or low edge when
   * reversed).
   *
   * @param oneColumnMode True when fills one column and stops, false when checks if condition
   *     matches before filling first column.
   ...
   */
  public void fillDisappearingItems(
      int[] positions, int positionsLength, SparseIntArray positionToRow) {
    final int lastPos = getLastVisibleIndex();
    final int resultSearchLast =
        lastPos >= 0 ? Arrays.binarySearch(positions, 0, positionsLength, lastPos) : 0;
    ...
    final int resultSearchFirst =
        firstPos >= 0 ? Arrays.binarySearch(positions, 0, positionsLength, firstPos) : 0;
