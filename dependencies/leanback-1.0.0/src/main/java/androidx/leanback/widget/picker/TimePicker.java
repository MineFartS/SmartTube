 ...
 *
 * <p>This class is a widget for selecting time and displays it according to the formatting for the
 * current system locale. The time can be selected by hour, minute, and AM/PM picker columns. The
 * AM/PM mode is determined by either explicitly setting the current mode through {@link
 * #setIs24Hour(boolean)} or the widget attribute {@code is24HourFormat} (true for 24-hour mode,
 * false for 12-hour mode). Otherwise, TimePicker retrieves the mode based on the current context.
 * In 24-hour mode, TimePicker displays only the hour and minute columns.
 *
 * <p>This widget can show the current time as the initial value if {@code useCurrentTime} is set to
...
public class TimePicker extends Picker {
  ...
  /**
   * Constructor called when inflating a TimePicker widget. This version uses a default style of 0,
   * so the only attribute values applied are those in the Context's Theme and the given
   * AttributeSet.
   *
   ...
   *     resource that supplies default values for the widget. Can be 0 to not look for defaults.
   */
  public TimePicker(Context context, AttributeSet attrs, int defStyleAttr) {
    super(context, attrs, defStyleAttr);

    mConstant = PickerUtility.getTimeConstantInstance(Locale.getDefault(), context.getResources());

    final TypedArray attributesArray =
        context.obtainStyledAttributes(attrs, R.styleable.lbTimePicker);
    mIs24hFormat =
        attributesArray.getBoolean(
            R.styleable.lbTimePicker_is24HourFormat, DateFormat.is24HourFormat(context));
    boolean useCurrentTime =
        attributesArray.getBoolean(R.styleable.lbTimePicker_useCurrentTime, true);
    ...
    if (useCurrentTime) {
      Calendar currentDate = PickerUtility.getCalendarForLocale(null, mConstant.locale);
  ...
  String getBestHourMinutePattern() {
    ...
    if (PickerUtility.SUPPORTS_BEST_DATE_TIME_PATTERN) {
      hourPattern =
          DateFormat.getBestDateTimePattern(mConstant.locale, mIs24hFormat ? "Hma" : "hma");
    } else {
  ...
  /**
   * Extracts the separators used to separate time fields (including before the first and after the
   * last time field). The separators can vary based on the individual locale and 12 or 24 hour time
   * format, defined in the Unicode CLDR and cannot be supposed to be ":".
   *
   * <p>See http://unicode.org/cldr/trac/browser/trunk/common/main
   *
   * <p>For example, for english in 12 hour format (time pattern of "h:mm a"), this will return {"",
   * ":", "", ""}, where the first separator indicates nothing needs to be displayed to the left of
   * the hour field, ":" needs to be displayed to the right of hour field, and so forth.
   *
  ...
  private String extractTimeFields() {
    ...
    boolean isRTL =
        TextUtils.getLayoutDirectionFromLocale(mConstant.locale) == View.LAYOUT_DIRECTION_RTL;
    boolean isAmPmAtEnd =
        (hmaPattern.indexOf('a') >= 0) ? (hmaPattern.indexOf("a") > hmaPattern.indexOf("m")) : true;
   ...
   *
   * @param is24Hour {@code true} to display in 24-hour mode, {@code false} ti display in 12-hour
   *     mode with AM/PM.
   * @see #is24Hour()
   */
  ...
  /**
   * @return {@code true} if this widget displays time in 24-hour mode, {@code false} otherwise.
   ...
   *
   * @return {@code true} if the currently selected time is in PM, {@code false} if the currently
   *     selected time in in AM.
   */
  public boolean isPm() {
