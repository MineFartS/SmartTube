...
/**
 * ParallaxEffect class drives changes in {@link ParallaxTarget} in response to changes in variables
 * defined in {@link Parallax}.
 *
 * <p>ParallaxEffect has a list of {@link Parallax.PropertyMarkerValue}s which represents the range
 * of values that source variables can take. The main function is {@link
 * ParallaxEffect#performMapping(Parallax)} which computes a fraction between 0 and 1 based on the
 * current values of variables in {@link Parallax}. As the parallax effect goes on, the fraction
 * increases from 0 at beginning to 1 at the end. Then the fraction is passed on to {@link
 * ParallaxTarget#update(float)}.
 *
 * <p>App use {@link Parallax#addEffect(PropertyMarkerValue...)} to create a ParallaxEffect.
 */
public abstract class ParallaxEffect {
  ...
  /**
   * Sets the list of {@link PropertyMarkerValue}s, which represents the range of values that source
   * variables can take.
   *
  ...
  /**
   * Creates a {@link ParallaxTarget} from {@link PropertyValuesHolder} and adds it to the list of
   * targets.
   *
   ...
   */
  public final <T, V extends Number> ParallaxEffect target(
      T targetObject, Property<T, V> targetProperty) {
   ...
   * variables in {@link Parallax}. As the parallax effect goes on, the fraction increases from 0 at
   * beginning to 1 at the end.
   *
  ...
  /**
   * This method is expected to get the current value of the single {@link IntProperty} or {@link
   * FloatProperty}.
   *
   ...
   * fraction inside a range to fraction of whole range. e.g. four marker values, three weight
   * values: 6, 2, 2. totalWeights are 6, 8, 10 When markerValueIndex is 3, the fraction is inside
   * last range. adjusted_fraction = 8 / 10 + 2 / 10 * fraction.
   */
  ...
  static final class IntEffect extends ParallaxEffect {
    ...
    Number calculateDirectValue(Parallax source) {
      ...
      int value1 = ((Parallax.IntPropertyMarkerValue) mMarkerValues.get(0)).getMarkerValue(source);
      int value2 = ((Parallax.IntPropertyMarkerValue) mMarkerValues.get(1)).getMarkerValue(source);
      if (value1 > value2) {
    ...
    float calculateFraction(Parallax source) {
      ...
      for (int i = 0; i < mMarkerValues.size(); i++) {
        Parallax.IntPropertyMarkerValue k = (Parallax.IntPropertyMarkerValue) mMarkerValues.get(i);
        ...
        if (i == 0) {
          ...
          if (lastIndex == index && lastMarkerValue < markerValue) {
            throw new IllegalStateException(
                "marker value of same variable must be " + "descendant order");
          }
  ...
  static final class FloatEffect extends ParallaxEffect {
    ...
    Number calculateDirectValue(Parallax source) {
      ...
      float value1 = ((FloatPropertyMarkerValue) mMarkerValues.get(0)).getMarkerValue(source);
      float value2 = ((FloatPropertyMarkerValue) mMarkerValues.get(1)).getMarkerValue(source);
      if (value1 > value2) {
