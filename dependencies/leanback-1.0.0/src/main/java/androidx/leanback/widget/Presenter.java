...
/**
 * A Presenter is used to generate {@link View}s and bind Objects to them on demand. It is closely
 * related to the concept of an {@link RecyclerView.Adapter RecyclerView.Adapter}, but is not
 * position-based. The leanback framework implements the adapter concept using {@link ObjectAdapter}
 * which refers to a Presenter (or {@link PresenterSelector}) instance.
 *
 * <p>Presenters should be stateless. Presenters typically extend {@link ViewHolder} to store all
 * necessary view state information, such as references to child views to be used when binding to
 * avoid expensive calls to {@link View#findViewById(int)}.
 *
 * <p>A trivial Presenter that takes a string and renders it into a {@link android.widget.TextView
 * TextView}:
 *
 ...
 * </pre>
 *
 * In addition to view creation and binding, Presenter allows dynamic interface (facet) to be added:
 * {@link #setFacet(Class, Object)}. Supported facets:
 * <li>{@link ItemAlignmentFacet} is used by {@link HorizontalGridView} and {@link VerticalGridView}
 *     to customize child alignment.
 */
public abstract class Presenter implements FacetProvider {
  /**
   * ViewHolder can be subclassed and used to cache any view accessors needed to improve binding
   * performance (for example, results of findViewById) without needing to subclass a View.
   */
  ...
  public abstract static class ViewHolderTask {
    /**
  ...
  /**
   * Unbinds a {@link View} from an item. Any expensive references may be released here, and any
   * fields that are not bound for every item should be cleared here.
   */
   ...
   *
   * <p>This can be used as a reasonable signal that the view is about to be seen by the user. If
   * the adapter previously freed any resources in {@link #onViewDetachedFromWindow(ViewHolder)}
   ...
   *
   * <p>Becoming detached from the window is not necessarily a permanent condition; the consumer of
   * an presenter's views may choose to cache views offscreen while they are not visible, attaching
   * and detaching them as appropriate. Any view property animations should be cancelled here or the
   * view may fail to be recycled.
   *
   ...
   *
   * <p>The default implementation sets the click listener on the root view in the view holder. If
   * the root view isn't focusable this method should be overridden to set the listener on the
   * appropriate focusable child view(s).
   *
