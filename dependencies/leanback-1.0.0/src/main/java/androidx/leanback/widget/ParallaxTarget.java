 ...
 * or the {@link #directUpdate(Number)} method when {@link #isDirectMapping()} is true. When {@link
 * #isDirectMapping()} is false, {@link ParallaxEffect} transforms the values of {@link Parallax},
 * which represents the current state of UI, into a float value between 0 and 1. That float value is
 * passed into {@link #update(float)} method.
 */
public abstract class ParallaxTarget {

  /**
   * Implementation class is supposed to update target with the provided fraction (between 0 and 1).
   * The fraction represents percentage of completed change (e.g. scroll) on target. Called only
   * when {@link #isDirectMapping()} is false.
   *
  ...
  /**
   * Returns true if the ParallaxTarget is directly mapping from source value, {@link
   * #directUpdate(Number)} will be used to update value, otherwise update(fraction) will be called
   * to update value. Default implementation returns false.
   *
  ...
  /**
   * PropertyValuesHolderTarget is an implementation of {@link ParallaxTarget} that uses {@link
  ...
  public static final class PropertyValuesHolderTarget extends ParallaxTarget {
    ...
    /**
     * We simulate a parallax effect on target object using an ObjectAnimator. PSEUDO_DURATION is
     * used on the ObjectAnimator.
     */
   ...
   * Property. App uses convenient method {@link ParallaxEffect#target(Object, Property)} to add a
   * direct mapping.
   *
  ...
  public static final class DirectPropertyTarget<T extends Object, V extends Number>
    ...
    /**
     * Returns true as DirectPropertyTarget receives a number to update Property in {@link
     * #directUpdate(Number)}.
     */
    @Override
    public boolean isDirectMapping() {
