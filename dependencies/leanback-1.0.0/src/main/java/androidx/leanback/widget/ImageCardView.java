...
/**
 * A subclass of {@link BaseCardView} with an {@link ImageView} as its main region. The {@link
 * ImageCardView} is highly customizable and can be used for various use-cases by adjusting the
 * ImageViewCard's type to any combination of Title, Content, Badge or ImageOnly.
 *
 * <p>
 *
 * <h3>Styling</h3>
 *
 * There are two different ways to style the ImageCardView. <br>
 ...
 * }</pre>
 *
 * <p>The first possibility is to set custom Styles in the Leanback Theme's attributes <code>
 * imageCardViewStyle</code>, <code>imageCardViewTitleStyle</code> etc. The styles set here, is the
 * default style for all ImageCardViews.
 *
 * <p>The second possibility allows you to style a particular ImageCardView. This is useful if you
 * want to create multiple types of cards. E.g. you might want to display a card with only a title
 * and another one with title and content. Thus you need to define two different <code>
 * ImageCardViewStyles</code> and two different themes and apply them to the ImageCardViews. You can
 * do this by using a the {@link #ImageCardView(Context)} constructor and passing a
 * ContextThemeWrapper with the custom ImageCardView theme id.
 *
 * <p><u>Example (using constructor):</u><br>
 *
 * <pre>{@code
 * new ImageCardView(new ContextThemeWrapper(context, R.style.CustomImageCardTheme));
 * }</pre>
 *
 * <p>You can style all ImageCardView's components such as the title, content, badge, infoArea and
 * the image itself by extending the corresponding style and overriding the specific attribute in
 * your custom ImageCardView theme.
 *
 * <h3>Components</h3>
 *
 ...
 *
 * <ul>
 *   <li>Title: The card's title
 *   <li>Content: A short description
 *   <li>Badge: An icon which can be displayed on the right or left side of the card.
 * </ul>
 *
 ...
 * You can combine the following values: <code>Title, Content, IconOnRight, IconOnLeft, ImageOnly
 * </code>.
 *
 * <p><u>Examples:</u><br>
 *
...
public class ImageCardView extends BaseCardView {
   ...
   *     you should share it in card Presenter: wrapper = new ContextThemeWrapper(context,
   *     themResId); return new ImageCardView(wrapper);
   */
  @Deprecated
  ...
  private void buildImageCardView(AttributeSet attrs, int defStyleAttr, int defStyle) {
    ...
    TypedArray cardAttrs =
        getContext()
            .obtainStyledAttributes(attrs, R.styleable.lbImageCardView, defStyleAttr, defStyle);
    int cardType =
        cardAttrs.getInt(
    ...
    if (hasTitle) {
      mTitleView =
          (TextView) inflater.inflate(R.layout.lb_image_card_view_themed_title, mInfoArea, false);
      mInfoArea.addView(mTitleView);
    }

    if (hasContent) {
      mContentView =
          (TextView) inflater.inflate(R.layout.lb_image_card_view_themed_content, mInfoArea, false);
