...
/**
 * ListRowPresenter renders {@link ListRow} using a {@link HorizontalGridView} hosted in a {@link
 * ListRowView}.
 *
 * <h3>Hover card</h3>
 *
 * Optionally, {@link #setHoverCardPresenterSelector(PresenterSelector)} can be used to display a
 * view for the currently focused list item below the rendered list. This view is known as a hover
 * card.
 *
 * <h3>Row selection animation</h3>
 *
 * ListRowPresenter disables {@link RowPresenter}'s default full row dimming effect and draws a dim
 * overlay on each child individually. A subclass may disable the overlay on each child by
 * overriding {@link #isUsingDefaultListSelectEffect()} to return false and write its own child dim
 * effect in {@link #applySelectLevelToChild(ViewHolder, View)}.
 *
 * <h3>Shadow</h3>
 *
 * ListRowPresenter applies a default shadow to each child view. Call {@link
 * #setShadowEnabled(boolean)} to disable shadows. A subclass may override and return false in
 * {@link #isUsingDefaultShadow()} and replace with its own shadow implementation.
 */
public class ListRowPresenter extends RowPresenter {
  ...
  public static class ViewHolder extends RowPresenter.ViewHolder {
    ...
    /**
     * Gets ViewHolder at a position in adapter. Returns null if the item does not exist or the item
    ...
    public Presenter.ViewHolder getItemViewHolder(int position) {
      ItemBridgeAdapter.ViewHolder ibvh =
          (ItemBridgeAdapter.ViewHolder) mGridView.findViewHolderForAdapterPosition(position);
      if (ibvh == null) {
        return null;
      }
    ...
    public Object getSelectedItem() {
      ItemBridgeAdapter.ViewHolder ibvh =
          (ItemBridgeAdapter.ViewHolder)
              mGridView.findViewHolderForAdapterPosition(getSelectedPosition());
      if (ibvh == null) {
        return null;
      }
      return ibvh.getItem();
  ...
  public static class SelectItemViewHolderTask extends Presenter.ViewHolderTask {
    ...
    public void run(Presenter.ViewHolder holder) {
      if (holder instanceof ListRowPresenter.ViewHolder) {
        ...
        if (mItemTask != null) {
          task =
              new androidx.leanback.widget.ViewHolderTask() {
  ...
  class ListRowPresenterItemBridgeAdapter extends ItemBridgeAdapter {
    ...
    public void onAddPresenter(Presenter presenter, int type) {
      mRowViewHolder
          .getGridView()
          .getRecycledViewPool()
          .setMaxRecycledViews(type, getRecycledPoolSize(presenter));
    }
  }
  ...
  /**
   * Constructs a ListRowPresenter with defaults. Uses {@link FocusHighlight#ZOOM_FACTOR_MEDIUM} for
   * focus zooming and disabled dimming on focus.
   */
   ...
   *     {@link FocusHighlight#ZOOM_FACTOR_NONE}, {@link FocusHighlight#ZOOM_FACTOR_SMALL}, {@link
   *     FocusHighlight#ZOOM_FACTOR_XSMALL}, {@link FocusHighlight#ZOOM_FACTOR_MEDIUM}, {@link
   *     FocusHighlight#ZOOM_FACTOR_LARGE} Dimming on focus defaults to disabled.
   */
  ...
  /**
   * Sets the row height for rows created by this Presenter. Rows created before calling this method
   * will not be updated.
   *
   * @param rowHeight Row height in pixels, or WRAP_CONTENT, or 0 to use the default height.
   */
  ...
  /**
   * Sets the expanded row height for rows created by this Presenter. If not set, expanded rows have
   * the same height as unexpanded rows.
   *
   * @param rowHeight The row height in to use when the row is expanded, in pixels, or WRAP_CONTENT,
  ...
  protected void initializeRowViewHolder(RowPresenter.ViewHolder holder) {
    ...
    if (mShadowOverlayHelper == null) {
      mShadowOverlayHelper =
          new ShadowOverlayHelper.Builder()
              .needsOverlay(needsDefaultListSelectEffect())
              .needsShadow(needsDefaultShadow())
              .needsRoundedCorner(
                  isUsingOutlineClipping(context) && areChildRoundedCornersEnabled())
      ...
      if (mShadowOverlayHelper.needsWrapper()) {
        mShadowOverlayWrapper = new ItemBridgeAdapterShadowOverlayWrapper(mShadowOverlayHelper);
      }
    }
    ...
    FocusHighlightHelper.setupBrowseItemFocusHighlight(
        rowViewHolder.mItemBridgeAdapter, mFocusZoomFactor, mUseFocusDimmer);
    rowViewHolder.mGridView.setFocusDrawingOrderEnabled(
        mShadowOverlayHelper.getShadowType() != ShadowOverlayHelper.SHADOW_DYNAMIC);
        ...
        new BaseGridView.OnUnhandledKeyListener() {
          ...
          public boolean onUnhandledKey(KeyEvent event) {
            return rowViewHolder.getOnKeyListener() != null
                && rowViewHolder
                    .getOnKeyListener()
                    .onKey(rowViewHolder.view, event.getKeyCode(), event);
          }
        });
  ...
  public int getRecycledPoolSize(Presenter presenter) {
    return mRecycledPoolSize.containsKey(presenter)
        ? mRecycledPoolSize.get(presenter)
  ...
  void selectChildView(ViewHolder rowViewHolder, View view, boolean fireEvent) {
    if (view != null) {
      if (rowViewHolder.mSelected) {
        ItemBridgeAdapter.ViewHolder ibh =
            (ItemBridgeAdapter.ViewHolder) rowViewHolder.mGridView.getChildViewHolder(view);
        ...
        if (fireEvent && rowViewHolder.getOnItemViewSelectedListener() != null) {
          rowViewHolder
              ...
              .onItemSelected(ibh.mHolder, ibh.mItem, rowViewHolder, rowViewHolder.mRow);
        }
      }
    } else {
      if (mHoverCardPresenterSelector != null) {
      ...
      if (fireEvent && rowViewHolder.getOnItemViewSelectedListener() != null) {
        rowViewHolder
            .getOnItemViewSelectedListener()
            .onItemSelected(null, null, rowViewHolder, rowViewHolder.mRow);
      }
    }
  }
  ...
  private static void initStatics(Context context) {
    if (sSelectedRowTopPadding == 0) {
      sSelectedRowTopPadding =
          context.getResources().getDimensionPixelSize(R.dimen.lb_browse_selected_row_top_padding);
      sExpandedSelectedRowTopPadding =
          context
              .getResources()
              .getDimensionPixelSize(R.dimen.lb_browse_expanded_selected_row_top_padding);
      sExpandedRowNoHovercardBottomPadding =
          context
              .getResources()
              .getDimensionPixelSize(R.dimen.lb_browse_expanded_row_no_hovercard_bottom_padding);
    }
  }
  ...
  private void setVerticalPadding(ListRowPresenter.ViewHolder vh) {
    ...
    if (vh.isExpanded()) {
      ...
      // MOD: fix different row padding when the row is unselected (channel fragment with search
      // header)
      ...
      paddingBottom =
          mHoverCardPresenterSelector == null
              ? sExpandedRowNoHovercardBottomPadding
              : vh.mPaddingBottom;
  ...
  /**
   * Dispatch item selected event using current selected item in the {@link HorizontalGridView}. The
   * method should only be called from onRowViewSelected().
   */
  @Override
  protected void dispatchItemSelectedListener(RowPresenter.ViewHolder holder, boolean selected) {
    ViewHolder vh = (ViewHolder) holder;
    ItemBridgeAdapter.ViewHolder itemViewHolder =
        (ItemBridgeAdapter.ViewHolder)
    ...
    if (selected) {
      if (holder.getOnItemViewSelectedListener() != null) {
        holder
            .getOnItemViewSelectedListener()
            .onItemSelected(itemViewHolder.getViewHolder(), itemViewHolder.mItem, vh, vh.getRow());
      }
    }
  }
  ...
  private void setupFadingEffect(ListRowView rowView) {
    ...
    if (mBrowseRowsFadingEdgeLength < 0) {
      ...
      mBrowseRowsFadingEdgeLength =
          (int) ta.getDimension(R.styleable.LeanbackTheme_browseRowsFadingEdgeLength, 0);
      ta.recycle();
    }
  ...
  /**
   * Returns true so that default select effect is applied to each individual child of {@link
   * HorizontalGridView}. Subclass may return false to disable the default implementation and
   * implement {@link #applySelectLevelToChild(ViewHolder, View)}.
   *
   ...
   * based) will be applied to each individual child of {@link HorizontalGridView}. Subclass may
   * return false to disable default implementation of shadow and provide its own.
   */
  ...
  /**
   * Returns true if SDK >= L, where Z shadow is enabled so that Z order is enabled on each child of
   * horizontal list. If subclass returns false in isUsingDefaultShadow() and does not use Z-shadow
   * on SDK >= L, it should override isUsingZOrder() return false.
   */
  ...
  /**
   * Returns true if leanback view outline is enabled on the system or false otherwise. When false,
   * rounded corner will not be enabled even {@link #enableChildRoundedCorners(boolean)} is called
   * with true.
   *
  ...
  /**
   * Enables or disables child shadow. This is not only for enable/disable default shadow
   * implementation but also subclass must respect this flag.
   */
  ...
  /**
   * Returns true if child shadow is enabled. This is not only for enable/disable default shadow
   * implementation but also subclass must respect this flag.
   */
   ...
   * Drawable. If application uses child's foreground for other purposes such as ripple effect, it
   * needs tell ListRowPresenter to keep the child's foreground. The default value is true.
   *
  ...
  /**
   * Returns true if keeps foreground of child of this row, false otherwise. When ListRowPresenter
   * applies overlay color on the child, it may change child's foreground Drawable. If application
   * uses child's foreground for other purposes such as ripple effect, it needs tell
   * ListRowPresenter to keep the child's foreground. The default value is true.
   *
  ...
  /**
   * Create ShadowOverlayHelper Options. Subclass may override. e.g. <code>
  ...
  /**
   * Applies select level to header and draws a default color dim over each child of {@link
   * HorizontalGridView}.
   *
   * <p>Subclass may override this method and starts with calling super if it has views to apply
   * select effect other than header and HorizontalGridView. To override the default color dim over
   * each child of {@link HorizontalGridView}, app should override {@link
   * #isUsingDefaultListSelectEffect()} to return false and override {@link
  ...
  /**
   * Applies select level to a child. Default implementation draws a default color dim over each
   * child of {@link HorizontalGridView}. This method is called on all children in {@link
   * #onSelectLevelChanged(RowPresenter.ViewHolder)} and when a child is attached to {@link
   * HorizontalGridView}.
   *
   * <p>Subclass may disable the default implementation by override {@link
   * #isUsingDefaultListSelectEffect()} to return false and deal with the individual item select
   * level by itself.
   *
  ...
  public void setEntranceTransitionState(RowPresenter.ViewHolder holder, boolean afterEntrance) {
    ...
    ((ViewHolder) holder)
        .mGridView.setChildrenVisibility(afterEntrance ? View.VISIBLE : View.INVISIBLE);
  }
}
