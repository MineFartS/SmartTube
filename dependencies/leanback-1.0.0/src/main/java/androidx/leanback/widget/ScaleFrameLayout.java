...
public class ScaleFrameLayout extends FrameLayout {
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    ...
    final float pivotX =
        (layoutDirection == View.LAYOUT_DIRECTION_RTL) ? getWidth() - getPivotX() : getPivotX();
    if (mLayoutScaleX != 1f) {
      parentLeft = getPaddingLeft() + (int) (pivotX - pivotX / mLayoutScaleX + 0.5f);
      parentRight =
          (int) (pivotX + (right - left - pivotX) / mLayoutScaleX + 0.5f) - getPaddingRight();
    } else {
    ...
    if (mLayoutScaleY != 1f) {
      ...
      parentBottom =
          (int) (pivotY + (bottom - top - pivotY) / mLayoutScaleY + 0.5f) - getPaddingBottom();
    } else {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    if (mLayoutScaleX != 1f || mLayoutScaleY != 1f) {
      final int scaledWidthMeasureSpec = getScaledMeasureSpec(widthMeasureSpec, mLayoutScaleX);
      final int scaledHeightMeasureSpec = getScaledMeasureSpec(heightMeasureSpec, mLayoutScaleY);
      super.onMeasure(scaledWidthMeasureSpec, scaledHeightMeasureSpec);
      setMeasuredDimension(
          (int) (getMeasuredWidth() * mLayoutScaleX + 0.5f),
