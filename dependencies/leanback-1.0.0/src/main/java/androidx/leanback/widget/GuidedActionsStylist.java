...
/**
 * GuidedActionsStylist is used within a {@link androidx.leanback.app.GuidedStepFragment} to supply
 * the right-side panel where users can take actions. It consists of a container for the list of
 * actions, and a stationary selector view that indicates visually the location of focus.
 ...
 *
 * <p>Many aspects of the base GuidedActionsStylist can be customized through theming; see the theme
 * attributes below. Note that these attributes are not set on individual elements in layout XML,
 * but instead would be set in a custom theme. See <a
 * href="http://developer.android.com/guide/topics/ui/themes.html">Styles and Themes</a> for more
 * information.
 *
 * <p>If these hooks are insufficient, this class may also be subclassed. Subclasses may wish to
 * override the {@link #onProvideLayoutId} method to change the layout used to display the list
 * container and selector; override {@link #onProvideItemLayoutId(int)} and {@link
 * #getItemViewType(GuidedAction)} method to change the layout used to display each action.
 *
 * <p>To support a "click to activate" view similar to DatePicker, app needs:
 ...
 *
 *     <p>Note: If an alternate list layout is provided, the following view IDs must be supplied:
 *
 *     <ul>
 *       <li>{@link androidx.leanback.R.id#guidedactions_list}
 *     </ul>
 *
 *     <p>These view IDs must be present in order for the stylist to function. The list ID must
 *     correspond to a {@link VerticalGridView} or subclass.
 *
 *     <p>If an alternate item layout is provided, the following view IDs should be used to refer to
 *     base elements:
 *
 *     <ul>
 *       <li>{@link androidx.leanback.R.id#guidedactions_item_content}
 *       <li>{@link androidx.leanback.R.id#guidedactions_item_title}
 *       <li>{@link androidx.leanback.R.id#guidedactions_item_description}
 *       <li>{@link androidx.leanback.R.id#guidedactions_item_icon}
 *       <li>{@link androidx.leanback.R.id#guidedactions_item_checkmark}
 *       <li>{@link androidx.leanback.R.id#guidedactions_item_chevron}
 *     </ul>
 *
 *     <p>These view IDs are allowed to be missing, in which case the corresponding views in {@link
 *     GuidedActionsStylist.ViewHolder} will be null.
 *
 *     <p>In order to support editable actions, the view associated with guidedactions_item_title
 *     should be a subclass of {@link android.widget.EditText}, and should satisfy the {@link
 ...
 * @attr ref
 *     androidx.leanback.R.styleable#LeanbackGuidedStepTheme_guidedStepImeDisappearingAnimation
...
public class GuidedActionsStylist implements FragmentAnimationProvider {
  ...
  static {
    ...
    sGuidedActionItemAlignFacet.setAlignmentDefs(
        new ItemAlignmentFacet.ItemAlignmentDef[] {alignedDef});
  }
  ...
  public static class ViewHolder extends RecyclerView.ViewHolder implements FacetProvider {
    ...
    /**
     * Convenience method to return an editable version of the title, if possible, or null if the
     * title view isn't an EditText.
     */
    ...
    /**
     * Convenience method to return an editable version of the description, if possible, or null if
     * the description view isn't an EditText.
     */
    ...
    /**
     * @return Current editing title view or description view or activator view or null if not in
     *     editing.
     */
    ...
    /**
     * @return True if bound action is inside {@link GuidedAction#getSubActions()}, false otherwise.
     */
    ...
    void press(boolean pressed) {
      ...
      final int themeAttrId =
          pressed ? R.attr.guidedActionPressedAnimation : R.attr.guidedActionUnpressedAnimation;
  ...
  /**
   * Creates a view appropriate for displaying a list of GuidedActions, using the provided inflater
   * and container.
   *
   * <p><i>Note: Does not actually add the created view to the container; the caller should do
   * this.</i>
   *
   * @param inflater The layout inflater to be used when constructing the view.
   * @param container The view group to be passed in the call to <code>LayoutInflater.inflate</code>
   *     .
  ...
  public View onCreateView(LayoutInflater inflater, final ViewGroup container) {
    ...
    mContentView =
        mMainView.findViewById(
            mButtonActions ? R.id.guidedactions_content2 : R.id.guidedactions_content);
    mBgView =
        mMainView.findViewById(
            mButtonActions
                ? R.id.guidedactions_list_background2
    ...
    if (mMainView instanceof VerticalGridView) {
      ...
      mActionsGridView =
          (VerticalGridView)
              mMainView.findViewById(
                  mButtonActions ? R.id.guidedactions_list2 : R.id.guidedactions_list);
      ...
      if (!mButtonActions) {
        mSubActionsGridView =
            (VerticalGridView) mMainView.findViewById(R.id.guidedactions_sub_list);
    ...
    mEnabledTextAlpha =
        getFloatValue(ctx.getResources(), val, R.dimen.lb_guidedactions_item_unselected_text_alpha);
    mDisabledTextAlpha =
        getFloatValue(ctx.getResources(), val, R.dimen.lb_guidedactions_item_disabled_text_alpha);
    mEnabledDescriptionAlpha =
        getFloatValue(
            ctx.getResources(),
            val,
            R.dimen.lb_guidedactions_item_unselected_description_text_alpha);
    mDisabledDescriptionAlpha =
        getFloatValue(
            ctx.getResources(), val, R.dimen.lb_guidedactions_item_disabled_description_text_alpha);
    ...
    if (mContentView instanceof GuidedActionsRelativeLayout) {
      ((GuidedActionsRelativeLayout) mContentView)
          .setInterceptKeyEventListener(
              new GuidedActionsRelativeLayout.InterceptKeyEventListener() {
                ...
                public boolean onInterceptKeyEvent(KeyEvent event) {
                  if (event.getKeyCode() == KeyEvent.KEYCODE_BACK
                    ...
                    if ((mExpandedAction.hasSubActions() && isBackKeyToCollapseSubActions())
  ...
  public void setAsButtonActions() {
    if (mMainView != null) {
      throw new IllegalStateException(
          "setAsButtonActions() must be called before creating " + "views");
    }
    mButtonActions = true;
  }
   ...
   * returns {@link androidx.leanback.R.layout#lb_guidedactions} or {@link
   * androidx.leanback.R.layout#lb_guidedbuttonactions} if {@link #isButtonActions()} is true. If
   ...
   * Subclasses may override to provide their own customized layouts. The base implementation
   * returns {@link androidx.leanback.R.layout#lb_guidedactions_item}. If overridden, the
   * substituted layout should contain matching IDs for any views that should be managed by the base
   * class; this can be achieved by starting with a copy of the base layout file. Note that in order
   * for the item to support editing, the title view should both subclass {@link
   * android.widget.EditText} and implement {@link ImeKeyMonitor}, {@link
   * GuidedActionAutofillSupport}; see {@link GuidedActionEditText}. To support different types of
   * Layouts, override {@link #onProvideItemLayoutId(int)}.
   *
   ...
   * <li>{{@link androidx.leanback.R.layout#lb_guidedactions_datepicker_item}. If overridden, the
   *     substituted layout should contain matching IDs for any views that should be managed by the
   *     base class; this can be achieved by starting with a copy of the base layout file. Note that
   *     in order for the item to support editing, the title view should both subclass {@link
  ...
  /**
   * Constructs a {@link ViewHolder} capable of representing {@link GuidedAction}s. Subclasses may
   * choose to return a subclass of ViewHolder. To support different view types, override {@link
   * #onCreateViewHolder(ViewGroup, int)}
   *
   * <p><i>Note: Should not actually add the created view to the parent; the caller will do
   * this.</i>
   *
  ...
  /**
   * Constructs a {@link ViewHolder} capable of representing {@link GuidedAction}s. Subclasses may
   * choose to return a subclass of ViewHolder.
   *
   * <p><i>Note: Should not actually add the created view to the parent; the caller will do
   * this.</i>
   *
  ...
  public void onBindViewHolder(ViewHolder vh, GuidedAction action) {
    ...
    if (vh.mDescriptionView != null) {
      ...
      vh.mDescriptionView.setVisibility(
          TextUtils.isEmpty(action.getDescription()) ? View.GONE : View.VISIBLE);
      vh.mDescriptionView.setAlpha(
          action.isEnabled() ? mEnabledDescriptionAlpha : mDisabledDescriptionAlpha);
    ...
    if (action.hasMultilineDescription()) {
      if (vh.mTitleView != null) {
        ...
        if (vh.mDescriptionView != null) {
          vh.mDescriptionView.setInputType(
              vh.mDescriptionView.getInputType() | InputType.TYPE_TEXT_FLAG_MULTI_LINE);
          vh.mDescriptionView.setMaxHeight(
              getDescriptionMaxHeight(vh.itemView.getContext(), vh.mTitleView));
        }
      }
    } else {
      if (vh.mTitleView != null) {
  ...
  /**
   * Called when editing mode of an ViewHolder is changed. Subclass must call <code>
   * super.onEditingModeChange(vh,editing,withTransition)</code>.
   *
  ...
  /**
   * Animates the view holder's view (or subviews thereof) when the action has had its focus state
   * changed.
   *
  ...
  /**
   * Animates the view holder's view (or subviews thereof) when the action has had its press state
   * changed.
   *
   * @param vh The view holder associated with the relevant action.
   ...
   * changed. Default implementation calls setChecked() if {@link ViewHolder#getCheckmarkView()} is
   * instance of {@link Checkable}.
   *
   * @param vh The view holder associated with the relevant action.
   ...
   * implementation assigns drawable loaded from theme attribute {@link
   * android.R.attr#listChoiceIndicatorMultiple} for checkbox or {@link
   * android.R.attr#listChoiceIndicatorSingle} for radio button. Subclass rarely needs override the
   * method, instead app can provide its own drawable that supports transition animations, change
   * theme attributes {@link android.R.attr#listChoiceIndicatorMultiple} and {@link
   * android.R.attr#listChoiceIndicatorSingle} in {androidx.leanback.R.
  ...
  public void onBindCheckMarkView(ViewHolder vh, GuidedAction action) {
    if (action.getCheckSetId() != GuidedAction.NO_CHECK_SET) {
      ...
      int attrId =
          action.getCheckSetId() == GuidedAction.CHECKBOX_CHECK_SET_ID
  ...
  public void onBindActivatorView(ViewHolder vh, GuidedAction action) {
    if (action instanceof GuidedDatePickerAction) {
      ...
      dateView.updateDate(
          c.get(Calendar.YEAR), c.get(Calendar.MONTH), c.get(Calendar.DAY_OF_MONTH), false);
    }
  }
  ...
  void onEditActivatorView(final ViewHolder vh, boolean editing, final boolean withTransition) {
    if (editing) {
      ...
      vh.mActivatorView.setOnClickListener(
          new View.OnClickListener() {
            @Override
            public void onClick(View v) {
              if (!isInExpandTransition()) {
                ((GuidedActionAdapter) getActionsGridView().getAdapter()).performOnActionClick(vh);
              }
            }
          });
    } else {
  ...
  public void onBindChevronView(ViewHolder vh, GuidedAction action) {
    ...
    if (hasNext || hasSubActions) {
      ...
      if (hasNext) {
        float r =
            mMainView != null && mMainView.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL
                ? 180f
                : 0f;
   ...
   * @deprecated use {@link #expandAction(GuidedAction, boolean)} and {@link
  ...
  /**
   * Returns if expand/collapse animation is supported. When this method returns true, {@link
   * #startExpandedTransition(ViewHolder)} will be used. When this method returns false, {@link
   * #onUpdateExpandedViewHolder(ViewHolder)} will be called.
   *
   * @return True if it is running an expanding or collapsing transition, false otherwise.
   */
   ...
   *
   * @param avh When not null, the GuidedActionStylist expands the sub actions of avh. When null the
   *     GuidedActionStylist will collapse sub actions.
   * @deprecated use {@link #expandAction(GuidedAction, boolean)} and {@link
   *     #collapseAction(boolean)}
   */
  @Deprecated
   ...
   *
   * @param backToCollapse True to enable using BACK key to collapse sub actions list, false to
   *     disable.
  ...
  /**
   * @return True if using BACK key to collapse sub actions list, false otherwise. Default value is
   *     true.
   * @see GuidedAction#hasSubActions
   * @see GuidedAction#getSubActions
   */
  ...
  /**
   * Enable or disable using BACK key to collapse {@link GuidedAction} with editable activator view.
   * Default is enabled.
   *
  ...
  /**
   * @return True if using BACK key to collapse {@link GuidedAction} with editable activator view,
  ...
  void startExpanded(ViewHolder avh, final boolean withTransition) {
    ...
    for (int i = 0; i < count; i++) {
      ViewHolder vh =
          (ViewHolder) mActionsGridView.getChildViewHolder(mActionsGridView.getChildAt(i));
    ...
    if (withTransition) {
      ...
      float slideDistance =
          isSubActionTransition
              ? focusAvh.itemView.getHeight()
              : focusAvh.itemView.getHeight() * 0.5f;
      Object slideAndFade =
          TransitionHelper.createFadeAndShortSlide(Gravity.TOP | Gravity.BOTTOM, slideDistance);
      TransitionHelper.setEpicenterCallback(
          slideAndFade,
          new TransitionEpicenterCallback() {
            Rect mRect = new Rect();
  ...
  void onUpdateSubActionsGridView(GuidedAction action, boolean expand) {
    if (mSubActionsGridView != null) {
      ...
      if (expand) {
        ...
        int actionPosition = ((GuidedActionAdapter) mActionsGridView.getAdapter()).indexOf(action);
        lp.topMargin =
            mActionsGridView.getLayoutManager().findViewByPosition(actionPosition).getBottom();
        lp.height = 0;
  ...
  /**
   * @return the max height in pixels the description can be such that the action nicely takes up
   *     the entire screen.
   */
  private int getDescriptionMaxHeight(Context context, TextView title) {
    ...
    return (int)
        (mDisplayHeight - 2 * mVerticalPadding - 2 * mTitleMaxLines * title.getLineHeight());
  }
}
