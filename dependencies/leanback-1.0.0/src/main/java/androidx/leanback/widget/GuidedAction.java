 ...
 * androidx.leanback.app.GuidedStepFragment}. GuidedActions contain at minimum a title and a
 * description, and typically also an icon.
 *
 * <p>A GuidedAction typically represents a single action a user may take, but may also represent a
 ...
 *
 * <p>GuidedActions may optionally be checked. They may also indicate that they will request further
 * user input on selection, in which case they will be displayed with a chevron indicator.
 *
 * <p>GuidedAction recommends to use {@link Builder}. When application subclass GuidedAction, it can
 * subclass {@link BuilderBase}, implement its own builder() method where it should call {@link
 * BuilderBase#applyValues(GuidedAction)}.
 */
public class GuidedAction extends Action {
  ...
  /**
   * Base builder class to build a {@link GuidedAction} object. When subclass GuidedAction, you can
   * override this BuilderBase class, implements your build() method which should call {@link
   * #applyValues(GuidedAction)}. When using GuidedAction directly, use {@link Builder}.
   */
  public abstract static class BuilderBase<B extends BuilderBase> {
    ...
    private int mInputType = InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS;
     ...
     *     {@link GuidedAction#ACTION_ID_FINISH} {@link GuidedAction#ACTION_ID_CONTINUE} {@link
     *     GuidedAction#ACTION_ID_YES} {@link GuidedAction#ACTION_ID_NO}.
    ...
    /**
     * Sets the ID associated with this action. The ID can be any value the client wishes; it is
     * typically used to determine what to do when an action is clicked.
     *
    ...
    /**
     * Sets the title for this action. The title is typically a short string indicating the action
     * to be taken on click, e.g. "Continue" or "Cancel".
     *
    ...
    /**
     * Sets the optional title text to edit. When TextView is activated, the edit title replaces the
     * string of title.
     *
    ...
    /**
     * Sets the description for this action. The description is typically a longer string providing
     * extra information on what the action will do.
     *
    ...
    /**
     * Sets the optional description text to edit. When TextView is activated, the edit description
     * replaces the string of description.
     *
     ...
     * replaces the string of description.
     *
     * @param descriptionResourceId String resource id of the description to edit for this action.
     */
     ...
     * This is a convenience function that simply looks up the drawable and calls {@link
     * #icon(Drawable)}.
     *
    ...
    /**
     * Sets the action's icon drawable by retrieving it by resource ID from Builder's context. This
     * is a convenience function that simply looks up the drawable and calls {@link
     * #icon(Drawable)}.
     *
     * @param iconResourceId The resource ID for the icon associated with this action.
     */
    ...
    /**
     * Indicates whether this action is part of a single-select group similar to radio buttons or
     * this action is a checkbox. When one item in a check set is checked, all others with the same
     * check set ID will be checked automatically.
     *
    ...
    public B multilineDescription(boolean multilineDescription) {
      setFlags(multilineDescription ? PF_MULTI_LINE_DESCRIPTION : 0, PF_MULTI_LINE_DESCRIPTION);
      return (B) this;
    }
  ...
  /**
   * Returns the optional title text to edit. When not null, it is being edited instead of {@link
   * #getTitle()}.
   *
   ...
   *
   * @param editDescription Optional description text to edit instead of {@link
  ...
  /**
   * Returns true if {@link #getEditTitle()} is not null. When true, the {@link #getEditTitle()} is
   * being edited instead of {@link #getTitle()}.
   *
  ...
  /**
   * Returns InputType of action description in editing; only valid when {@link
   ...
   *
   * @return an integer representing the check set this action is a part of, or {@link
   *     #CHECKBOX_CHECK_SET_ID} if this is a checkbox, or {@link #NO_CHECK_SET} if this action is
   ...
   *
   * @return true if the action was constructed as having a multiline description, false otherwise.
   */
  ...
  /**
   * Returns true if Action will be saved to instanceState and restored later, false otherwise. The
   * default value is true. When isAutoSaveRestoreEnabled() is true and {@link #getId()} is not
   * {@link #NO_ID}:
   * <li>{@link #isEditable()} is true: save text of {@link #getTitle()}
   * <li>{@link #isDescriptionEditable()} is true: save text of {@link #getDescription()}
   * <li>{@link #getCheckSetId()} is not {@link #NO_CHECK_SET}: save {@link #isChecked()}}
   * <li>{@link GuidedDatePickerAction} will be saved App may explicitly disable auto restore and
   *     handle by itself. App should override Fragment onSaveInstanceState() and onCreateActions()
   *
   ...
   * <li>{@link #isDescriptionEditable()} is true: save text of {@link #getDescription()}
   * <li>{@link #getCheckSetId()} is not {@link #NO_CHECK_SET}: save {@link #isChecked()}}
   * <li>{@link GuidedDatePickerAction} will be saved Subclass may override this method.
   *
