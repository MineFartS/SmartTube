...
public class ThumbsBar extends LinearLayout {
  ...
  public ThumbsBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    mThumbWidthInPixel =
        context.getResources().getDimensionPixelSize(R.dimen.lb_playback_transport_thumbs_width);
    mThumbHeightInPixel =
        context.getResources().getDimensionPixelSize(R.dimen.lb_playback_transport_thumbs_height);
    // According to the spec, the width of HeroThumb should be 192dp
    mHeroThumbHeightInPixel =
        context
            .getResources()
            .getDimensionPixelSize(R.dimen.lb_playback_transport_hero_thumbs_width);
    mHeroThumbWidthInPixel =
        context
            .getResources()
            .getDimensionPixelSize(R.dimen.lb_playback_transport_hero_thumbs_height);
    // According to the spec, the margin between thumbs to be 4dp
    mMeasuredMarginInPixel =
        context.getResources().getDimensionPixelSize(R.dimen.lb_playback_transport_thumbs_margin);
  }
  ...
  /**
   * Helper function for setNumberOfThumbs. Will Update the layout settings in ThumbsBar based on
   * mNumOfThumbs
   */
  private void setNumberOfThumbsInternal() {
    ...
    while (getChildCount() < mNumOfThumbs) {
      ...
      LinearLayout.LayoutParams lp =
          new LinearLayout.LayoutParams(mThumbWidthInPixel, mThumbHeightInPixel);
      addView(view, lp);
    }
    int heroIndex = getHeroIndex();
    for (int i = 0; i < getChildCount(); i++) {
  ...
  /**
   * Helper function to compute how many thumbs should be put in the screen Assume we should put x's
   * non-hero thumbs in the screen, the equation should be 192dp (width of hero thumbs) + 154dp
   * (width of common thumbs) * x + 4dp (width of the margin between thumbs) * x = width So the
   * calculated number of non-hero thumbs should be (width - 192dp) / 158dp. If the calculated
   * number of non-hero thumbs is less than 2, it will be updated to 2 or if the calculated number
   * or non-hero thumbs is not an even number, it will be decremented by one. This processing is
   * used to make sure the arrangement of non-hero thumbs in ThumbsBar is symmetrical. Also there
   * should be a hero thumb in the middle of the ThumbsBar, the final result should be non-hero
   * thumbs (after processing) + 1.
   *
  ...
  private int calculateNumOfThumbs(int widthInPixel) {
    int nonHeroThumbNum =
        roundUp(widthInPixel - mHeroThumbWidthInPixel, mThumbWidthInPixel + mMeasuredMarginInPixel);
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    heroView.layout(
        heroLeft, getPaddingTop(), heroRight, getPaddingTop() + heroView.getMeasuredHeight());
