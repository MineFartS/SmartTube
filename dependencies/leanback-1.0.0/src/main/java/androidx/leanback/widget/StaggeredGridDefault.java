 ...
 *
 * <p>This implementation tries to fill items in consecutive row order. The next item is always in
 * same row or in the next row.
 */
final class StaggeredGridDefault extends StaggeredGrid {

  /**
   * Returns the max edge value of item (visible or cached) in a row. This will be the place to
   * append or prepend item not in cache.
   */
  ...
  /**
   * Returns the min edge value of item (visible or cached) in a row. This will be the place to
   * prepend or append item not in cache.
   */
  ...
  /**
   * Note this method has assumption that item is filled either in the same row next row of last
   * item. Search until row index wrapped.
   */
  @Override
  ...
  protected boolean appendVisibleItemsWithoutCache(int toLimit, boolean oneColumnMode) {
    ...
    if (mLastVisibleIndex >= 0) {
      ...
      rowIndex =
          (mLocations.size() > 0 ? getLocation(getLastIndex()).row + 1 : itemIndex) % mNumRows;
      edgeLimit = 0;
  ...
  protected boolean prependVisibleItemsWithoutCache(int toLimit, boolean oneColumnMode) {
    ...
    if (mFirstVisibleIndex >= 0) {
      ...
      if (mReversedFlow ? getRowMax(rowIndex) >= edgeLimit : getRowMin(rowIndex) <= edgeLimit) {
        // if current row exceeds previous column, fill from next row
        rowIndex = rowIndex - 1;
        if (rowIndex < 0) {
          // start a new column and using edge limit of current column
      ...
      rowIndex =
          (mLocations.size() > 0 ? getLocation(getFirstIndex()).row + mNumRows - 1 : itemIndex)
              % mNumRows;
      edgeLimit = 0;
      edgeLimitIsValid = false;
    }
