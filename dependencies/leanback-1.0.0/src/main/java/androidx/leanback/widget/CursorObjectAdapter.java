...
public class CursorObjectAdapter extends ObjectAdapter {
   ...
   * if it is different than the new cursor.
   *
  ...
  /**
   * Swap in a new Cursor, returning the old Cursor. Unlike changeCursor(Cursor), the returned old
  ...
  /** Returns the {@link CursorMapper} used to convert {@link Cursor} rows into Objects. */
  ...
  /**
   * Returns true if the adapter, and hence the backing {@link Cursor}, is closed; false otherwise.
   */
  ...
  /**
   * Removes an item from the cache. This will force the item to be re-read from the data source the
   * next time {@link #get(int)} is called.
   */
