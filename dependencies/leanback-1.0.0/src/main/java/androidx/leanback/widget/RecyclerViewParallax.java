...
/**
 * Implementation of {@link Parallax} class for {@link RecyclerView}. This class allows users to
 * track position of specific views inside {@link RecyclerView} relative to itself. See {@link
 * ChildPositionProperty} for details.
 */
public class RecyclerViewParallax extends Parallax<RecyclerViewParallax.ChildPositionProperty> {
  ...
  View.OnLayoutChangeListener mOnLayoutChangeListener =
      new View.OnLayoutChangeListener() {
        ...
        public void onLayoutChange(
            View view, int l, int t, int r, int b, int oldL, int oldT, int oldR, int oldB) {
          updateValues();
        }
      };

  /**
   * Subclass of {@link Parallax.IntProperty}. Using this Property, users can track a RecylerView
   * child's position inside recyclerview. i.e.
   *
   * <p>tracking_pos = view.top + fraction * view.height() + offset
   *
   * <p>This way we can track top using fraction 0 and bottom using fraction 1.
   */
  public static final class ChildPositionProperty extends Parallax.IntProperty {
    ...
    /**
     * Sets fraction of size to be added to view's start position. e.g. to track the center position
     * of the view, use fraction 0.5; to track the end position of the view use fraction 1.
     *
    ...
    /**
     * Returns fraction of size to be added to view's start position. e.g. to track the center
     * position of the view, use fraction 0.5; to track the end position of the view use fraction 1.
     */
    ...
    void updateValue(RecyclerViewParallax source) {
      ...
      if (viewHolder == null) {
        ...
        if (source.mIsVertical) {
          source.setIntPropertyValue(
              getIndex(), rect.top + mOffset + (int) (mFraction * rect.height()));
        } else {
          source.setIntPropertyValue(
              getIndex(), rect.left + mOffset + (int) (mFraction * rect.width()));
        }
      }
    }
  }
  ...
  public void setRecyclerView(RecyclerView recyclerView) {
    ...
    if (mRecylerView != null) {
      LayoutManager.Properties properties =
          mRecylerView.getLayoutManager().getProperties(mRecylerView.getContext(), null, 0, 0);
  ...
  /**
   * Manually update values. This is used for changes not controlled by RecyclerView. E.g. called by
   * a Slide transition that changes translation of the view.
   */
  @Override
