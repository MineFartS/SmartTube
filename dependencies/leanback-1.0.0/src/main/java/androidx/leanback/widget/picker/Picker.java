 ...
 *
 * <p>Picker has two states and will change height:
 * <li>{@link #isActivated()} is true: Picker shows typically three items vertically (see {@link
 *     #getActivatedVisibleItemCount()}}. Columns other than {@link #getSelectedColumn()} still
 ...
 * <li>{@link #isActivated()} is false: Picker shows one item vertically (see {@link
 *     #getVisibleItemCount()}) on all columns. The size of Picker shrinks.
 */
public class Picker extends FrameLayout {
  ...
  /**
   * Sets the list of separators that will be populated between the Picker columns. The number of
   * the separators should be either 1 indicating the same separator used between all the columns
   * fields (and nothing will be placed before the first and after the last column), or must be one
   * unit larger than the number of columns passed to {@link #setColumns(List)}. In the latter case,
   * the list of separators corresponds to the positions before the first column all the way to the
   * position after the last column. An empty string for a given position indicates no separators
   * needs to be placed for that position, otherwise a TextView with the given String will be
  ...
  /**
   * Classes extending {@link Picker} can choose to override this method to supply the {@link
  ...
  /**
   * Returns the {@link Picker}'s item's {@link TextView}'s id from within the layout provided by
   * {@link Picker#getPickerItemLayoutId()} or 0 if the layout provided by {@link
   * Picker#getPickerItemLayoutId()} is a {link TextView}.
   */
  ...
  /**
   * Sets the {@link Picker}'s item's {@link TextView}'s id from within the layout provided by
   * {@link Picker#getPickerItemLayoutId()} or 0 if the layout provided by {@link
   * Picker#getPickerItemLayoutId()} is a {link TextView}.
   *
   ...
   *
   * @param columns The actual focusable columns of a picker which are scrollable if the field takes
   *     more than one value (e.g. for a DatePicker, day, month, and year fields and for TimePicker,
   *     hour, minute, and am/pm fields form the columns).
   */
  public void setColumns(List<PickerColumn> columns) {
    ...
    if (!TextUtils.isEmpty(mSeparators.get(0))) {
      TextView separator =
          (TextView) inflater.inflate(R.layout.lb_picker_separator, mPickerView, false);
      separator.setText(mSeparators.get(0));
      mPickerView.addView(separator);
    }
    for (int i = 0; i < totalCol; i++) {
      final int colIndex = i;
      final VerticalGridView columnView =
          (VerticalGridView) inflater.inflate(R.layout.lb_picker_column, mPickerView, false);
      ...
      columnView.setAdapter(
          new PickerScrollArrayAdapter(
              getContext(), getPickerItemLayoutId(), getPickerItemTextViewId(), colIndex));
  ...
  private void setOrAnimateAlpha(
      View view, boolean animate, float destAlpha, float startAlpha, Interpolator interpolator) {
    view.animate().cancel();
    if (!animate) {
      ...
      view.animate()
          .alpha(destAlpha)
          .setDuration(mAlphaAnimDuration)
  ...
  /**
   * Classes extending {@link Picker} can override this function to supply the behavior when a list
   * has been scrolled. Subclass may call {@link #setColumnValue(int, int, boolean)} and or {@link
   * #setColumnAt(int, PickerColumn)}. Subclass should not directly call {@link
   * PickerColumn#setCurrentValue(int)} which does not update internal state or notify listeners.
   *
      ...
      new OnChildViewHolderSelectedListener() {
        ...
        public void onChildViewHolderSelected(
          ...
          PickerScrollArrayAdapter pickerScrollArrayAdapter =
              (PickerScrollArrayAdapter) parent.getAdapter();
  ...
  private void updateColumnSize(VerticalGridView columnView) {
    ...
    lp.height =
        (int)
            (getPickerItemHeightPixels() * itemCount
  ...
  /**
   * Returns number of visible items showing in a column when it's activated. The default value is
   * 3.
   *
  ...
  /**
   * Changes number of visible items showing in a column when it's activated. The default value is
   * 3.
   *
  ...
  /**
   * Returns number of visible items showing in a column when it's not activated. The default value
   * is 1.
   *
  ...
  /**
   * Changes number of visible items showing in a column when it's not activated. The default value
   * is 1.
   *
