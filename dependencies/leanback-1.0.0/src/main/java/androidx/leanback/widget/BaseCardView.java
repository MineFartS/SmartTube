...
/**
 * A card style layout that responds to certain state changes. It arranges its children in a
 * vertical column, with different regions becoming visible at different times.
 *
 * <p>A BaseCardView will draw its children based on its type, the region visibilities of the child
 * types, and the state of the widget. A child may be marked as belonging to one of three regions:
 * main, info, or extra. The main region is always visible, while the info and extra regions can be
 * set to display based on the activated or selected state of the View. The card states are set by
 * calling {@link #setActivated(boolean) setActivated} and {@link #setSelected(boolean)
 * setSelected}.
 *
 * <p>See {@link BaseCardView.LayoutParams} for layout attributes.
 */
public class BaseCardView extends FrameLayout {
  ...
  /**
   * A simple card type with a single layout area. This card type does not change its layout or size
   * as it transitions between Activated/Not-Activated or Selected/Unselected states.
   *
   * @see #getCardType()
   */
  ...
  /**
   * A Card type with 2 layout areas: A main area which is always visible, and an info area that
   * fades in over the main area when it is visible. The card height will not change.
   *
   * @see #getCardType()
   */
  ...
  /**
   * A Card type with 2 layout areas: A main area which is always visible, and an info area that
   * appears below the main area. When the info area is visible the total card height will change.
   *
   * @see #getCardType()
   */
  ...
  /**
   * A Card type with 3 layout areas: A main area which is always visible; an info area which will
   * appear below the main area, and an extra area that only appears after a short delay. The info
   * area appears below the main area, causing the total card height to change. The extra area
   * animates in at the bottom of the card, shifting up the info view without affecting the card
   * height.
   *
   * @see #getCardType()
   */
  ...
  public BaseCardView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.lbBaseCardView, defStyleAttr, 0);
    ...
    try {
      ...
      mInfoVisibility =
          a.getInteger(R.styleable.lbBaseCardView_infoVisibility, CARD_REGION_VISIBLE_ACTIVATED);
      mExtraVisibility =
          a.getInteger(R.styleable.lbBaseCardView_extraVisibility, CARD_REGION_VISIBLE_SELECTED);
      ...
      mActivatedAnimDuration =
          a.getInteger(
              R.styleable.lbBaseCardView_activatedAnimationDuration,
  ...
  /**
   * Sets a flag indicating if the Selected animation (if the selected card type implements one)
   * should run immediately after the card is selected, or if it should be delayed. The default
   * behavior is to delay this animation. This is a one-shot override. If set to false, after the
   * card is selected and the selected animation is triggered, this flag is automatically reset to
   * true. This is useful when you want to change the default behavior, and have the selected
   * animation run immediately. One such case could be when focus moves from one row to the other,
   * when instead of delaying the selected animation until the user pauses on a card, it may be
   * desirable to trigger the animation for that card immediately.
   *
   * @param delay True (default) if the selected animation should be delayed after the card is
   *     selected, or false if the animation should run immediately the next time the card is
   *     Selected.
   */
  ...
  /**
   * Returns a boolean indicating if the selected animation will run immediately or be delayed the
   * next time the card is Selected.
   *
   * @return true if this card is set to delay the selected animation the next time it is selected,
   *     or false if the selected animation will run immediately the next time the card is selected.
   */
  ...
  public void setCardType(int type) {
    if (mCardType != type) {
      if (type >= CARD_TYPE_MAIN_ONLY && type < CARD_TYPE_INVALID) {
        ...
        Log.e(
            TAG,
            "Invalid card type specified: " + type + ". Defaulting to type CARD_TYPE_MAIN_ONLY.");
   ...
   *
   * @param visibility The region visibility to use for the info region. Must be one of {@link
   *     #CARD_REGION_VISIBLE_ALWAYS}, {@link #CARD_REGION_VISIBLE_SELECTED}, or {@link
  ...
  final float getFinalInfoVisFraction() {
    return mCardType == CARD_TYPE_INFO_UNDER
            && mInfoVisibility == CARD_REGION_VISIBLE_SELECTED
            && !isSelected()
        ? 0.0f
        : 1.0f;
  }
  ...
  final float getFinalInfoAlpha() {
    return mCardType == CARD_TYPE_INFO_OVER
            && mInfoVisibility == CARD_REGION_VISIBLE_SELECTED
            && !isSelected()
        ? 0.0f
        : 1.0f;
   ...
   *
   * @param visibility The region visibility to use for the extra region. Must be one of {@link
   *     #CARD_REGION_VISIBLE_ALWAYS}, {@link #CARD_REGION_VISIBLE_SELECTED}, or {@link
   *     #CARD_REGION_VISIBLE_ACTIVATED}.
  ...
  /**
   * Sets the Activated state of this Card. This can trigger changes in the card layout, resulting
   * in views to become visible or hidden. A card is normally set to Activated state when its parent
   * container (like a Row) receives focus, and then activates all of its children.
   *
  ...
  /**
   * Sets the Selected state of this Card. This can trigger changes in the card layout, resulting in
   * views to become visible or hidden. A card is normally set to Selected state when it receives
   * input focus.
   *
    ...
    mMeasuredHeight =
        (int)
            (mainHeight
                ...
                + extraHeight
                - (infoAnimating ? 0 : mInfoOffset));

    // Report our final dimensions.
    setMeasuredDimension(
        View.resolveSizeAndState(
            mMeasuredWidth + getPaddingLeft() + getPaddingRight(), widthMeasureSpec, state),
        View.resolveSizeAndState(
            mMeasuredHeight + getPaddingTop() + getPaddingBottom(),
            heightMeasureSpec,
            state << View.MEASURED_HEIGHT_STATE_SHIFT));
  }
  ...
  private void findChildrenViews() {
    ...
    for (int i = 0; i < count; i++) {
      ...
      BaseCardView.LayoutParams lp = (BaseCardView.LayoutParams) child.getLayoutParams();
  ...
  protected LayoutParams generateDefaultLayoutParams() {
    return new BaseCardView.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
  }
