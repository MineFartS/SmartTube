...
/**
 * Helper class to assist delayed shared element activity transition for view created by {@link
 * FullWidthDetailsOverviewRowPresenter}. User must call {@link
 * #setSharedElementEnterTransition(Activity, String, long)} during activity onCreate() and call
 * {@link
 * FullWidthDetailsOverviewRowPresenter#setListener(FullWidthDetailsOverviewRowPresenter.Listener)}.
 * The helper implements {@link FullWidthDetailsOverviewRowPresenter.Listener} and starts delayed
 * activity transition once {@link
 * FullWidthDetailsOverviewRowPresenter.Listener#onBindLogo(ViewHolder)} is called.
 */
public class FullWidthDetailsOverviewSharedElementHelper
  ...
  /**
   * Enable or disable auto startPostponedEnterTransition() when bound to logo. When it's disabled,
   * app must call {@link #startPostponedEnterTransition()} to kick off windowEnterTransition. By
   * default, it is disabled when there is no windowEnterSharedElementTransition set on the
   * activity.
   */
  ...
  /**
   * Returns true if auto startPostponedEnterTransition() when bound to logo. When it's disabled,
   * app must call {@link #startPostponedEnterTransition()} to kick off windowEnterTransition. By
   * default, it is disabled when there is no windowEnterSharedElementTransition set on the
   * activity.
   */
  ...
  public void onBindLogo(ViewHolder vh) {
    ...
    if (mViewHolder != null) {
      if (DEBUG) {
        Log.d(
            TAG,
            "rebind? clear transitionName on current viewHolder " + mViewHolder.getOverviewView());
      }
            ...
            new Runnable() {
              ...
              public void run() {
                ...
                ViewCompat.setTransitionName(
                    mViewHolder.getLogoViewHolder().view, mSharedElementName);
                Object transition =
                    TransitionHelper.getSharedElementEnterTransition(
