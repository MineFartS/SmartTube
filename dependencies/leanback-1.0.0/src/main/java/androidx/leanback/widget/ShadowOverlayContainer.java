 ...
 * corners. It's not always preferred to create a ShadowOverlayContainer, use {@link
 * ShadowOverlayHelper} instead.
 *
 * <p>{@link #prepareParentForShadow(ViewGroup)} must be called on parent of container before using
 * shadow. Depending on sdk version, optical bounds might be applied to parent.
 *
 * <p>If shadows can appear outside the bounds of the parent view, setClipChildren(false) must be
 * called on the grandparent view.
 *
 * <p>{@link #initialize(boolean, boolean, boolean)} must be first called on the container. Then
 * call {@link #wrap(View)} to insert the wrapped view into the container.
 *
 * <p>Call {@link #setShadowFocusLevel(float)} to control the strength of the shadow (focused
 * shadows cast stronger shadows).
 *
 * <p>Call {@link #setOverlayColor(int)} to control overlay color.
 */
public class ShadowOverlayContainer extends FrameLayout {
  ...
  /**
   * Sets the shadow type to {@link #SHADOW_DYNAMIC} if supported and sets the elevation/Z values to
  ...
  /**
   * Returns the shadow type, one of {@link #SHADOW_NONE}, {@link #SHADOW_STATIC}, or {@link
   * #SHADOW_DYNAMIC}.
   */
  ...
  /**
   * Initialize shadows, color overlay, and rounded corners. All are optional. Shadow type are
   * auto-selected based on {@link #useStaticShadow()} and {@link #useDynamicShadow()} call.
   *
   * @deprecated use {@link ShadowOverlayHelper#createShadowOverlayContainer(Context)} instead.
   */
  @Deprecated
  public void initialize(boolean hasShadow, boolean hasColorDimOverlay, boolean roundedCorners) {
    ...
    int roundedCornerRadius =
        roundedCorners
            ? getContext()
                .getResources()
                .getDimensionPixelSize(R.dimen.lb_rounded_rect_corner_radius)
            : 0;
    initialize(shadowType, hasColorDimOverlay, roundedCornerRadius);
  }
  ...
  void initialize(int shadowType, boolean hasColorDimOverlay, int roundedCornerRadius) {
    ...
    switch (mShadowType) {
      case SHADOW_DYNAMIC:
        mShadowImpl =
            ShadowHelper.addDynamicShadow(this, mUnfocusedZ, mFocusedZ, mRoundedCornerRadius);
        break;
      case SHADOW_STATIC:
  ...
  public void draw(Canvas canvas) {
    ...
    if (mOverlayPaint != null && mOverlayColor != Color.TRANSPARENT) {
      canvas.drawRect(
          mWrappedView.getLeft(),
          mWrappedView.getTop(),
          mWrappedView.getRight(),
          mWrappedView.getBottom(),
          mOverlayPaint);
    }
  }
