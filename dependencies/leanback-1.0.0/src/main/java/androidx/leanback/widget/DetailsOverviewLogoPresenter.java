 ...
 *
 * <p>Default implementation assumes no scaleType on ImageView and uses intrinsic width and height
 * of {@link DetailsOverviewRow#getImageDrawable()} to initialize ImageView's layout params. To
 ...
 *
 * <p>Subclass may override and has its own image view. Subclass may also download image from URL
...
public class DetailsOverviewLogoPresenter extends Presenter {
  ...
  public static class ViewHolder extends Presenter.ViewHolder {
     ...
     *     false otherwise. Used by {@link
     *     DetailsOverviewLogoPresenter#onBindViewHolder(Presenter.ViewHolder, Object)} .
    ...
    /**
     * Change if the ImageView layout size should be synchronized to Drawable intrinsic size. Used
     * by {@link DetailsOverviewLogoPresenter#onBindViewHolder(Presenter.ViewHolder, Object)}.
     *
  ...
  /**
   * Create a View for the Logo, default implementation loads from {@link
   * R.layout#lb_fullwidth_details_overview_logo}. Subclass may override this method to use a fixed
   * layout size and change ImageView scaleType. If the layout params is WRAP_CONTENT for both width
   * and size, the ViewHolder would be using intrinsic size of Drawable in {@link
  ...
  public Presenter.ViewHolder onCreateViewHolder(ViewGroup parent) {
    ...
    vh.setSizeFromDrawableIntrinsic(
  ...
  /**
   * Called from {@link FullWidthDetailsOverviewRowPresenter} to setup
   * FullWidthDetailsOverviewRowPresenter and FullWidthDetailsOverviewRowPresenter.ViewHolder that
   * hosts the logo.
   *
   * @param viewHolder
   ...
   * implementation returns true when {@link DetailsOverviewRow#getImageDrawable()} is not null. If
   * subclass of DetailsOverviewLogoPresenter manages its own image drawable, it should override
   * this function to report status correctly and invoke {@link
   * FullWidthDetailsOverviewRowPresenter#notifyOnBindLogo(FullWidthDetailsOverviewRowPresenter.ViewHolder)}
  ...
  /**
   * Bind logo View to drawable of DetailsOverviewRow and call notifyOnBindLogo(). The default
   * implementation assumes the Logo View is an ImageView and change layout size to intrinsic size
   * of ImageDrawable if {@link ViewHolder#isSizeFromDrawableIntrinsic()} is true.
   *
