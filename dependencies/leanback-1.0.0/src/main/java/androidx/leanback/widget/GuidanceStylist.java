...
/**
 * GuidanceStylist is used within a {@link androidx.leanback.app.GuidedStepFragment} to display
 * contextual information for the decision(s) required at that step.
 *
 * <p>Many aspects of the base GuidanceStylist can be customized through theming; see the theme
 ...
 * override the {@link #onProvideLayoutId} method to change the layout file used to display the
 * guidance; more complex layouts may be supported by also providing a subclass of {@link
 * GuidanceStylist.Guidance} with extra fields.
 *
 * <p>Note: If an alternate layout is provided, the following view IDs should be used to refer to
 * base elements:
 *
 * <ul>
 *   <li>{@link androidx.leanback.R.id#guidance_title}
 *   <li>{@link androidx.leanback.R.id#guidance_description}
 *   <li>{@link androidx.leanback.R.id#guidance_breadcrumb}
 *   <li>{@link androidx.leanback.R.id#guidance_icon}
 * </ul>
 *
 * <p>View IDs are allowed to be missing, in which case the corresponding views will be null.
 *
...
public class GuidanceStylist implements FragmentAnimationProvider {
   ...
   * androidx.leanback.app.GuidedStepFragment}. Guidance consists of a short title, a longer
   * description, a breadcrumb to help with global navigation (often indicating where the back
   * button will lead), and an optional icon. All this information is intended to provide users with
   * the appropriate context to make the decision(s) required by the current step.
   *
   * <p>Clients may provide a subclass of this if they wish to remember auxiliary data for use in a
   * customized GuidanceStylist.
   */
  public static class Guidance {
    ...
    /**
     * Constructs a Guidance object with the specified title, description, breadcrumb, and icon
     * drawable.
     *
  ...
  /**
   * Creates an appropriately configured view for the given Guidance, using the provided inflater
   * and container.
   *
  ...
  /**
   * Provides the resource ID of the layout defining the guidance view. Subclasses may override to
   * provide their own customized layouts. The base implementation returns {@link
   * androidx.leanback.R.layout#lb_guidance}. If overridden, the substituted layout should contain
