...
final class GridLayoutManager extends RecyclerView.LayoutManager {
  ...
  static final class LayoutParams extends RecyclerView.LayoutParams {
    ...
    void calculateItemAlignments(int orientation, View view) {
      ...
      for (int i = 0; i < defs.length; i++) {
        mAlignMultiple[i] =
            ItemAlignmentFacetHelper.getAlignmentPosition(view, defs[i], orientation);
      }
  ...
  abstract class GridLinearSmoothScroller extends LinearSmoothScroller {
    ...
    protected int calculateTimeForScrolling(int dx) {
      ...
      if (mWindowAlignment.mainAxis().getSize() > 0) {
        float minMs =
            (float) MIN_MS_SMOOTH_SCROLL_MAIN_SCREEN / mWindowAlignment.mainAxis().getSize() * dx;
        if (ms < minMs) {
          ms = (int) minMs;
        }
      }
  ...
  final class PendingMoveSmoothScroller extends GridLinearSmoothScroller {
    ...
    /**
     * Called before laid out an item when non-staggered grid can handle pending movements by
     * skipping "mNumRows" per movement; staggered grid will have to wait the item has been laid out
     * in consumePendingMovesAfterLayout().
     */
    ...
    /**
     * Called after laid out an item. Staggered grid should find view on same Row and consume
     * pending movements.
     */
    ...
    public PointF computeScrollVectorForPosition(int targetPosition) {
      ...
      int direction =
          ((mFlag & PF_REVERSE_FLOW_PRIMARY) != 0 ? mPendingMoves > 0 : mPendingMoves < 0) ? -1 : 1;
      if (mOrientation == HORIZONTAL) {
  ...
  /**
   * Note on conventions in the presence of RTL layout directions: Many properties and method names
   * reference entities related to the beginnings and ends of things. In the presence of RTL flows,
   * it may not be clear whether this is intended to reference a quantity that changes direction in
   * RTL cases, or a quantity that does not. Here are the conventions in use:
   *
   * <p>start/end: coordinate quantities - do reverse (optical) left/right: coordinate quantities -
   * do not reverse low/high: coordinate quantities - do not reverse min/max: coordinate quantities
   * - do not reverse scroll offset - coordinate quantities - do not reverse first/last: positional
   * indices - do not reverse front/end: positional indices - do not reverse prepend/append: related
   * to positional indices - do not reverse
   *
   * <p>Note that although quantities do not reverse in RTL flows, their relationship does. In LTR
   * flows, the first positional index is leftmost; in RTL flows, it is rightmost. Thus, anywhere
   * that positional quantities are mapped onto coordinate quantities, the flow must be checked and
   * the logic reversed.
   */

  /** The orientation of a "row". */
  @RecyclerView.Orientation int mOrientation = HORIZONTAL;
  ...
  /**
   * During full layout pass, when GridView had focus: onLayoutChildren will skip non-focusable
   * child and adjust mFocusPosition.
   */
  ...
  /**
   * Flag controlling whether the current/next layout should be updating the secondary size of rows.
   */
  ...
  /** Allow DPAD key to navigate out of second axis. default is true. */
  ...
  /**
   * The focused position, it's not the currently visually aligned position but it is the final
   * position that we intend to focus on. If there are multiple setSelection() called,
   * mFocusPosition saves last value.
   */
  int mFocusPosition = NO_POSITION;

  /**
   * A view can have multiple alignment position, this is the index of which alignment is used, by
   * default is 0.
   */
  ...
  /**
   * The offset to be applied to mFocusPosition, due to adapter change, on the next layout. Set to
   * Integer.MIN_VALUE means we should stop adding delta to mFocusPosition until next layout cycler.
   * TODO: This is somewhat duplication of RecyclerView getOldPosition() which is unfortunately
  ...
  @Visibility int mChildVisibility;

  /**
   * Pixels that scrolled in secondary forward direction. Negative value means backward. Note that
   * we treat secondary differently than main. For the main axis, update scroll min/max based on
   * first/last item's view location. For second axis, we don't use item's view location. We are
   * using the {@link #getRowSizeSecondary(int)} plus mScrollOffsetSecondary. see details in {@link
  ...
  /**
   * The fixed size of each grid item in the secondary direction. This corresponds to the row
   * height, equal for all rows. Grid items may have variable length in the primary direction.
   */
  ...
  /** Number of rows requested, can be 0 to be determined by parent size and rowHeight. */
  ...
  void fireOnChildViewHolderSelected(
      RecyclerView parent, RecyclerView.ViewHolder child, int position, int subposition) {
    if (mChildViewHolderSelectedListeners == null) {
      return;
    }
    for (int i = mChildViewHolderSelectedListeners.size() - 1; i >= 0; i--) {
      mChildViewHolderSelectedListeners
          .get(i)
          .onChildViewHolderSelected(parent, child, position, subposition);
    }
  }
    ...
    if (view != null) {
      ...
      if (mChildSelectedListener != null) {
        mChildSelectedListener.onChildSelected(
            mBaseGridView, view, mFocusPosition, vh == null ? NO_ID : vh.getItemId());
      }
  ...
  void dispatchChildSelectedAndPositioned() {
    ...
    if (view != null) {
      RecyclerView.ViewHolder vh = mBaseGridView.getChildViewHolder(view);
      fireOnChildViewHolderSelectedAndPositioned(
  ...
  /**
   * Re-initialize data structures for a data change or handling invisible selection. The method
   * tries its best to preserve position information so that staggered grid looks same before and
   * after re-initialize.
   *
    ...
    if (view != null) {
      ...
      int childWidthSpec =
          ViewGroup.getChildMeasureSpec(
              widthSpec, getPaddingLeft() + getPaddingRight() + widthUsed, p.width);
      int childHeightSpec =
          ViewGroup.getChildMeasureSpec(
              heightSpec, getPaddingTop() + getPaddingBottom() + heightUsed, p.height);
    ...
    for (int rowIndex = 0; rowIndex < mNumRows; rowIndex++) {
      ...
      if (!mBaseGridView.hasFixedSize() && measure && rowSize < 0 && itemCount > 0) {
        if (scrapeChildSize < 0) {
          ...
          if (getChildCount() > 0) {
            int firstPos = mBaseGridView.getChildViewHolder(getChildAt(0)).getLayoutPosition();
            int lastPos =
                mBaseGridView
            ...
            if (position >= firstPos && position <= lastPos) {
              position =
                  (position - firstPos <= lastPos - position) ? (firstPos - 1) : (lastPos + 1);
          ...
          if (position >= 0 && position < itemCount) {
            measureScrapChild(
                ...
                mMeasuredDimension);
            scrapeChildSize =
                mOrientation == HORIZONTAL ? mMeasuredDimension[1] : mMeasuredDimension[0];
            if (DEBUG) {
              Log.v(
                  TAG,
                  "measured scrap child: " + mMeasuredDimension[0] + " " + mMeasuredDimension[1]);
            }
          }
        }
        if (scrapeChildSize >= 0) {
      ...
      if (mRowSizeSecondary[rowIndex] != rowSize) {
        if (DEBUG) {
          Log.v(
              getTag(),
              "row size secondary changed: " + mRowSizeSecondary[rowIndex] + ", " + rowSize);
        }
  ...
  public void onMeasure(Recycler recycler, State state, int widthSpec, int heightSpec) {
    ...
    mIsPortrait =
        mBaseGridView.getContext().getResources().getConfiguration().orientation
      ...
      switch (modeSecondary) {
        ...
        case MeasureSpec.AT_MOST:
          measuredSizeSecondary =
              Math.min(getSizeSecondary() + paddingSecondary, mMaxSizeSecondary);
          break;
        ...
        case MeasureSpec.UNSPECIFIED:
          mFixedRowSizeSecondary =
              mRowSizeSecondaryRequested == 0
                  ? sizeSecondary - paddingSecondary
                  : mRowSizeSecondaryRequested;
          mNumRows = mNumRowsRequested == 0 ? 1 : mNumRowsRequested;
          measuredSizeSecondary =
              mFixedRowSizeSecondary * mNumRows
                  + mSpacingSecondary * (mNumRows - 1)
                  + paddingSecondary;
          break;
        case MeasureSpec.AT_MOST:
        case MeasureSpec.EXACTLY:
    ...
    if (DEBUG) {
      Log.v(
          ...
          "measureChild secondarySpec "
              + Integer.toHexString(secondarySpec)
              + " widthSpec "
              + Integer.toHexString(widthSpec)
              + " heightSpec "
              + Integer.toHexString(heightSpec)
      ...
      new Grid.Provider() {
        ...
        public int createItem(int index, boolean append, Object[] item, boolean disappearingItem) {
          ...
          if (!lp.isItemRemoved()) {
            ...
            if ((mFlag & PF_STAGE_MASK) != PF_STAGE_LAYOUT) {
              ...
              if ((mFlag & PF_IN_LAYOUT_SEARCH_FOCUS) == 0
                  && index == mFocusPosition
                  && subindex == mSubFocusPosition) {
                dispatchChildSelected();
              } else if ((mFlag & PF_IN_LAYOUT_SEARCH_FOCUS) != 0
                  && index >= mFocusPosition
                  && v.hasFocusable()) {
        ...
        public void addItem(Object item, int index, int length, int rowIndex, int edge) {
          ...
          if (edge == Integer.MIN_VALUE || edge == Integer.MAX_VALUE) {
            edge =
                !mGrid.isReversedFlow()
                    ? mWindowAlignment.mainAxis().getPaddingMin()
          ...
          int startSecondary =
              getRowStartSecondary(rowIndex)
                  + mWindowAlignment.secondAxis().getPaddingMin()
          ...
          if (mChildLaidOutListener != null) {
            RecyclerView.ViewHolder vh = mBaseGridView.getChildViewHolder(v);
            mChildLaidOutListener.onChildLaidOut(
                mBaseGridView, v, index, vh == null ? NO_ID : vh.getItemId());
          }
        }
  ...
  void layoutChild(int rowIndex, View v, int start, int end, int startSecondary) {
    ...
    final int horizontalGravity =
        (mFlag & PF_REVERSE_FLOW_MASK) != 0
            ? Gravity.getAbsoluteGravity(
                mGravity & Gravity.RELATIVE_HORIZONTAL_GRAVITY_MASK, View.LAYOUT_DIRECTION_RTL)
    ...
    params.setOpticalInsets(
        left - sTempRect.left,
        top - sTempRect.top,
        sTempRect.right - right,
  ...
  private void removeInvisibleViewsAtEnd() {
    if ((mFlag & (PF_PRUNE_CHILD | PF_SLIDING)) == PF_PRUNE_CHILD) {
      mGrid.removeInvisibleItemsAtEnd(
          mFocusPosition,
          (mFlag & PF_REVERSE_FLOW_PRIMARY) != 0
              ? -mExtraLayoutSpace
  ...
  private void removeInvisibleViewsAtFront() {
    if ((mFlag & (PF_PRUNE_CHILD | PF_SLIDING)) == PF_PRUNE_CHILD) {
      mGrid.removeInvisibleItemsAtFront(
          mFocusPosition,
          (mFlag & PF_REVERSE_FLOW_PRIMARY) != 0
              ? mSizePrimary + mExtraLayoutSpace
              : -mExtraLayoutSpace);
    ...
    if ((mFlag & PF_SLIDING) != 0) {
      ...
      if ((mFlag & PF_LAYOUT_EATEN_IN_SLIDING) != 0) {
        ...
        if (mBaseGridView.getScrollState() != SCROLL_STATE_IDLE || isSmoothScrolling()) {
          mBaseGridView.addOnScrollListener(
  ...
  void slideOut() {
    ...
    if (mOrientation == VERTICAL) {
      mBaseGridView.smoothScrollBy(
          0, getSlideOutDistance(), new AccelerateDecelerateInterpolator());
    } else {
      mBaseGridView.smoothScrollBy(
          getSlideOutDistance(), 0, new AccelerateDecelerateInterpolator());
    }
  }
  ...
  /**
   * Fast layout when there is no structure change, adapter change, etc. It will layout all views
   * was layout requested or updated, until hit a view with different size, then it break and
   * detachAndScrap all views after that.
   */
  private void fastRelayout() {
    ...
    for (; index < childCount; index++, position++) {
      ...
      int startSecondary =
          getRowStartSecondary(location.row)
              + mWindowAlignment.secondAxis().getPaddingMin()
              - mScrollOffsetSecondary;
      int primarySize, end;
  ...
  private void focusToViewInLayout(
      boolean hadFocus, boolean alignToView, int extraDelta, int extraDeltaSecondary) {
  ...
  @VisibleForTesting OnLayoutCompleteListener mLayoutCompleteListener;
  ...
  public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
    ...
    if (state.isPreLayout()) {
      ...
      if (mGrid != null && childCount > 0) {
        ...
        int minOldAdapterPosition =
            mBaseGridView.getChildViewHolder(getChildAt(0)).getOldPosition();
        int maxOldAdapterPosition =
            mBaseGridView.getChildViewHolder(getChildAt(childCount - 1)).getOldPosition();
        for (int i = 0; i < childCount; i++) {
        ...
        try {
          ...
          // IndexOutOfBoundsException: Invalid item position -1(-1). Item count:12
          // androidx.leanback.widget.VerticalGridView
          // NullPointerException: Attempt to invoke virtual method
          // 'android.view.ViewGroup$LayoutParams android.view.View.getLayoutParams()'
          // IllegalArgumentException: VideoCardPresenter$1 is not a direct child of
          // HorizontalGridView
          e.printStackTrace();
        }
      }
    ...
    final boolean scrollToFocus =
        !isSmoothScrolling() && mFocusScrollStrategy == BaseGridView.FOCUS_SCROLL_ALIGNED;
    ...
    if (layoutInit()) {
      ...
      try {
        ...
        // IndexOutOfBoundsException: Invalid item position 2(2). Item count:2 HorizontalGridView
        // (Aftmm)
        // IllegalArgumentException: Called attach on a child which is not detached:
        // ViewHolder{434061b0 position=10 id=-1, oldPos=-1, pLpos:-1 no parent}
        // IllegalStateException: Layout state should be one of 100 but it is 10
        // NullPointerException: Attempt to read from field 'int
        // androidx.recyclerview.widget.ViewInfoStore$InfoRecord.flags' on a null object reference
        e.printStackTrace();
      }
    } else {
      mFlag &= ~PF_FAST_RELAYOUT;
      // layoutInit() has detached all views, so start from scratch
      mFlag = (mFlag & ~PF_IN_LAYOUT_SEARCH_FOCUS) | (hadFocus ? PF_IN_LAYOUT_SEARCH_FOCUS : 0);
      int startFromPosition, endPos;
      if (scrollToFocus
          && (firstVisibleIndex < 0
              || mFocusPosition > lastVisibleIndex
      ...
      if (endPos != NO_POSITION) {
        ...
        try {
          ...
          // IndexOutOfBoundsException: Invalid item position 20(20). Item count:6
          // androidx.leanback.widget.VerticalGridView
          // NullPointerException: Attempt to invoke virtual method
          // 'android.content.res.StringBlock.get(int)' on a null object reference
          // InflateException: Binary XML file line #60: Binary XML file line #60: Error inflating
          // class <unknown>
          e.printStackTrace();
        }
      }
    ...
    try {
      ...
      // NullPointerException: Attempt to invoke virtual method 'android.view.ViewGroup$LayoutParams
  ...
  public int scrollHorizontallyBy(int dx, Recycler recycler, RecyclerView.State state) {
    ...
    if (mOrientation == HORIZONTAL) {
      ...
      try {
        ...
        // Attempt to invoke virtual method 'android.view.ViewGroup$LayoutParams
        // android.view.View.getLayoutParams()' on a null object reference
        // IllegalArgumentException: VideoCardPresenter$1 is not a direct child of
  ...
  public void collectAdjacentPrefetchPositions(
      int dx, int dy, State state, LayoutPrefetchRegistry layoutPrefetchRegistry) {
    try {
      ...
      int fromLimit = da < 0 ? -mExtraLayoutSpace : mSizePrimary + mExtraLayoutSpace;
  ...
  public void collectInitialPrefetchPositions(
      int adapterItemCount, LayoutPrefetchRegistry layoutPrefetchRegistry) {
    ...
    if (adapterItemCount != 0 && numToPrefetch != 0) {
      ...
      int initialPos =
          Math.max(
              0,
              Math.min(mFocusPosition - (numToPrefetch - 1) / 2, adapterItemCount - numToPrefetch));
  ...
  void updateScrollLimits() {
    ...
    if (!highAvailable
        && mWindowAlignment.mainAxis().isMaxUnknown()
        && !lowAvailable
        && mWindowAlignment.mainAxis().isMinUnknown()) {
      return;
    }
  ...
  /**
   * Update secondary axis's scroll min/max, should be updated in {@link
   * #scrollDirectionSecondary(int)}.
   */
  ...
  private void initScrollController() {
    ...
    if (DEBUG) {
      Log.v(
          getTag(),
          "initScrollController mSizePrimary "
              + mSizePrimary
              + " mWindowAlignment "
              + mWindowAlignment);
    }
  ...
  private void updateScrollController() {
    ...
    if (DEBUG) {
      Log.v(
          getTag(),
          "updateScrollController mSizePrimary "
              + mSizePrimary
              + " mWindowAlignment "
              + mWindowAlignment);
    }
  ...
  void scrollToSelection(int position, int subposition, boolean smooth, int primaryScrollExtra) {
    ...
    if (notSmoothScrolling
        && !mBaseGridView.isLayoutRequested()
        && view != null
  ...
  /**
   * when start a new SmoothScroller or scroll to a different location, dont need current
   * SmoothScroller.onStopInternal() doing the scroll work.
   */
  ...
  private void processPendingMovement(boolean forward) {
    ...
    if (mPendingMoveSmoothScroller == null) {
      ...
      mBaseGridView.stopScroll();
      PendingMoveSmoothScroller linearSmoothScroller =
          new PendingMoveSmoothScroller(forward ? 1 : -1, mNumRows > 1);
      mFocusPositionOffset = 0;
      startSmoothScroll(linearSmoothScroller);
    } else {
      if (forward) {
  ...
  public void onItemsAdded(RecyclerView recyclerView, int positionStart, int itemCount) {
    if (DEBUG)
      Log.v(getTag(), "onItemsAdded positionStart " + positionStart + " itemCount " + itemCount);
  ...
  public void onItemsRemoved(RecyclerView recyclerView, int positionStart, int itemCount) {
    if (DEBUG)
      Log.v(getTag(), "onItemsRemoved positionStart " + positionStart + " itemCount " + itemCount);
    if (mFocusPosition != NO_POSITION
        && mGrid != null
        && mGrid.getFirstVisibleIndex() >= 0
        && mFocusPositionOffset != Integer.MIN_VALUE) {
  ...
  public void onItemsMoved(
      RecyclerView recyclerView, int fromPosition, int toPosition, int itemCount) {
    if (DEBUG)
      Log.v(getTag(), "onItemsMoved fromPosition " + fromPosition + " toPosition " + toPosition);
    if (mFocusPosition != NO_POSITION && mFocusPositionOffset != Integer.MIN_VALUE) {
      int pos = mFocusPosition + mFocusPositionOffset;
  ...
  public void onItemsUpdated(RecyclerView recyclerView, int positionStart, int itemCount) {
    if (DEBUG)
      Log.v(getTag(), "onItemsUpdated positionStart " + positionStart + " itemCount " + itemCount);
  ...
  public boolean requestChildRectangleOnScreen(
      RecyclerView parent, View view, Rect rect, boolean immediate) {
  ...
  /**
   * Return the scroll delta on primary direction to make the view selected. If the return value is
   * 0, there is no need to scroll.
   */
   ...
   * on the view).
   */
  private int getAdjustedPrimaryAlignedScrollDistance(
      int scrollPrimary, View view, View childView) {
  ...
  void scrollToView(View view, boolean smooth, int extraDelta, int extraDeltaSecondary) {
    scrollToView(
        view, view == null ? null : view.findFocus(), smooth, extraDelta, extraDeltaSecondary);
  }
  ...
  private void scrollToView(
      View view, View childView, boolean smooth, int extraDelta, int extraDeltaSecondary) {
    if ((mFlag & PF_SLIDING) != 0) {
      return;
    }
  ...
  public View onInterceptFocusSearch(View focused, int direction) {
    ...
    if (direction == View.FOCUS_FORWARD || direction == View.FOCUS_BACKWARD) {
      ...
      if (canScrollHorizontally()) {
        ...
        final int absDir =
            (direction == View.FOCUS_FORWARD) ^ rtl ? View.FOCUS_RIGHT : View.FOCUS_LEFT;
        result = ff.findNextFocus(mBaseGridView, focused, absDir);
      }
    } else {
  ...
  public boolean onAddFocusables(
    ...
    if (recyclerView.hasFocus()) {
      ...
      final View immediateFocusedChild =
          (focusedPos == NO_POSITION) ? null : findViewByPosition(focusedPos);
      ...
      final int focusedRow =
          mGrid != null && immediateFocusedChild != null
              ? mGrid.getLocation(focusedPos).row
              : NO_POSITION;
  ...
  boolean isItemFullyVisible(int pos) {
    ...
    return vh.itemView.getLeft() >= 0
        && vh.itemView.getRight() <= mBaseGridView.getWidth()
        && vh.itemView.getTop() >= 0
        && vh.itemView.getBottom() <= mBaseGridView.getHeight();
  }
    ...
    switch (mFocusScrollStrategy) {
      ...
      default:
        return gridOnRequestFocusInDescendantsAligned(
            recyclerView, direction, previouslyFocusedRect);
      case BaseGridView.FOCUS_SCROLL_PAGE:
      case BaseGridView.FOCUS_SCROLL_ITEM:
  ...
  int processSelectionMoves(boolean preventScroll, int moves) {
    ...
    int focusedRow = focusPosition != NO_POSITION ? mGrid.getRowIndex(focusPosition) : NO_POSITION;
    View newSelected = null;
