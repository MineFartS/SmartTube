...
/**
 * State: each State has incoming Transitions and outgoing Transitions. When {@link
 * State#mBranchStart} is true, all the outgoing Transitions may be triggered, when {@link
 * State#mBranchStart} is false, only first outgoing Transition will be triggered. When {@link
 * State#mBranchEnd} is true, all the incoming Transitions must be triggered for the State to run.
 * When {@link State#mBranchEnd} is false, only need one incoming Transition triggered for the State
 * to run. Transition: three types: 1. Event based transition, transition will be triggered when
 * {@link #fireEvent(Event)} is called. 2. Auto transition, transition will be triggered when {@link
 * Transition#mFromState} is executed. 3. Condiitonal Auto transition, transition will be triggered
 * when {@link Transition#mFromState} is executed and {@link Transition#mCondition} passes.
 *
 * @hide
 */
...
public final class StateMachine {
  ...
  public static class State {
     ...
     * Create a State
     *
     * @param branchStart True if can run all out going transitions or false execute the first out
     *     going transition.
     * @param branchEnd True if wait all incoming transitions executed or false only need one of the
     *     transition executed.
     */
