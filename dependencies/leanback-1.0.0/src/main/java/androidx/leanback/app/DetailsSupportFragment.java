 ...
 *
 * <p>A DetailsSupportFragment renders the elements of its {@link ObjectAdapter} as a set of rows in
 * a vertical list.The Adapter's {@link PresenterSelector} must maintain subclasses of {@link
 * RowPresenter}. When {@link FullWidthDetailsOverviewRowPresenter} is found in adapter,
 * DetailsSupportFragment will setup default behavior of the DetailsOverviewRow:
 ...
 *
 *     <p>The recommended activity themes to use with a DetailsSupportFragment are
 ...
 *
 *     <p>DetailsSupportFragment can use {@link DetailsSupportFragmentBackgroundController} to add a
 *     parallax drawable background and embedded video playing fragment.
 */
public class DetailsSupportFragment extends BaseSupportFragment {
  ...
  public View onCreateView(
    ...
    mRowsSupportFragment =
        (RowsSupportFragment) getChildFragmentManager().findFragmentById(R.id.details_rows_dock);
    if (mRowsSupportFragment == null) {
      mRowsSupportFragment = new RowsSupportFragment();
      getChildFragmentManager()
          .beginTransaction()
          .replace(R.id.details_rows_dock, mRowsSupportFragment)
          .commit();
    }
    ...
    if (Build.VERSION.SDK_INT >= 21) {
      ...
      mRowsSupportFragment.setExternalAdapterListener(
   ...
   * DetailsSupportFragment. For example how a row is aligned in details Fragment. The default
   * implementation invokes {@link
   * #setupDetailsOverviewRowPresenter(FullWidthDetailsOverviewRowPresenter)}
   */
  ...
  /**
   * This method asks DetailsSupportFragmentBackgroundController to add a fragment for rendering
   * video. In case the fragment is already there, it will return the existing one. The method must
   * be called after calling super.onCreate(). App usually does not call this method directly.
   *
   ...
   * control embedded video playback. App usually does not use this method directly. App may use
   * this method for other custom parallax tasks.
   *
            ...
            if (mRowsSupportFragment.getVerticalGridView() != null
              ...
              if (direction == View.FOCUS_UP) {
                if (mDetailsBackgroundController != null
                    ...
                    && mVideoSupportFragment != null
                    && mVideoSupportFragment.getView() != null) {
