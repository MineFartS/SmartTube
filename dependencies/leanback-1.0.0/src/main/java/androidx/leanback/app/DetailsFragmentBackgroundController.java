 ...
 *
 * <p>To customize playback fragment, app should override {@link #onCreateVideoFragment()} and
...
public class DetailsFragmentBackgroundController {
  ...
  /**
   * Creates a DetailsFragmentBackgroundController for a DetailsFragment. Note that each
   * DetailsFragment can only associate with one DetailsFragmentBackgroundController.
   *
  ...
  public DetailsFragmentBackgroundController(DetailsFragment fragment) {
    if (fragment.mDetailsBackgroundController != null) {
      throw new IllegalStateException(
          "Each DetailsFragment is allowed to initialize "
   ...
   * VideoFragment and VideoFragmentGlueHost to host the PlaybackGlue. This method must be called
   * after calling details Fragment super.onCreate(). This method can be called multiple times to
   * replace existing PlaybackGlue or calling setupVideoPlayback(null) to clear. Note a typical
   * {@link PlaybackGlue} subclass releases resources in {@link PlaybackGlue#onDetachedFromHost()},
   * when the {@link PlaybackGlue} subclass is not doing that, it's app's responsibility to release
   * the resources.
   *
   ...
   * #canNavigateToVideoFragment()}. If the method is called in DetailsFragment.onCreate() it will
   * make video fragment to be initially focused once it is created.
   *
   * <p>Calling switchToVideo() in DetailsFragment.onCreate() will clear the activity enter
   * transition and shared element transition.
   *
   * <p>If switchToVideo() is called after {@link DetailsFragment#prepareEntranceTransition()} and
   * before {@link DetailsFragment#onEntranceTransitionEnd()}, it will be ignored.
   *
   * <p>If {@link DetailsFragment#prepareEntranceTransition()} is called after switchToVideo(), an
   ...
   * #onCreateVideoFragment()}. This method must be called after calling Fragment super.onCreate().
   * When override this method, app may call {@link #findOrCreateVideoFragment()} to get or create a
   * fragment.
   *
  ...
  /**
   * Adds or gets fragment for rendering video in DetailsFragment. A subclass that overrides {@link
   * #onCreateGlueHost()} should call this method to get a fragment for creating a {@link
   * PlaybackGlueHost}.
   *
