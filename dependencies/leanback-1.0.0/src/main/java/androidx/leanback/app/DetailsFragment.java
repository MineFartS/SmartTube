 ...
 *
 * <p>A DetailsFragment renders the elements of its {@link ObjectAdapter} as a set of rows in a
 * vertical list.The Adapter's {@link PresenterSelector} must maintain subclasses of {@link
 * RowPresenter}. When {@link FullWidthDetailsOverviewRowPresenter} is found in adapter,
 * DetailsFragment will setup default behavior of the DetailsOverviewRow:
 * <li>The alignment of FullWidthDetailsOverviewRowPresenter is setup in {@link
 *     #setupDetailsOverviewRowPresenter(FullWidthDetailsOverviewRowPresenter)}.
 * <li>The view status switching of FullWidthDetailsOverviewRowPresenter is done in {@link
 ...
 *
 *     <p>The recommended activity themes to use with a DetailsFragment are
 * <li>{@link androidx.leanback.R.style#Theme_Leanback_Details} with activity shared element
 *     transition for {@link FullWidthDetailsOverviewRowPresenter}.
 * <li>{@link androidx.leanback.R.style#Theme_Leanback_Details_NoSharedElementTransition} if shared
 *     element transition is not needed, for example if first row is not rendered by {@link
 *     FullWidthDetailsOverviewRowPresenter}.
 *
 *     <p>DetailsFragment can use {@link DetailsFragmentBackgroundController} to add a parallax
 *     drawable background and embedded video playing fragment.
 *
...
public class DetailsFragment extends BaseFragment {
  ...
  final State STATE_SWITCH_TO_VIDEO_IN_ON_CREATE =
      new State("STATE_SWITCH_TO_VIDEO_IN_ON_CREATE", false, false) {
        @Override
        public void run() {
  ...
  final State STATE_ENTER_TRANSITION_CANCEL =
      new State("STATE_ENTER_TRANSITION_CANCEL", false, false) {
        @Override
        public void run() {
          ...
          if (getActivity() != null) {
            ...
            Object sharedReturnTransition =
                TransitionHelper.getSharedElementReturnTransition(window);
  ...
  final State STATE_ENTER_TRANSITION_COMPLETE =
      new State("STATE_ENTER_TRANSIITON_COMPLETE", true, false);

  final State STATE_ENTER_TRANSITION_ADDLISTENER =
      new State("STATE_ENTER_TRANSITION_PENDING") {
        @Override
        public void run() {
  ...
  void createStateMachineTransitions() {
    ...
    mStateMachine.addTransition(
        STATE_ENTER_TRANSITION_INIT, STATE_ENTER_TRANSITION_COMPLETE, EVT_NO_ENTER_TRANSITION);
    // if switchToVideo is called before EVT_ON_CREATEVIEW, clear enter transition and skip to
    // complete.
    mStateMachine.addTransition(
        STATE_ENTER_TRANSITION_INIT, STATE_ENTER_TRANSITION_CANCEL, EVT_SWITCH_TO_VIDEO);
    ...
    // it to finish
    mStateMachine.addTransition(
        STATE_ENTER_TRANSITION_INIT, STATE_ENTER_TRANSITION_ADDLISTENER, EVT_ON_CREATEVIEW);
    ...
    mStateMachine.addTransition(
        STATE_ENTER_TRANSITION_ADDLISTENER, STATE_ENTER_TRANSITION_PENDING, EVT_DETAILS_ROW_LOADED);
    ...
    mStateMachine.addTransition(
        STATE_ENTER_TRANSITION_PENDING, STATE_ENTER_TRANSITION_COMPLETE, EVT_ENTER_TRANSIITON_DONE);
    ...
    mStateMachine.addTransition(
        STATE_ENTRANCE_INIT, STATE_SWITCH_TO_VIDEO_IN_ON_CREATE, EVT_SWITCH_TO_VIDEO);
    ...
    mStateMachine.addTransition(
        STATE_ENTRANCE_COMPLETE, STATE_SWITCH_TO_VIDEO_IN_ON_CREATE, EVT_SWITCH_TO_VIDEO);

    // for once the view is created in onStart and prepareEntranceTransition was called, we
    // could setEntranceStartState:
    mStateMachine.addTransition(
        STATE_ENTRANCE_ON_PREPARED, STATE_SET_ENTRANCE_START_STATE, EVT_ONSTART);
  ...
  public View onCreateView(
    ...
    mRootView =
        (BrowseFrameLayout) inflater.inflate(R.layout.lb_details_fragment, container, false);
    mBackgroundView = mRootView.findViewById(R.id.details_background_view);
    if (mBackgroundView != null) {
      mBackgroundView.setBackground(mBackgroundDrawable);
    }
    mRowsFragment =
        (RowsFragment) getChildFragmentManager().findFragmentById(R.id.details_rows_dock);
    if (mRowsFragment == null) {
      mRowsFragment = new RowsFragment();
      getChildFragmentManager()
          .beginTransaction()
          .replace(R.id.details_rows_dock, mRowsFragment)
          .commit();
    }
    ...
    if (Build.VERSION.SDK_INT >= 21) {
      ...
      mRowsFragment.setExternalAdapterListener(
            ...
            public void onCreate(ItemBridgeAdapter.ViewHolder vh) {
              if (mDetailsParallax != null
                ...
                FullWidthDetailsOverviewRowPresenter.ViewHolder rowVh =
                    (FullWidthDetailsOverviewRowPresenter.ViewHolder) vh.getViewHolder();
                rowVh.getOverviewView().setTag(R.id.lb_parallax_source, mDetailsParallax);
              }
            }
          });
    }
   ...
   * example how a row is aligned in details Fragment. The default implementation invokes {@link
   * #setupDetailsOverviewRowPresenter(FullWidthDetailsOverviewRowPresenter)}
   */
  ...
  /**
   * Called to setup {@link FullWidthDetailsOverviewRowPresenter}. The default implementation adds
   * two alignment positions({@link ItemAlignmentFacet}) for ViewHolder of
  ...
  protected void setupDetailsOverviewRowPresenter(FullWidthDetailsOverviewRowPresenter presenter) {
    ...
    alignDef1.setItemAlignmentOffset(
        -getResources().getDimensionPixelSize(R.dimen.lb_details_v2_align_pos_for_actions));
    ...
    alignDef2.setItemAlignmentOffset(
        -getResources().getDimensionPixelSize(R.dimen.lb_details_v2_align_pos_for_description));
  ...
  /**
   * This method asks DetailsFragmentBackgroundController to add a fragment for rendering video. In
   * case the fragment is already there, it will return the existing one. The method must be called
   * after calling super.onCreate(). App usually does not call this method directly.
   *
  ...
  final Fragment findOrCreateVideoFragment() {
    ...
    if (fragment == null && mDetailsBackgroundController != null) {
      ...
      ft2.add(
          androidx.leanback.R.id.video_surface_container,
  ...
  void onRowSelected(int selectedPosition, int selectedSubPosition) {
    ...
    if (adapter != null && adapter.size() > selectedPosition) {
      ...
      for (int i = 0; i < count; i++) {
        ItemBridgeAdapter.ViewHolder bridgeViewHolder =
            (ItemBridgeAdapter.ViewHolder) gridView.getChildViewHolder(gridView.getChildAt(i));
  ...
  /**
   * Called when onStart and enter transition (postponed/none postponed) and entrance transition are
   * all finished.
   */
  @CallSuper
  void onSafeStart() {
  ...
  /**
   * Called on every visible row to change view status when current selected row position or
   * selected sub position changed. Subclass may override. The default implementation calls {@link
   * #onSetDetailsOverviewRowStatus(FullWidthDetailsOverviewRowPresenter,
   * FullWidthDetailsOverviewRowPresenter.ViewHolder, int, int, int)} if presenter is instance of
   * {@link FullWidthDetailsOverviewRowPresenter}.
   *
   * @param presenter The presenter used to create row ViewHolder.
   * @param viewHolder The visible (attached) row ViewHolder, note that it may or may not be
   *     selected.
   ...
   * @param selectedSubPosition The sub position within currently selected row. This is used When a
   *     row has multiple alignment positions.
   */
  ...
  /**
   * Called to change DetailsOverviewRow view status when current selected row position or selected
   * sub position changed. Subclass may override. The default implementation switches between three
   * states based on the positions: {@link FullWidthDetailsOverviewRowPresenter#STATE_HALF}, {@link
   * FullWidthDetailsOverviewRowPresenter#STATE_FULL} and {@link
  ...
  protected Object createEntranceTransition() {
    return TransitionHelper.loadTransition(
        FragmentUtil.getContext(DetailsFragment.this), R.transition.lb_details_enter_transition);
  }
  ...
  /**
   * Returns the {@link DetailsParallax} instance used by {@link
   * DetailsFragmentBackgroundController} to configure parallax effect of background and control
   * embedded video playback. App usually does not use this method directly. App may use this method
   * for other custom parallax tasks.
   *
  ...
  /**
   * Set background drawable shown below foreground rows UI and above {@link
   ...
   *
   * <ul>
   *   <li>sets up focus search handling logic in the root view to enable transitioning between half
   *       screen/full screen/no video mode.
   *   <li>Sets up the key listener in the root view to intercept events like UP/DOWN and transition
   *       to appropriate mode like half/full screen video.
   * </ul>
   */
  void setupDpadNavigation() {
    mRootView.setOnChildFocusListener(
    ...
    mRootView.setOnFocusSearchListener(
    ...
    mRootView.setOnDispatchKeyListener(
