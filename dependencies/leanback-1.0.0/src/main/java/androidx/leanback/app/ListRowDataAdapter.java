...
/**
 * Wrapper class for {@link ObjectAdapter} used by {@link BrowseFragment} to initialize {@link
 * RowsFragment}. We use invisible rows to represent {@link androidx.leanback.widget.DividerRow},
 * {@link androidx.leanback.widget.SectionRow} and {@link androidx.leanback.widget.PageRow} in
 * RowsFragment. In case we have an invisible row at the end of a RowsFragment, it creates a jumping
 * effect as the layout manager thinks there are items even though they're invisible. This class
 * takes care of filtering out the invisible rows at the end. In case the data inside the adapter
 * changes, it adjusts the bounds to reflect the latest data. {@link #detach()} must be called to
 * release DataObserver from Adapter.
 */
class ListRowDataAdapter extends ObjectAdapter {
  ...
  private class SimpleDataObserver extends DataObserver {
    ...
    public void onItemRangeRemoved(int positionStart, int itemCount) {
      ...
      if (totalItems > 0) {
        onEventFired(
            ON_ITEM_RANGE_REMOVED, Math.min(mLastVisibleRowIndex + 1, positionStart), totalItems);
      }
    }
  ...
  /**
   * When using custom {@link ObjectAdapter}, it's possible that the user may make multiple changes
   * to the underlying data at once. The notifications about those updates may be batched and the
   * underlying data would have changed to reflect latest updates as opposed to intermediate
   * changes. In order to force RecyclerView to refresh the view with access only to the final data,
   * we call notifyChange().
   */
