...
/**
 * Fragment class for managing search and branding using a view that implements {@link
...
public class BrandedSupportFragment extends Fragment {
  ...
  /**
   * Called by {@link #installTitleView(LayoutInflater, ViewGroup, Bundle)} to inflate title view.
   * Default implementation uses layout file lb_browse_title. Subclass may override and use its own
   * layout, the layout must have a descendant with id browse_title_group that implements {@link
   * TitleViewAdapter.Provider}. Subclass may return null if no title is needed.
   *
  ...
  public View onInflateTitleView(
    ...
    boolean found =
        parent
            .getContext()
            .getTheme()
            .resolveAttribute(R.attr.browseTitleViewLayout, typedValue, true);
    return inflater.inflate(
        found ? typedValue.resourceId : R.layout.lb_browse_title, parent, false);
  }

  /**
   * Inflate title view and add to parent. This method should be called in {@link
   ...
   *
   * <p>The presence of a listener will change the visibility of the search affordance in the
   * fragment title. When set to non-null, the title will contain an element that a user may click
   * to begin a search.
   *
   * <p>The listener's {@link View.OnClickListener#onClick onClick} method will be invoked when the
   * user clicks on the search element.
   *
  ...
  /**
   * Sets the {@link androidx.leanback.widget.SearchOrbView.Colors} used to draw the search
   * affordance.
   *
  ...
  /**
   * Returns the {@link androidx.leanback.widget.SearchOrbView.Colors} used to draw the search
   * affordance.
   */
  ...
  /**
   * Sets the color used to draw the search affordance. A default brighter color will be set by the
   * framework.
   *
