 ...
 *
 * <p>The parallax background drawable is made of two parts: cover drawable (by default {@link
 * FitWidthBitmapDrawable}) above the details overview row and bottom drawable (by default {@link
 * ColorDrawable}) below the details overview row. While vertically scrolling rows, the size of
 * cover drawable and bottom drawable will be updated and the cover drawable will by default perform
 * a parallax shift using {@link FitWidthBitmapDrawable#PROPERTY_VERTICAL_OFFSET}.
 *
 * <pre>
 *        ***************************
 *        *      Cover Drawable     *
 ...
 * </pre>
 *
 * Both parallax background drawable and embedded video play are optional. App must call {@link
 * #enableParallax()} and/or {@link #setupVideoPlayback(PlaybackGlue)} explicitly. The PlaybackGlue
 * is automatically {@link PlaybackGlue#play()} when fragment starts and {@link
 * PlaybackGlue#pause()} when fragment stops. When video is ready to play, cover drawable will be
 * faded out. Example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>To customize cover drawable and/or bottom drawable, app should call {@link
 * #enableParallax(Drawable, Drawable, ParallaxTarget.PropertyValuesHolderTarget)}. If app supplies
 * a custom cover Drawable, it should not call {@link #setCoverBitmap(Bitmap)}. If app supplies a
 * custom bottom Drawable, it should not call {@link #setSolidColor(int)}.
 *
 * <p>To customize playback fragment, app should override {@link #onCreateVideoSupportFragment()}
 * and {@link #onCreateGlueHost()}.
 */
public class DetailsSupportFragmentBackgroundController {
   ...
   * each DetailsSupportFragment can only associate with one
  ...
  public DetailsSupportFragmentBackgroundController(DetailsSupportFragment fragment) {
    if (fragment.mDetailsBackgroundController != null) {
      throw new IllegalStateException(
          "Each DetailsSupportFragment is allowed to initialize "
   ...
   * and {@link ColorDrawable} as bottom drawable. A vertical parallax movement will be applied to
   * the FitWidthBitmapDrawable. App may use {@link #setSolidColor(int)} and {@link
   * #setCoverBitmap(Bitmap)} to change the content of bottom drawable and cover drawable. This
   * method must be called before {@link #setupVideoPlayback(PlaybackGlue)}.
   *
    ...
    enableParallax(
        ...
        new ParallaxTarget.PropertyValuesHolderTarget(
            ...
            PropertyValuesHolder.ofInt(
                FitWidthBitmapDrawable.PROPERTY_VERTICAL_OFFSET, 0, -offset)));
  }
   ...
   *
   * @param coverDrawable Custom cover drawable shown at top. {@link #setCoverBitmap(Bitmap)} will
   *     not work if coverDrawable is not {@link FitWidthBitmapDrawable}; in that case it's app's
   *     responsibility to set content into coverDrawable.
   * @param bottomDrawable Drawable shown at bottom. {@link #setSolidColor(int)} will not work if
   *     bottomDrawable is not {@link ColorDrawable}; in that case it's app's responsibility to set
   *     content of bottomDrawable.
   * @param coverDrawableParallaxTarget Target to perform parallax effect within coverDrawable. Use
   *     null for no parallax movement effect. Example to move bitmap within FitWidthBitmapDrawable:
   *     new ParallaxTarget.PropertyValuesHolderTarget( coverDrawable, PropertyValuesHolder.ofInt(
   *     FitWidthBitmapDrawable.PROPERTY_VERTICAL_OFFSET, 0, -120))
   * @throws IllegalStateException If {@link #setupVideoPlayback(PlaybackGlue)} was called.
   */
  public void enableParallax(
    ...
    mVideoHelper =
        new DetailsBackgroundVideoHelper(
            null, mFragment.getParallax(), mParallaxDrawable.getCoverDrawable());
  }

  /**
   * Enable video playback and set proper {@link PlaybackGlueHost}. This method by default creates a
   * VideoSupportFragment and VideoSupportFragmentGlueHost to host the PlaybackGlue. This method
   * must be called after calling details Fragment super.onCreate(). This method can be called
   * multiple times to replace existing PlaybackGlue or calling setupVideoPlayback(null) to clear.
   * Note a typical {@link PlaybackGlue} subclass releases resources in {@link
   * PlaybackGlue#onDetachedFromHost()}, when the {@link PlaybackGlue} subclass is not doing that,
   * it's app's responsibility to release the resources.
   *
  ...
  /**
   * Precondition allows user navigate to video fragment using DPAD. Default implementation returns
   * true if PlaybackGlue is not null. Subclass may override, e.g. only allow navigation when {@link
   * PlaybackGlue#isPrepared()} is true. Note this method does not block app calls {@link
   * #switchToVideo}.
   *
  ...
  /**
   * Switch to video fragment, note that this method is not affected by result of {@link
   * #canNavigateToVideoSupportFragment()}. If the method is called in
   * DetailsSupportFragment.onCreate() it will make video fragment to be initially focused once it
   * is created.
   *
   * <p>Calling switchToVideo() in DetailsSupportFragment.onCreate() will clear the activity enter
   * transition and shared element transition.
   *
   * <p>If switchToVideo() is called after {@link
   * DetailsSupportFragment#prepareEntranceTransition()} and before {@link
   * DetailsSupportFragment#onEntranceTransitionEnd()}, it will be ignored.
   *
   * <p>If {@link DetailsSupportFragment#prepareEntranceTransition()} is called after
   * switchToVideo(), an IllegalStateException will be thrown.
   */
  ...
  /**
   * When fragment is started and no running transition. First set host if not yet set, second start
   * playing if it was paused before.
   */
  void onStart() {
    if (!mCanUseHost) {
      mCanUseHost = true;
  ...
  /**
   * Returns the cover drawable at top. Returns null if {@link #enableParallax()} is not called. By
   * default it's a {@link FitWidthBitmapDrawable}.
   *
  ...
  /**
   * Returns the drawable at bottom. Returns null if {@link #enableParallax()} is not called. By
   * default it's a {@link ColorDrawable}.
   *
   ...
   * default. App may override and return a different fragment and it also must override {@link
   * #onCreateGlueHost()}.
   *
  ...
  /**
   * Creates a PlaybackGlueHost to host PlaybackGlue. App may override this if it overrides {@link
   * #onCreateVideoSupportFragment()}. This method must be called after calling Fragment
   * super.onCreate(). When override this method, app may call {@link
   * #findOrCreateVideoSupportFragment()} to get or create a fragment.
   *
  ...
  public PlaybackGlueHost onCreateGlueHost() {
    return new VideoSupportFragmentGlueHost(
        (VideoSupportFragment) findOrCreateVideoSupportFragment());
  }
  ...
  /**
   * Adds or gets fragment for rendering video in DetailsSupportFragment. A subclass that overrides
   * {@link #onCreateGlueHost()} should call this method to get a fragment for creating a {@link
   * PlaybackGlueHost}.
   *
  ...
  /**
   * Convenient method to set Bitmap in cover drawable. If app is not using default {@link
   * FitWidthBitmapDrawable}, app should not use this method It's safe to call setCoverBitmap()
  ...
  /**
   * Convenient method to set color in bottom drawable. If app is not using default {@link
   * ColorDrawable}, app should not use this method. It's safe to call setSolidColor() before
   * calling {@link #enableParallax()}.
   *
  ...
  /**
   * Sets default parallax offset in pixels for bitmap moving vertically. This method must be called
   * before {@link #enableParallax()}.
   *
  ...
  /**
   * Returns Default parallax offset in pixels for bitmap moving vertically. When 0, a default value
   * would be used.
   *
