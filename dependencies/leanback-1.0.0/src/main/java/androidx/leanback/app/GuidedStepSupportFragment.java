 ...
 * <h3>Basic Usage</h3>
 *
 * <p>Clients of GuidedStepSupportFragment must create a custom subclass to attach to their
 * Activities. This custom subclass provides the information necessary to construct the user
 * interface and respond to user actions. At a minimum, subclasses should override:
 *
 * <ul>
 *   <li>{@link #onCreateGuidance}, to provide instructions to the user
 *   <li>{@link #onCreateActions}, to provide a set of {@link GuidedAction}s the user can take
 *   <li>{@link #onGuidedActionClicked}, to respond to those actions
 * </ul>
 *
 * <p>Clients use following helper functions to add GuidedStepSupportFragment to Activity or
 * FragmentManager:
 *
 * <ul>
 *   <li>{@link #addAsRoot(FragmentActivity, GuidedStepSupportFragment, int)}, to be called during
 *       Activity onCreate, adds GuidedStepSupportFragment as the first Fragment in activity.
 *   <li>{@link #add(FragmentManager, GuidedStepSupportFragment)} or {@link #add(FragmentManager,
 *       GuidedStepSupportFragment, int)}, to add GuidedStepSupportFragment on top of existing
 *       Fragments or replacing existing GuidedStepSupportFragment when moving forward to next step.
 ...
 *       {@link #setUiStyle(int)} to select fragment transition and remember the stack entry where
 *       it need pops to.
 * </ul>
 *
 * <h3>Theming and Stylists</h3>
 *
 * <p>GuidedStepSupportFragment delegates its visual styling to classes called stylists. The {@link
 ...
 * Most simple visual aspects of GuidanceStylist and GuidedActionsStylist can be customized via
 * theming; see their documentation for more information.
 *
 * <p>GuidedStepSupportFragments must have access to an appropriate theme in order for the stylists
 * to function properly. Specifically, the fragment must receive {@link
 * androidx.leanback.R.style#Theme_Leanback_GuidedStep}, or a theme whose parent is is set to that
 * theme. Themes can be provided in one of three ways:
 *
 * <ul>
 ...
 *       androidx.leanback.R.styleable#LeanbackGuidedStepTheme_guidedStepTheme}. If present, this
 *       theme will be used by GuidedStepSupportFragment as an overlay to the Activity's theme.
 *   <li>Finally, custom subclasses of GuidedStepSupportFragment may provide a theme through the
 *       {@link #onProvideTheme} method. This can be useful if a subclass is used across multiple
 *       Activities.
 * </ul>
 *
 * <p>If the theme is provided in multiple ways, the onProvideTheme override has priority, followed
 * by the Activity's theme. (Themes whose parent theme is already set to the guided step theme do
 * not need to set the guidedStepTheme attribute; if set, it will be ignored.)
 *
 * <p>If themes do not provide enough customizability, the stylists themselves may be subclassed and
 * provided to the GuidedStepSupportFragment through the {@link #onCreateGuidanceStylist} and {@link
 * #onCreateActionsStylist} methods. The stylists have simple hooks so that subclasses may override
 * layout files; subclasses may also have more complex logic to determine styling.
 *
 * <p>
 *
 * <h3>Guided sequences</h3>
 *
 * <p>GuidedStepSupportFragments can be grouped together to provide a guided sequence.
 * GuidedStepSupportFragments grouped as a sequence use custom animations provided by {@link
 * GuidanceStylist} and {@link GuidedActionsStylist} (or subclasses) during transitions between
 * steps. Clients should use {@link #add} to place subsequent GuidedFragments onto the fragment
 * stack so that custom animations are properly configured. (Custom animations are triggered
 * automatically when the fragment stack is subsequently popped by any normal mechanism.)
 *
 * <p><i>Note: Currently GuidedStepSupportFragments grouped in this way must all be defined
 * programmatically, rather than in XML. This restriction may be removed in the future.</i>
 *
 ...
 * @attr ref
 *     androidx.leanback.R.styleable#LeanbackGuidedStepTheme_guidedActionContentWidthWeightTwoPanels
...
public class GuidedStepSupportFragment extends Fragment
  ...
  /**
   * Fragment argument name for UI style. The argument value is persisted in fragment state and used
   * to select fragment transition. The value is initially {@link #UI_STYLE_ENTRANCE} and might be
   * changed in one of the three helper functions:
   *
   * <ul>
   *   <li>{@link #addAsRoot(FragmentActivity, GuidedStepSupportFragment, int)} sets to {@link
   *       #UI_STYLE_ACTIVITY_ROOT}
   *   <li>{@link #add(FragmentManager, GuidedStepSupportFragment)} or {@link #add(FragmentManager,
   *       GuidedStepSupportFragment, int)} sets it to {@link #UI_STYLE_REPLACE} if there is already
   *       a GuidedStepSupportFragment on stack.
   ...
   *       GuidedStepSupportFragment run a return transition of {@link #UI_STYLE_ENTRANCE}
   * </ul>
   *
   * <p>Argument value can be either:
   *
   * <ul>
   *   <li>{@link #UI_STYLE_REPLACE}
   *   <li>{@link #UI_STYLE_ENTRANCE}
   *   <li>{@link #UI_STYLE_ACTIVITY_ROOT}
   * </ul>
   */
   ...
   *
   * <ul>
   *   <li>Enter transition slides in from END(right), exit transition same as {@link
   *       #UI_STYLE_ENTRANCE}.
   * </ul>
   */
   ...
   */
  @Deprecated public static final int UI_STYLE_DEFAULT = 0;

  /**
   * Default value for argument {@link #EXTRA_UI_STYLE}. The default value is assigned in
   * GuidedStepSupportFragment constructor. This is the case that we show GuidedStepSupportFragment
   * on top of other content. The default behavior of this style:
   *
   * <ul>
   ...
   * </ul>
   *
   * When popping multiple GuidedStepSupportFragment, {@link #finishGuidedStepSupportFragments()}
   * also changes the top GuidedStepSupportFragment to UI_STYLE_ENTRANCE in order to run the return
   * transition (reverse of enter transition) of UI_STYLE_ENTRANCE.
   */
   ...
   *       same as {@link #UI_STYLE_ENTRANCE}. Note: Changing exit transition by UI style is not
   *       working because fragment transition asks for exit transition before UI style is restored
  ...
  /**
   * Creates the presenter used to style the guidance panel. The default implementation returns a
   * basic GuidanceStylist.
   *
  ...
  /**
   * Creates the presenter used to style a sided actions panel for button only. The default
   * implementation returns a basic GuidedActionsStylist.
   *
   * @return The GuidedActionsStylist used in this fragment.
   */
   ...
   * GuidanceStylist.Guidance} for use in a subclass of {@link GuidanceStylist}. The default returns
   * a Guidance object with empty fields; subclasses should override.
   *
  ...
  /**
   * Callback invoked when an action is taken by the user. Subclasses should override in order to
   * act on the user's decisions.
   *
  ...
  /**
   * Callback invoked when an action in sub actions is taken by the user. Subclasses should override
   * in order to act on the user's decisions. Default return value is true to close the sub actions
   * list.
   *
   * @param action The chosen action.
  ...
  /**
   * @return True if is current expanded including subactions list or action with {@link
  ...
  /**
   * Expand a given action with sub actions list or {@link GuidedAction#hasEditableActivatorView()}
   * is true. The method must be called after {@link #onCreateView(LayoutInflater, ViewGroup,
   * Bundle)} creates fragment view.
   *
   * @param action GuidedAction to expand.
  ...
  /**
   * Collapse action which either has a sub actions list or action with {@link
   * GuidedAction#hasEditableActivatorView()} is true.
   *
   * @param withTransition True to run transition animation, false otherwise.
   */
   ...
   *
   * @deprecated Override {@link #onGuidedActionEditedAndProceed(GuidedAction)} and/or {@link
  ...
  /**
   * Callback invoked when an action has been canceled editing, for example when user closes IME
   * window by BACK key. Default implementation calls deprecated method {@link
  ...
  /**
   * Callback invoked when an action has been edited, for example when user clicks confirm button in
   * IME window. Default implementation calls deprecated method {@link
   ...
   * @return ID of the action will be focused or {@link GuidedAction#ACTION_ID_NEXT}, {@link
   ...
   * pressed.
   * <li>If current fragment on stack is GuidedStepSupportFragment: assign {@link #UI_STYLE_REPLACE}
   *     and {@link #onAddSharedElementTransition(FragmentTransaction, GuidedStepSupportFragment)}
   ...
   * when the GuidedStepSupportFragment replacing an existing GuidedStepSupportFragment). Default
   * implementation establishes connections between action background views to morph action
   * background bounds change from disappearing GuidedStepSupportFragment into this
   * GuidedStepSupportFragment. The default implementation heavily relies on {@link
   * GuidedActionsStylist}'s layout, app may override this method when modifying the default layout
  ...
  protected void onAddSharedElementTransition(
    ...
    addNonNullSharedElementTransition(
        ft, fragmentView.findViewById(R.id.action_fragment_root), "action_fragment_root");
    addNonNullSharedElementTransition(
        ft,
        fragmentView.findViewById(R.id.action_fragment_background),
        "action_fragment_background");
    addNonNullSharedElementTransition(
        ft, fragmentView.findViewById(R.id.action_fragment), "action_fragment");
    addNonNullSharedElementTransition(
        ft, fragmentView.findViewById(R.id.guidedactions_root), "guidedactions_root");
    addNonNullSharedElementTransition(
        ft, fragmentView.findViewById(R.id.guidedactions_content), "guidedactions_content");
    addNonNullSharedElementTransition(
        ft,
        fragmentView.findViewById(R.id.guidedactions_list_background),
        "guidedactions_list_background");
    addNonNullSharedElementTransition(
        ft, fragmentView.findViewById(R.id.guidedactions_root2), "guidedactions_root2");
    addNonNullSharedElementTransition(
        ft, fragmentView.findViewById(R.id.guidedactions_content2), "guidedactions_content2");
    addNonNullSharedElementTransition(
        ft,
        fragmentView.findViewById(R.id.guidedactions_list_background2),
        "guidedactions_list_background2");
  }

  private static void addNonNullSharedElementTransition(
      FragmentTransaction ft, View subView, String transitionName) {
   ...
   * associated. Note {@link #UI_STYLE_ACTIVITY_ROOT} will return empty String. The method returns
  ...
  /**
   * Generates BackStackEntry name for GuidedStepSupportFragment class or empty String if no entry
   * is associated. Note {@link #UI_STYLE_ACTIVITY_ROOT} is not allowed and returns empty String.
   *
  ...
  /**
   * Returns true if the backstack entry represents GuidedStepSupportFragment with {@link
   * #UI_STYLE_ENTRANCE}, i.e. this is the first GuidedStepSupportFragment pushed to stack; false
   * otherwise.
   *
   * @see #generateStackEntryName(int, Class)
   * @param backStackEntryName Name of BackStackEntry.
   * @return True if the backstack represents GuidedStepSupportFragment with {@link
   *     #UI_STYLE_ENTRANCE}; false otherwise.
   */
   ...
   * added so the activity will be dismissed when BACK key is pressed. The method is typically
   * called in Activity.onCreate() when savedInstanceState is null. When savedInstanceState is not
   * null, the Activity is being restored, do not call addAsRoot() to duplicate the Fragment
   * restored by FragmentManager. {@link #UI_STYLE_ACTIVITY_ROOT} is assigned.
   *
  ...
  public static int addAsRoot(
    ...
    if (fragmentManager.findFragmentByTag(TAG_LEAN_BACK_ACTIONS_FRAGMENT) != null) {
      Log.w(
          TAG,
          "Fragment is already exists, likely calling "
   ...
   * @return The View corresponding to the button action at the indicated position, or null if that
   *     action is not currently onscreen.
   */
  public View getButtonActionItemView(int position) {
    final RecyclerView.ViewHolder holder =
        mButtonActionsStylist.getActionsGridView().findViewHolderForPosition(position);
  ...
  /**
   * Sets the list of GuidedActions that the user may take in this fragment. Uses DiffCallback set
   * by {@link #setActionsDiffCallback(DiffCallback)}.
   *
   ...
   * GuidedStepSupportFragment uses {@link androidx.leanback.widget.GuidedActionDiffCallback}. Sets
   * it to null if app wants to refresh the whole list.
   *
  ...
  public View getActionItemView(int position) {
    final RecyclerView.ViewHolder holder =
        mActionsStylist.getActionsGridView().findViewHolderForPosition(position);
    return holder == null ? null : holder.itemView;
  }
   ...
   *       activity transition, exit transition is same as {@link #UI_STYLE_REPLACE}, no shared
   *       element enter transition.
   * </ul>
   *
   * <p>The default implementation heavily relies on {@link GuidedActionsStylist} and {@link
   * GuidanceStylist} layout, app may override this method when modifying the default layout of
   * {@link GuidedActionsStylist} or {@link GuidanceStylist}.
   *
   * <p>TIP: because the fragment view is removed during fragment transition, in general app cannot
   * use two Visibility transition together. Workaround is to create your own Visibility transition
   * that controls multiple animators (e.g. slide and fade animation in one Transition class).
   */
  protected void onProvideFragmentTransitions() {
    if (Build.VERSION.SDK_INT >= 21) {
      ...
      if (uiStyle == UI_STYLE_REPLACE) {
        ...
        Object fade =
            TransitionHelper.createFadeTransition(
        ...
        if (entranceTransitionType == SLIDE_FROM_SIDE) {
          ...
          Object slideFromSide =
              TransitionHelper.createFadeAndShortSlide(Gravity.END | Gravity.START);
  ...
  /**
   * Called by onCreateView to inflate background view. Default implementation loads view from
   * {@link R.layout#lb_guidedstep_background} which holds a reference to guidedStepBackground.
   *
   ...
   * determine if this is the first GuidedStepSupportFragment on backstack. In most cases app does
   * not directly call this method, app calls helper function {@link #add(FragmentManager,
   * GuidedStepSupportFragment, int)}. However if the app creates Fragment transaction and controls
   * backstack by itself, it would need call setUiStyle() to select the fragment transition to use.
   *
   * @param style {@link #UI_STYLE_ACTIVITY_ROOT} {@link #UI_STYLE_REPLACE} or {@link
   *     #UI_STYLE_ENTRANCE}.
   */
   ...
   *
   * @return {@link #UI_STYLE_ACTIVITY_ROOT} {@link #UI_STYLE_REPLACE} or {@link
   *     #UI_STYLE_ENTRANCE}.
   * @see #onProvideFragmentTransitions()
   */
  ...
  public View onCreateView(
    ...
    GuidedStepRootLayout root =
        (GuidedStepRootLayout) inflater.inflate(R.layout.lb_guidedstep_fragment, container, false);
    ...
    mAdapter =
        new GuidedActionAdapter(
            mActions,
            new GuidedActionAdapter.ClickListener() {
              @Override
              public void onGuidedActionClicked(GuidedAction action) {
    ...
    if (mButtonActions.size() == 0) {
      ...
      LinearLayout.LayoutParams lp =
          (LinearLayout.LayoutParams) buttonActionsView.getLayoutParams();
      lp.weight = 0;
      ...
      if (ctx.getTheme()
          .resolveAttribute(R.attr.guidedActionContentWidthWeightTwoPanels, typedValue, true)) {
    ...
    if (backgroundView != null) {
      FrameLayout backgroundViewRoot =
          (FrameLayout) root.findViewById(R.id.guidedstep_background_view_root);
  ...
  /**
   * Convenient method to close GuidedStepSupportFragments on top of other content or finish
   * Activity if GuidedStepSupportFragments were started in a separate activity. Pops all stack
   * entries including {@link #UI_STYLE_ENTRANCE}; if {@link #UI_STYLE_ENTRANCE} is not found,
   * finish the activity. Note that this method must be paired with {@link #add(FragmentManager,
   * GuidedStepSupportFragment, int)} which sets up the stack entry name for finding which fragment
   * we need to pop back to.
   */
   ...
   * Default value is false, the reason is to disable FocusFinder to find focusable views beneath
   * content of GuidedStepSupportFragment. Subclass may override.
   *
  ...
  /**
   * Sets the transition type to be used for {@link #UI_STYLE_ENTRANCE} animation. Currently we
   * provide 2 different variations for animation - slide in from side (default) or bottom.
   *
   * <p>Ideally we can retrieve the screen mode settings from the theme attribute {@code
   * Theme.Leanback.GuidedStep#guidedStepHeightWeight} and use that to determine the transition. But
   * the fragment context to retrieve the theme isn't available on platform v23 or earlier.
   *
   * <p>For now clients(subclasses) can call this method inside the constructor.
   *
   * @hide
   */
  @RestrictTo(LIBRARY_GROUP)
  ...
  /**
   * Opens the provided action in edit mode and raises ime. This can be used to programmatically
   * skip the extra click required to go into edit mode. This method can be invoked in {@link
