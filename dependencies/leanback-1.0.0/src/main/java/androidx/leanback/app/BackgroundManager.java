 ...
 *
 * <p>An Activity should instantiate a BackgroundManager and {@link #attach} to the Activity's
 * window. When the Activity is started, the background is initialized to the current background
 * values stored in a continuity service. The background continuity service is updated as the
 * background is updated.
 *
 * <p>At some point, for example when it is stopped, the Activity may release its background state.
 *
 ...
 *
 * <ul>
 *   <li>the background Drawable of the theme
 *   <li>a solid color (set via {@link #setColor})
 *   <li>two Drawables, previous and current (set via {@link #setBitmap} or {@link #setDrawable}),
 *       which may be in transition
 * </ul>
 *
 * <p>BackgroundManager holds references to potentially large bitmap Drawables. Call {@link
 * #release} to release these references when the Activity is not visible.
 */
...
public final class BackgroundManager {
  ...
  /**
   * Synchronizes state when the owning Activity is started. At that point the view becomes visible.
   */
  ...
  /**
   * Makes the background visible on the given Window. The background manager must be attached when
  ...
  public void attachToView(View sceneRoot) {
    ...
    mContext
        .getWindow()
        .getDecorView()
        .setBackground(Build.VERSION.SDK_INT >= 26 ? null : new ColorDrawable(Color.TRANSPARENT));
  }
   ...
   * overhead when not visible. It's app's responsibility to restore the drawable/bitmap in Activity
   * onStart(). The method is automatically called in onStop() when {@link #isAutoReleaseOnStop()}
   * is true.
   *
  ...
  private void lazyInit() {
    ...
    LayerDrawable layerDrawable =
        (LayerDrawable) ContextCompat.getDrawable(mContext, R.drawable.lb_background).mutate();
  ...
  /**
   * Sets the background to the given color. The timing for when this becomes visible in the app is
