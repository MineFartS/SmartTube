...
/**
 * Helper class responsible for controlling video playback in {@link DetailsFragment}. This takes
 * {@link DetailsParallax}, {@link PlaybackGlue} and a drawable as input. Video is played when
 * {@link DetailsParallax#getOverviewRowTop()} moved bellow top edge of screen. Video is stopped
 * when {@link DetailsParallax#getOverviewRowTop()} reaches or scrolls above top edge of screen. The
 * drawable will change alpha to 0 when video is ready to play. App does not directly use this
 * class.
 *
...
final class DetailsBackgroundVideoHelper {
   ...
   *     start/stop. Video is played when {@link DetailsParallax#getOverviewRowTop()} moved bellow
   *     top edge of screen. Video is stopped when {@link DetailsParallax#getOverviewRowTop()}
   *     reaches or scrolls above top edge of screen.
   * @param backgroundDrawable The drawable will change alpha to 0 when video is ready to play.
   */
  DetailsBackgroundVideoHelper(
      PlaybackGlue playbackGlue, DetailsParallax detailsParallax, Drawable backgroundDrawable) {
  ...
  void crossFadeBackgroundToVideo(boolean crossFadeToVideo, boolean immediate) {
    ...
    mBackgroundAnimator.addUpdateListener(
        new ValueAnimator.AnimatorUpdateListener() {
          @Override
          public void onAnimationUpdate(ValueAnimator valueAnimator) {
            mBackgroundDrawable.setAlpha((int) ((Float) (valueAnimator.getAnimatedValue()) * 255));
          }
        });
