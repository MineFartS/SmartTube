...
/**
 * A GuidedStepFragment is used to guide the user through a decision or series of decisions. It is
 * composed of a guidance view on the left and a view on the right containing a list of possible
 * actions.
 *
 * <p>
 *
 * <h3>Basic Usage</h3>
 *
 * <p>Clients of GuidedStepFragment must create a custom subclass to attach to their Activities.
 ...
 * </ul>
 *
 * <p>Clients use following helper functions to add GuidedStepFragment to Activity or
 * FragmentManager:
 *
 * <ul>
 *   <li>{@link #addAsRoot(Activity, GuidedStepFragment, int)}, to be called during Activity
 *       onCreate, adds GuidedStepFragment as the first Fragment in activity.
 ...
 *       replacing existing GuidedStepFragment when moving forward to next step.
 ...
 *
 * <p>GuidedStepFragment delegates its visual styling to classes called stylists. The {@link
 ...
 *
 * <p>GuidedStepFragments must have access to an appropriate theme in order for the stylists to
 ...
 *       theme will be used by GuidedStepFragment as an overlay to the Activity's theme.
 ...
 *
 * <p>GuidedStepFragments can be grouped together to provide a guided sequence. GuidedStepFragments
 * grouped as a sequence use custom animations provided by {@link GuidanceStylist} and {@link
 * GuidedActionsStylist} (or subclasses) during transitions between steps. Clients should use {@link
 * #add} to place subsequent GuidedFragments onto the fragment stack so that custom animations are
 * properly configured. (Custom animations are triggered automatically when the fragment stack is
 * subsequently popped by any normal mechanism.)
 *
 * <p><i>Note: Currently GuidedStepFragments grouped in this way must all be defined
...
public class GuidedStepFragment extends Fragment implements GuidedActionAdapter.FocusListener {
   ...
   *
   * <ul>
   *   <li>{@link #addAsRoot(Activity, GuidedStepFragment, int)} sets to {@link
   *       #UI_STYLE_ACTIVITY_ROOT}
   *   <li>{@link #add(FragmentManager, GuidedStepFragment)} or {@link #add(FragmentManager,
   *       GuidedStepFragment, int)} sets it to {@link #UI_STYLE_REPLACE} if there is already a
   *       GuidedStepFragment on stack.
   *   <li>{@link #finishGuidedStepFragments()} changes current GuidedStepFragment to {@link
   *       #UI_STYLE_ENTRANCE} for the non activity case. This is a special case that changes the
   *       transition settings after fragment has been created, in order to force current
   *       GuidedStepFragment run a return transition of {@link #UI_STYLE_ENTRANCE}
   * </ul>
   *
  ...
  /**
   * This is the case that we use GuidedStepFragment to replace another existing GuidedStepFragment
   * when moving forward to next step. Default behavior of this style is:
   *
   * <ul>
   ...
   * </ul>
   *
   * When popping multiple GuidedStepFragment, {@link #finishGuidedStepFragments()} also changes the
   * top GuidedStepFragment to UI_STYLE_ENTRANCE in order to run the return transition (reverse of
   * enter transition) of UI_STYLE_ENTRANCE.
   */
   ...
   * GuidedStepFragments in the stack, and configuring the fragment-to-fragment custom transitions.
   * A backstack entry is added, so the fragment will be dismissed when BACK key is pressed.
   ...
   *     {@link #onAddSharedElementTransition(FragmentTransaction, GuidedStepFragment)} will be
   *     called to perform shared element transition between GuidedStepFragments.
   * <li>If current fragment on stack is not GuidedStepFragment: assign {@link #UI_STYLE_ENTRANCE}
   *
   *     <p>Note: currently fragments added using this method must be created programmatically
   *     rather than via XML.
  ...
  protected void onAddSharedElementTransition(
      FragmentTransaction ft, GuidedStepFragment disappearing) {
  ...
  /**
   * Returns true if the backstack entry represents GuidedStepFragment with {@link
   * #UI_STYLE_ENTRANCE}, i.e. this is the first GuidedStepFragment pushed to stack; false
   * otherwise.
   *
   ...
   * Activity.onCreate() when savedInstanceState is null. When savedInstanceState is not null, the
   * Activity is being restored, do not call addAsRoot() to duplicate the Fragment restored by
   * FragmentManager. {@link #UI_STYLE_ACTIVITY_ROOT} is assigned.
   *
   ...
   * GuidedStepFragment uses {@link androidx.leanback.widget.GuidedActionDiffCallback}. Sets it to
   * null if app wants to refresh the whole list.
   *
   ...
   * directly call this method, app calls helper function {@link #add(FragmentManager,
   * GuidedStepFragment, int)}. However if the app creates Fragment transaction and controls
  ...
  public View onCreateView(
    ...
    if (mButtonActions.size() == 0) {
      ...
      Context ctx =
          mThemeWrapper != null ? mThemeWrapper : FragmentUtil.getContext(GuidedStepFragment.this);
  ...
  /**
   * Returns true if allows focus out of start edge of GuidedStepFragment, false otherwise. Default
   * value is false, the reason is to disable FocusFinder to find focusable views beneath content of
   * GuidedStepFragment. Subclass may override.
   *
  ...
  /**
   * Returns true if allows focus out of end edge of GuidedStepFragment, false otherwise. Default
   * value is false, the reason is to disable FocusFinder to find focusable views beneath content of
   * GuidedStepFragment. Subclass may override.
   *
