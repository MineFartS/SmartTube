 ...
 * common parts are composed of icon, title, description and page navigator and the custom parts are
 * composed of background, contents and foreground.
 *
 * <p>To build the screen views, the inherited class should override:
 *
 * <ul>
 *   <li>{@link #onCreateBackgroundView} to provide the background view. Background view has the
 *       same size as the screen and the lowest z-order.
 *   <li>{@link #onCreateContentView} to provide the contents view. The content view is located in
 *       the content area at the center of the screen.
 *   <li>{@link #onCreateForegroundView} to provide the foreground view. Foreground view has the
 *       same size as the screen and the highest z-order
 * </ul>
 *
 * <p>Each of these methods can return {@code null} if the application doesn't want to provide it.
 *
 * <p>
 *
 ...
 *
 * <ul>
 *   <li>{@link #getPageCount} to provide the number of pages.
 *   <li>{@link #getPageTitle} to provide the title of the page.
 *   <li>{@link #getPageDescription} to provide the description of the page.
 * </ul>
 *
 * <p>Note that the information is used in {@link #onCreateView}, so should be initialized before
 ...
 *
 * <p>In most cases, the logo animation needs to be customized because the logo images of
 * applications are different from each other, or some applications may want to show their own
 * animations.
 *
 * <p>The logo animation can be customized in two ways:
 *
 * <ul>
 *   <li>The simplest way is to provide the logo image by calling {@link #setLogoResourceId} to show
 *       the default logo animation. This method should be called in {@link Fragment#onCreateView}.
 *   <li>If the logo animation is complex, then override {@link #onCreateLogoAnimation} and return
 *       the {@link Animator} object to run.
 * </ul>
 *
 * <p>If the inherited class provides neither the logo image nor the animation, the logo animation
 * will be omitted.
 *
 * <h4>Page enter animation</h4>
 *
 * After logo animation finishes, page enter animation starts, which causes the header section -
 * title and description views to fade and slide in. Users can override the default fade + slide
 * animation by overriding {@link #onCreateTitleAnimator()} & {@link
 * #onCreateDescriptionAnimator()}. By default we don't animate the custom views but users can
 * provide animation by overriding {@link #onCreateEnterAnimation}.
 *
 ...
 *
 * <p>If the user finishes the onboarding screen after navigating all the pages, {@link
 ...
 * <h3>Theming</h3>
 *
 * <p>OnboardingFragment must have access to an appropriate theme. Specifically, the fragment must
 ...
 *
 * <ul>
 *   <li>The simplest way is to set the theme for the host Activity to the Onboarding theme or a
 ...
 *       R.styleable#LeanbackOnboardingTheme_onboardingTheme}. If present, this theme will be used
 *       by OnboardingFragment as an overlay to the Activity's theme.
 *   <li>Finally, custom subclasses of OnboardingFragment may provide a theme through the {@link
 ...
 * by the Activity's theme. (Themes whose parent theme is already set to the onboarding theme do not
...
public abstract class OnboardingFragment extends Fragment {
  ...
  @ColorInt private int mTitleViewTextColor = Color.TRANSPARENT;
  private boolean mTitleViewTextColorSet;

  @ColorInt private int mDescriptionViewTextColor = Color.TRANSPARENT;
  private boolean mDescriptionViewTextColorSet;

  @ColorInt private int mDotBackgroundColor = Color.TRANSPARENT;
  private boolean mDotBackgroundColorSet;

  @ColorInt private int mArrowColor = Color.TRANSPARENT;
  private boolean mArrowColorSet;

  @ColorInt private int mArrowBackgroundColor = Color.TRANSPARENT;
  ...
  public View onCreateView(
    ...
    final ViewGroup view =
        (ViewGroup) localInflater.inflate(R.layout.lb_onboarding_fragment, container, false);
    ...
    if (sSlideDistance == 0) {
      sSlideDistance =
          (int) (SLIDE_DISTANCE * context.getResources().getDisplayMetrics().scaledDensity);
    }
    view.requestFocus();
    return view;
  }
  ...
  /**
   * Sets the text color for TitleView. If not set, the default textColor set in style referenced by
   * attr {@link R.attr#onboardingTitleStyle} will be used.
   *
  ...
  /**
   * Returns the text color of TitleView if it's set through {@link #setTitleViewTextColor(int)}. If
   * no color was set, transparent is returned.
   */
  @ColorInt
  ...
  /**
   * Returns the text color of DescriptionView if it's set through {@link
   * #setDescriptionViewTextColor(int)}. If no color was set, transparent is returned.
   */
  @ColorInt
  ...
  /**
   * Sets the background color of the dots. If not set, the default color from attr {@link
   * R.styleable#PagingIndicator_dotBgColor} in the theme will be used.
   *
  ...
  /**
   * Returns the background color of the dot if it's set through {@link
   * #setDotBackgroundColor(int)}. If no color was set, transparent is returned.
   */
  @ColorInt
  ...
  /**
   * Returns the color of the arrow if it's set through {@link #setArrowColor(int)}. If no color was
   * set, transparent is returned.
   */
  @ColorInt
  ...
  /**
   * Sets the background color of the arrow. If not set, the default color from attr {@link
   * R.styleable#PagingIndicator_arrowBgColor} in the theme will be used.
   *
   * @param color the color to use for arrow background
   */
  ...
  /**
   * Returns the background color of the arrow if it's set through {@link
   * #setArrowBackgroundColor(int)}. If no color was set, transparent is returned.
   */
  @ColorInt
  ...
  /**
   * Returns the start button text if it's set through {@link #setStartButtonText(CharSequence)}}.
  ...
  /**
   * Sets the text on the start button text. If not set, the default text set in {@link
   * R.styleable#LeanbackOnboardingTheme_onboardingStartButtonStyle} will be used.
   *
   ...
   *
   * <p>This is called only if the logo image resource ID is not set by {@link #setLogoResourceId}.
  ...
  boolean startLogoAnimation() {
    ...
    if (mLogoResourceId != 0) {
      ...
      Animator outAnimator =
          AnimatorInflater.loadAnimator(context, R.animator.lb_onboarding_logo_exit);
  ...
  void hideLogoView() {
    ...
    LayoutInflater inflater =
        getThemeInflater(LayoutInflater.from(FragmentUtil.getContext(OnboardingFragment.this)));
   ...
   * method can also be called when the activity is recreated, i.e. when no logo animation are
   * performed. By default, this method will hide the logo view and start the entrance animation for
   * this fragment. Overriding subclasses can provide their own data loading logic as to when the
   * entrance animation should be executed.
   */
  ...
  protected final void startEnterAnimation(boolean force) {
    ...
    Animator animator =
        AnimatorInflater.loadAnimator(context, R.animator.lb_onboarding_page_indicator_enter);
  ...
  /**
   * Provides the entry animation for description view. This allows users to override the default
   * fade and slide animation. Returning null will disable the animation.
   */
  ...
  /**
   * Provides the entry animation for title view. This allows users to override the default fade and
   * slide animation. Returning null will disable the animation.
   */
  protected Animator onCreateTitleAnimator() {
    return AnimatorInflater.loadAnimator(
        FragmentUtil.getContext(OnboardingFragment.this), R.animator.lb_onboarding_title_enter);
  }
   ...
   * which doesn't have the background view can return {@code null}. This is called inside {@link
   * #onCreateView}.
   *
   * @param inflater The LayoutInflater object that can be used to inflate the views,
   * @param container The parent view that the additional views are attached to.The fragment should
   *     not add the view by itself.
  ...
  /**
   * Called to have the inherited class create content view. This is optional and the fragment which
   * doesn't have the content view can return {@code null}. This is called inside {@link
   * #onCreateView}.
   *
   ...
   * which doesn't need the foreground view can return {@code null}. This is called inside {@link
   * #onCreateView}.
   *
  ...
  private void onPageChangedInternal(int previousPage) {
    ...
    if (previousPage < getCurrentPageIndex()) {
      ...
      animators.add(
          fadeAnimator =
              createAnimator(mDescriptionView, false, Gravity.START, DESCRIPTION_START_DELAY_MS));
      ...
      animators.add(
          fadeAnimator =
              createAnimator(mDescriptionView, false, Gravity.END, DESCRIPTION_START_DELAY_MS));
    ...
    if (getCurrentPageIndex() == getPageCount() - 1) {
      ...
      Animator navigatorFadeOutAnimator =
          AnimatorInflater.loadAnimator(context, R.animator.lb_onboarding_page_indicator_fade_out);
      navigatorFadeOutAnimator.setTarget(mPageIndicator);
      navigatorFadeOutAnimator.addListener(
          new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
      ...
      Animator buttonFadeInAnimator =
          AnimatorInflater.loadAnimator(context, R.animator.lb_onboarding_start_button_fade_in);
      buttonFadeInAnimator.setTarget(mStartButton);
      animators.add(buttonFadeInAnimator);
    } else if (previousPage == getPageCount() - 1) {
      mPageIndicator.setVisibility(View.VISIBLE);
      Animator navigatorFadeInAnimator =
          AnimatorInflater.loadAnimator(context, R.animator.lb_onboarding_page_indicator_fade_in);
      navigatorFadeInAnimator.setTarget(mPageIndicator);
      animators.add(navigatorFadeInAnimator);
      Animator buttonFadeOutAnimator =
          AnimatorInflater.loadAnimator(context, R.animator.lb_onboarding_start_button_fade_out);
      buttonFadeOutAnimator.setTarget(mStartButton);
      buttonFadeOutAnimator.addListener(
          new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
  ...
  private Animator createAnimator(View view, boolean fadeIn, int slideDirection, long startDelay) {
    ...
    boolean slideRight =
        (isLtr && slideDirection == Gravity.END)
    ...
    if (fadeIn) {
      ...
      slideAnimator =
          ObjectAnimator.ofFloat(
              view, View.TRANSLATION_X, slideRight ? sSlideDistance : -sSlideDistance, 0);
      ...
      slideAnimator =
          ObjectAnimator.ofFloat(
              view, View.TRANSLATION_X, 0, slideRight ? sSlideDistance : -sSlideDistance);
