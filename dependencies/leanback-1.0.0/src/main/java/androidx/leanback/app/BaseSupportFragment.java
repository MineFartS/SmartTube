...
public class BaseSupportFragment extends BrandedSupportFragment {
  ...
  /**
   * prepareEntranceTransition is just called, but view not ready yet. We can enable the busy
   * spinner.
   */
  final State STATE_ENTRANCE_ON_PREPARED =
      new State("ENTRANCE_ON_PREPARED", true, false) {
        @Override
        public void run() {
   ...
   * in this State, the process is very different in subclass, e.g. BrowseSupportFragment hide
   * header views and hide main fragment view in two steps.
   */
  final State STATE_ENTRANCE_ON_PREPARED_ON_CREATEVIEW =
      new State("ENTRANCE_ON_PREPARED_ON_CREATEVIEW") {
        @Override
        public void run() {
  ...
  final State STATE_ENTRANCE_PERFORM =
      new State("STATE_ENTRANCE_PERFORM") {
        @Override
        public void run() {
  ...
  final State STATE_ENTRANCE_ON_ENDED =
      new State("ENTRANCE_ON_ENDED") {
        @Override
        public void run() {
  ...
  final Condition COND_TRANSITION_NOT_SUPPORTED =
      new Condition("EntranceTransitionNotSupport") {
        @Override
  ...
  void createStateMachineTransitions() {
    ...
    mStateMachine.addTransition(
        STATE_ENTRANCE_INIT, STATE_ENTRANCE_COMPLETE, COND_TRANSITION_NOT_SUPPORTED);
    mStateMachine.addTransition(STATE_ENTRANCE_INIT, STATE_ENTRANCE_COMPLETE, EVT_ON_CREATEVIEW);
    mStateMachine.addTransition(
        STATE_ENTRANCE_INIT, STATE_ENTRANCE_ON_PREPARED, EVT_PREPARE_ENTRANCE);
    mStateMachine.addTransition(
        STATE_ENTRANCE_ON_PREPARED, STATE_ENTRANCE_ON_PREPARED_ON_CREATEVIEW, EVT_ON_CREATEVIEW);
    mStateMachine.addTransition(
        STATE_ENTRANCE_ON_PREPARED, STATE_ENTRANCE_PERFORM, EVT_START_ENTRANCE);
    mStateMachine.addTransition(STATE_ENTRANCE_ON_PREPARED_ON_CREATEVIEW, STATE_ENTRANCE_PERFORM);
    mStateMachine.addTransition(STATE_ENTRANCE_PERFORM, STATE_ENTRANCE_ON_ENDED, EVT_ENTRANCE_END);
   ...
   *
   * <p>Entrance transition is the standard slide-in transition that shows rows of data in browse
   * screen and details screen.
   *
   * <p>The method is ignored before LOLLIPOP (API21).
   *
   * <p>This method must be called in or before onCreate(). Typically entrance transition should be
   * enabled when savedInstance is null so that fragment restored from instanceState does not run an
   * extra entrance transition. When the entrance transition is enabled, the fragment will make
   * headers and content hidden initially. When data of rows are ready, app must call {@link
   * #startEntranceTransition()} to kick off the transition, otherwise the rows will be invisible
   * forever.
   *
   * <p>It is similar to android:windowsEnterTransition and can be considered a late-executed
   * android:windowsEnterTransition controlled by app. There are two reasons that app needs it:
   * <li>Workaround the problem that activity transition is not available between launcher and app.
   *     Browse activity must programmatically start the slide-in transition.
   * <li>Separates DetailsOverviewRow transition from other rows transition. So that the
   *     DetailsOverviewRow transition can be executed earlier without waiting for all rows to be
   *     loaded.
   *
   *     <p>Transition object is returned by createEntranceTransition(). Typically the app does not
   *     need override the default transition that browse and details provides.
   */
  ...
  /**
   * Create entrance transition. Subclass can override to load transition from resource or construct
   * manually. Typically app does not need to override the default transition that browse and
   * details provides.
   */
  ...
  /**
   * Run entrance transition. Subclass may use TransitionManager to perform go(Scene) or
   * beginDelayedTransition(). App should not override the default implementation of browse and
   * details fragment.
   */
  protected void runEntranceTransition(Object entranceTransition) {}

  /**
   * Callback when entrance transition is prepared. This is when fragment should stop user input and
   * animations.
   */
  protected void onEntranceTransitionPrepare() {}

  /**
   * Callback when entrance transition is started. This is when fragment should stop processing
   * layout.
   */
