...
/**
 * A base class for all V8 resources. V8 resources must be closed/released. The rules for releasing
 * resources is as follows:
 *
 * <p>1. If you created it, you must close it, with one exception; if the object is being passed
 * pack via a return statement, the system will release it for you.
 *
 * <p>2. If the system created it, you don’t need to worry about it, with one caveat; if the object
 * was returned to you as a result of a method call, you must close it.
 */
public abstract class V8Value implements Releasable {
  ...
  /**
   * Returns the 'type' of this V8Value. The available types are defined as constants in {@link
   * V8Value}. Only types that inherit from {@link V8Value} can be returned here.
   *
   ...
   * adding new members or changing existing ones) then both the original and twin will be updated.
   * Twins are .equal and .strict equals, but not == in Java.
   *
   * <p>Twins must be closed separately since they have their own native resources.
   *
   * @return A new Java object pointing at the same V8 Value as this.
   */
  public V8Value twin() {
    if (isUndefined()) {
      return this;
  ...
  /**
   * Sets the V8Value as weak reference. A weak reference will eventually be closed when no more
   * references exist to this object. Once setWeak is called, you should check if {@link
   * V8Value#isReleased()} is true before invoking any methods on this object.
   *
   * <p>If any other references exist to this object, the object will not be reclaimed. Even if no
   * reference exist, V8 does not give any guarantee the object will be closed, so this should only
   * be used if there is no other way to track object usage.
   *
   * @return The receiver.
   */
  ...
  /**
   * Clears any weak reference set on this V8Value and makes this a strong reference. Strong
   * references will not be garbage collected and this Object must be explicitly released.
   *
   * <p>Calling clearWeak does nothing if the object is not currently set to weak.
   *
   * @return The receiver.
   */
  ...
  /**
   * If {@link V8Value#setWeak()} has been called on this Object, this method will return true.
  ...
  /**
   * Determine if the native resources have been released. Once released a V8 Value can no longer be
   * used.
   *
