...
/**
 * A lightweight handle to a V8TypedArray. This handle provides access to a V8TypedArray. This
 * handle does not need to be closed, but if the type array is accessed using getV8TypedArray then
 * the result must be closed.
 *
 * <p>The underlying V8TypedArray may be reclaimed by the JavaScript garbage collector. To check if
 * it's still available, use isAvailable.
 */
public class TypedArray {
   ...
   */
  public TypedArray(
      final V8 v8, final ArrayBuffer buffer, final int type, final int offset, final int size) {
  ...
  /**
   * Determine if the underlying V8TypedArray is still available, or if it's been cleaned up by the
