/**
 * Copyright 2016, Genuitec, LLC All rights reserved. This program and the accompanying materials
 * are made available under the terms of the Eclipse Public License v1.0 which accompanies this
 * distribution, and is available at http://www.eclipse.org/legal/epl-v10.html
 *
 * <p>Contributors: Genuitec LLC - initial API and implementation
 * ****************************************************************************
 */
...
/**
 * V8DebugServer enables debuggers to connect to J2V8 via V8 server sockets debug protocol. Server
 * has to be created in the same thread as the provided V8 runtime has been created (the V8 thread).
 * You can specify port and whether the {@link #start()} method should block until a client
 * connects. {@link #setTraceCommunication(boolean)} allows to output communication details for
 * debugging purposes. Before creating V8 runtime you need to set V8 flag to expose debug object. If
 * you do not intend to set other flags, than you can use {@link #configureV8ForDebugging()} method,
 * otherwise set {@code -expose-debug-as=__j2v8_Debug} flag through {@link V8#setFlags(String)}.
 *
 * <p>Client connection is handled in a separate thread, however, commands are processed in the V8
 * thread. Therefore it is vital to provide an opportunity to process requests by calling {@link
 * #processRequests(long)} method from the V8 thread. This will for instance allow to install
 * breakpoints before the JavaScript code starts to execute. It is also good to call that method
 * when V8 thread is idle to promptly provide responses to the debugger to avoid timeouts.
 *
...
public class V8DebugServer {
  ...
  /**
   * Name under which internal V8 debug object is going to be exposed in the runtime. You can change
   * the name if you are passing a different one through {@code -expose-debug-as} flag.
   */
  ...
  private static final byte[] PROTOCOL_CONTENT_LENGTH_BYTES =
      PROTOCOL_CONTENT_LENGTH_HEADER.getBytes(PROTOCOL_CHARSET);
  ...
  public V8DebugServer(final V8 runtime, final int port, final boolean waitForConnection) {
    ...
    runtime.executeVoidScript(
        "(function() {\n"
            ...
            + ".Debug. "
            + MAKE_BREAK_EVENT
            + " = function (break_id,breakpoints_hit) {\n"
            + "  return new "
            + DEBUG_OBJECT_NAME
            + ".BreakEvent(break_id,breakpoints_hit);\n"
            + " }\n"
            + " "
            + DEBUG_OBJECT_NAME
            + ".Debug. "
  ...
  /**
   * Starts accepting client connections and blocks until a client connects if {@code
   * waitForConnection} has been passed to V8DebugServer constructor.
   */
  public void start() {
    if (server == null) {
      return;
    ...
    runningStateDcp =
        runtime.executeObjectScript(
            "(function() {return new "
                + DEBUG_OBJECT_NAME
                + ".DebugCommandProcessor(null, true)})()");
  ...
  private void sendMessage(final String header, final String contents) throws IOException {
    synchronized (clientLock) {
      ...
      client.getOutputStream().write(sb.toString().getBytes(PROTOCOL_CHARSET));
  ...
  private void processRequest(final String message) throws IOException {
    ...
    if ((stoppedStateDcp == null) && json.contains("\"running\":false")) {
      ...
      json =
          json.replace("\"running\":false", "\"running\":true")
              .replace("\"success\":true", "\"success\":false")
              .replace(
                  "{\"",
                  "{\"message\":\"Client requested suspension is not supported on J2V8.\",\"");
  ...
  private void enterBreakLoop(final V8Object execState, final V8Object eventData)
      throws IOException {
    try {
      V8Array params = new V8Array(runtime);
      try {
