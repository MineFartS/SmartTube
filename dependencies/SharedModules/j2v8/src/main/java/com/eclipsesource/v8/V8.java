...
/**
 * An isolated V8Runtime. All JavaScript execution must exist on a single runtime, and data is not
 * shared between runtimes. A runtime must be created and released when finished.
 *
 * <p>All access to a runtime must come from the same thread, unless the thread explicitly gives up
 * control using the V8Locker.
 *
 * <p>A public static factory method can be used to create the runtime.
 *
 * <p>V8 runtime = V8.createV8Runtime();
 */
public class V8 extends V8Object {
  ...
  private static synchronized void load(final String tmpDirectory) {
    try {
   ...
   *
   * @return Returns true if the native libraries are loaded, false otherwise.
   */
  ...
  /**
   * Sets the V8 flags on the platform. All runtimes will be created with the same flags. Flags must
   * be set before the runtime is created.
   *
  ...
  /**
   * Creates a new V8Runtime and loads the required native libraries if they are not already loaded.
   * An alias is also set for the global scope. For example, 'window' can be set as the global scope
   * name.
   *
   * <p>The current thread is given the lock to this runtime.
   *
   ...
   *
   * @param globalAlias The name to associate with the global scope.
   * @param tempDirectory The name of the directory to extract the native libraries too.
   * @return A new isolated V8 Runtime.
   */
  ...
  /**
   * Adds a handler that will be called when the runtime is being released. The runtime will still
   * be available when the handler is executed.
   *
  ...
  /**
   * Removes an existing ReferenceHandler from the collection of reference handlers. If the
   * ReferenceHandler does not exist in the collection, it is ignored.
   *
  ...
  /**
   * Removes an existing release handler from the collection of release handlers. If the release
   * handler does not exist in the collection, it is ignored.
   *
  ...
  /**
   * Returns the data object associated with this runtime, null if no object has been associated.
   *
   * @param key The key used to reference this object
    ...
    if (!nativeLibraryLoaded) {
      ...
      if (nativeLoadError != null) {
        throw new IllegalStateException(
            message + ": " + nativeLoadError.getMessage(), nativeLoadError);
      } else if (nativeLoadException != null) {
        throw new IllegalStateException(
            message + ": " + nativeLoadException.getMessage(), nativeLoadException);
      } else {
        throw new IllegalStateException(message);
      }
    }
  ...
  public long createInspector(
      final V8InspectorDelegate inspectorDelegate, final String contextName) {
  ...
  /**
   * Returns the revision ID of this version as specified by the source code management system.
   * Currently we use Git, so this will return the commit ID for this revision.
   *
  ...
  /**
   * Terminates any JavaScript executing on this runtime. Once the runtime is released, any
   * executors that were spawned will also be force terminated.
   */
  ...
  /**
   * Release native resources associated with this runtime. Once released, a runtime cannot be
   * reused.
   *
   * @param reportMemoryLeaks True if memory leaks should be reported by throwing an
   *     IllegalStateException if any objects were not released.
   */
  public void release(final boolean reportMemoryLeaks) {
    ...
    try {
      ...
      if (reportMemoryLeaks && (getObjectReferenceCount() > 0)) {
        throw new IllegalStateException(
            getObjectReferenceCount() + " Object(s) still exist in runtime");
      }
    }
  }
  ...
  /**
   * Registers an executor with this runtime. An executor is another runtime with its own thread. By
   * registering an executor, it can be terminated when this runtime is released.
   *
  ...
  /**
   * Removes the executor from this runtime. The executor is *NOT* shutdown, simply removed from the
  ...
  /**
   * Shutdown all executors associated with this runtime. If force terminate is specified, it will
   * forcefully terminate the executors, otherwise it will simply signal that they should terminate.
   *
   * @param forceTerminate Specify if the executors should be forcefully terminated, or simply
   *     notified to shutdown when ready.
   */
  ...
  /**
   * Registers a resource with this runtime. All registered resources will be released before the
   * runtime is released.
   *
  ...
  /**
   * Executes a JS Script on this runtime and returns the result as an integer. If the result is not
   * an integer, then a V8ResultUndefinedException is thrown.
   *
   * @param script The script to execute.
   * @return The result of the script as an integer, or V8ResultUndefinedException if the result is
   *     not an integer.
   */
   ...
   * @param lineNumber The line number that is considered to be the first line of the script.
   *     Typically 0, but could be set to another value for excepton purposes.
   * @return The result of the script as an integer, or V8ResultUndefinedException if the result is
   *     not an integer.
   */
  public int executeIntegerScript(
      final String script, final String scriptName, final int lineNumber) {
    checkThread();
    checkScript(script);
  ...
  /**
   * Executes a JS Script on this runtime and returns the result as a double. If the result is not a
   * double, then a V8ResultUndefinedException is thrown.
   *
   * @param script The script to execute.
   * @return The result of the script as a double, or V8ResultUndefinedException if the result is
   *     not a double.
   */
   ...
   * @param scriptName The name of the script
   * @param lineNumber The line number that is considered to be the first line of the script.
   *     Typically 0, but could be set to another value for exception stack trace purposes.
   * @return The result of the script as a double, or V8ResultUndefinedException if the result is
   *     not a double.
   */
  public double executeDoubleScript(
      final String script, final String scriptName, final int lineNumber) {
    checkThread();
    checkScript(script);
  ...
  /**
   * Executes a JS Script on this runtime and returns the result as a String. If the result is not a
   * String, then a V8ResultUndefinedException is thrown.
   *
   * @param script The script to execute.
   * @return The result of the script as a String, or V8ResultUndefinedException if the result is
   *     not a String.
   */
   ...
   *     not a String.
   */
  public String executeStringScript(
      final String script, final String scriptName, final int lineNumber) {
    checkThread();
    checkScript(script);
  ...
  /**
   * Executes a JS Script on this runtime and returns the result as a boolean. If the result is not
   * a boolean, then a V8ResultUndefinedException is thrown.
   *
   * @param script The script to execute.
   * @return The result of the script as a boolean, or V8ResultUndefinedException if the result is
   *     not a boolean.
   */
   ...
   *     not a boolean.
   */
  public boolean executeBooleanScript(
      final String script, final String scriptName, final int lineNumber) {
    checkThread();
    checkScript(script);
  ...
  /**
   * Executes a JS Script on this runtime and returns the result as a V8Array. If the result is not
   * a V8Array, then a V8ResultUndefinedException is thrown.
   *
   * @param script The script to execute.
   * @return The result of the script as a V8Array, or V8ResultUndefinedException if the result is
   *     not a V8Array.
   */
   ...
   *     not a V8Array.
   */
  public V8Array executeArrayScript(
      final String script, final String scriptName, final int lineNumber) {
    checkThread();
  ...
  /**
   * Executes a JS Script on this runtime and returns the result as a Java Object. Primitives will
   * be boxed.
   *
   * @param script The script to execute.
  ...
  /**
   * Executes a JS Script module on this runtime and returns the result as a Java Object. Primitives
   * will be boxed.
   *
   * <p>If the script does not match the signature (as verified with the public key) then a
   ...
   * @return The result of the script as a Java Object.
   */
  public Object executeModule(
      final String script,
      final String modulePrefix,
      final String modulePostfix,
      final String uri) {
    checkThread();
    checkScript(script);
  ...
  /**
   * Executes a JS Script on this runtime and returns the result as a V8Object. If the result is not
   * a V8Object, then a V8ResultUndefinedException is thrown.
   *
   * @param script The script to execute.
   * @return The result of the script as a V8Object, or V8ResultUndefinedException if the result is
   *     not a V8Object.
   */
   ...
   *     not a V8Object.
   */
  public V8Object executeObjectScript(
      final String script, final String scriptName, final int lineNumber) {
    checkThread();
    Object result = this.executeScript(script, scriptName, lineNumber);
  ...
  /**
   * Returns the locker associated with this runtime. The locker allows threads to give up control
   * of the runtime and other threads to acquire control.
   *
  ...
  /**
   * Indicates to V8 that the system is low on memory. V8 may use this to attempt to recover space
  ...
  void registerCallback(
      ...
      final boolean includeReceiver) {
    MethodDescriptor methodDescriptor = new MethodDescriptor();
    methodDescriptor.object = object;
    methodDescriptor.method = method;
    methodDescriptor.includeReceiver = includeReceiver;
    long methodID =
        registerJavaMethod(getV8RuntimePtr(), objectHandle, jsFunctionName, isVoidMethod(method));
    functionRegistry.put(methodID, methodDescriptor);
  }

  void registerVoidCallback(
      final JavaVoidCallback callback, final long objectHandle, final String jsFunctionName) {
    MethodDescriptor methodDescriptor = new MethodDescriptor();
  ...
  void registerCallback(
      final JavaCallback callback, final long objectHandle, final String jsFunctionName) {
  ...
  protected Object callObjectJavaMethod(
      final long methodID, final V8Object receiver, final V8Array parameters) throws Throwable {
  ...
  protected void callVoidJavaMethod(
      final long methodID, final V8Object receiver, final V8Array parameters) throws Throwable {
    MethodDescriptor methodDescriptor = functionRegistry.get(methodID);
  ...
  private Object[] getArgs(
      final V8Object receiver,
      final MethodDescriptor methodDescriptor,
      final V8Array parameters,
    ...
    Object[] args =
        setDefaultValues(
            ...
            methodDescriptor.method.getParameterTypes(),
            receiver,
            methodDescriptor.includeReceiver);
    ...
    if (hasVarArgs) {
      Object varArgContainer =
          getVarArgContainer(methodDescriptor.method.getParameterTypes(), varArgs.size());
  ...
  private void populateParamters(
      final V8Array parameters,
      final int varArgIndex,
      final Object[] args,
  ...
  protected void createTwin(
      final long v8RuntimePtr, final long objectHandle, final long twinHandle) {
    _createTwin(v8RuntimePtr, objectHandle, twinHandle);
  }

  protected int executeIntegerScript(
      final long v8RuntimePtr, final String script, final String scriptName, final int lineNumber) {
    return _executeIntegerScript(v8RuntimePtr, script, scriptName, lineNumber);
  }

  protected double executeDoubleScript(
      final long v8RuntimePtr, final String script, final String scriptName, final int lineNumber) {
    return _executeDoubleScript(v8RuntimePtr, script, scriptName, lineNumber);
  }

  protected String executeStringScript(
      final long v8RuntimePtr, final String script, final String scriptName, final int lineNumber) {
    return _executeStringScript(v8RuntimePtr, script, scriptName, lineNumber);
  }

  protected boolean executeBooleanScript(
      final long v8RuntimePtr, final String script, final String scriptName, final int lineNumber) {
  ...
  protected void executeVoidScript(
      final long v8RuntimePtr, final String script, final String scriptName, final int lineNumber) {
  ...
  protected int executeIntegerFunction(
      final long v8RuntimePtr,
      final long objectHandle,
      final String name,
      final long parametersHandle) {
    return _executeIntegerFunction(v8RuntimePtr, objectHandle, name, parametersHandle);
  }

  protected double executeDoubleFunction(
      final long v8RuntimePtr,
      final long objectHandle,
      final String name,
      final long parametersHandle) {
  ...
  protected String executeStringFunction(
  ...
  protected boolean executeBooleanFunction(
      final long v8RuntimePtr, final long handle, final String name, final long parametersHandle) {
    return _executeBooleanFunction(v8RuntimePtr, handle, name, parametersHandle);
  }

  protected Object executeFunction(
      final long v8RuntimePtr,
      final int expectedType,
      final long objectHandle,
      final String name,
  ...
  protected void addObject(
      final long v8RuntimePtr, final long objectHandle, final String key, final long value) {
    _addObject(v8RuntimePtr, objectHandle, key, value);
  }

  protected void add(
      final long v8RuntimePtr, final long objectHandle, final String key, final boolean value) {
    _add(v8RuntimePtr, objectHandle, key, value);
  }

  protected void add(
      final long v8RuntimePtr, final long objectHandle, final String key, final double value) {
    _add(v8RuntimePtr, objectHandle, key, value);
  }

  protected void add(
      final long v8RuntimePtr, final long objectHandle, final String key, final String value) {
    _add(v8RuntimePtr, objectHandle, key, value);
  }
  ...
  protected long registerJavaMethod(
      final long v8RuntimePtr,
      final long objectHandle,
  ...
  protected long initNewV8ArrayBuffer(
  ...
  protected boolean arrayGetBoolean(
      final long v8RuntimePtr, final long arrayHandle, final int index) {
  ...
  protected double arrayGetDouble(
      final long v8RuntimePtr, final long arrayHandle, final int index) {
  ...
  protected int arrayGetBooleans(
      final long v8RuntimePtr,
      final long objectHandle,
      final int index,
      final int length,
  ...
  private native long _createInspector(
      long v8RuntimePtr, final V8InspectorDelegate inspectorDelegate, final String contextName);

  private native void _dispatchProtocolMessage(
      final long v8RuntimePtr, long v8InspectorPtr, final String protocolMessage);

  private native void _schedulePauseOnNextStatement(
      final long v8RuntimePtr, long v8InspectorPtr, final String reason);

  private native int _executeIntegerScript(
      long v8RuntimePtr, final String script, final String scriptName, final int lineNumber);

  private native double _executeDoubleScript(
      long v8RuntimePtr, final String script, final String scriptName, final int lineNumber);

  private native String _executeStringScript(
      long v8RuntimePtr, final String script, final String scriptName, final int lineNumber);

  private native boolean _executeBooleanScript(
      long v8RuntimePtr, final String script, final String scriptName, final int lineNumber);

  private native Object _executeScript(
      long v8RuntimePtr, int expectedType, String script, String scriptName, int lineNumber);

  private native void _executeVoidScript(
      long v8RuntimePtr, String script, String scriptName, int lineNumber);
  ...
  private native Object _get(
      long v8RuntimePtr, int expectedType, long objectHandle, final String key);

  private native int _executeIntegerFunction(
      long v8RuntimePtr, long objectHandle, String name, long parametersHandle);

  private native double _executeDoubleFunction(
      long v8RuntimePtr, long objectHandle, String name, long parametersHandle);

  private native String _executeStringFunction(
      long v8RuntimePtr2, long handle, String name, long parametersHandle);

  private native boolean _executeBooleanFunction(
      long v8RuntimePtr2, long handle, String name, long parametersHandle);

  private native Object _executeFunction(
      long v8RuntimePtr, int expectedType, long objectHandle, String name, long parametersHandle);

  private native Object _executeFunction(
      long v8RuntimePtr, long receiverHandle, long functionHandle, long parametersHandle);

  private native void _executeVoidFunction(
      long v8RuntimePtr, long objectHandle, final String name, final long parametersHandle);
  ...
  private native void _addObject(
      long v8RuntimePtr, long objectHandle, final String key, final long value);

  private native void _add(
      long v8RuntimePtr, long objectHandle, final String key, final boolean value);

  private native void _add(
      long v8RuntimePtr, long objectHandle, final String key, final double value);

  private native void _add(
      long v8RuntimePtr, long objectHandle, final String key, final String value);
  ...
  private native long _registerJavaMethod(
      long v8RuntimePtr, long objectHandle, final String functionName, final boolean voidMethod);
  ...
  private native int _getType(
      long v8RuntimePtr, long objectHandle, final int index, final int length);

  private native double[] _arrayGetDoubles(
      final long v8RuntimePtr, final long objectHandle, final int index, final int length);

  private native int[] _arrayGetIntegers(
      final long v8RuntimePtr, final long objectHandle, final int index, final int length);

  private native boolean[] _arrayGetBooleans(
      final long v8RuntimePtr, final long objectHandle, final int index, final int length);

  private native byte[] _arrayGetBytes(
      final long v8RuntimePtr, final long objectHandle, final int index, final int length);

  private native String[] _arrayGetStrings(
      final long v8RuntimePtr, final long objectHandle, final int index, final int length);
  ...
  private native int _arrayGetDoubles(
      final long v8RuntimePtr,
      final long objectHandle,
      final int index,
      final int length,
  ...
  private native int _arrayGetBooleans(
      final long v8RuntimePtr,
      final long objectHandle,
      final int index,
      final int length,
  ...
  private native int _arrayGetBytes(
      final long v8RuntimePtr,
      final long objectHandle,
      final int index,
      final int length,
  ...
  private native int _arrayGetStrings(
      final long v8RuntimePtr,
      final long objectHandle,
      final int index,
      final int length,
  ...
  private native long _initNewV8Int32Array(
      long runtimePtr, long bufferHandle, int offset, int size);

  private native long _initNewV8UInt32Array(
      long runtimePtr, long bufferHandle, int offset, int size);

  private native long _initNewV8Float32Array(
      long runtimePtr, long bufferHandle, int offset, int size);

  private native long _initNewV8Float64Array(
      long runtimePtr, long bufferHandle, int offset, int size);

  private native long _initNewV8Int16Array(
      long runtimePtr, long bufferHandle, int offset, int size);

  private native long _initNewV8UInt16Array(
      long runtimePtr, long bufferHandle, int offset, int size);

  private native long _initNewV8Int8Array(long runtimePtr, long bufferHandle, int offset, int size);

  private native long _initNewV8UInt8Array(
      long runtimePtr, long bufferHandle, int offset, int size);

  private native long _initNewV8UInt8ClampedArray(
      long runtimePtr, long bufferHandle, int offset, int size);
  ...
  private native ByteBuffer _createV8ArrayBufferBackingStore(
      final long v8RuntimePtr, final long objectHandle, final int capacity);
  ...
  private static native void _startNodeJS(final long v8RuntimePtr, final String fileName);

  private static native boolean _pumpMessageLoop(final long v8RuntimePtr);

  private static native boolean _isRunning(final long v8RuntimePtr);
