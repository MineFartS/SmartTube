...
/**
 * The entry point for the Debug API. The debug API is a Java API that exposes V8's JavaScript API.
 *
 * <p>The API requires that V8 be initialized with the runtime flag
...
public class DebugHandler implements Releasable {
  ...
  /**
   * Creates the Debug Handler for a particular V8 runtime. Before the runtime was created,
   * V8.setFlags("expose-debug-as=__j2v8_debug_handler"); must be called.
   *
  ...
  /**
   * Removes a handler from the list of breakpoint handlers. If the handler is not present in the
   * list, the list is unchanged.
   *
  ...
  /**
   * Registers a function breakpoint. When the JavaScript function is invoked, the breakpoint will
   * be 'hit'.
   *
  ...
  /**
   * Registers a breakpoint given a scriptID and line number. The breakpoint will be 'hit' when the
   * script is executed and the given line is reached.
   *
  ...
  private class BreakpointHandler implements JavaVoidCallback {
    ...
    private void invokeHandler(
        final V8Array parameters, final int event, final BreakHandler handler) {
