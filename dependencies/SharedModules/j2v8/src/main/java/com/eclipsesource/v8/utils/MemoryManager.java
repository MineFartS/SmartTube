...
/**
 * A memory manager that tracks all V8 Handles while the object is registered. Once released, all V8
 * handles that were created while the memory manager was active, will be released.
 *
 * <p>It is important that no V8 handles (V8Objects, V8Arrays, etc...) that are created while the
 * memory manager is active, are persisted.
 */
public class MemoryManager {
  ...
  /**
   * Returns the number of handles currently being tracked by this memory manager.
   *
   * <p>Throws an IllegalStateException if the memory manager is used after it's been released.
   *
   ...
   *
   * <p>Objects that are being managed by a MemoryManager will be released once the MemoryManager is
   * released. If an object is persisted, it will be remove from the MemoryManager's control and
  ...
  /**
   * Checks if the memory manager has been released or not. Released memory managers can no longer
   * be used.
   *
  ...
  /**
   * Releases this Memory Manager and all V8Objects that were created while this memory manager was
   * active.
   */
  public void release() {
    v8.getLocker().checkThread();
