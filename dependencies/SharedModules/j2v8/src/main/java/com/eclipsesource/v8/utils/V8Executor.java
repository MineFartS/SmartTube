...
/**
 * Executes a JS Script on a new V8 runtime in its own thread, and once finished, will optionally
 * wait on a message queue. If the executor is *not* long running, when the JS Script finishes, the
 * executor will shutdown. If the executor *is* long running, the the script will execute, and when
 * finished, the executor will wait for messages to arrive. When messages arrive, the messageHandler
 * will be invoked with the contents of the message.
 *
 * <p>Executors can be shutdown in two different ways. forceTermination() will stop any executing
 * scripts and immediately terminate the executor. shutdown() will indicate that the executor should
 * shutdown, but this will only happen once any scripts finish executing and the message queue
 * becomes empty.
 */
public class V8Executor extends Thread {
  ...
  /**
   * Create a new executor and execute the given script on it. Once the script has finished
   * executing, the executor can optionally wait on a message queue.
   *
   ...
   * @param messageHandler The name of the message handler that should be notified when messages are
   *     delivered.
   */
  ...
  /**
   * Override to provide a custom setup for this V8 runtime. This method can be overridden to
   * configure the V8 runtime, for example, to add callbacks or to add some additional functionality
   * to the global scope.
   *
   ...
   *
   * @return The result of the JS Script that was executed on this executor.
   */
  ...
  /**
   * Posts a message to the receiver to be processed by the executor and sent to the V8 runtime via
   * the messageHandler.
   *
  ...
  public void run() {
    ...
    try {
      if (!forceTerminating) {
        Object scriptResult =
            runtime.executeScript("__j2v8__checkThreadTerminate();\n" + script, getName(), -1);
   ...
   *
   * @return True if an exception was thrown during the JavaScript execution, false otherwise.
   */
   ...
   *
   * @return The exception that was thrown during the JavaScript execution, or null if no such
  ...
  /**
   * Forces the executor to shutdown immediately. Any currently executing JavaScript will be
   * interrupted and all outstanding messages will be ignored.
   */
  ...
  /**
   * Indicates to the executor that it should shutdown. Any currently executing JavaScript will be
   * allowed to finish, and any outstanding messages will be processed. Only once the message queue
   * is empty, will the executor actually shtutdown.
   */
