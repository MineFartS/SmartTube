...
/**
 * A representation of a JS TypedArray in Java. The typed array is simply a 'view' onto a back
 * buffer.
 */
public class V8TypedArray extends V8Array {

  /**
   * Create a new TypedArray from a specified ArrayBuffer, type, offset and size. For example, a
   * V8Int32Array is a typed array where each value is a 32-bit integer. The typed array is backed
   * by the V8ArrayBuffer.
   *
   ...
   */
  public V8TypedArray(
      final V8 v8, final V8ArrayBuffer buffer, final int type, final int offset, final int size) {
  ...
  /**
   * Provide access to the underlying ByteBuffer used for this TypedArray. The V8ArrayBuffer must be
   * released.
   *
  ...
  private long createTypedArray(final long runtimePtr, final V8ArrayData arrayData) {
    switch (arrayData.type) {
      case V8Value.FLOAT_32_ARRAY:
        return v8.initNewV8Float32Array(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      case V8Value.FLOAT_64_ARRAY:
        return v8.initNewV8Float64Array(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      case V8Value.UNSIGNED_INT_32_ARRAY:
        return v8.initNewV8UInt32Array(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      case V8Value.INT_16_ARRAY:
        return v8.initNewV8Int16Array(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      case V8Value.UNSIGNED_INT_16_ARRAY:
        return v8.initNewV8UInt16Array(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      case V8Value.INTEGER:
        return v8.initNewV8Int32Array(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      case V8Value.UNSIGNED_INT_8_ARRAY:
        return v8.initNewV8UInt8Array(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      case V8Value.INT_8_ARRAY:
        return v8.initNewV8Int8Array(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      case V8Value.UNSIGNED_INT_8_CLAMPED_ARRAY:
        return v8.initNewV8UInt8ClampedArray(
            runtimePtr, arrayData.buffer.objectHandle, arrayData.offset, arrayData.size);
      default:
        throw new IllegalArgumentException(
            "Cannot create a typed array of type "
                + V8Value.getStringRepresentation(arrayData.type));
    }
  }
  ...
  public static int getStructureSize(final int type) {
    switch (type) {
      ...
      default:
        throw new IllegalArgumentException(
            "Cannot create a typed array of type " + V8Value.getStringRepresentation(type));
    }
  }
  ...
  private void checkOffset(final V8ArrayData arrayData) {
    if ((arrayData.offset % getStructureSize(arrayData.type)) != 0) {
      throw new IllegalStateException(
          "RangeError: Start offset of Int32Array must be a multiple of "
              + getStructureSize(arrayData.type));
    }
  }
  ...
  private static class V8ArrayData {
    ...
    public V8ArrayData(
        final V8ArrayBuffer buffer, final int offset, final int size, final int type) {
      this.buffer = buffer;
      this.offset = offset;
      this.size = size;
      this.type = type;
