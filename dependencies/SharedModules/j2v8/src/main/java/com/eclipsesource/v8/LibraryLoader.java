...
class LibraryLoader {
   ...
   *     <p>NOTE: Vendors are only included for linux systems
  ...
  public static String computeLibraryShortName(boolean withLinuxVendor) {
    String prefix = "j2v8";
    String vendor =
        withLinuxVendor && PlatformDetector.OS.isLinux() ? PlatformDetector.Vendor.getName() : null;
    String os = PlatformDetector.OS.getName();
    String arch = PlatformDetector.Arch.getName();

    final String separator = "-";

    return prefix + (vendor != null ? separator + vendor : "") + separator + os + separator + arch;
  }

  public static String computeLibraryFullName(boolean withLinuxVendor) {
    return "lib"
        + computeLibraryShortName(withLinuxVendor)
        + "."
        + PlatformDetector.OS.getLibFileExtension();
  }
  ...
  static boolean tryLoad(boolean withLinuxVendor, StringBuffer message) {
    ...
    String ideLocation =
        System.getProperty("user.dir") + SEPARATOR + "jni" + SEPARATOR + libFullName;
  ...
  static void loadLibrary(final String tempDirectory) {
    ...
    if (extract(path, true, message)) return;

    // if there is no vendor-specific library, just try to extract the default OS library
    if (extract(path, false, message)) return;

    /* Failed to find the library */
    throw new UnsatisfiedLinkError(
        "Could not load J2V8 library. Reasons: " + message.toString()); // $NON-NLS-1$
  }
  ...
  static boolean extract(
      final String fileName, final String mappedName, final StringBuffer message) {
