...
public class V8RuntimeNotLoadedTest {
  ...
  /**
   * NOTE: we need to skip these tests, because on Android the SystemClassLoader can not be cast to
   * an URLClassLoader and some other issues (see TestClassLoader below)
   */
  ...
  public static class SeparateClassloaderTestRunner extends BlockJUnit4ClassRunner {
    ...
    private static Class<?> getFromTestClassloader(final Class<?> clazz)
        throws InitializationError {
      try {
        if (skipTest()) return clazz;
