...
public class V8ArrayTest {
  ...
  public void testLargeArrayGetInt() {
    V8Array array =
        v8.executeArrayScript("foo = []; for ( var i = 0; i < 10000; i++) {foo[i] = i;}; foo");
  ...
  public void testSparseArrayLength() {
    V8Array array =
        v8.executeArrayScript("x = []; x[0] = 'foo'; x[100] = 'bar'; x['boo'] = 'baz'; x");
  ...
  public void testCreateMatrix() {
    ...
    v8.executeVoidScript(
        "var total = 0; function add(matrix) { for(var i = 0; i < 3; i++) { for (var j = 0; j < 3;"
            + " j++) { total = total + matrix[i][j]; }}};");
