...
public class TestHelpers {
  public static boolean isEqual(InputStream i1, InputStream i2) throws IOException {
    ...
    try {
      while (true) {
        ...
        for (int i = 0; i < Math.min(n1, n2); i++) if (buf1.get() != buf2.get()) return false;
   ...
   *       0xFF).
   *   <li><strong>&#92;uXXXX</strong> : Hexadecimal based Unicode character.
   * </ul>
   *
      ...
      if (ch == '\\') {
        ...
        switch (nextChar) {
          ...
          case 'u':
            ...
            int code =
                Integer.parseInt(
                    "" + st.charAt(i + 2) + st.charAt(i + 3) + st.charAt(i + 4) + st.charAt(i + 5),
                    16);
