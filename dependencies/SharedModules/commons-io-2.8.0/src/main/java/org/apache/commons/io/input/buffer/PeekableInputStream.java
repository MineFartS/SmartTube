 ...
 * This comes in handy when manually implementing scanners, lexers, parsers, or the like.
 */
public class PeekableInputStream extends CircularBufferInputStream {
  ...
  /**
   * Creates a new instance, which filters the given input stream, and uses a reasonable default
   * buffer size (8192).
   *
   * @param inputStream The input stream, which is being buffered.
   */
  ...
  /**
   * Returns, whether the next bytes in the buffer are as given by {@code sourceBuffer}. This is
   * equivalent to {@link #peek(byte[], int, int)} with {@code offset} == 0, and {@code length} ==
   * {@code sourceBuffer.length}
   *
  ...
  public boolean peek(final byte[] sourceBuffer) throws IOException {
    ...
    if (sourceBuffer.length > bufferSize) {
      throw new IllegalArgumentException(
          "Peek request size of "
              + sourceBuffer.length
              + " bytes exceeds buffer size of "
              + bufferSize
              + " bytes");
    }
  ...
  /**
   * Returns, whether the next bytes in the buffer are as given by {@code sourceBuffer}, {code
   * offset}, and {@code length}.
   *
   * @param sourceBuffer the buffer to compare against
