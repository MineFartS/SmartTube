...
public final class PathUtils {
  ...
  /**
   * Private worker/holder that computes and tracks relative path names and their equality. We reuse
   * the sorted relative lists when comparing directories.
   */
   ...
   */
  private static AccumulatorPathVisitor accumulate(
      final Path directory, final int maxDepth, final FileVisitOption[] fileVisitOptions)
      throws IOException {
   ...
   * @since 2.8.0
   */
  public static PathCounters cleanDirectory(final Path directory, final DeleteOption... options)
      throws IOException {
   ...
   * @throws IOException if an I/O error is thrown by a visitor method.
   */
  public static PathCounters copyDirectory(
      final Path sourceDirectory, final Path targetDirectory, final CopyOption... copyOptions)
      throws IOException {
    return visitFileTree(
            new CopyDirectoryVisitor(
                Counters.longPathCounters(), sourceDirectory, targetDirectory, copyOptions),
            sourceDirectory)
        .getPathCounters();
  }
  ...
  public static Path copyFile(
      final URL sourceFile, final Path targetFile, final CopyOption... copyOptions)
      throws IOException {
   ...
   */
  public static Path copyFileToDirectory(
      final Path sourceFile, final Path targetDirectory, final CopyOption... copyOptions)
      throws IOException {
   ...
   */
  public static Path copyFileToDirectory(
      final URL sourceFile, final Path targetDirectory, final CopyOption... copyOptions)
      throws IOException {
    try (final InputStream inputStream = sourceFile.openStream()) {
  ...
  public static PathCounters countDirectory(final Path directory) throws IOException {
    return visitFileTree(new CountingPathVisitor(Counters.longPathCounters()), directory)
        .getPathCounters();
  }
   ...
   *
   * <ul>
   *   <li>A directory to delete does not have to be empty.
   *   <li>You get exceptions when a file or directory cannot be deleted; {@link
   ...
   * @since 2.8.0
   */
  public static PathCounters delete(final Path path, final DeleteOption... options)
      throws IOException {
   ...
   * @since 2.8.0
   */
  public static PathCounters deleteDirectory(final Path directory, final DeleteOption... options)
      throws IOException {
   ...
   * @return A visitor with path counts set to 1 file, 0 directories, and the size of the deleted
   *     file.
   * @throws IOException if an I/O error occurs.
   ...
   * @since 2.8.0
   */
  public static PathCounters deleteFile(final Path file, final DeleteOption... options)
      throws IOException {
  ...
  /**
   * Compares the file sets of two Paths to determine if they are equal or not while considering
   * file contents. The comparison includes all files in all sub-directories.
   *
  ...
  public static boolean directoryAndFileContentEquals(
    ...
    final RelativeSortedPaths relativeSortedPaths =
        new RelativeSortedPaths(path1, path2, Integer.MAX_VALUE, linkOptions, fileVisitOption);
    ...
    for (final Path path : fileList1) {
      ...
      if (binarySearch > -1) {
        if (!fileContentEquals(
            path1.resolve(path), path2.resolve(path), linkOptions, openOptions)) {
          return false;
        }
      } else {
  ...
  /**
   * Compares the file sets of two Paths to determine if they are equal or not without considering
   * file contents. The comparison includes all files in all sub-directories.
   *
   * @param path1 The first directory.
   * @param path2 The second directory.
   ...
   * @throws IOException if an I/O error is thrown by a visitor method
   */
  public static boolean directoryContentEquals(final Path path1, final Path path2)
      throws IOException {
    return directoryContentEquals(
        path1, path2, Integer.MAX_VALUE, EMPTY_LINK_OPTION_ARRAY, EMPTY_FILE_VISIT_OPTION_ARRAY);
  }
   ...
   * @throws IOException if an I/O error is thrown by a visitor method
   */
  public static boolean directoryContentEquals(
      final Path path1,
      final Path path2,
      final int maxDepth,
      final LinkOption[] linkOptions,
   ...
   *
   * <p>File content is accessed through {@link Files#newInputStream(Path,OpenOption...)}.
   *
   ...
   * @return a file attribute view of the specified type, or null ifthe attribute view type is not
   *     available.
   * @throws IOException if an I/O error occurs.
   * @since 2.8.0
   */
   ...
   */
  static List<Path> relativize(
      final Collection<Path> collection,
      final Path parent,
      final boolean sort,
   ...
   *
   * <p>This behavior is OS dependent.
   *
  ...
  public static Path setReadOnly(
      final Path path, final boolean readOnly, final LinkOption... options) throws IOException {
   ...
   *
   * <p>Note that {@link Files#walkFileTree(Path,FileVisitor)} returns the given path.
   *
   ...
   */
  public static <T extends FileVisitor<? super Path>> T visitFileTree(
      final T visitor, final Path start, final Set<FileVisitOption> options, final int maxDepth)
      throws IOException {
   ...
   */
  public static <T extends FileVisitor<? super Path>> T visitFileTree(
      final T visitor, final String first, final String... more) throws IOException {
   ...
   */
  public static <T extends FileVisitor<? super Path>> T visitFileTree(
      final T visitor, final URI uri) throws IOException {
