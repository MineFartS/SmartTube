...
/**
 * Reader proxy that transparently writes a copy of all characters read from the proxied reader to a
 * given Reader. Using {@link #skip(long)} or {@link #mark(int)}/{@link #reset()} on the reader will
 * result on some characters from the reader being skipped or duplicated in the writer.
 *
 * <p>The proxied reader is closed when the {@link #close()} method is called on this proxy. You may
 * configure whether the reader closes the writer.
 *
 * @since 2.7
 */
public class TeeReader extends ProxyReader {
  ...
  /**
   * Creates a TeeReader that proxies the given {@link Reader} and copies all read characters to the
   * given {@link Writer}. The given writer will not be closed when this reader gets closed.
   *
  ...
  /**
   * Creates a TeeReader that proxies the given {@link Reader} and copies all read characters to the
   * given {@link Writer}. The given writer will be closed when this reader gets closed if the
  ...
  /**
   * Closes the proxied reader and, if so configured, the associated writer. An exception thrown
   * from the reader will not prevent closing of the writer.
   *
  ...
  /**
   * Reads characters from the proxied reader and writes the read characters to the associated
   * writer.
   *
