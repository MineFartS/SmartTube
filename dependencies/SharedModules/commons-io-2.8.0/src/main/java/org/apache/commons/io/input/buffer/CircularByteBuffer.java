 ...
 * This works particularly well, if reading from the buffer takes place at the same time than
 * writing to. Such is the case, for example, when using the buffer within a filtering input stream,
...
public class CircularByteBuffer {
  ...
  /**
   * Creates a new instance with a reasonable default buffer size ({@link
  ...
  /**
   * Returns the next byte from the buffer, removing it at the same time, so that following
   * invocations won't return it again.
   *
   * @return The byte, which is being returned.
   * @throws IllegalStateException The buffer is empty. Use {@link #hasBytes()}, or {@link
   *     #getCurrentNumberOfBytes()}, to prevent this exception.
   */
  public byte read() {
  ...
  /**
   * Returns the given number of bytes from the buffer by storing them in the given byte array at
   * the given offset.
   *
   ...
   * @throws IllegalArgumentException Either of {@code pOffset}, or {@code pLength} is negative, or
   *     the length of the byte array {@code pBuffer} is too small.
   * @throws IllegalStateException The buffer doesn't hold the given number of bytes. Use {@link
   *     #getCurrentNumberOfBytes()} to prevent this exception.
   */
  public void read(final byte[] targetBuffer, final int targetOffset, final int length) {
    ...
    if (targetOffset + length > targetBuffer.length) {
      throw new IllegalArgumentException(
          "The supplied byte array contains only "
              + targetBuffer.length
              + " bytes, but offset, and length would require "
  ...
  /**
   * Adds a new byte to the buffer, which will eventually be returned by following invocations of
   * {@link #read()}.
   *
   * @param value The byte, which is being added to the buffer.
   * @throws IllegalStateException The buffer is full. Use {@link #hasSpace()}, or {@link
   *     #getSpace()}, to prevent this exception.
   */
  ...
  /**
   * Returns, whether the next bytes in the buffer are exactly those, given by {@code sourceBuffer},
   * {@code offset}, and {@code length}. No bytes are being removed from the buffer. If the result
   * is true, then the following invocations of {@link #read()} are guaranteed to return exactly
   * those bytes.
   *
   * @param sourceBuffer the buffer to compare against
   * @param offset start offset
   * @param length length to compare
   * @return True, if the next invocations of {@link #read()} will return the bytes at offsets
   *     {@code pOffset}+0, {@code pOffset}+1, ..., {@code pOffset}+{@code pLength}-1 of byte array
   *     {@code pBuffer}.
  ...
  /**
   * Adds the given bytes to the buffer. This is the same as invoking {@link #add(byte)} for the
   * bytes at offsets {@code offset+0}, {@code offset+1}, ..., {@code offset+length-1} of byte array
   * {@code targetBuffer}.
   *
   * @param targetBuffer the buffer to copy
   * @param offset start offset
   * @param length length to copy
   * @throws IllegalStateException The buffer doesn't have sufficient space. Use {@link #getSpace()}
  ...
  /**
   * Returns, whether there is currently room for a single byte in the buffer. Same as {@link
   * #hasSpace(int) hasSpace(1)}.
   *
