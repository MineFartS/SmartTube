...
/**
 * Abstracts an OS' file system details, currently supporting the single use case of converting a
 * file name String to a legal file name with {@link #toLegalFileName(String, char)}.
 *
 * <p>The starting point of any operation is {@link #getCurrent()} which gets you the enum for the
 * file system that matches the OS hosting the running JVM.
 *
 * @since 2.7
 */
public enum FileSystem {
  ...
  /**
   * Checks if a candidate file name (without a path) such as {@code "filename.ext"} or {@code
   * "filename"} is a potentially legal file name. If the file name length exceeds {@link
   * #getMaxFileNameLength()}, or if it contains an illegal character then the check fails.
   *
   * @param candidate a candidate file name (without a path) like {@code "filename.ext"} or {@code
   *     "filename"}
   ...
   * "filename"} to a legal file name. Illegal characters in the candidate name are replaced by the
   * {@code replacement} character. If the file name length exceeds {@link #getMaxFileNameLength()},
   * then the name is truncated to {@link #getMaxFileNameLength()}.
   *
   * @param candidate a candidate file name (without a path) like {@code "filename.ext"} or {@code
   *     "filename"}
  ...
  public String toLegalFileName(final String candidate, final char replacement) {
    if (isIllegalFileNameChar(replacement)) {
      throw new IllegalArgumentException(
          String.format(
              "The replacement character '%s' cannot be one of the %s illegal characters: %s",
              // %s does not work properly with NUL
              replacement == '\0' ? "\\0" : replacement,
              name(),
              Arrays.toString(illegalFileNameChars)));
    }
    final String truncated =
        candidate.length() > maxFileNameLength
            ? candidate.substring(0, maxFileNameLength)
            : candidate;
