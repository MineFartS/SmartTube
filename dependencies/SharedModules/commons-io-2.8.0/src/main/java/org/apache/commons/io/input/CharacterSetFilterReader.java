...
/**
 * A filter reader that removes a given set of characters represented as <code>int</code> code
 * points, handy to remove known junk characters from CSV files for example.
 *
 * <p>This class must convert each <code>int</code> read to an <code>Integer</code>. You can
 * increase the Integer cache with a system property, see {@link Integer}.
 */
public class CharacterSetFilterReader extends AbstractCharacterFilterReader {
  ...
  protected boolean filter(final int ch) {
    // Note WRT Integer.valueOf(): You can increase the Integer cache with a system property, see
    // {@link Integer}.
