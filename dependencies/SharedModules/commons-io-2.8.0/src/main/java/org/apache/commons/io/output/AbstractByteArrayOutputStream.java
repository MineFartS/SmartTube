...
/**
 * This is the base class for implementing an output stream in which the data is written into a byte
 ...
 * Closing an {@code AbstractByteArrayOutputStream} has no effect. The methods in this class can be
 * called after the stream has been closed without generating an {@code IOException}.
 *
 * <p>This is the base for an alternative implementation of the {@link
 * java.io.ByteArrayOutputStream} class. The original implementation only allocates 32 bytes at the
 * beginning. As this class is designed for heavy duty it starts at {@value #DEFAULT_SIZE} bytes. In
 * contrast to the original it doesn't reallocate the whole memory block but allocates additional
 * buffers. This way no buffers need to be garbage collected and the contents don't have to be
 * copied to the new buffer. This class is designed to behave exactly like the original. The only
 * exception is the deprecated {@link java.io.ByteArrayOutputStream#toString(int)} method that has
 * been ignored.
 *
 * @since 2.7
 */
