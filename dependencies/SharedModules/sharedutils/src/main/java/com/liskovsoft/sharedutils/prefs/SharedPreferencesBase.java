...
public class SharedPreferencesBase {
  ...
  public SharedPreferencesBase(
      Context context, String prefName, int defValResId, boolean limitMaxSize) {
    if (limitMaxSize) {
    ...
    if (defValResId != -1) {
      try {
        PreferenceManager.setDefaultValues(
            context, prefName, Context.MODE_PRIVATE, defValResId, true);
      } catch (NoSuchMethodError e) {
        // NoSuchMethodError: No interface method putBoolean in class SharedPreferences$Editor
        // (Android 7.0)
        e.printStackTrace();
      }
    }
  ...
  private void limitMaxSize(Context context, String prefName) {
    File sharedPrefs =
        new File(context.getApplicationInfo().dataDir, "shared_prefs" + "/" + prefName + ".xml");
