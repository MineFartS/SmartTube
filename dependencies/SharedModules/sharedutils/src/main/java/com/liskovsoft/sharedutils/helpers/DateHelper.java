...
public class DateHelper {
  ...
  public static long toUnixTimeMs(String timestamp) {
    ...
    String longPattern =
        "yyyy-MM-dd'T'HH:mm:ss" + (timestamp.contains("+") && supportXPattern ? "X" : "");
    String shortPattern = "yyyy-MM-dd";
    SimpleDateFormat format =
        new SimpleDateFormat(timestamp.contains("T") ? longPattern : shortPattern, Locale.US);
  ...
  public static String toShortDate(
      long timeMs, boolean showDate, boolean showYear, boolean showHours) {
   ...
   */
  public static String toShortDate(
      String timestamp, boolean showDate, boolean showYear, boolean showHours) {
    return toShortDate(toDate(timestamp), showDate, showYear, showHours);
  }

  private static String toShortDate(
      Date date, boolean showDate, boolean showYear, boolean showHours) {
    Locale locale = Locale.getDefault();
    boolean is24HourLocale =
        GlobalPreferences.sInstance != null
            ? GlobalPreferences.sInstance.is24HourLocaleEnabled()
            : is24HourLocale(locale);
    ...
    SimpleDateFormat sdf =
        new SimpleDateFormat(
            Helpers.combineItems(" ", datePattern, yearPattern, hoursPattern), locale);
  ...
  private static boolean is24HourLocale(Locale locale) {
    return !Helpers.equalsAny(
        locale.getLanguage(), "en", "es", "pt", "fr", "hi", "tl", "ar", "sw", "bn", "ur");
  }
}
