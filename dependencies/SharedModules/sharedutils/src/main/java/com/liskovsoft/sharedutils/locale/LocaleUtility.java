...
public class LocaleUtility {
  ...
  private static void initScriptsByLocale() {
    ...
    scriptsByLocale.put(
        "zh",
        getScriptsMap(
            "", "Hant", "CN", "Hans", "HK", "Hans", "MO", "Hans", "SG", "Hans", "MN", "Hans"));
  ...
  /**
   * Gets the script (writing type) for the given locale. For example, if a US citizen uses German
   * Locale, and calls this method with Locale.getDefault(), the result would be "Runr"
   *
   * @param locale
   * @return
   */
  ...
  /**
   * Obtain right locale even if the user changes their Locale in settings after your application
   * process is running. Android N (Api level 24) update (no warnings).
   *
  ...
  public static boolean is24HourLocale(Context context) {
    ...
    java.text.DateFormat natural =
        java.text.DateFormat.getTimeInstance(java.text.DateFormat.LONG, currentLocale);
