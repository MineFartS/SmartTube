...
public class UnzippingInterceptor implements Interceptor {
      ...
      return response
          ...
          .body(
              new RealResponseBody(
                  toString(response.body().contentType()),
                  contentLength,
                  Okio.buffer(responseBody)))
          .build();
    } else if (contentEncoding != null && contentEncoding.equals("deflate")) {
      long contentLength = -1;
      InflaterSource responseBody =
          new InflaterSource(response.body().source(), new Inflater(true));
      Headers strippedHeaders = stripHeaders(response.headers());
      return response
          .newBuilder()
          .headers(strippedHeaders)
  ...
  private static Headers stripHeaders(Headers headers) {
    return headers.newBuilder().removeAll("Content-Encoding").removeAll("Content-Length").build();
  }
