...
public abstract class GenericSelectorDialog implements OnClickListener {
  ...
  private View buildView(Context context) {
    ...
    for (DialogItem item : mDialogSource.getItems()) {
      ...
      dialogItem.setTextSize(
          TypedValue.COMPLEX_UNIT_PX,
          mActivity.getResources().getDimension(R.dimen.dialog_text_size));
  ...
  private void updateViews(View root, boolean focusDefault) {
    ...
    boolean enableFocus =
        focusDefault
            && (mDialogSource instanceof CombinedDialogSource
                || mDialogSource instanceof SingleDialogSource);
    ...
    for (DialogItem item : items) {
      ...
      if (view == null) {
        throw new IllegalStateException(
            "Please, don't create 'DialogItems' in the 'getItems' method. For such purposed use"
                + " constructor.");
      }
  ...
  protected abstract CheckedTextView createDialogItem(
