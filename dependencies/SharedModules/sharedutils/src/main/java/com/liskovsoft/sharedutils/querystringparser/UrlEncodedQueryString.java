...
public class UrlEncodedQueryString implements UrlQueryString {
  ...
  @Nullable private String mQueryPrefix;
  @Nullable private UrlEncodedQueryStringBase mQueryString;
  private String mUrl;
  ...
  private UrlEncodedQueryString(String url) {
    ...
    if (Helpers.isValidUrl(url)) {
      ...
      if (parsedUrl != null) {
        mQueryPrefix =
            String.format(
                "%s://%s%s", parsedUrl.getScheme(), parsedUrl.getHost(), parsedUrl.getPath());
  ...
  private URI getURI(String url) {
    ...
    try {
      ...
      return new URI(
          url.length() > 100
              ? // OOM fix: don't replace long string
              url
              : url.replace(" ", "+").replace("|", "%7C").replace("\\", "/").replace("#", ""));
  ...
  public String toString() {
    ...
    return mQueryPrefix != null
        ? String.format("%s?%s", mQueryPrefix, mQueryString)
        : mQueryString.toString();
  }
