...
final class OkHttpCommons {
  ...
  private static void setupConnectionParams(OkHttpClient.Builder okBuilder) {
    ...
    // okBuilder.connectionPool(new ConnectionPool(10, 24, TimeUnit.HOURS)); // Video unavailable
    // fix???
    okBuilder.connectionPool(
        new ConnectionPool(20, 5, TimeUnit.MINUTES)); // fix npe on pool dispose???
  }
   ...
   * Setting testMode configuration. If set as testMode, the connection will skip certification
   * check
   */
