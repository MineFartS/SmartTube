...
public class KeyHelpers {
  ...
  /**
   * Key code constant: Contents menu key. Goes to the title list. Corresponds to Contents Menu
   * (0x0B) of CEC User Control Code
   */
  ...
  /**
   * Whether this key is a media key, which can be send to apps that are interested in media key
   * events.
   *
   * @hide
   */
  ...
  public static void fixShowKeyboard(EditText... editFields) {
    ...
    for (EditText editField : editFields) {
      ...
      editField.setOnFocusChangeListener(
          (v, hasFocus) -> {
            if (hasFocus) {
              Helpers.showKeyboardAlt(v.getContext(), v);
            }
