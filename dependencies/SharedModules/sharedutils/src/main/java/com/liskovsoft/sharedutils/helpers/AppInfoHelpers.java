...
public class AppInfoHelpers {
  ...
  public static String getAppVersionRobust(Context context, String launchActivityName) {
    return formatAppVersion(
        getAppVersionName(context), getActivityLabelRobust(context, launchActivityName));
  }
    ...
    try {
      ...
      if (info != null) {
        if (info.metaData != null && info.metaData.containsKey("AppLabel")) {
          ...
          label =
              Helpers.getSimpleClassName(info.name); // label not found, return simple class name
        }
      }
    }
