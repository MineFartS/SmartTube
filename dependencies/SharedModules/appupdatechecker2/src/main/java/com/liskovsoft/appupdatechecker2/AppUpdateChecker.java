...
public class AppUpdateChecker implements AppVersionCheckerListener, AppDownloaderListener {
  ...
  /**
   * You normally shouldn't need to call this, as {@link #checkForUpdates(String[] versionListUrls)}
   * checks it before doing any updates.
   *
  ...
  private boolean isStale() {
    ...
    return System.currentTimeMillis() - mSettingsManager.getLastCheckedMs()
        > mSettingsManager.getMinIntervalMs();
  }
  ...
  public void checkForUpdates(String[] updateManifestUrls) {
    if (isUpdateCheckEnabled() && isStale()) {
      ...
      mListener.onUpdateError(
          new IllegalStateException(AppUpdateCheckerListener.UPDATE_CHECK_DISABLED));
    }
  }
