...
/**
 * A fairly simple non-Market app update checker. Give it a URL pointing to a JSON file and it will
 * compare its version (from the manifest file) to the versions listed in the JSON. If there are
 * newer version(s), it will provide the changelog between the installed version and the latest
 * version. The updater checks against the versionCode, but displays the versionName.
 *
 * <p>While you can create your own OnAppUpdateListener to listen for new updates, OnUpdateDialog is
 * a handy implementation that displays a Dialog with a bulleted list and a button to do the
 * upgrade.
 *
 * <p>The JSON format looks like this:
 *
 * <pre>
 * {
 * "package": {
...
public class AppVersionChecker {
  ...
  /**
   * Checks for updates regardless of when the last check happened or if checking for updates is
   * enabled.<br>
  ...
  private void triggerFromJson(JSONObject jo) throws JSONException {
    ...
    if (mVersionInfo.has("downloadUrlList_" + DeviceHelpers.getPrimaryAbi())) {
      JSONArray urls =
          mVersionInfo.getJSONArray("downloadUrlList_" + DeviceHelpers.getPrimaryAbi());
    ...
    if (mCurrentAppVersion > latestVersionNumber) {
      Log.d(
          TAG,
          "We're newer than the latest published version ("
              + latestVersionName
              + "). Living in the future...");
      mListener.onChangelogReceived(
          true, latestVersionName, latestVersionNumber, null, downloadUrls);
      return;
    }
    ...
    for (final Entry<Integer, JSONObject> version :
        versionMap.headMap(mCurrentAppVersion).entrySet()) {
      final JSONObject versionInfo = version.getValue();

      JSONArray versionChangelog =
          versionInfo.optJSONArray("changelog_" + LocaleUtility.getCurrentLanguage(mContext));
    ...
    mListener.onChangelogReceived(
        false, latestVersionName, latestVersionNumber, changelog, downloadUrls);
  }
    ...
    protected void onPostExecute(JSONObject result) {
      if (result != null) {
        ...
        mListener.onCheckError(
            mLastException != null
                ? mLastException
                : new Exception("Unknown error. JSON content is null"));
      }
