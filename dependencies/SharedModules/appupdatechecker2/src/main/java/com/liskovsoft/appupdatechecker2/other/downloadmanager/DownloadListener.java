...
public class DownloadListener {
  ...
  private void enqueueFile(String uriStr, String fileName) {
    ...
    if (fileName == null) fileName = getFileName(uri);
  ...
  class GetFileInfo extends AsyncTask<String, Integer, String> {
    ...
    protected String doInBackground(String... urls) {
      ...
      try {
        ...
        if (depo != null) filename = depo.replaceFirst("(?i)^.*filename=\"?([^\"]+)\"?.*$", "$1");
        else filename = URLUtil.guessFileName(urls[0], null, null);
