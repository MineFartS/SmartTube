...
class FragmentManagerViewModel extends ViewModel {
   ...
   *
   * <ol>
   *   <li>Automatically saved: in this model, it is assumed that the ViewModel is added to an
   *       appropriate {@link ViewModelStore} that has the same lifecycle as the FragmentManager and
   *       that {@link #onCleared()} indicates that the Fragment's host is being permanently
   *       destroyed.
   *   <li>Not automatically saved: in this model, the FragmentManager is responsible for calling
   *       {@link #getSnapshot()} and later restoring the ViewModel with {@link
   *       #restoreFromSnapshot(FragmentManagerNonConfig)}.
   * </ol>
   *
  ...
  /**
   * @deprecated Ideally, we only support mStateAutomaticallySaved = true and remove this code,
   *     alongside {@link FragmentController#restoreAllState(android.os.Parcelable,
  ...
  /**
   * @deprecated Ideally, we only support mStateAutomaticallySaved = true and remove this code,
   *     alongside {@link FragmentController#retainNestedNonConfig()}.
   */
  @Deprecated
  @Nullable
  FragmentManagerNonConfig getSnapshot() {
    ...
    if (mRetainedFragments.isEmpty() && childNonConfigs.isEmpty() && mViewModelStores.isEmpty()) {
      return null;
    }
    return new FragmentManagerNonConfig(
        new ArrayList<>(mRetainedFragments), childNonConfigs, new HashMap<>(mViewModelStores));
  }
