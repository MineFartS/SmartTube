...
public class DialogFragment extends Fragment
  ...
  /**
   * Style for {@link #setStyle(int, int)}: don't draw any frame at all; the view hierarchy returned
   * by {@link #onCreateView} is entirely responsible for drawing the dialog.
   */
  ...
  /**
   * Call to customize the basic appearance and behavior of the fragment's dialog. This can be used
   * for some common dialog behaviors, taking care of selecting flags, theme, and other options for
   * you. The same effect can be achieve by manually setting Dialog and Window attributes yourself.
  ...
  /**
   * Display the dialog, adding the fragment to the given FragmentManager. This is a convenience for
   * explicitly creating a transaction, adding the fragment to it with the given tag, and {@link
   * FragmentTransaction#commit() committing} it. This does <em>not</em> add the transaction to the
   * fragment back stack. When the fragment is dismissed, a new transaction will be executed to
  ...
  /**
   * Display the dialog, adding the fragment using an existing transaction and then {@link
   * FragmentTransaction#commit() committing} the transaction.
   *
   * @param transaction An existing transaction in which to add the fragment.
   * @param tag The tag for this fragment, as per {@link FragmentTransaction#add(Fragment, String)
   *     FragmentTransaction.add}.
   * @return Returns the identifier of the committed transaction, as per {@link
   ...
   * transaction to the fragment back stack. When the fragment is dismissed, a new transaction will
   * be executed to remove it from the activity.
   *
   * @param manager The FragmentManager this fragment will be added to.
   * @param tag The tag for this fragment, as per {@link FragmentTransaction#add(Fragment, String)
   *     FragmentTransaction.add}.
   ...
   * stack state up to and including this entry will be popped. Otherwise, a new transaction will be
   * committed to remove the fragment.
   */
  public void dismiss() {
  ...
  void dismissInternal(boolean allowStateLoss, boolean fromOnDismiss) {
    ...
    mViewDestroyed = true;
    if (mBackStackId >= 0) {
      requireFragmentManager().popBackStack(mBackStackId, FragmentManager.POP_BACK_STACK_INCLUSIVE);
      mBackStackId = -1;
    } else {
   ...
   * Return the {@link Dialog} this fragment is currently controlling.
   *
   * @throws IllegalStateException if the Dialog has not yet been created (before {@link
   *     #onCreateDialog(Bundle)}) or has been destroyed (after {@link #onDestroyView()}.
   * @see #getDialog()
   */
  @NonNull
  ...
  /**
   * Control whether the shown Dialog is cancelable. Use this instead of directly calling {@link
   * Dialog#setCancelable(boolean) Dialog.setCancelable(boolean)}, because DialogFragment needs to
   * change its behavior based on this.
   *
   ...
   * created in {@link #onActivityCreated(Bundle)}, and the fragment's view hierarchy will thus not
   * be added to it. This allows you to instead use it as a normal fragment (embedded inside of its
   * activity).
   *
   * <p>This is normally set for you based on whether the fragment is associated with a container
  ...
  public LayoutInflater onGetLayoutInflater(@Nullable Bundle savedInstanceState) {
    ...
    if (mDialog != null) {
      ...
      return (LayoutInflater)
          mDialog.getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    }
   ...
   * AlertDialog instead of a generic Dialog; when doing so, {@link #onCreateView(LayoutInflater,
   * ViewGroup, Bundle)} does not need to be implemented since the AlertDialog takes care of its own
   * content.
   *
   * <p>This method will be called after {@link #onCreate(Bundle)} and before {@link
   ...
   * Dialog.setOnCancelListener} and {@link Dialog#setOnDismissListener Dialog.setOnDismissListener}
   * callbacks. You must not set them yourself.</em> To find out about these events, override {@link
   * #onCancel(DialogInterface)} and {@link #onDismiss(DialogInterface)}.
   *
