...
final class BackStackRecord extends FragmentTransaction
  ...
  public FragmentTransaction setMaxLifecycle(
    ...
    if (fragment.mFragmentManager != mManager) {
      throw new IllegalArgumentException(
          "Cannot setMaxLifecycle for Fragment not attached to" + " FragmentManager " + mManager);
    }
    if (!state.isAtLeast(Lifecycle.State.CREATED)) {
      throw new IllegalArgumentException(
          "Cannot set maximum Lifecycle below " + Lifecycle.State.CREATED);
    }
  ...
  void bumpBackStackNesting(int amt) {
    ...
    if (FragmentManagerImpl.DEBUG) Log.v(TAG, "Bump nesting in " + this + " by " + amt);
    final int numOps = mOps.size();
    for (int opNum = 0; opNum < numOps; opNum++) {
      final Op op = mOps.get(opNum);
  ...
  /**
   * Implementation of {@link FragmentManagerImpl.OpGenerator}. This operation is added to the list
   * of pending actions during {@link #commit()}, and will be executed on the UI thread to run this
   * FragmentTransaction.
   *
  ...
  /**
   * Executes the operations contained within this transaction. The Fragment states will only be
   * modified if optimizations are not allowed.
   */
  void executeOps() {
    final int numOps = mOps.size();
    for (int opNum = 0; opNum < numOps; opNum++) {
  ...
  /**
   * Reverses the execution of the operations within this transaction. The Fragment states will only
   * be modified if reordering is not allowed.
   *
   * @param moveToState {@code true} if added fragments should be moved to their final state in
  ...
  /**
   * Expands all meta-ops into their more primitive equivalents. This must be called prior to {@link
   * #executeOps()} or any other call that operations on mOps for forward navigation. It should not
   * be called for pop/reverse navigation operations.
   *
   * <p>Removes all OP_REPLACE ops and replaces them with the proper add and remove operations that
   * are equivalent to the replace.
   *
   * <p>Adds OP_UNSET_PRIMARY_NAV ops to match OP_SET_PRIMARY_NAV, OP_REMOVE and OP_DETACH ops so
   * that we can restore the old primary nav fragment later. Since callers call this method in a
   * loop before running ops from several transactions at once, the caller should pass the return
   * value from this method as the oldPrimaryNav parameter for the next call. The first call in such
   * a loop should pass the value of {@link FragmentManager#getPrimaryNavigationFragment()}.
   *
   * @param added Initialized to the fragments that are in the mManager.mAdded, this will be
   *     modified to contain the fragments that will be in mAdded after the execution ({@link
   *     #executeOps()}.
   * @param oldPrimaryNav The tracked primary navigation fragment as of the beginning of this set of
   *     ops
