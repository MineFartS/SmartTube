...
/**
 * Contains the Fragment Transition functionality for both ordered and reordered Fragment
 * Transactions. With reordered fragment transactions, all Views have been added to the View
 * hierarchy prior to calling startTransitions. With ordered fragment transactions, Views will be
 * removed and added after calling startTransitions.
 */
class FragmentTransition {
  /**
   * The inverse of all BackStackRecord operation commands. This assumes that REPLACE operations
   * have already been replaced by add/remove operations.
   */
  ...
  private static final FragmentTransitionImpl PLATFORM_IMPL =
      Build.VERSION.SDK_INT >= 21 ? new FragmentTransitionCompat21() : null;
  ...
  private static FragmentTransitionImpl resolveSupportImpl() {
    try {
      ...
      Class<FragmentTransitionImpl> impl =
  ...
  /**
   * The main entry point for Fragment Transitions, this starts the transitions set on the leaving
   * Fragment's {@link Fragment#getExitTransition()}, the entering Fragment's {@link
   * Fragment#getEnterTransition()} and {@link Fragment#getSharedElementEnterTransition()}. When
   * popping, the leaving Fragment's {@link Fragment#getReturnTransition()} and {@link
   * Fragment#getSharedElementReturnTransition()} and the entering {@link
   * Fragment#getReenterTransition()} will be run.
   *
   * <p>With reordered Fragment Transitions, all Views have been added to the View hierarchy prior
   * to calling this method. The incoming Fragment's Views will be INVISIBLE. With ordered Fragment
   * Transitions, this method is called before any change has been made to the hierarchy. That means
   * that the added Fragments have not created their Views yet and the hierarchy is unknown.
   *
   ...
   * @param startIndex The first index into records and isRecordPop to execute as part of this
   *     transition.
   * @param endIndex One past the last index into records and isRecordPop to execute as part of this
   *     transition.
   * @param isReordered true if this is a reordered transaction, meaning that the Views of incoming
   *     fragments have been added. false if the transaction has yet to be run and Views haven't
   *     been created.
   */
  static void startTransitions(
    ...
    if (transitioningFragments.size() != 0) {
      ...
      for (int i = 0; i < numContainers; i++) {
        ...
        ArrayMap<String, String> nameOverrides =
            calculateNameOverrides(containerId, records, isRecordPop, startIndex, endIndex);

        FragmentContainerTransition containerTransition = transitioningFragments.valueAt(i);

        if (isReordered) {
          configureTransitionsReordered(
              fragmentManager, containerId, containerTransition, nonExistentView, nameOverrides);
        } else {
  ...
  /**
   * Iterates through the transactions that affect a given fragment container and tracks the shared
   * element names across transactions. This is most useful in pop transactions where the names of
   * shared elements are known.
   *
   ...
   * @param endIndex One past the last index into records and isRecordPop to execute as part of this
   *     transition.
   * @return A map from the initial shared element name to the final shared element name before any
   *     onMapSharedElements is run.
   */
  private static ArrayMap<String, String> calculateNameOverrides(
      int containerId,
      ArrayList<BackStackRecord> records,
      ArrayList<Boolean> isRecordPop,
      int startIndex,
   ...
   * reordered. That means that all Fragment Views have been added and incoming fragment Views are
   * marked invisible.
   *
   * @param fragmentManager The executing FragmentManagerImpl
   * @param containerId The container ID that is executing the transition.
   * @param fragments A structure holding the transitioning fragments in this container.
   * @param nonExistentView A View that does not exist in the hierarchy. This is used to prevent
   *     transitions from acting on other Views when there is no other target.
   * @param nameOverrides A map of the shared element names from the starting fragment to the final
   *     fragment's Views as given in {@link FragmentTransaction#addSharedElement(View, String)}.
   */
  private static void configureTransitionsReordered(
      FragmentManagerImpl fragmentManager,
      int containerId,
    ...
    ArrayList<View> exitingViews =
        configureEnteringExitingViews(
            impl, exitTransition, outFragment, sharedElementsOut, nonExistentView);

    ArrayList<View> enteringViews =
        configureEnteringExitingViews(
            impl, enterTransition, inFragment, sharedElementsIn, nonExistentView);

    setViewVisibility(enteringViews, View.INVISIBLE);

    Object transition =
        mergeTransitions(
            impl, enterTransition, exitTransition, sharedElementTransition, inFragment, inIsPop);
    ...
    if (transition != null) {
      ...
      impl.setNameOverridesReordered(
          sceneRoot, sharedElementsOut, sharedElementsIn, inNames, nameOverrides);
  ...
  /**
   * Replace hide operations with visibility changes on the exiting views. Instead of making the
   * entire fragment's view GONE, make each exiting view INVISIBLE. At the end of the transition,
   * make the fragment's view GONE.
   */
  private static void replaceHide(
    ...
    if (exitingFragment != null
        && exitTransition != null
        && exitingFragment.mAdded
        && exitingFragment.mHidden
        && exitingFragment.mHiddenChanged) {
  ...
  /**
   * Configures a transition for a single fragment container for which the transaction was ordered.
   * That means that the transaction has not been executed yet, so incoming Views are not yet known.
   *
   * @param fragmentManager The executing FragmentManagerImpl
   * @param containerId The container ID that is executing the transition.
   * @param fragments A structure holding the transitioning fragments in this container.
   ...
   *     fragment's Views as given in {@link FragmentTransaction#addSharedElement(View, String)}.
   */
  private static void configureTransitionsOrdered(
      FragmentManagerImpl fragmentManager,
      int containerId,
      FragmentContainerTransition fragments,
      View nonExistentView,
  ...
  /**
   * This method is used for fragment transitions for ordrered transactions to change the enter and
   * exit transition targets after the call to {@link
   * FragmentTransitionCompat21#beginDelayedTransition(ViewGroup, Object)}. The exit transition must
   * ensure that it does not target any Views and the enter transition must start targeting the
   * Views of the incoming Fragment.
   *
   * @param sceneRoot The fragment container View
   * @param inFragment The last fragment that is entering
   * @param nonExistentView A view that does not exist in the hierarchy that is used as a transition
   *     target to ensure no View is targeted.
  ...
  private static void scheduleTargetChange(
      ...
      final ArrayList<View> enteringViews,
   ...
   *     exists.
   */
  private static Object getSharedElementTransition(
      FragmentTransitionImpl impl, Fragment inFragment, Fragment outFragment, boolean isPop) {
    if (inFragment == null || outFragment == null) {
      return null;
    }
    Object transition =
        impl.cloneTransition(
            isPop
  ...
  private static Object getEnterTransition(
      FragmentTransitionImpl impl, Fragment inFragment, boolean isPop) {
    if (inFragment == null) {
      return null;
    }
    return impl.cloneTransition(
        isPop ? inFragment.getReenterTransition() : inFragment.getEnterTransition());
  }

  /** Returns a clone of the exit transition or null if no such transition exists. */
  private static Object getExitTransition(
      FragmentTransitionImpl impl, Fragment outFragment, boolean isPop) {
    if (outFragment == null) {
      return null;
    }
    return impl.cloneTransition(
        isPop ? outFragment.getReturnTransition() : outFragment.getExitTransition());
  }

  /**
   * Configures the shared elements of a reordered fragment transaction's transition. This retrieves
   * the shared elements of the outgoing and incoming fragments, maps the views, and sets up the
   * epicenter on the transitions.
   *
   * <p>The epicenter of exit and shared element transitions is the first shared element in the
   * outgoing fragment. The epicenter of the entering transition is the first shared element in the
   * incoming fragment.
   *
   * @param sceneRoot The fragment container View
   * @param nonExistentView A View that does not exist in the hierarchy. This is used to prevent
   ...
   *     fragment
   * @param sharedElementsIn A list modified to contain the shared elements in the incoming fragment
   * @param enterTransition The transition used for entering Views, modified by applying the
   *     epicenter
   * @param exitTransition The transition used for exiting Views, modified by applying the epicenter
   * @return The shared element transition or null if no shared elements exist
   */
  private static Object configureSharedElementsReordered(
      final FragmentTransitionImpl impl,
      final ViewGroup sceneRoot,
      final View nonExistentView,
      final ArrayMap<String, String> nameOverrides,
    ...
    final boolean inIsPop = fragments.lastInIsPop;
    Object sharedElementTransition =
        nameOverrides.isEmpty()
            ? null
            : getSharedElementTransition(impl, inFragment, outFragment, inIsPop);

    final ArrayMap<String, View> outSharedElements =
        captureOutSharedElements(impl, nameOverrides, sharedElementTransition, fragments);

    final ArrayMap<String, View> inSharedElements =
        captureInSharedElements(impl, nameOverrides, sharedElementTransition, fragments);
    ...
    if (nameOverrides.isEmpty()) {
      ...
      addSharedElementsWithMatchingNames(
          sharedElementsOut, outSharedElements, nameOverrides.keySet());
      addSharedElementsWithMatchingNames(
          sharedElementsIn, inSharedElements, nameOverrides.values());
    }
  ...
  /**
   * Add Views from sharedElements into views that have the transitionName in the nameOverridesSet.
   *
   * @param views Views list to add shared elements to
   * @param sharedElements List of shared elements
   * @param nameOverridesSet The transition names for all views to be copied from sharedElements to
   *     views.
   */
  private static void addSharedElementsWithMatchingNames(
      ArrayList<View> views,
      ArrayMap<String, View> sharedElements,
      Collection<String> nameOverridesSet) {
  ...
  /**
   * Configures the shared elements of an ordered fragment transaction's transition. This retrieves
   * the shared elements of the incoming fragments, and schedules capturing the incoming fragment's
   * shared elements. It also maps the views, and sets up the epicenter on the transitions.
   *
   * <p>The epicenter of exit and shared element transitions is the first shared element in the
   * outgoing fragment. The epicenter of the entering transition is the first shared element in the
   * incoming fragment.
   ...
   * @return The shared element transition or null if no shared elements exist
   */
  private static Object configureSharedElementsOrdered(
      final FragmentTransitionImpl impl,
      final ViewGroup sceneRoot,
      final View nonExistentView,
      final ArrayMap<String, String> nameOverrides,
          ...
          public void run() {
            ArrayMap<String, View> inSharedElements =
                captureInSharedElements(
                    impl, nameOverrides, finalSharedElementTransition, fragments);
            ...
            if (finalSharedElementTransition != null) {
              impl.swapSharedElementTargets(
                  finalSharedElementTransition, sharedElementsOut, sharedElementsIn);

              final View inEpicenterView =
                  getInEpicenterView(inSharedElements, fragments, enterTransition, inIsPop);
  ...
  /**
   * Finds the shared elements in the outgoing fragment. It also calls {@link
   * SharedElementCallback#onMapSharedElements(List, Map)} to allow more control of the shared
   * element mapping. {@code nameOverrides} is updated to match the actual transition name of the
   * mapped shared elements.
   *
   * @param nameOverrides A map of the shared element names from the starting fragment to the final
   *     fragment's Views as given in {@link FragmentTransaction#addSharedElement(View, String)}.
   * @param sharedElementTransition The shared element transition
   * @param fragments A structure holding the transitioning fragments in this container.
   * @return The mapping of shared element names to the Views in the hierarchy or null if there is
  ...
  /**
   * Finds the shared elements in the incoming fragment. It also calls {@link
   * SharedElementCallback#onMapSharedElements(List, Map)} to allow more control of the shared
   * element mapping. {@code nameOverrides} is updated to match the actual transition name of the
   * mapped shared elements.
   *
  ...
  static ArrayMap<String, View> captureInSharedElements(
      FragmentTransitionImpl impl,
      ArrayMap<String, String> nameOverrides,
      Object sharedElementTransition,
      FragmentContainerTransition fragments) {
   ...
   *
   * @param inSharedElements The mapping of shared element names to Views in the incoming fragment.
   * @param fragments A structure holding the transitioning fragments in this container.
   ...
   */
  @SuppressWarnings("WeakerAccess") /* synthetic access */
  static View getInEpicenterView(
      ArrayMap<String, View> inSharedElements,
      FragmentContainerTransition fragments,
   ...
   */
  private static void setOutEpicenter(
      FragmentTransitionImpl impl,
      Object sharedElementTransition,
      Object exitTransition,
      ArrayMap<String, View> outSharedElements,
  ...
  /**
   * A utility to retain only the mappings in {@code nameOverrides} that have a value that has a key
   * in {@code namedViews}. This is a useful equivalent to {@link ArrayMap#retainAll(Collection)}
   * for values.
   */
  private static void retainValues(
      ArrayMap<String, String> nameOverrides, ArrayMap<String, View> namedViews) {
  ...
  /**
   * Calls the {@link SharedElementCallback#onSharedElementStart(List, List, List)} or {@link
   * SharedElementCallback#onSharedElementEnd(List, List, List)} on the appropriate incoming or
   * outgoing fragment.
   *
   * @param inFragment The incoming fragment
   * @param outFragment The outgoing fragment
   ...
   */
  @SuppressWarnings("WeakerAccess") /* synthetic access */
  static void callSharedElementStartEnd(
      Fragment inFragment,
    ...
    SharedElementCallback sharedElementCallback =
        isPop ? outFragment.getEnterTransitionCallback() : inFragment.getEnterTransitionCallback();
    if (sharedElementCallback != null) {
  ...
  @SuppressWarnings("WeakerAccess") /* synthetic access */
  static ArrayList<View> configureEnteringExitingViews(
      FragmentTransitionImpl impl,
      Object transition,
      Fragment fragment,
  ...
  /**
   * Merges exit, shared element, and enter transitions so that they act together or sequentially as
   * defined in the fragments.
   */
  private static Object mergeTransitions(
      FragmentTransitionImpl impl,
      Object enterTransition,
      Object exitTransition,
      Object sharedElementTransition,
    ...
    if (enterTransition != null && exitTransition != null && inFragment != null) {
      overlap =
          isPop
              ? inFragment.getAllowReturnTransitionOverlap()
    ...
    if (overlap) {
      ...
      transition =
          impl.mergeTransitionsTogether(exitTransition, enterTransition, sharedElementTransition);
    } else {
      // First do exit, then enter, but allow shared element transition to happen
      // during both.
      transition =
          impl.mergeTransitionsInSequence(exitTransition, enterTransition, sharedElementTransition);
    }
    return transition;
  }

  /**
   * Finds the first removed fragment and last added fragments when going forward. If none of the
   * fragments have transitions, then both lists will be empty.
   *
   * @param transitioningFragments Keyed on the container ID, the first fragments to be removed, and
   *     last fragments to be added. This will be modified by this method.
   */
  public static void calculateFragments(
      BackStackRecord transaction,
  ...
  /**
   * Finds the first removed fragment and last added fragments when popping the back stack. If none
   * of the fragments have transitions, then both lists will be empty.
   *
   * @param transitioningFragments Keyed on the container ID, the first fragments to be removed, and
   *     last fragments to be added. This will be modified by this method.
   */
  public static void calculatePopFragments(
      BackStackRecord transaction,
      SparseArray<FragmentContainerTransition> transitioningFragments,
      boolean isReordered) {
   ...
   * @param transitioningFragments A structure holding the first in and last out fragments for each
   *     fragment container.
   * @param isPop Is the operation a pop?
   * @param isReorderedTransaction True if the operations have been partially executed and the added
   *     fragments have Views in the hierarchy or false if the operations haven't been executed yet.
   */
  @SuppressWarnings("ReferenceEquality")
  private static void addToFirstInLastOut(
      BackStackRecord transaction,
    ...
    if (!isReorderedTransaction && wasAdded) {
      ...
      if (fragment.mState < Fragment.CREATED
          && manager.mCurState >= Fragment.CREATED
  ...
  /**
   * Ensures that a FragmentContainerTransition has been added to the SparseArray. If so, it returns
   * the existing one. If not, one is created and added to the SparseArray and returned.
   */
  ...
  /**
   * Tracks the last fragment added and first fragment removed for fragment transitions. This also
   * tracks which fragments are changed by push or pop transactions.
   */
