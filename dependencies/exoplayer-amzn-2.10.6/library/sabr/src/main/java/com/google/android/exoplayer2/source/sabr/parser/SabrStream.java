...
public class SabrStream {
  ...
  public SabrPart parse(@NonNull ExtractorInput extractorInput) {
    ...
    return result != null
        ? result
        : multiResult != null && !multiResult.isEmpty() ? multiResult.remove(0) : null;
  }
  ...
  private MediaSegmentInitSabrPart processMediaHeader(UMPPart part) {
    ...
    try {
      ...
      // For segments near stream head, it estimates using segment duration, which can cause
      // off-by-one segment mismatches.
      // If a segment is much longer or shorter than expected, the server may return a segment ahead
      // or behind.
      ...
      if (processor.isLive() && e.receivedSequenceNumber == e.expectedSequenceNumber - 1) {
        ...
        ClientAbrState state =
            processor.getClientAbrState().toBuilder()
                .setPlayerTimeMs(
                    processor.getClientAbrState().getPlayerTimeMs()
  ...
  private FormatInitializedSabrPart processFormatInitializationMetadata(UMPPart part) {
    ...
    ProcessFormatInitializationMetadataResult result =
        processor.processFormatInitializationMetadata(fmtInitMetadata);
  ...
  private RefreshPlayerResponseSabrPart processReloadPlayerResponse(UMPPart part) {
    ...
    return new RefreshPlayerResponseSabrPart(
        ...
        reloadPlayerResponse.hasReloadPlaybackParams()
            ...
            ? reloadPlayerResponse.getReloadPlaybackParams().getToken()
            : null);
  }
  ...
  private void setUrl(String url) {
    ...
    if (processor.isLive() && this.url != null && !Helpers.equals(bn, bc)) {
      throw new SabrStreamError(
          String.format(
              "Broadcast ID changed from %s to %s. The download will need to be restarted.",
              bc, bn));
    }
    this.url = url;
