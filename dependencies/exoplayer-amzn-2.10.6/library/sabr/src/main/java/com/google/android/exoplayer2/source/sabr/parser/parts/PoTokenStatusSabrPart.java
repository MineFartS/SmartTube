...
public class PoTokenStatusSabrPart implements SabrPart {
  ...
  public enum PoTokenStatus {
    ...
    PENDING, // PO Token is provided, but probably only a cold start token. A full PO Token should
             // be provided ASAP
    NOT_REQUIRED, // PO Token is not provided, and is not required
    PENDING_MISSING // PO Token is not provided, but is pending. A full PO Token should be
                    // (probably) provided ASAP
  }
}
