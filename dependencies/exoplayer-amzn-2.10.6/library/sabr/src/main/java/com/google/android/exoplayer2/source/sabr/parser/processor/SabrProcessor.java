...
public class SabrProcessor {
  ...
  public SabrProcessor(
    ...
    this.liveSegmentTargetDurationSec =
        liveSegmentTargetDurationSec != NO_VALUE ? liveSegmentTargetDurationSec : 5;
    this.liveSegmentTargetDurationToleranceMs =
        liveSegmentTargetDurationToleranceMs != NO_VALUE
            ? liveSegmentTargetDurationToleranceMs
            : 100;
    if (this.liveSegmentTargetDurationToleranceMs
        >= (this.liveSegmentTargetDurationSec * 1_000) / 2) {
      throw new IllegalArgumentException(
          "liveSegmentTargetDurationToleranceMs must be less than half of"
              + " liveSegmentTargetDurationSec in milliseconds");
    }
  ...
  public ProcessMediaHeaderResult processMediaHeader(MediaHeader mediaHeader) {
    if (mediaHeader.hasVideoId()
        && videoId != null
        && !Helpers.equals(mediaHeader.getVideoId(), videoId)) {
      throw new SabrStreamError(
          String.format(
              "Received unexpected MediaHeader for video %s (expecting %s)",
              mediaHeader.getVideoId(), videoId));
    }

    if (!mediaHeader.hasFormatId()) {
      throw new SabrStreamError(
          String.format("FormatId not found in MediaHeader (media_header=%s)", mediaHeader));
    }
    ...
    if (partialSegments.containsKey(Utils.toLong(mediaHeader.getHeaderId()))) {
      throw new SabrStreamError(
          String.format("Header ID %s already exists", mediaHeader.getHeaderId()));
    }

    InitializedFormat initializedFormat =
        initializedFormats.get(mediaHeader.getFormatId().toString());

    if (initializedFormat == null) {
      throw new SabrStreamError(
          String.format("Initialized format not found for %s", mediaHeader.getFormatId()));
    }

    if (mediaHeader.hasCompression()) {
      // Unknown when this is used, but it is not supported currently
      throw new SabrStreamError(
          String.format("Compression not supported in MediaHeader (media_header=%s)", mediaHeader));
    }

    int sequenceNumber =
        mediaHeader.hasSequenceNumber() ? mediaHeader.getSequenceNumber() : NO_VALUE;
    boolean isInitSegment = mediaHeader.getIsInitSegment();

    if (sequenceNumber == NO_VALUE && !isInitSegment) {
      throw new SabrStreamError(
          String.format("Sequence number not found in MediaHeader (media_header=%s)", mediaHeader));
    }

    initializedFormat.sequenceLmt =
    ...
    if (!isInitSegment
        && Helpers.findFirst(
                initializedFormat.consumedRanges,
                cr ->
                    cr.startSequenceNumber <= sequenceNumber
                        && sequenceNumber <= cr.endSequenceNumber)
            == null) {
      Log.d(
          TAG,
          "%s segment %s already consumed, marking segment as consumed",
    ...
    if (previousSegment != null
        && !isInitSegment
        && !previousSegment.discard
        && !discard
        && !consumed
        && sequenceNumber != previousSegment.sequenceNumber + 1) {
      // Bail out as the segment is not in order when it is expected to be
      throw new MediaSegmentMismatchError(
          mediaHeader.getFormatId(), previousSegment.sequenceNumber + 1, sequenceNumber);
    }

    if (initializedFormat.initSegment != null && isInitSegment) {
      Log.d(
          TAG,
          "Init segment %s already seen for format %s, marking segment as consumed",
          sequenceNumber,
          initializedFormat.formatId);
      consumed = true;
    }
    ...
    int startMs =
        mediaHeader.hasStartMs()
            ? mediaHeader.getStartMs()
    ...
    int actualDurationMs =
        mediaHeader.hasDurationMs()
            ? mediaHeader.getDurationMs()
    ...
    if (isLive()) {
      ...
      estimatedDurationMs =
          (getLiveSegmentTargetDurationSec() * 1_000) - getLiveSegmentTargetDurationToleranceMs();
    ...
    if (durationMs == NO_VALUE) {
      throw new SabrStreamError(
          String.format(
              "Cannot determine duration of segment %s (media_header=%s)",
              sequenceNumber, mediaHeader));
    }
    ...
    if (isLive() && !mediaHeader.hasContentLength() && mediaHeader.hasBitrateBps()) {
      estimatedContentLength =
          (long) Math.ceil(mediaHeader.getBitrateBps() * ((double) durationMs / 1_000));
    }
    ...
    Segment segment =
        new Segment(
            ...
            sequenceNumber,
            mediaHeader.hasContentLength()
                ? mediaHeader.getContentLength()
                : estimatedContentLength,
    ...
    Log.d(
        TAG,
        "Initialized Media Header %s for sequence %s. Segment: %s",
        Utils.toLong(mediaHeader.getHeaderId()),
        sequenceNumber,
        segment);
  ...
  public ProcessMediaResult processMedia(long headerId, int contentLength, ExtractorInput data) {
    ...
    if (segment == null) {
      ...
      throw new SabrStreamError(
          String.format("Header ID %s not found in partial segments", headerId));
    }
  ...
  public ProcessMediaEndResult processMediaEnd(long headerId) {
    ...
    Log.d(
        TAG,
        "MediaEnd for %s (sequence %s, data length = %s)",
        segment.formatId,
        segment.sequenceNumber,
        segment.receivedDataLength);

    if (segment.contentLength != -1 && segment.receivedDataLength != segment.contentLength) {
      if (segment.contentLengthEstimated) {
        Log.d(
            TAG,
            "Content length for %s (sequence %s) was estimated, "
        ...
        throw new SabrStreamError(
            String.format(
                "Content length mismatch for %s (sequence %s): "
    ...
    if (!segment.discard) {
      ...
      // So the consumer can see the updated consumed ranges and use them for e.g. syncing between
      // concurrent streams
      result.sabrPart =
          new MediaSegmentEndSabrPart(
              segment.initializedFormat.formatSelector,
              segment.formatId,
              segment.sequenceNumber,
              segment.isInitSegment,
    ...
    if (segment.initializedFormat.currentSegment != null && isLive()) {
      ...
      Log.d(
          TAG,
          "Previous segment %s for format %s "
              + "estimated duration difference from this segment (%s): %sms",
          previousSegment.sequenceNumber,
          segment.formatId,
          segment.sequenceNumber,
    ...
    if (segment.consumed) {
      ...
      // This can be expected to happen in the case of video-only, where we discard the audio track
      // (and mark it as entirely buffered)
      // We still want to create/update consumed range for discarded media IF it is not already
      // consumed
      Log.d(
          TAG,
          "%s} segment %s already consumed, not creating or updating consumed range (discard=%s)",
          segment.formatId,
          segment.sequenceNumber,
          segment.discard);
      return result;
    ...
    ConsumedRange consumedRange =
        Helpers.findFirst(
            segment.initializedFormat.consumedRanges,
            cr -> cr.endSequenceNumber == segment.sequenceNumber - 1);

    if (consumedRange == null) {
      // Create a new consumed range starting from this segment
      segment.initializedFormat.consumedRanges.add(
          new ConsumedRange(
              segment.startMs, segment.durationMs, segment.sequenceNumber, segment.sequenceNumber));
      Log.d(
          TAG,
          "Created new consumed range for %s %s",
          segment.initializedFormat.formatId,
          segment.initializedFormat.consumedRanges.get(
              segment.initializedFormat.consumedRanges.size() - 1));
      return result;
    }
  ...
  public ProcessStreamProtectionStatusResult processStreamProtectionStatus(
      StreamProtectionStatus streamProtectionStatus) {
    this.streamProtectionStatus =
        streamProtectionStatus.hasStatus() ? streamProtectionStatus.getStatus() : null;
    ...
    if (status == StreamProtectionStatus.Status.OK) {
      resultStatus =
          poToken != null
              ? PoTokenStatusSabrPart.PoTokenStatus.OK
              : PoTokenStatusSabrPart.PoTokenStatus.NOT_REQUIRED;
    } else if (status == StreamProtectionStatus.Status.ATTESTATION_PENDING) {
      resultStatus =
          poToken != null
              ? PoTokenStatusSabrPart.PoTokenStatus.PENDING
              : PoTokenStatusSabrPart.PoTokenStatus.PENDING_MISSING;
    } else if (status == StreamProtectionStatus.Status.ATTESTATION_REQUIRED) {
      resultStatus =
          poToken != null
              ? PoTokenStatusSabrPart.PoTokenStatus.INVALID
              : PoTokenStatusSabrPart.PoTokenStatus.MISSING;
    } else {
  ...
  public ProcessFormatInitializationMetadataResult processFormatInitializationMetadata(
      FormatInitializationMetadata formatInitMetadata) {
    ProcessFormatInitializationMetadataResult result =
        new ProcessFormatInitializationMetadataResult();

    if (formatInitMetadata.hasFormatId()
        && initializedFormats.containsKey(formatInitMetadata.getFormatId().toString())) {
      Log.d(TAG, "Format %s already initialized", formatInitMetadata.getFormatId());
      return result;
    }

    if (formatInitMetadata.hasVideoId()
        && videoId != null
        && !formatInitMetadata.getVideoId().equals(videoId)) {
      throw new SabrStreamError(
          String.format(
              "Received unexpected Format Initialization Metadata for video"
                  + "  %s (expecting %s)",
              formatInitMetadata.getVideoId(), videoId));
    }
    ...
    if (formatSelector == null) {
      ...
      throw new SabrStreamError(
          String.format(
              "Received format %s but it does not match any format selector",
              formatInitMetadata.getFormatId()));
    }
    ...
    for (InitializedFormat izf : initializedFormats.values()) {
      if (izf.formatSelector == formatSelector) {
        throw new SabrStreamError(
            "Server changed format. Changing formats is not currently supported");
      }
    }
    ...
    int durationMs =
        Utils.ticksToMs(
            ...
            formatInitMetadata.hasDurationTimescale()
                ? formatInitMetadata.getDurationTimescale()
                : -1);

    int totalSegments =
        formatInitMetadata.hasTotalSegments() ? formatInitMetadata.getTotalSegments() : -1;
    ...
    totalDurationMs =
        Math.max(
            ...
            Math.max(
                formatInitMetadata.hasEndTimeMs() ? formatInitMetadata.getEndTimeMs() : 0,
    ...
    if (initializedFormat.discard) {
      ...
      // Note: Using JS_MAX_SAFE_INTEGER but could use any maximum value as long as the server
      // accepts it.
      initializedFormat.consumedRanges.clear();
      initializedFormat.consumedRanges.add(
          new ConsumedRange(
              0,
  ...
  public ProcessLiveMetadataResult processLiveMetadata(LiveMetadata liveMetadata) {
    ...
    // If we have a head sequence number, we need to update the total sequences for each initialized
    // format
    ...
    // If the current player time is less than the min dvr time, simulate a server seek to the min
    // dvr time.
    // The server SHOULD send us a SABR_SEEK part in this case, but it does not always happen (e.g.
    // ANDROID_VR)
    // The server SHOULD NOT send us segments before the min dvr time, so we should assume that the
    // player time is correct.
    int minSeekableTimeMs =
        Utils.ticksToMs(
    ...
    if (minSeekableTimeMs != -1
        && clientAbrState.hasPlayerTimeMs()
        && clientAbrState.getPlayerTimeMs() < minSeekableTimeMs) {
      Log.d(
          TAG,
          "Player time %s is less than min seekable time %s, simulating server seek",
          clientAbrState.getPlayerTimeMs(),
          minSeekableTimeMs);
      clientAbrState = clientAbrState.toBuilder().setPlayerTimeMs(minSeekableTimeMs).build();
      for (InitializedFormat izf : initializedFormats.values()) {
        izf.currentSegment =
            null; // Clear the current segment as we expect segments to no longer be in order.
        result.seekSabrParts.add(
            new MediaSeekSabrPart(
                MediaSeekSabrPart.Reason.SERVER_SEEK, izf.formatId, izf.formatSelector));
      }
    }
  ...
  public ProcessSabrSeekResult processSabrSeek(SabrSeek sabrSeek) {
    int seekTo =
        Utils.ticksToMs(
            sabrSeek.hasSeekTimeTicks() ? sabrSeek.getSeekTimeTicks() : -1,
            sabrSeek.hasTimescale() ? sabrSeek.getTimescale() : -1);
    if (seekTo == -1) {
      throw new SabrStreamError(
          String.format(
              "Server sent a SabrSeek part that is missing required seek data: %s", sabrSeek));
    }
  ...
  public void processSabrContextUpdate(SabrContextUpdate sabrCtxUpdate) {
    ...
    if (sabrCtxUpdate.getWritePolicy()
  ...
  public VideoPlaybackAbrRequest buildVideoPlaybackAbrRequest() {
    return VideoPlaybackAbrRequest.newBuilder()
        ...
        .setVideoPlaybackUstreamerConfig(
            ByteString.copyFrom(Base64.decode(videoPlaybackUstreamerConfig, Base64.URL_SAFE)))
  ...
  private StreamerContext createStreamerContext() {
    return StreamerContext.newBuilder()
        .setPoToken(ByteString.copyFrom(Base64.decode(poToken, Base64.URL_SAFE)))
        .setPlaybackCookie(
  ...
  private FormatSelector matchFormatSelector(FormatInitializationMetadata formatInitMetadata) {
    for (FormatSelector formatSelector :
        new FormatSelector[] {videoFormatSelector, audioFormatSelector, captionFormatSelector}) {
      if (formatSelector == null) {
        continue;
      }

      if (formatSelector.match(
          formatInitMetadata.getFormatId(), formatInitMetadata.getMimeType())) {
