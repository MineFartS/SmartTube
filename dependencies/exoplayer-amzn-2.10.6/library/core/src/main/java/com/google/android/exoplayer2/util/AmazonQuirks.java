...
public final class AmazonQuirks {
  ...
  /**
   * Updates log level based on a local system property of the device. This can be very useful to
   * enable logging in scenarios when a 3P developer has issues we need to assist Example: adb shell
   * setprop com.amazon.exoplayer.forcelog Video:verbose#Audio:debug
   */
  ...
  /**
   * Called to set quirk which determines if codec profile checks should be skipped. Call this api
   * with true to make the player skip checking the support for profile levels of the content. Only
   * use this if you are sure that the target device is capable playing the content, but under
