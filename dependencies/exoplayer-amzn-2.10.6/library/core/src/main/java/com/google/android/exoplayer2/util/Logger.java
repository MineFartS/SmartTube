...
/**
 * A common logger module that allows any module to log. Supports log level configuration for each
 * module. call {@link Logger#setLogLevel} API to configure the modules and log levels from the app
 */
public class Logger {
  public enum Module {
    Unknown,
   ...
   *
   * @param module The module to which this file belongs to. One of the MOD_xxx values Setting All
   *     enables logging in all modules. Setting AudioVideo enables logging in both Audio and Video
   *     Setting Audio or Video enables logging in AudioVideoCommon
   * @param logLevel Log level for this module. One of the constants in android.util.Log. i.e
   *     Log.ERROR, Log.WARNING, Log.INFO, Log.DEBUG and Log.VERBOSE Info , error and warning logs
   *     are always printed. Setting to Log.INFO, Log.ERROR, Log.WARNING etc disables DEBUG and
   *     VERBOSE logs. Setting to Log.VERBOSE prints Debug and Verbose logs. Setting to Log.DEBUG
   *     prints Debug logs (excludes Verbose)
   */
  ...
  /**
   * Function to print verbose level logs. Prints only if the log level is Log.VERBOSE
   *
  ...
  /**
   * Function to print debug level logs. Prints only if the log level is Log.DEBUG or Log.VERBOSE
   *
   * @param msg The log message
   */
