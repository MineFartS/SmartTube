...
public interface ExoMediaDrm<T extends ExoMediaCrypto> {
    ...
    interface OnEventListener<T extends ExoMediaCrypto> {
         ...
         */
        void onEvent(ExoMediaDrm<? extends T> mediaDrm, @Nullable byte[] sessionId, int event,
                int extra, @Nullable byte[] data);
    }
    ...
    interface OnKeyStatusChangeListener<T extends ExoMediaCrypto> {
        ...
        void onKeyStatusChange(ExoMediaDrm<? extends T> mediaDrm, byte[] sessionId,
                List<KeyStatus> exoKeyInformation, boolean hasNewUsableKey);
    }
     ...
     *        the session id that the keys will be provided to. If {@code keyType} is
     *        {@link #KEY_TYPE_RELEASE}, the keySetId of the keys to release.
     ...
     * @param keyType The type of the request. Either {@link #KEY_TYPE_STREAMING} to acquire keys
     *        for streaming, {@link #KEY_TYPE_OFFLINE} to acquire keys for offline usage, or
     *        {@link #KEY_TYPE_RELEASE} to release acquired keys. Releasing keys invalidates them
     *        for all sessions.
     * @param optionalParameters Are included in the key request message to allow a client
     *        application to provide additional message parameters to the server. This may be
     *        {@code null} if no additional parameters are to be sent.
     * @return The generated key request.
     * @see MediaDrm#getKeyRequest(byte[], byte[], String, int, HashMap)
     */
    KeyRequest getKeyRequest(byte[] scope, @Nullable List<SchemeData> schemeDatas, int keyType,
