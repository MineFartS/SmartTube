...
public interface RendererCapabilities {
  ...
  /**
   * A mask to apply to the result of {@link #supportsFormat(Format)} to obtain one of {@link
   * #FORMAT_HANDLED}, {@link #FORMAT_EXCEEDS_CAPABILITIES}, {@link #FORMAT_UNSUPPORTED_DRM}, {@link
   * #FORMAT_UNSUPPORTED_SUBTYPE} and {@link #FORMAT_UNSUPPORTED_TYPE}.
   */
   ...
   *
   * <p>Example: The {@link Renderer} is capable of rendering H264 and the format's mime type is
  ...
  /**
   * The {@link Renderer} is a general purpose renderer for formats of the same top-level type, but
   * is not capable of rendering the format or any other format with the same mime type because the
   * sub-type is not supported.
   *
   * <p>Example: The {@link Renderer} is a general purpose audio renderer and the format's mime type
   * matches audio/[subtype], but there does not exist a suitable decoder for [subtype].
   */
  int FORMAT_UNSUPPORTED_SUBTYPE = 0b001;

  /**
   * The {@link Renderer} is not capable of rendering the format, either because it does not support
   * the format's top-level type, or because it's a specialized renderer for a different mime type.
   *
   * <p>Example: The {@link Renderer} is a general purpose video renderer, but the format has an
   * audio mime type.
   */
  ...
  /**
   * A mask to apply to the result of {@link #supportsFormat(Format)} to obtain one of {@link
   * #ADAPTIVE_SEAMLESS}, {@link #ADAPTIVE_NOT_SEAMLESS} and {@link #ADAPTIVE_NOT_SUPPORTED}.
   */
  ...
  /**
   * A mask to apply to the result of {@link #supportsFormat(Format)} to obtain one of {@link
   * #TUNNELING_SUPPORTED} and {@link #TUNNELING_NOT_SUPPORTED}.
   */
   ...
   *
   * <ul>
   *   <li>The level of support for the format itself. One of {@link #FORMAT_HANDLED}, {@link
   *       #FORMAT_EXCEEDS_CAPABILITIES}, {@link #FORMAT_UNSUPPORTED_DRM}, {@link
   *       #FORMAT_UNSUPPORTED_SUBTYPE} and {@link #FORMAT_UNSUPPORTED_TYPE}.
   *   <li>The level of support for adapting from the format to another format of the same mime
   *       type. One of {@link #ADAPTIVE_SEAMLESS}, {@link #ADAPTIVE_NOT_SEAMLESS} and {@link
   ...
   *   <li>The level of support for tunneling. One of {@link #TUNNELING_SUPPORTED} and {@link
   *       #TUNNELING_NOT_SUPPORTED}. Only set if the level of support for the format itself is
   *       {@link #FORMAT_HANDLED} or {@link #FORMAT_EXCEEDS_CAPABILITIES}.
   * </ul>
   *
   * The individual properties can be retrieved by performing a bitwise AND with {@link
   * #FORMAT_SUPPORT_MASK}, {@link #ADAPTIVE_SUPPORT_MASK} and {@link #TUNNELING_SUPPORT_MASK}
   * respectively.
   *
