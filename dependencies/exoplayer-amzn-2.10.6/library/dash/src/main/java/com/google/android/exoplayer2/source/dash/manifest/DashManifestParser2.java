...
public class DashManifestParser2 {
  ...
  private DashManifest parseDashManifest(MediaItemFormatInfo formatInfo) {
    ...
    long minUpdateTimeMs =
        C.TIME_UNSET; // 3155690800000L, "P100Y" no refresh (there is no dash url)
  ...
  private AdaptationSet parseAdaptationSet(Set<MediaFormat> formats) {
    ...
    for (int i = 0; i < representationInfos.size(); i++) {
      representations.add(
          buildRepresentation(representationInfos.get(i), label, drmSchemeType, drmSchemeDatas));
    }

    return new AdaptationSet(
        id, contentType, representations, new ArrayList<>(), new ArrayList<>());
  }
  ...
  private SegmentTemplate parseSegmentTemplate(MediaFormat format) {
    ...
    List<SegmentTimelineElement> timeline =
        parseSegmentTimeline(offsetUnits, segmentDurationUnits, segmentCount);
  ...
  private List<SegmentTimelineElement> parseSegmentTimeline(
      long elapsedTime, long duration, int segmentCount) {
    List<SegmentTimelineElement> timeline = new ArrayList<>();
  ...
  private RepresentationInfo parseRepresentation(MediaFormat mediaFormat) {
    ...
    int audioSamplingRate =
        Helpers.parseInt(ITagUtils.getAudioRateByTag(mediaFormat.getITag()), Format.NO_VALUE);
    ...
    if (MediaFormatUtils.isLiveMedia(mediaFormat)) {
      ...
      segmentBase = parseSegmentList(mediaFormat);
        ...
        && !mediaFormat
            .getIndex()
            .equals(NULL_INDEX_RANGE)) { // json mediaFormat fix: index is null
