package minefarts.exoplayer2.source.ads;
...
import minefarts.exoplayer2.source.CompositeMediaSource;
import minefarts.exoplayer2.source.DeferredMediaPeriod;
...
import minefarts.exoplayer2.source.MediaSourceEventListener.LoadEventInfo;
import minefarts.exoplayer2.source.MediaSourceEventListener.MediaLoadData;
import minefarts.exoplayer2.source.ProgressiveMediaSource;
...
public final class AdsMediaSource extends CompositeMediaSource<MediaPeriodId> {
    ...
    public interface MediaSourceFactory {
        ...
        /**
         * Creates a new {@link MediaSource} for loading the ad media with the specified
         * {@code uri}.
         *
        ...
        /**
         * Returns the content types supported by media sources created by this factory. Each
         * element should be one of {@link C#TYPE_DASH}, {@link C#TYPE_SS}, {@link C#TYPE_HLS} or
         * {@link C#TYPE_OTHER}.
         *
     ...
     * {@link MediaSourceEventListener#onLoadError(int, MediaPeriodId, LoadEventInfo, MediaLoadData, IOException, boolean)}.
     */
    public static final class AdLoadException extends IOException {

        /**
         * Types of ad load exceptions. One of {@link #TYPE_AD}, {@link #TYPE_AD_GROUP},
         * {@link #TYPE_ALL_ADS} or {@link #TYPE_UNEXPECTED}.
         */
        @Documented
        ...
        public static AdLoadException createForAdGroup(Exception error, int adGroupIndex) {
            return new AdLoadException(TYPE_AD_GROUP,
                    new IOException("Failed to load ad group " + adGroupIndex, error));
        }
        ...
        /**
         * Returns the {@link RuntimeException} that caused the exception if its type is
    ...
    public AdsMediaSource(MediaSource contentMediaSource, DataSource.Factory dataSourceFactory,
            AdsLoader adsLoader, AdsLoader.AdViewProvider adViewProvider) {
        this(contentMediaSource, new ProgressiveMediaSource.Factory(dataSourceFactory), adsLoader,
                adViewProvider);
    }
     ...
     * @param adViewProvider Provider of views for the ad UI.
     */
    public AdsMediaSource(MediaSource contentMediaSource, MediaSourceFactory adMediaSourceFactory,
            AdsLoader adsLoader, AdsLoader.AdViewProvider adViewProvider) {
        ...
        if (adPlaybackState.adGroupCount > 0 && id.isAd()) {
            ...
            if (adGroupMediaSources[adGroupIndex].length <= adIndexInAdGroup) {
                ...
                if (adIndexInAdGroup >= oldAdCount) {
                    ...
                    adGroupTimelines[adGroupIndex] =
                            Arrays.copyOf(adGroupTimelines[adGroupIndex], adCount);
                }
            ...
            List<DeferredMediaPeriod> mediaPeriods =
                    deferredMediaPeriodByAdMediaSource.get(mediaSource);
            if (mediaPeriods == null) {
                Object periodUid = adGroupTimelines[adGroupIndex][adIndexInAdGroup]
                        .getUidOfPeriod(/* periodIndex= */ 0);
                MediaPeriodId adSourceMediaPeriodId =
                        new MediaPeriodId(periodUid, id.windowSequenceNumber);
                deferredMediaPeriod.createPeriod(adSourceMediaPeriodId);
            } else {
                // Keep track of the deferred media period so it can be populated with the real
                // media period
    ...
    @Override
    protected void onChildSourceInfoRefreshed(MediaPeriodId mediaPeriodId, MediaSource mediaSource,
            Timeline timeline, @Nullable Object manifest) {
    ...
    @Override
    protected @Nullable MediaPeriodId getMediaPeriodIdForChildMediaPeriodId(MediaPeriodId childId,
            MediaPeriodId mediaPeriodId) {
        // The child id for the content period is just DUMMY_CONTENT_MEDIA_PERIOD_ID. That's why we
        // need
    ...
    private void maybeUpdateSourceInfo() {
        if (adPlaybackState != null && contentTimeline != null) {
            adPlaybackState =
                    adPlaybackState.withAdDurationsUs(getAdDurations(adGroupTimelines, period));
    ...
    private static long[][] getAdDurations(Timeline[][] adTimelines, Timeline.Period period) {
        ...
        for (int i = 0; i < adTimelines.length; i++) {
            ...
            for (int j = 0; j < adTimelines[i].length; j++) {
                adDurations[i][j] = adTimelines[i][j] == null ? C.TIME_UNSET
    ...
    private final class ComponentListener implements AdsLoader.EventListener {
        ...
        /**
         * Creates new listener which forwards ad playback states on the creating thread and all
         * other events on the external event listener thread.
         */
        ...
        public void onAdLoadError(final AdLoadException error, DataSpec dataSpec) {
            ...
            createEventDispatcher(/* mediaPeriodId= */ null).loadError(dataSpec, dataSpec.uri,
                    /* responseHeaders= */ Collections.emptyMap(), C.DATA_TYPE_AD,
                    C.TRACK_TYPE_UNKNOWN, /* loadDurationMs= */ 0, /* bytesLoaded= */ 0, error,
                    /* wasCanceled= */ true);
        }
    }
    ...
    private final class AdPrepareErrorListener implements DeferredMediaPeriod.PrepareErrorListener {
        ...
        public void onPrepareError(MediaPeriodId mediaPeriodId, final IOException exception) {
            createEventDispatcher(mediaPeriodId).loadError(new DataSpec(adUri), adUri,
                    /* responseHeaders= */ Collections.emptyMap(), C.DATA_TYPE_AD,
                    C.TRACK_TYPE_UNKNOWN, /* loadDurationMs= */ 0, /* bytesLoaded= */ 0,
                    AdLoadException.createForAd(exception), /* wasCanceled= */ true);
            mainHandler.post(
