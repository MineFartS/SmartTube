...
import static minefarts.exoplayer2.offline.Download.STOP_REASON_NONE;
...
import minefarts.exoplayer2.scheduler.Requirements;
import minefarts.exoplayer2.scheduler.Scheduler;
import minefarts.exoplayer2.util.Assertions;
import minefarts.exoplayer2.util.Log;
import minefarts.exoplayer2.util.NotificationUtil;
...
public abstract class DownloadService extends Service {
     ...
     * added.
     * <li>{@link #KEY_STOP_REASON} - An initial stop reason for the download. If omitted
     * {@link Download#STOP_REASON_NONE} is used.
     * <li>{@link #KEY_FOREGROUND} - See {@link #KEY_FOREGROUND}.
     * </ul>
     */
    ...
    /**
     * Sets the stop reason for one or all downloads. To clear the stop reason, pass
     * {@link Download#STOP_REASON_NONE}. Extras:
     *
     * <ul>
    ...
    /**
     * Key for the {@link String} content id in {@link #ACTION_SET_STOP_REASON} and
     * {@link #ACTION_REMOVE_DOWNLOAD} intents.
     */
    public static final String KEY_CONTENT_ID = "content_id";

    /**
     * Key for the integer stop reason in {@link #ACTION_SET_STOP_REASON} and
     * {@link #ACTION_ADD_DOWNLOAD} intents.
     */
     ...
     * started in the foreground. If set, the service is guaranteed to call
    ...
    // Keep DownloadManagerListeners for each DownloadService as long as there are downloads (and
    // the
     ...
     *
     * <p>
     * If {@code foregroundNotificationId} is {@link #FOREGROUND_NOTIFICATION_ID_NONE} then the
     * service will only ever run in the background. No foreground notification will be displayed
     * and {@link #getScheduler()} will not be called.
     *
     * <p>
     * If {@code foregroundNotificationId} is not {@link #FOREGROUND_NOTIFICATION_ID_NONE} then the
     ...
     *
     * @param foregroundNotificationId The notification id for the foreground notification, or
     *        {@link #FOREGROUND_NOTIFICATION_ID_NONE} if the service should only ever run in the
     *        background.
     */
     ...
     *        foreground notification, in milliseconds. Ignored if {@code foregroundNotificationId}
     *        is {@link #FOREGROUND_NOTIFICATION_ID_NONE}.
     */
    protected DownloadService(int foregroundNotificationId,
            long foregroundNotificationUpdateInterval) {
        this(foregroundNotificationId, foregroundNotificationUpdateInterval, /* channelId= */ null,
                /* channelNameResourceId= */ 0, /* channelDescriptionResourceId= */ 0);
    }

    /** @deprecated Use {@link #DownloadService(int, long, String, int, int)}. */
    @Deprecated
    protected DownloadService(int foregroundNotificationId,
            long foregroundNotificationUpdateInterval, @Nullable String channelId,
            @StringRes int channelNameResourceId) {
        this(foregroundNotificationId, foregroundNotificationUpdateInterval, channelId,
                channelNameResourceId, /* channelDescriptionResourceId= */ 0);
    }
     ...
     *        the app will take care of creating a notification channel if needed. If specified,
     *        must be unique per package. The value may be truncated if it's too long. Ignored if
     *        {@code
     ...
     *        notification channel. The recommended maximum length is 40 characters. The value may
     *        be truncated if it's too long. Ignored if {@code channelId} is null or if {@code
     *     foregroundNotificationId} is {@link #FOREGROUND_NOTIFICATION_ID_NONE}.
     ...
     *        recommended maximum length is 300 characters. The value may be truncated if it is too
     *        long. Ignored if {@code channelId} is null or if {@code foregroundNotificationId} is
     *        {@link #FOREGROUND_NOTIFICATION_ID_NONE}.
     */
    protected DownloadService(int foregroundNotificationId,
            long foregroundNotificationUpdateInterval, @Nullable String channelId,
            @StringRes int channelNameResourceId, @StringRes int channelDescriptionResourceId) {
        if (foregroundNotificationId == FOREGROUND_NOTIFICATION_ID_NONE) {
            ...
            this.foregroundNotificationUpdater = new ForegroundNotificationUpdater(
    ...
    public static Intent buildAddDownloadIntent(Context context,
            Class<? extends DownloadService> clazz, DownloadRequest downloadRequest,
            boolean foreground) {
        return buildAddDownloadIntent(context, clazz, downloadRequest, STOP_REASON_NONE,
                foreground);
    }
     ...
     * @param clazz The concrete download service being targeted by the intent.
     * @param downloadRequest The request to be executed.
     * @param stopReason An initial stop reason for the download, or
     *        {@link Download#STOP_REASON_NONE} if the download should be started.
     * @param foreground Whether this intent will be used to start the service in the foreground.
     * @return The created intent.
     */
    public static Intent buildAddDownloadIntent(Context context,
            Class<? extends DownloadService> clazz, DownloadRequest downloadRequest, int stopReason,
            boolean foreground) {
    ...
    public static Intent buildRemoveDownloadIntent(Context context,
            Class<? extends DownloadService> clazz, String id, boolean foreground) {
    ...
    public static Intent buildRemoveAllDownloadsIntent(Context context,
            Class<? extends DownloadService> clazz, boolean foreground) {
    ...
    public static Intent buildSetStopReasonIntent(Context context,
            Class<? extends DownloadService> clazz, @Nullable String id, int stopReason,
            boolean foreground) {
        return getIntent(context, clazz, ACTION_SET_STOP_REASON, foreground)
                .putExtra(KEY_CONTENT_ID, id).putExtra(KEY_STOP_REASON, stopReason);
    }
    ...
    public static Intent buildSetRequirementsIntent(Context context,
            Class<? extends DownloadService> clazz, Requirements requirements, boolean foreground) {
     ...
     */
    public static void sendAddDownload(Context context, Class<? extends DownloadService> clazz,
    ...
    public static void sendAddDownload(Context context, Class<? extends DownloadService> clazz,
        ...
        Intent intent =
                buildAddDownloadIntent(context, clazz, downloadRequest, stopReason, foreground);
        startService(context, intent, foreground);
    }
     ...
     * @param foreground Whether the service is started in the foreground.
     */
    public static void sendRemoveDownload(Context context, Class<? extends DownloadService> clazz,
            String id, boolean foreground) {
     ...
     * @param foreground Whether the service is started in the foreground.
     */
    public static void sendResumeDownloads(Context context, Class<? extends DownloadService> clazz,
            boolean foreground) {
     ...
     * @param foreground Whether the service is started in the foreground.
     */
    public static void sendPauseDownloads(Context context, Class<? extends DownloadService> clazz,
            boolean foreground) {
     ...
     * To clear stop reason, pass {@link Download#STOP_REASON_NONE}.
     *
     * @param context A {@link Context}.
     * @param clazz The concrete download service to be started.
     * @param id The content id, or {@code null} to set the stop reason for all downloads.
     ...
     * @param foreground Whether the service is started in the foreground.
     */
    public static void sendSetStopReason(Context context, Class<? extends DownloadService> clazz,
            @Nullable String id, int stopReason, boolean foreground) {
     ...
     * @param foreground Whether the service is started in the foreground.
     */
    public static void sendSetRequirements(Context context, Class<? extends DownloadService> clazz,
     ...
     * not finished downloads and the requirements are met, the service resumes downloading.
    ...
    public void onCreate() {
        if (channelId != null) {
            NotificationUtil.createNotificationChannel(this, channelId, channelNameResourceId,
                    channelDescriptionResourceId, NotificationUtil.IMPORTANCE_LOW);
        }
        ...
        if (downloadManagerHelper == null) {
            ...
            downloadManagerHelper = new DownloadManagerHelper(getApplicationContext(),
                    downloadManager, getScheduler(), clazz);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            startedInForeground |= intent.getBooleanExtra(KEY_FOREGROUND, false)
                    || ACTION_RESTART.equals(intentAction);
    ...
    /**
     * Throws {@link UnsupportedOperationException} because this service is not designed to be
     * bound.
     */
    @Nullable
    @Override
    ...
    /**
     * Returns a {@link Scheduler} to restart the service when requirements allowing downloads to
     * take place are met. If {@code null}, the service will only be restarted if the process is
     * still in memory when the requirements are met.
     */
    protected abstract @Nullable Scheduler getScheduler();

    /**
     * Returns a notification to be displayed when this service running in the foreground. This
     * method is called when there is a download state change and periodically while there are
     * active downloads. The periodic update interval can be set using
     * {@link #DownloadService(int, long)}.
     *
     * <p>
     * On API level 26 and above, this method may also be called just before the service stops, with
     * an empty {@code downloads} array. The returned notification is used to satisfy system
     ...
     * {@code foregroundNotificationId} constructor argument to
     * {@link #FOREGROUND_NOTIFICATION_ID_NONE}. This method will not be called in this case,
     * meaning it can be implemented to throw {@link UnsupportedOperationException}.
     *
     ...
     * {@link #getForegroundNotification(List)} to be invoked again if the service isn't stopped.
     */
    ...
    private void stop() {
        if (foregroundNotificationUpdater != null) {
            foregroundNotificationUpdater.stopPeriodicUpdates();
            // Make sure startForeground is called before stopping. Workaround for [Internal:
            // b/69424260].
    ...
    private static Intent getIntent(Context context, Class<? extends DownloadService> clazz,
            String action, boolean foreground) {
    ...
    private static final class DownloadManagerHelper implements DownloadManager.Listener {
        ...
        private DownloadManagerHelper(Context context, DownloadManager downloadManager,
                @Nullable Scheduler scheduler, Class<? extends DownloadService> serviceClass) {
            ...
            if (scheduler != null) {
                ...
                setSchedulerEnabled(/* enabled= */ !requirements.checkRequirements(context),
                        requirements);
            }
        }
        ...
        @Override
        public void onRequirementsStateChanged(DownloadManager downloadManager,
                Requirements requirements, @Requirements.RequirementFlags int notMetRequirements) {
            ...
            if (downloadService == null && requirementsMet) {
                try {
                    ...
                    /*
                     * startService fails if the app is in the background then don't stop the
                     * scheduler.
                     */
                    return;
                }
