...
public final class Id3Decoder implements MetadataDecoder {
   ...
   */
  public static final int ID3_TAG = Utils.getIntegerCodeForString("ID3");
  /**
  ...
  private static ApicFrame decodeApicFrame(ParsableByteArray id3Data, int frameSize,
    ...
    if (majorVersion == 2) {
      mimeTypeEndIndex = 2;
      mimeType = "image/" + Utils.toLowerInvariant(new String(data, 0, 3, "ISO-8859-1"));
      if ("image/jpg".equals(mimeType)) {
        mimeType = "image/jpeg";
      }
    } else {
      mimeTypeEndIndex = indexOfZeroByte(data, 0);
      mimeType = Utils.toLowerInvariant(new String(data, 0, mimeTypeEndIndex, "ISO-8859-1"));
