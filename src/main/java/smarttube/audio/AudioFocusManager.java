...
public final class AudioFocusManager {
    ...
    /**
     * Player commands. One of {@link #PLAYER_COMMAND_DO_NOT_PLAY},
     * {@link #PLAYER_COMMAND_WAIT_FOR_CALLBACK} or {@link #PLAYER_COMMAND_PLAY_WHEN_READY}.
     */
    @Documented
    @Retention(RetentionPolicy.SOURCE)
    @IntDef({PLAYER_COMMAND_DO_NOT_PLAY, PLAYER_COMMAND_WAIT_FOR_CALLBACK,
    ...
    @Documented
    @Retention(RetentionPolicy.SOURCE)
    @IntDef({AUDIO_FOCUS_STATE_LOST_FOCUS, AUDIO_FOCUS_STATE_NO_FOCUS, AUDIO_FOCUS_STATE_HAVE_FOCUS,
            AUDIO_FOCUS_STATE_LOSS_TRANSIENT, AUDIO_FOCUS_STATE_LOSS_TRANSIENT_DUCK})
     ...
     */
    @PlayerCommand
    public int setAudioAttributes(@Nullable AudioAttributes audioAttributes, boolean playWhenReady,
            int playerState) {
        if (!Utils.areEqual(this.audioAttributes, audioAttributes)) {
            ...
            if (playWhenReady && (playerState == Player.STATE_BUFFERING
                    || playerState == Player.STATE_READY)) {
                return requestAudioFocus();
            }
        }

        return playerState == Player.STATE_IDLE ? handleIdle(playWhenReady)
    ...
    private int requestAudioFocus() {
        ...
        if (audioFocusState == AUDIO_FOCUS_STATE_NO_FOCUS) {
            ...
            audioFocusState = focusRequestResult == AudioManager.AUDIOFOCUS_REQUEST_GRANTED
    ...
    private int requestAudioFocusDefault() {
        return audioManager.requestAudioFocus(focusListener,
                Utils.getStreamTypeForAudioUsage(Assertions.checkNotNull(audioAttributes).usage),
                focusGain);
    }
    ...
    private int requestAudioFocusV26() {
        if (audioFocusRequest == null || rebuildAudioFocusRequest) {
            AudioFocusRequest.Builder builder =
                    audioFocusRequest == null ? new AudioFocusRequest.Builder(focusGain)
                            : new AudioFocusRequest.Builder(audioFocusRequest);

            boolean willPauseWhenDucked = willPauseWhenDucked();
            audioFocusRequest = builder
                    .setAudioAttributes(
                            Assertions.checkNotNull(audioAttributes).getAudioAttributesV21())
            ...
            switch (audioFocusState) {
                ...
                default:
                    throw new IllegalStateException(
                            "Unknown audio focus state: " + audioFocusState);
            }

            float volumeMultiplier = (audioFocusState == AUDIO_FOCUS_STATE_LOSS_TRANSIENT_DUCK)
