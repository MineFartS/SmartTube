...
/* package */ final class ClearKeyUtil {
  ...
  public static byte[] adjustRequestData(byte[] request) {
    ...
    String requestString = Utils.fromUtf8Bytes(request);
    return Utils.getUtf8Bytes(base64ToBase64Url(requestString));
  }
  ...
  public static byte[] adjustResponseData(byte[] response) {
    ...
    try {
      JSONObject responseJson = new JSONObject(Utils.fromUtf8Bytes(response));
      ...
      adjustedResponseBuilder.append("]}");
      return Utils.getUtf8Bytes(adjustedResponseBuilder.toString());
    } catch (JSONException e) {
      Log.e(TAG, "Failed to adjust response data: " + Utils.fromUtf8Bytes(response), e);
      return response;
    }
  }
