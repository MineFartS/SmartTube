...
public final class FrameworkMediaDrm implements ExoMediaDrm<FrameworkMediaCrypto> {
    ...
    public void setOnEventListener(
        ...
        mediaDrm.setOnEventListener(listener == null ? null
                : (mediaDrm, sessionId, event, extra, data) -> listener
                        .onEvent(FrameworkMediaDrm.this, sessionId, event, extra, data));
    }
    ...
    public void setOnKeyStatusChangeListener(
        ...
        mediaDrm.setOnKeyStatusChangeListener(
                listener == null ? null : (mediaDrm, sessionId, keyInfo, hasNewUsableKey) -> {
    ...
    @Override
    public KeyRequest getKeyRequest(byte[] scope,
            @Nullable List<DrmInitData.SchemeData> schemeDatas, int keyType,
    ...
    public FrameworkMediaCrypto createMediaCrypto(byte[] initData) throws MediaCryptoException {
        ...
        boolean forceAllowInsecureDecoderComponents = Utils.SDK_INT < 21
    ...
    private static SchemeData getSchemeData(UUID uuid, List<SchemeData> schemeDatas) {
        ...
        if (Utils.SDK_INT >= 28 && schemeDatas.size() > 1) {
            ...
            for (int i = 0; i < schemeDatas.size(); i++) {
                ...
                byte[] schemeDataData = Utils.castNonNull(schemeData.data);
                if (schemeData.requiresSecureDecryption == firstSchemeData.requiresSecureDecryption
                        && Utils.areEqual(schemeData.mimeType, firstSchemeData.mimeType)
                        && Utils.areEqual(schemeData.licenseServerUrl,
        ...
        for (int i = 0; i < schemeDatas.size(); i++) {
            SchemeData schemeData = schemeDatas.get(i);
            int version = PsshAtomUtil.parseVersion(Utils.castNonNull(schemeData.data));
            if (Utils.SDK_INT < 23 && version == 0) {
                return schemeData;
            } else if (Utils.SDK_INT >= 23 && version == 1) {
                return schemeData;
            }
        }
    ...
    private static UUID adjustUuid(UUID uuid) {
        ...
        return Utils.SDK_INT < 27 && C.CLEARKEY_UUID.equals(uuid) ? C.COMMON_PSSH_UUID : uuid;
    }
    ...
    private static byte[] adjustRequestInitData(UUID uuid, byte[] initData) {
        ...
        if (C.PLAYREADY_UUID.equals(uuid)) {
            ...
            initData = PsshAtomUtil.buildPsshAtom(C.PLAYREADY_UUID,
                    addLaUrlAttributeIfMissing(schemeSpecificData));
        }

        // Prior to L the Widevine CDM required data to be extracted from the PSSH atom. Some Amazon
        // devices also required data to be extracted from the PSSH atom for PlayReady.
        if (
            (Utils.SDK_INT < 21 && C.WIDEVINE_UUID.equals(uuid))
    ...
    private static String adjustRequestMimeType(UUID uuid, String mimeType) {
        ...
        if (Utils.SDK_INT < 26 && C.CLEARKEY_UUID.equals(uuid)
     ...
     *
     * <p>
     * See <a href="https://github.com/google/ExoPlayer/issues/4413">GitHub issue #4413</a>.
     */
    private static boolean needsForceWidevineL3Workaround() {
        return "ASUS_Z00AD".equals(Utils.MODEL);
    }
    ...
    private static byte[] addLaUrlAttributeIfMissing(byte[] data) {
        ...
        String xmlWithMockLaUrl =
                xml.substring(/* beginIndex= */ 0, /* endIndex= */ endOfDataTagIndex) + MOCK_LA_URL
