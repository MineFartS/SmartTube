...
public final class MediaCodecUtil {
  ...
  public static synchronized List<MediaCodecInfo> getDecoderInfos(
    ...
    if (secure && decoderInfos.isEmpty() && 21 <= Utils.SDK_INT && Utils.SDK_INT <= 23) {
  ...
  public static int maxH264DecodableFrameSize() throws DecoderQueryException {
    if (maxH264DecodableFrameSize == -1) {
      ...
      if (decoderInfo != null) {
        ...
        result = Math.max(result, Utils.SDK_INT >= 21 ? (720 * 480) : (480 * 360));
      }
    ...
    try {
      ...
      for (int i = 0; i < numberOfCodecs; i++) {
        ...
        try {
          ...
          if (Utils.SDK_INT <= 23 && !decoderInfos.isEmpty()) {
  ...
  private static boolean isCodecUsableDecoder(
    ...
    if (Utils.SDK_INT < 18
        ...
        && ("a70".equals(Utils.DEVICE)
        || ("Xiaomi".equals(Utils.MANUFACTURER) && Utils.DEVICE.startsWith("HM")))) {
      return false;
    }
    ...
    if (Utils.SDK_INT == 16
        ...
        && ("dlxu".equals(Utils.DEVICE) // HTC Butterfly
        || "protou".equals(Utils.DEVICE) // HTC Desire X
        || "ville".equals(Utils.DEVICE) // HTC One S
        || "villeplus".equals(Utils.DEVICE)
        || "villec2".equals(Utils.DEVICE)
        || Utils.DEVICE.startsWith("gee") // LGE Optimus G
        || "C6602".equals(Utils.DEVICE) // Sony Xperia Z
        || "C6603".equals(Utils.DEVICE)
        || "C6606".equals(Utils.DEVICE)
        || "C6616".equals(Utils.DEVICE)
        || "L36h".equals(Utils.DEVICE)
        || "SO-02E".equals(Utils.DEVICE))) {
      return false;
    }
    ...
    if (Utils.SDK_INT == 16
        ...
        && ("C1504".equals(Utils.DEVICE) // Sony Xperia E
        || "C1505".equals(Utils.DEVICE)
        || "C1604".equals(Utils.DEVICE) // Sony Xperia E dual
        || "C1605".equals(Utils.DEVICE))) {
      return false;
    }
    ...
    if (Utils.SDK_INT < 24
        ...
        && "samsung".equals(Utils.MANUFACTURER)
        && (Utils.DEVICE.startsWith("zeroflte") // Galaxy S6
        || Utils.DEVICE.startsWith("zerolte") // Galaxy S6 Edge
        || Utils.DEVICE.startsWith("zenlte") // Galaxy S6 Edge+
        || "SC-05G".equals(Utils.DEVICE) // Galaxy S6
        || "marinelteatt".equals(Utils.DEVICE) // Galaxy S6 Active
        || "404SC".equals(Utils.DEVICE) // Galaxy S6 Edge
        || "SC-04G".equals(Utils.DEVICE)
        || "SCV31".equals(Utils.DEVICE))) {
      return false;
    }
    ...
    if (Utils.SDK_INT <= 19
        ...
        && "samsung".equals(Utils.MANUFACTURER)
        && (Utils.DEVICE.startsWith("d2")
        || Utils.DEVICE.startsWith("serrano")
        || Utils.DEVICE.startsWith("jflte")
        || Utils.DEVICE.startsWith("santos")
        || Utils.DEVICE.startsWith("t0"))) {
      return false;
    }

    // VP8 decoder on Samsung Galaxy S4 cannot be queried.
    if (Utils.SDK_INT <= 19 && Utils.DEVICE.startsWith("jflte")
  ...
  private static void applyWorkarounds(String mimeType, List<MediaCodecInfo> decoderInfos) {
    ...
    } else if (Utils.SDK_INT < 21 && decoderInfos.size() > 1) {
  ...
  private static boolean codecNeedsDisableAdaptationWorkaround(String name) {
    return Utils.SDK_INT <= 22
        && ("ODROID-XU3".equals(Utils.MODEL) || "Nexus 10".equals(Utils.MODEL))
  ...
  private static final class RawAudioCodecComparator implements Comparator<MediaCodecInfo> {
    ...
    private static int scoreMediaCodecInfo(MediaCodecInfo mediaCodecInfo) {
      ...
      if (Utils.SDK_INT < 26 && name.equals("OMX.MTK.AUDIO.DECODER.RAW")) {
