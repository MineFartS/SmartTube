...
public final class DownloadRequest implements Parcelable {
     ...
     */
    public DownloadRequest(String id, String type, Uri uri, List<StreamKey> streamKeys,
            @Nullable String customCacheKey, @Nullable byte[] data) {
        if (TYPE_DASH.equals(type) || TYPE_HLS.equals(type) || TYPE_SS.equals(type)) {
            Assertions.checkArgument(customCacheKey == null,
                    "customCacheKey must be null for type: " + type);
        }
        this.id = id;
        this.type = type;
        this.uri = uri;
        ...
        this.data = data != null ? Arrays.copyOf(data, data.length) : Utils.EMPTY_BYTE_ARRAY;
    }
    ...
    /**
     * Returns the result of merging {@code newRequest} into this request. The requests must have
     * the same {@link #id} and {@link #type}.
     *
     * <p>
     * If the requests have different {@link #uri}, {@link #customCacheKey} and {@link #data}
     * values, then those from the request being merged are included in the result.
     *
     * @param newRequest The request being merged.
     * @return The merged result.
     * @throws IllegalArgumentException If the requests do not have the same {@link #id} and
     *         {@link #type}.
     */
    public DownloadRequest copyWithMergedRequest(DownloadRequest newRequest) {
        ...
        return new DownloadRequest(id, type, newRequest.uri, mergedKeys, newRequest.customCacheKey,
                newRequest.data);
    }
    ...
    public boolean equals(@Nullable Object o) {
        ...
        return id.equals(that.id) && type.equals(that.type) && uri.equals(that.uri)
                && streamKeys.equals(that.streamKeys)
                && Utils.areEqual(customCacheKey, that.customCacheKey)
