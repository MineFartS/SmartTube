...
import minefarts.smarttube.metadata.emsg.EventMessageDecoder;
import minefarts.smarttube.metadata.id3.PrivFrame;
import minefarts.smarttube.source.MediaSourceEventListener.EventDispatcher;
import minefarts.smarttube.source.SampleQueue;
import minefarts.smarttube.source.SampleQueue.UpstreamFormatChangedListener;
...
/* package */ final class HlsSampleStreamWrapper implements Loader.Callback<Chunk>,
    ...
    public interface Callback extends SequenceableLoader.Callback<HlsSampleStreamWrapper> {
         ...
         *
         * <p>
         * Note: This method will be called on a later handler loop than the one on which either
         * {@link #prepareWithMasterPlaylistInfo} or {@link #continuePreparing} are invoked.
         */
        void onPrepared();

        /**
         * Called to schedule a {@link #continueLoading(long)} call when the playlist referred by
    ...
    private static final Set<Integer> MAPPABLE_TYPES = Collections.unmodifiableSet(new HashSet<>(
     ...
     *        (i.e. {@link DrmInitData#schemeType}). If the stream has {@link DrmInitData} and uses
     *        a protection scheme type for which overriding {@link DrmInitData} is provided, then
     *        the stream's {@link DrmInitData} will be overridden.
     * @param allocator An {@link Allocator} from which to obtain media buffer allocations.
     * @param positionUs The position from which to start loading media.
     * @param muxedAudioFormat Optional muxed audio {@link Format} as defined by the master
     *        playlist.
    ...
    public HlsSampleStreamWrapper(int trackType, Callback callback, HlsChunkSource chunkSource,
            Map<String, DrmInitData> overridingDrmInitData, Allocator allocator, long positionUs,
            Format muxedAudioFormat, LoadErrorHandlingPolicy loadErrorHandlingPolicy,
            EventDispatcher eventDispatcher, @HlsMetadataType int metadataType) {
     ...
     * @param optionalTrackGroups A subset of {@code trackGroups} that should not trigger a failure
     *        if not found in the media playlist's segments.
     */
    public void prepareWithMasterPlaylistInfo(TrackGroupArray trackGroups,
            int primaryTrackGroupIndex, TrackGroupArray optionalTrackGroups) {
        prepared = true;
     ...
     * @param mayRetainStreamFlags Flags indicating whether the existing sample stream can be
     *        retained for each selection. A {@code true} value indicates that the selection is
     *        unchanged, and that the caller does not require that the sample stream be recreated.
     * @param streams The existing sample streams, which will be updated to reflect the provided
     *        selections.
     * @param streamResetFlags Will be updated to indicate new sample streams, and sample streams
     *        that have been retained but with the requirement that the consuming renderer be reset.
     * @param positionUs The current playback position in microseconds.
     * @param forceReset If true then a reset is forced (i.e. a seek will be performed with
     *        in-buffer seeking disabled).
    ...
    public boolean selectTracks(TrackSelection[] selections, boolean[] mayRetainStreamFlags,
        ...
        // We'll always need to seek if we're being forced to reset, or if this is a first selection
        // to
        ...
        boolean seekRequired =
                forceReset || (seenFirstTrackSelection ? oldEnabledTrackGroupCount == 0
                        : positionUs != lastSeekPositionUs);
        // Get the old (i.e. current before the loop below executes) primary track selection. The
        // new
        ...
        for (int i = 0; i < selections.length; i++) {
            ...
            if (streams[i] == null) {
                ...
                // If there's still a chance of avoiding a seek, try and seek within the sample
                // queue.
                if (sampleQueuesBuilt && !seekRequired) {
                    SampleQueue sampleQueue =
                            sampleQueues[trackGroupToSampleQueueIndex[trackGroupIndex]];
                    sampleQueue.rewind();
                    // A seek can be avoided if we're able to advance to the current playback
                    // position in the
                    // sample queue, or if we haven't read anything from the queue since the
                    // previous seek
                    // (this case is common for sparse tracks such as metadata tracks). In all other
                    // cases a
                    // seek is required.
                    seekRequired = sampleQueue.advanceTo(positionUs, true,
                            true) == SampleQueue.ADVANCE_FAILED && sampleQueue.getReadIndex() != 0;
                }
            }
        }
        ...
        if (enabledTrackGroupCount == 0) {
            ...
            if (!mediaChunks.isEmpty()
                    && !Utils.areEqual(primaryTrackSelection, oldPrimaryTrackSelection)) {
                // The primary track selection has changed and we have buffered media. The buffered
                // media
                ...
                if (!seenFirstTrackSelection) {
                    ...
                    primaryTrackSelection.updateSelectedTrack(positionUs, bufferedDurationUs,
                            C.TIME_UNSET, readOnlyMediaChunks, mediaChunkIterators);
                    int chunkIndex =
                            chunkSource.getTrackGroup().indexOf(lastMediaChunk.trackFormat);
                    if (primaryTrackSelection.getSelectedIndexInTrackGroup() != chunkIndex) {
                        // This is the first selection and the chunk loaded during preparation does
                        // not match
                        // the initially selected format.
                        primarySampleQueueDirty = true;
                    }
                } else {
                    // The primary sample queue contains media buffered for the old primary track
                    // selection.
                    primarySampleQueueDirty = true;
                }
     ...
     * @return Whether the wrapper was reset, meaning the wrapped sample queues were reset. If
     *         false, an in-buffer seek was performed.
     */
    ...
    public boolean isReady(int sampleQueueIndex) {
        return loadingFinished
                || (!isPendingReset() && sampleQueues[sampleQueueIndex].hasNextSample());
    }
    ...
    public int readData(int sampleQueueIndex, FormatHolder formatHolder, DecoderInputBuffer buffer,
        ...
        // TODO: Split into discard (in discardBuffer) and format change (here and in skipData)
        // steps.
        if (!mediaChunks.isEmpty()) {
            ...
            Utils.removeRange(mediaChunks, 0, discardToMediaChunkIndex);
        ...
        int result = sampleQueues[sampleQueueIndex].read(formatHolder, buffer, requireFormat,
        ...
        if (result == C.RESULT_FORMAT_READ) {
            ...
            if (sampleQueueIndex == primarySampleQueueIndex) {
                ...
                int chunkIndex = 0;
                while (chunkIndex < mediaChunks.size()
                        && mediaChunks.get(chunkIndex).uid != chunkUid) {
                    chunkIndex++;
                }
                Format trackFormat =
                        chunkIndex < mediaChunks.size() ? mediaChunks.get(chunkIndex).trackFormat
                                : upstreamTrackFormat;
    ...
    public long getBufferedPositionUs() {
        if (loadingFinished) {
            ...
            HlsMediaChunk lastMediaChunk = getLastMediaChunk();
            HlsMediaChunk lastCompletedMediaChunk = lastMediaChunk.isLoadCompleted()
                    ? lastMediaChunk
                    : mediaChunks.size() > 1 ? mediaChunks.get(mediaChunks.size() - 2) : null;
            if (lastCompletedMediaChunk != null) {
                bufferedPositionUs =
                        Math.max(bufferedPositionUs, lastCompletedMediaChunk.endTimeUs);
            }
            if (sampleQueuesBuilt) {
                for (SampleQueue sampleQueue : sampleQueues) {
                    bufferedPositionUs =
    ...
    public boolean continueLoading(long positionUs) {
        ...
        if (isPendingReset()) {
            ...
            HlsMediaChunk lastMediaChunk = getLastMediaChunk();
            loadPositionUs = lastMediaChunk.isLoadCompleted() ? lastMediaChunk.endTimeUs
        ...
        long elapsedRealtimeMs = loader.startLoading(loadable, this,
                loadErrorHandlingPolicy.getMinimumLoadableRetryCount(loadable.type));
        eventDispatcher.loadStarted(loadable.dataSpec, loadable.type, trackType,
    ...
    public void onLoadCompleted(Chunk loadable, long elapsedRealtimeMs, long loadDurationMs) {
        ...
        eventDispatcher.loadCompleted(loadable.dataSpec, loadable.getUri(),
                loadable.getResponseHeaders(), loadable.type, trackType, loadable.trackFormat,
    ...
    public void onLoadCanceled(Chunk loadable, long elapsedRealtimeMs, long loadDurationMs,
            boolean released) {
        eventDispatcher.loadCanceled(loadable.dataSpec, loadable.getUri(),
                loadable.getResponseHeaders(), loadable.type, trackType, loadable.trackFormat,
                loadable.trackSelectionReason, loadable.trackSelectionData, loadable.startTimeUs,
                loadable.endTimeUs, elapsedRealtimeMs, loadDurationMs, loadable.bytesLoaded());
        if (!released) {
    ...
    @Override
    public LoadErrorAction onLoadError(Chunk loadable, long elapsedRealtimeMs, long loadDurationMs,
            IOException error, int errorCount) {
        long bytesLoaded = loadable.bytesLoaded();
        boolean isMediaChunk = isMediaChunk(loadable);
        boolean blacklistSucceeded = false;
        LoadErrorAction loadErrorAction;

        long blacklistDurationMs = loadErrorHandlingPolicy.getBlacklistDurationMsFor(loadable.type,
        ...
        if (blacklistSucceeded) {
            ...
            long retryDelayMs = loadErrorHandlingPolicy.getRetryDelayMsFor(loadable.type,
                    loadDurationMs, error, errorCount);
            loadErrorAction = retryDelayMs != C.TIME_UNSET
                    ? Loader.createRetryAction(/* resetErrorCount= */ false, retryDelayMs)
                    : Loader.DONT_RETRY_FATAL;
        }

        eventDispatcher.loadError(loadable.dataSpec, loadable.getUri(),
                loadable.getResponseHeaders(), loadable.type, trackType, loadable.trackFormat,
                loadable.trackSelectionReason, loadable.trackSelectionData, loadable.startTimeUs,
     ...
     * @param reusingExtractor Whether the extractor for the chunk has already been used for
     *        preceding chunks.
     */
        ...
        if (type == C.TRACK_TYPE_METADATA) {
            if (emsgUnwrappingTrackOutput == null) {
                emsgUnwrappingTrackOutput =
                        new EmsgUnwrappingTrackOutput(trackOutput, metadataType);
            }
     ...
     *
     * <p>
     * If a {@link SampleQueue} for {@code type} has been created and is mapped, but it has a
     * different ID, then return a {@link DummyTrackOutput} that does nothing.
     *
     * <p>
     * If a {@link SampleQueue} for {@code type} has been created but is not mapped, then map it to
    ...
    private TrackOutput getMappedTrackOutput(int id, int type) {
        ...
        return sampleQueueTrackIds[sampleQueueIndex] == id ? sampleQueues[sampleQueueIndex]
    ...
    private void maybeFinishPrepare() {
        ...
        if (trackGroups != null) {
            // The track groups were created with master playlist information. They only need to be
            // mapped
            // to a sample queue.
    ...
    private void mapSampleQueuesToMatchTrackGroups() {
        ...
        for (int i = 0; i < trackGroupCount; i++) {
            for (int queueIndex = 0; queueIndex < sampleQueues.length; queueIndex++) {
                ...
                if (formatsMatch(sampleQueue.getUpstreamFormat(),
                        trackGroups.get(i).getFormat(0))) {
     ...
     * variant stream typically contains muxed video, audio and (possibly) additional audio,
     * metadata and caption tracks. We wish to allow the user to select between an adaptive track
     * that spans all variants, as well as each individual variant. If multiple audio tracks are
     * present within each variant then we wish to allow the user to select between those also.
     *
     * <p>
     * To do this, tracks are constructed as follows. The {@link HlsChunkSource} exposes (N+1)
     * tracks, where N is the number of variants defined in the HLS master playlist. These consist
     * of one adaptive track defined to span all variants and a track for each individual variant.
     * The adaptive track is initially selected. The extractor is then prepared to discover the
     * tracks inside of each variant stream. The two sets of tracks are then combined by this method
     * to create a third set, which is the set exposed by this {@link HlsSampleStreamWrapper}:
     *
     * <ul>
     ...
     * of switching the selected track on the chunk source.
        ...
        for (int i = 0; i < extractorTrackCount; i++) {
            ...
            if (i == primaryExtractorTrackIndex) {
                ...
                if (chunkSourceTrackCount == 1) {
                    formats[0] = sampleFormat
                            .copyWithManifestFormatInfo(chunkSourceTrackGroup.getFormat(0));
                } else {
                ...
                Format trackFormat = primaryExtractorTrackType == C.TRACK_TYPE_VIDEO
    ...
    private boolean seekInsideBufferUs(long positionUs) {
        ...
        for (int i = 0; i < sampleQueueCount; i++) {
            ...
            // If we have AV tracks then an in-queue seek is successful if the seek into every AV
            // queue
            // is successful. We ignore whether seeks within non-AV queues are successful in this
            // case, as
            // they may be sparse or poorly interleaved. If we only have non-AV tracks then a seek
            // is
     ...
     * @param propagateBitrate Whether the bitrate from the playlist format should be included in
     *        the derived format.
     * @return The derived track format.
     */
    private static Format deriveFormat(Format playlistFormat, Format sampleFormat,
            boolean propagateBitrate) {
        if (playlistFormat == null) {
            return sampleFormat;
        }
        int bitrate = propagateBitrate ? playlistFormat.bitrate : Format.NO_VALUE;
        int channelCount =
                playlistFormat.channelCount != Format.NO_VALUE ? playlistFormat.channelCount
                        : sampleFormat.channelCount;
        int sampleTrackType = MimeTypes.getTrackType(sampleFormat.sampleMimeType);
        String codecs = Utils.getCodecsOfType(playlistFormat.codecs, sampleTrackType);
        String mimeType = MimeTypes.getMediaMimeType(codecs);
        if (mimeType == null) {
            mimeType = sampleFormat.sampleMimeType;
        }
        return sampleFormat.copyWithContainerInfo(playlistFormat.id, playlistFormat.label, mimeType,
                codecs, playlistFormat.metadata, bitrate, playlistFormat.width,
                playlistFormat.height, playlistFormat.frameRate, // MOD: add fps to hls format info
                channelCount, playlistFormat.selectionFlags, playlistFormat.language);
    }
    ...
    private static boolean formatsMatch(Format manifestFormat, Format sampleFormat) {
        ...
        } else if (!Utils.areEqual(manifestFormatMimeType, sampleFormatMimeType)) {
            return false;
        }
    ...
    private static class EmsgUnwrappingTrackOutput implements TrackOutput {
        ...
        private static final Format ID3_FORMAT = Format.createSampleFormat(/* id= */ null,
                MimeTypes.APPLICATION_ID3, Format.OFFSET_SAMPLE_RELATIVE);
        private static final Format EMSG_FORMAT = Format.createSampleFormat(/* id= */ null,
        ...
        public void sampleMetadata(long timeUs, @C.BufferFlags int flags, int size, int offset,
            ...
            if (Utils.areEqual(format.sampleMimeType, delegateFormat.sampleMimeType)) {
                ...
                // Incoming sample is EMSG, and delegate track is not expecting EMSG, so try
                // unwrapping.
                ...
                sampleForDelegate = new ParsableByteArray(
                        Assertions.checkNotNull(emsg.getWrappedMetadataBytes()));
            } else {
        ...
        private boolean emsgContainsExpectedWrappedFormat(EventMessage emsg) {
            ...
            return wrappedMetadataFormat != null && Utils.areEqual(delegateFormat.sampleMimeType,
                    wrappedMetadataFormat.sampleMimeType);
        }
        ...
        /**
         * Removes a complete sample from the {@link #buffer} field & reshuffles the tail data
         * skipped by {@code offset} to the head of the array.
         *
