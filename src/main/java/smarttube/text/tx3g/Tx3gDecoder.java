package minefarts.smarttube.text.tx3g;
...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
  ...
  private static final int TYPE_STYL = Utils.getIntegerCodeForString("styl");
  private static final int TYPE_TBOX = Utils.getIntegerCodeForString("tbox");
  ...
  public Tx3gDecoder(List<byte[]> initializationData) {
    ...
    if (initializationData != null && initializationData.size() == 1
      ...
      String fontFamily =
          Utils.fromUtf8Bytes(initializationBytes, 43, initializationBytes.length - 43);
      ...
      if (customVerticalPlacement) {
        ...
        defaultVerticalPlacement = Utils.constrainValue(defaultVerticalPlacement, 0.0f, 0.95f);
      } else {
  ...
  protected Subtitle decode(byte[] bytes, int length, boolean reset)
    ...
    while (parsableByteArray.bytesLeft() >= SIZE_ATOM_HEADER) {
      ...
      if (atomType == TYPE_STYL) {
        ...
        verticalPlacement = Utils.constrainValue(verticalPlacement, 0.0f, 0.95f);
      }
