...
import minefarts.smarttube.extractor.SeekMap.SeekPoints;
import minefarts.smarttube.extractor.SeekMap.Unseekable;
import minefarts.smarttube.extractor.TrackOutput;
import minefarts.smarttube.extractor.mp3.Mp3Extractor;
...
import minefarts.smarttube.upstream.Loader.Loadable;
import minefarts.smarttube.upstream.StatsDataSource;
import minefarts.smarttube.utils.Assertions;
import minefarts.smarttube.utils.ConditionVariable;
...
/* package */ final class ProgressiveMediaPeriod implements MediaPeriod, ExtractorOutput,
        Loader.Callback<ProgressiveMediaPeriod.ExtractingLoadable>, Loader.ReleaseCallback,
    ...
    private static final Format ICY_FORMAT = Format.createSampleFormat("icy",
            MimeTypes.APPLICATION_ICY, Format.OFFSET_SAMPLE_RELATIVE);
     ...
     *        cache indexing. May be null.
     * @param continueLoadingCheckIntervalBytes The number of bytes that should be loaded between
     *        each invocation of {@link Callback#onContinueLoadingRequested(SequenceableLoader)}.
     */
    // maybeFinishPrepare is not posted to the handler until initialization completes.
    @SuppressWarnings({"nullness:argument.type.incompatible",
            "nullness:methodref.receiver.bound.invalid", "deprecation"})
    public ProgressiveMediaPeriod(Uri uri, DataSource dataSource, Extractor[] extractors,
            LoadErrorHandlingPolicy loadErrorHandlingPolicy, EventDispatcher eventDispatcher,
            Listener listener, Allocator allocator, @Nullable String customCacheKey,
    ...
    @Override
    public long selectTracks(@NullableType TrackSelection[] selections,
        ...
        // We'll always need to seek if this is a first selection to a non-zero position, or if
        // we're
        // making a selection having previously disabled all tracks.
        boolean seekRequired =
                seenFirstTrackSelection ? oldEnabledTrackCount == 0 : positionUs != 0;
        // Select new tracks.
        for (int i = 0; i < selections.length; i++) {
    ...
    public boolean continueLoading(long playbackPositionUs) {
        if (loadingFinished || loader.hasFatalError() || pendingDeferredRetry
    ...
    public long readDiscontinuity() {
        ...
        if (notifyDiscontinuity && (loadingFinished
                || getExtractedSamplesCount() > extractedSamplesCountAtStartOfLoad)) {
            notifyDiscontinuity = false;
    ...
    public long getAdjustedSeekPositionUs(long positionUs, SeekParameters seekParameters) {
        ...
        return Utils.resolveSeekPositionUs(positionUs, seekParameters, seekPoints.first.timeUs,
    ...
    /* package */ int readData(int track, FormatHolder formatHolder, DecoderInputBuffer buffer,
        ...
        int result = sampleQueues[track].read(formatHolder, buffer, formatRequired, loadingFinished,
                lastSeekPositionUs);
    ...
    private void maybeNotifyDownstreamFormat(int track) {
        ...
        if (!trackNotifiedDownstreamFormats[track]) {
            ...
            eventDispatcher.downstreamFormatChanged(
                    MimeTypes.getTrackType(trackFormat.sampleMimeType), trackFormat,
                    C.SELECTION_REASON_UNKNOWN, /* trackSelectionData= */ null, lastSeekPositionUs);
    ...
    public void onLoadCompleted(ExtractingLoadable loadable, long elapsedRealtimeMs,
        ...
        eventDispatcher.loadCompleted(loadable.dataSpec, loadable.dataSource.getLastOpenedUri(),
                loadable.dataSource.getLastResponseHeaders(), C.DATA_TYPE_MEDIA,
                C.TRACK_TYPE_UNKNOWN, /* trackFormat= */ null, C.SELECTION_REASON_UNKNOWN,
                /* trackSelectionData= */ null, /* mediaStartTimeUs= */ loadable.seekTimeUs,
                durationUs, elapsedRealtimeMs, loadDurationMs, loadable.dataSource.getBytesRead());
    ...
    public void onLoadCanceled(ExtractingLoadable loadable, long elapsedRealtimeMs,
        ...
        eventDispatcher.loadCanceled(loadable.dataSpec, loadable.dataSource.getLastOpenedUri(),
                loadable.dataSource.getLastResponseHeaders(), C.DATA_TYPE_MEDIA,
                C.TRACK_TYPE_UNKNOWN, /* trackFormat= */ null, C.SELECTION_REASON_UNKNOWN,
                /* trackSelectionData= */ null, /* mediaStartTimeUs= */ loadable.seekTimeUs,
                durationUs, elapsedRealtimeMs, loadDurationMs, loadable.dataSource.getBytesRead());
    ...
    @Override
    public LoadErrorAction onLoadError(ExtractingLoadable loadable, long elapsedRealtimeMs,
            long loadDurationMs, IOException error, int errorCount) {
        copyLengthFromLoader(loadable);
        LoadErrorAction loadErrorAction;
        long retryDelayMs = loadErrorHandlingPolicy.getRetryDelayMsFor(dataType, loadDurationMs,
                error, errorCount);
        if (retryDelayMs == C.TIME_UNSET) {
            ...
            loadErrorAction = configureRetry(loadable, extractedSamplesCount)
                    ? Loader.createRetryAction(/* resetErrorCount= */ madeProgress, retryDelayMs)
                    : Loader.DONT_RETRY;
        }

        eventDispatcher.loadError(loadable.dataSpec, loadable.dataSource.getLastOpenedUri(),
                loadable.dataSource.getLastResponseHeaders(), C.DATA_TYPE_MEDIA,
                C.TRACK_TYPE_UNKNOWN, /* trackFormat= */ null, C.SELECTION_REASON_UNKNOWN,
                /* trackSelectionData= */ null, /* mediaStartTimeUs= */ loadable.seekTimeUs,
                durationUs, elapsedRealtimeMs, loadDurationMs, loadable.dataSource.getBytesRead(),
                error, !loadErrorAction.isRetry());
    ...
    private TrackOutput prepareTrackOutput(TrackId id) {
        ...
        this.sampleQueueTrackIds = Utils.castNonNullTypeArray(sampleQueueTrackIds);
        @NullableType
        SampleQueue[] sampleQueues = Arrays.copyOf(this.sampleQueues, trackCount + 1);
        sampleQueues[trackCount] = trackOutput;
        this.sampleQueues = Utils.castNonNullTypeArray(sampleQueues);
        return trackOutput;
    }
    ...
    private void maybeFinishPrepare() {
        ...
        for (int i = 0; i < trackCount; i++) {
            ...
            if (icyHeaders != null) {
                if (isAudio || sampleQueueTrackIds[i].isIcyTrack) {
                    ...
                    trackFormat =
                            trackFormat.copyWithMetadata(metadata == null ? new Metadata(icyHeaders)
                                    : metadata.copyWithAppendedEntries(icyHeaders));
                }
                if (isAudio && trackFormat.bitrate == Format.NO_VALUE
        ...
        dataType = length == C.LENGTH_UNSET && seekMap.getDurationUs() == C.TIME_UNSET
    ...
    private void startLoading() {
        ExtractingLoadable loadable = new ExtractingLoadable(uri, dataSource, extractorHolder,
                /* extractorOutput= */ this, loadCondition);
        if (prepared) {
            SeekMap seekMap = getPreparedState().seekMap;
            ...
            loadable.setLoadPosition(seekMap.getSeekPoints(pendingResetPositionUs).first.position,
        ...
        eventDispatcher.loadStarted(loadable.dataSpec, C.DATA_TYPE_MEDIA, C.TRACK_TYPE_UNKNOWN,
                /* trackFormat= */ null, C.SELECTION_REASON_UNKNOWN, /* trackSelectionData= */ null,
                /* mediaStartTimeUs= */ loadable.seekTimeUs, durationUs, elapsedRealtimeMs);
    }
     ...
     * @param currentExtractedSampleCount The current number of samples that have been extracted
     *        into the sample queues.
    ...
    private boolean configureRetry(ExtractingLoadable loadable, int currentExtractedSampleCount) {
        if (length != C.LENGTH_UNSET
            ...
            // therefore that
            // the data at the uri is a continuously shifting window of the latest available media.
            // For
            // this case there's no way to continue loading from where a previous load finished, so
            // it's
            // necessary to load from the start whenever commencing a new load. Deferring the retry
            // until
            // we run out of buffered data makes for a much better user experience. See:
            // https://github.com/google/ExoPlayer/issues/1606.
            // Note that the suppressRead() check means only a single deferred retry can occur
            // without
            // progress being made. Any subsequent failures without progress will go through the
            // else
            // block below.
            pendingDeferredRetry = true;
            return false;
        } else {
            // This is the same case as above, except in this case there's no value in deferring the
            // retry
            // because there's no buffered data to be read. This case also covers an on-demand
            // stream with
            // unknown length that has yet to be prepared. This case cannot be disambiguated from
            // the live
    ...
    private boolean seekInsideBufferUs(boolean[] trackIsAudioVideoFlags, long positionUs) {
        ...
        for (int i = 0; i < trackCount; i++) {
            ...
            // If we have AV tracks then an in-buffer seek is successful if the seek into every AV
            // queue
    ...
    private final class SampleStreamImpl implements SampleStream {
        ...
        public int readData(FormatHolder formatHolder, DecoderInputBuffer buffer,
                boolean formatRequired) {
            return ProgressiveMediaPeriod.this.readData(track, formatHolder, buffer,
                    formatRequired);
        }
    ...
    /* package */ final class ExtractingLoadable implements Loadable, IcyDataSource.Listener {
        ...
        public ExtractingLoadable(Uri uri, DataSource dataSource, ExtractorHolder extractorHolder,
                ExtractorOutput extractorOutput, ConditionVariable loadCondition) {
            this.uri = uri;
            ...
            while (result == Extractor.RESULT_CONTINUE && !loadCanceled) {
                ...
                try {
                    ...
                    if (icyHeaders != null && icyHeaders.metadataInterval != C.LENGTH_UNSET) {
                        extractorDataSource =
                                new IcyDataSource(dataSource, icyHeaders.metadataInterval, this);
                    ...
                    Extractor extractor =
                            extractorHolder.selectExtractor(input, extractorOutput, uri);
        ...
        public void onIcyMetadata(ParsableByteArray metadata) {
            ...
            long timeUs = !seenIcyMetadata ? seekTimeUs
                    : Math.max(getLargestQueuedTimestampUs(), seekTimeUs);
            ...
            icyTrackOutput.sampleMetadata(timeUs, C.BUFFER_FLAG_KEY_FRAME, length, /* offset= */ 0,
                    /* encryptionData= */ null);
        ...
        private DataSpec buildDataSpec(long position) {
            // Disable caching if the content length cannot be resolved, since this is indicative of
            // a
            // progressive live stream.
            return new DataSpec(uri, position, C.LENGTH_UNSET, customCacheKey,
    ...
    private static final class ExtractorHolder {
        ...
        /**
         * Creates a holder that will select an extractor and initialize it using the specified
         * output.
         *
        ...
        /**
         * Returns an initialized extractor for reading {@code input}, and returns the same
         * extractor on later calls.
         *
         ...
         * @throws UnrecognizedInputFormatException Thrown if the input format could not be
         *         detected.
            ...
            if (extractors.length == 1) {
                ...
                if (extractor == null) {
                    throw new UnrecognizedInputFormatException("None of the available extractors ("
                            + Utils.getCommaDelimitedSimpleClassNames(extractors)
    ...
    private static final class PreparedState {
        ...
        public PreparedState(SeekMap seekMap, TrackGroupArray tracks,
