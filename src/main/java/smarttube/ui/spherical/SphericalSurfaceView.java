package minefarts.smarttube.ui.spherical;
...
public final class SphericalSurfaceView extends GLSurfaceView {
    ...
    public SphericalSurfaceView(Context context, @Nullable AttributeSet attributeSet) {
        ...
        sensorManager = (SensorManager) Assertions
                .checkNotNull(context.getSystemService(Context.SENSOR_SERVICE));
        Sensor orientationSensor = null;
        if (Utils.SDK_INT >= 18) {
            // TYPE_GAME_ROTATION_VECTOR is the easiest sensor since it handles all the complex math
            // for
            // fusion. It's used instead of TYPE_ROTATION_VECTOR since the latter uses the
            // magnetometer on
            // devices. When used indoors, the magnetometer can take some time to settle depending
            // on the
    ...
    /**
     * Sets the default stereo mode. If the played video doesn't contain a stereo mode the default
     * one is used.
     *
    ...
    private static void releaseSurface(@Nullable SurfaceTexture oldSurfaceTexture,
    ...
    /* package */ class Renderer
        ...
        // There is no model matrix for this scene so viewProjectionMatrix is used for the
        // mvpMatrix.
        private final float[] viewProjectionMatrix = new float[16];

        // Device orientation is derived from sensor data. This is accessed in the sensor's thread
        // and
        // the GL thread.
        private final float[] deviceOrientationMatrix = new float[16];

        // Optional pitch and yaw rotations are applied to the sensor orientation. These are
        // accessed on
        ...
        public void onDrawFrame(GL10 gl) {
            ...
            // Orientation = pitch * sensor * yaw since that is closest to what most users expect
            // the
            // behavior to be.
            synchronized (this) {
        ...
        /**
         * Updates the pitch matrix after a physical rotation or touch input. The pitch matrix
         * rotation is applied on an axis that is dependent on device rotation so this must be
         * called after either touch or sensor update.
         */
        @AnyThread
        private void updatePitchMatrix() {
            // The camera's pitch needs to be rotated along an axis that is parallel to the real
            // world's
            // horizon. This is the <1, 0, 0> axis after compensating for the device's roll.
            Matrix.setRotateM(touchPitchMatrix, 0, -touchPitch, (float) Math.cos(deviceRoll),
                    (float) Math.sin(deviceRoll), 0);
        }
