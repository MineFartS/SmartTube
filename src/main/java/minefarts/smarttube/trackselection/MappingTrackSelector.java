    ...
    public int getAdaptiveSupport(int rendererIndex, int groupIndex, int[] trackIndices) {
      ...
      for (int i = 0; i < trackIndices.length; i++) {
        ...
        if (handledTrackCount++ == 0) {
          ...
          multipleMimeTypes |= !Utils.areEqual(firstSampleMimeType, sampleMimeType);
        }
        adaptiveSupport =
            Math.min(
                adaptiveSupport,
  ...
  public final TrackSelectorResult selectTracks(
    ...
    for (int i = 0; i < rendererCapabilities.length; i++) {
      ...
      rendererTrackGroupArrays[i] =
          new TrackGroupArray(
              Utils.nullSafeArrayCopy(rendererTrackGroups[i], rendererTrackGroupCount));
      rendererFormatSupports[i] =
          Utils.nullSafeArrayCopy(rendererFormatSupports[i], rendererTrackGroupCount);
