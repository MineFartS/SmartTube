...
import minefarts.smarttube.video.spherical.Projection.Mesh;
import minefarts.smarttube.video.spherical.Projection.SubMesh;
...
public final class ProjectionDecoder {
  ...
  private static final int TYPE_YTMP = Utils.getIntegerCodeForString("ytmp");
  private static final int TYPE_MSHP = Utils.getIntegerCodeForString("mshp");
  private static final int TYPE_RAW = Utils.getIntegerCodeForString("raw ");
  private static final int TYPE_DFL8 = Utils.getIntegerCodeForString("dfl8");
  private static final int TYPE_MESH = Utils.getIntegerCodeForString("mesh");
  private static final int TYPE_PROJ = Utils.getIntegerCodeForString("proj");
  ...
  private static @Nullable ArrayList<Mesh> parseMshp(ParsableByteArray input) {
    ...
    if (encoding == TYPE_DFL8) {
      ...
      try {
        if (!Utils.inflate(input, output, inflater)) {
          return null;
        }
      } finally {
        inflater.end();
