package minefarts.smarttube.ms;
...
import minefarts.smarttube.utils.RepeatModeUtil;
 ...
 *
 * <p>
 * This connector does <em>not</em> call {@link MediaSessionCompat#setActive(boolean)}, and so
 ...
 * <li>A {@link MediaButtonEventHandler} can be set by calling
 * {@link #setMediaButtonEventHandler(MediaButtonEventHandler)}. By default media button events are
 * handled by {@link MediaSessionCompat.Callback#onMediaButtonEvent(Intent)}.
 * <li>An {@link ErrorMessageProvider} for providing human readable error messages and corresponding
 * error codes can be set by calling {@link #setErrorMessageProvider(ErrorMessageProvider)}.
 * <li>A {@link MediaMetadataProvider} can be set by calling
 * {@link #setMediaMetadataProvider(MediaMetadataProvider)}. By default the
 * {@link DefaultMediaMetadataProvider} is used.
 * </ul>
 */
...
public final class MediaSessionConnector {
    ...
    @LongDef(flag = true,
            value = {PlaybackStateCompat.ACTION_PLAY_PAUSE, PlaybackStateCompat.ACTION_PLAY,
    ...
    public interface CommandReceiver {
         ...
         * receiver may handle the command, but is not required to do so. Changes to the player
         * should be made via the {@link ControlDispatcher}.
         *
        ...
        boolean onCommand(Player player, ControlDispatcher controlDispatcher, String command,
                Bundle extras, ResultReceiver cb);
    }
    ...
    public interface QueueEditor extends CommandReceiver {
        ...
        /**
         * See
         * {@link MediaSessionCompat.Callback#onAddQueueItem(MediaDescriptionCompat description)}.
         */
        void onAddQueueItem(Player player, MediaDescriptionCompat description);

        /**
         * See
         * {@link MediaSessionCompat.Callback#onAddQueueItem(MediaDescriptionCompat description, int index)}.
         */
        void onAddQueueItem(Player player, MediaDescriptionCompat description, int index);

        /**
         * See
         * {@link MediaSessionCompat.Callback#onRemoveQueueItem(MediaDescriptionCompat description)}.
         */
    ...
    public interface MediaButtonEventHandler {
         ...
         */
        boolean onMediaButtonEvent(Player player, ControlDispatcher controlDispatcher,
                Intent mediaButtonEvent);
    }

    /**
     * Provides a {@link PlaybackStateCompat.CustomAction} to be published and handles the action
     * when sent by a media controller.
     */
    public interface CustomActionProvider {
         ...
         */
        void onCustomAction(Player player, ControlDispatcher controlDispatcher, String action,
                Bundle extras);

        /**
         * Returns a {@link PlaybackStateCompat.CustomAction} which will be published to the media
         * session by the connector or {@code null} if this action should not be published at the
         * given player state.
         *
         * @param player The player connected to the media session.
    ...
    public interface MediaMetadataProvider {
         ...
         * case the app should return a {@link MediaMetadataCompat} object that does not contain
         * these resources as a placeholder. The app should start an asynchronous operation to
         * download the bitmap and put it into a cache. Finally, the app should call
         * {@link #invalidateMediaSessionMetadata()}. This causes this callback to be called again
         * and the app can now return a {@link MediaMetadataCompat} object with all the resources
         * included.
         *
         * @param player The player connected to the media session.
    ...
    public MediaSessionConnector(MediaSessionCompat mediaSession) {
        ...
        mediaMetadataProvider = new DefaultMediaMetadataProvider(mediaSession.getController(),
                /* metadataExtrasPrefix= */ null);
    ...
    /**
     * Sets the player to be connected to the media session. Must be called on the same thread that
     * is used to access the player.
     *
    ...
    /**
     * Sets the {@link MediaButtonEventHandler}. Pass {@code null} if the media button event should
     * be handled by {@link MediaSessionCompat.Callback#onMediaButtonEvent(Intent)}.
     *
     * @param mediaButtonEventHandler The {@link MediaButtonEventHandler}, or null to let the event
     *        be handled by {@link MediaSessionCompat.Callback#onMediaButtonEvent(Intent)}.
     */
    ...
    /**
     * Sets the {@link QueueNavigator} to handle queue navigation actions
     * {@code ACTION_SKIP_TO_NEXT}, {@code ACTION_SKIP_TO_PREVIOUS} and
     * {@code ACTION_SKIP_TO_QUEUE_ITEM}.
     *
     ...
     *
     * <p>
     * This sets the error code via
     * {@link PlaybackStateCompat.Builder#setErrorMessage(int, CharSequence)}. By default, the error
     * code will be set to {@link PlaybackStateCompat#ERROR_CODE_APP_ERROR}.
     *
     ...
     *
     * @param customActionProviders The custom action providers, or null to remove all existing
    ...
    public final void invalidateMediaSessionMetadata() {
        ...
        if (mediaMetadataProvider == null) return;

        MediaMetadataCompat metadata = mediaMetadataProvider != null && player != null
     ...
     *
     * <p>
     * Apps normally only need to call this method when the custom actions provided by a
     * {@link CustomActionProvider} changed and the playback state needs to be updated immediately.
     */
    public final void invalidateMediaSessionPlaybackState() {
        PlaybackStateCompat.Builder builder = new PlaybackStateCompat.Builder();
        if (player == null) {
            builder.setActions(buildPrepareActions()).setState(PlaybackStateCompat.STATE_NONE,
                    /* position= */ 0, /* playbackSpeed= */ 0,
        ...
        for (CustomActionProvider customActionProvider : customActionProviders) {
            PlaybackStateCompat.CustomAction customAction =
                    customActionProvider.getCustomAction(player);
        ...
        int sessionPlaybackState = reportError ? PlaybackStateCompat.STATE_ERROR
                : getMediaSessionPlaybackState(player.getPlaybackState(),
        ...
        long activeQueueItemId =
                queueNavigator != null ? queueNavigator.getActiveQueueItemId(player)
        ...
        builder.setActions(buildPrepareActions() | buildPlaybackActions(player))
                ...
                .setState(sessionPlaybackState, player.getCurrentPosition(), sessionPlaybackSpeed,
                        /* updateTime= */ SystemClock.elapsedRealtime())
                .setExtras(extras);
        mediaSession.setPlaybackState(builder.build());
    }
     ...
     * {@link QueueNavigator#onTimelineChanged(Player)}.
     *
     * <p>
     ...
     * {@link MediaSessionCompat.Callback#onCommand(String, Bundle, ResultReceiver)}.
     *
     * <p>
    ...
    private long buildPrepareActions() {
        return playbackPreparer == null ? 0
    ...
    private boolean canDispatchToQueueNavigator(long action) {
        return player != null && queueNavigator != null
    ...
    private static int getMediaSessionPlaybackState(int exoPlayerPlaybackState,
            boolean playWhenReady) {
        switch (exoPlayerPlaybackState) {
            case Player.STATE_BUFFERING:
                return PlaybackStateCompat.STATE_BUFFERING;
            case Player.STATE_READY:
                return playWhenReady ? PlaybackStateCompat.STATE_PLAYING
    ...
    /**
     * Provides a default {@link MediaMetadataCompat} with properties and extras taken from the
     * {@link MediaDescriptionCompat} of the {@link MediaSessionCompat.QueueItem} of the active
     * queue item.
     */
    public static final class DefaultMediaMetadataProvider implements MediaMetadataProvider {
         ...
         */
        public DefaultMediaMetadataProvider(MediaControllerCompat mediaController,
                        ...
                        if (extras != null) {
                            for (String key : extras.keySet()) {
                                ...
                                if (value instanceof String) {
                                    ...
                                    builder.putText(metadataExtrasPrefix + key,
                                            (CharSequence) value);
                                    ...
                                    builder.putRating(metadataExtrasPrefix + key,
                                            (RatingCompat) value);
                                }
                            }
                        }
    ...
    private class ComponentListener extends MediaSessionCompat.Callback
        ...
        public void onRepeatModeChanged(@Player.RepeatMode int repeatMode) {
            mediaSession.setRepeatMode(
                    repeatMode == Player.REPEAT_MODE_ONE ? PlaybackStateCompat.REPEAT_MODE_ONE
        ...
        public void onPositionDiscontinuity(@Player.DiscontinuityReason int reason) {
            if (currentWindowIndex != player.getCurrentWindowIndex()) {
                ...
                // Update playback state after queueNavigator.onCurrentWindowIndexChanged has been
                // called
        ...
        public void onCustomAction(@NonNull String action, @Nullable Bundle extras) {
            if (player != null && customActionMap.containsKey(action)) {
                customActionMap.get(action).onCustomAction(player, controlDispatcher, action,
                        extras);
                invalidateMediaSessionPlaybackState();
            }
        }
        ...
        public void onCommand(String command, Bundle extras, ResultReceiver cb) {
            if (player != null) {
                for (int i = 0; i < commandReceivers.size(); i++) {
                    if (commandReceivers.get(i).onCommand(player, controlDispatcher, command,
                            extras, cb)) {
                        return;
                    }
                }
                for (int i = 0; i < customCommandReceivers.size(); i++) {
                    if (customCommandReceivers.get(i).onCommand(player, controlDispatcher, command,
                            extras, cb)) {
                        return;
                    }
                }
        ...
        public boolean onMediaButtonEvent(Intent mediaButtonEvent) {
            boolean isHandled = canDispatchMediaButtonEvent() && mediaButtonEventHandler
                    .onMediaButtonEvent(player, controlDispatcher, mediaButtonEvent);
