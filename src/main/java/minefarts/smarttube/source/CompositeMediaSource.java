...
public abstract class CompositeMediaSource<T> extends BaseMediaSource {
     ...
     */
    protected abstract void onChildSourceInfoRefreshed(T id, MediaSource mediaSource,
    ...
    protected final void prepareChildSource(final T id, MediaSource mediaSource) {
        ...
        SourceInfoRefreshListener sourceListener = (source, timeline,
                manifest) -> onChildSourceInfoRefreshed(id, source, timeline, manifest);
        MediaSourceEventListener eventListener = new ForwardingEventListener(id);
        childSources.put(id,
                new MediaSourceAndListener(mediaSource, sourceListener, eventListener));
    ...
    /**
     * Returns the window index in the composite source corresponding to the specified window index
     * in a child source. The default implementation does not change the window index.
     *
     * @param id The unique id used to prepare the child source.
    ...
    /**
     * Returns the {@link MediaPeriodId} in the composite source corresponding to the specified
     * {@link MediaPeriodId} in a child source. The default implementation does not change the media
     * period id.
     *
     * @param id The unique id used to prepare the child source.
     ...
     */
    protected @Nullable MediaPeriodId getMediaPeriodIdForChildMediaPeriodId(T id,
    ...
    private final class ForwardingEventListener implements MediaSourceEventListener {
        ...
        @Override
        public void onLoadStarted(int windowIndex, @Nullable MediaPeriodId mediaPeriodId,
                LoadEventInfo loadEventData, MediaLoadData mediaLoadData) {
            if (maybeUpdateEventDispatcher(windowIndex, mediaPeriodId)) {
        ...
        @Override
        public void onLoadCompleted(int windowIndex, @Nullable MediaPeriodId mediaPeriodId,
                LoadEventInfo loadEventData, MediaLoadData mediaLoadData) {
            if (maybeUpdateEventDispatcher(windowIndex, mediaPeriodId)) {
                eventDispatcher.loadCompleted(loadEventData,
                        maybeUpdateMediaLoadData(mediaLoadData));
            }
        }

        @Override
        public void onLoadCanceled(int windowIndex, @Nullable MediaPeriodId mediaPeriodId,
                LoadEventInfo loadEventData, MediaLoadData mediaLoadData) {
            if (maybeUpdateEventDispatcher(windowIndex, mediaPeriodId)) {
                eventDispatcher.loadCanceled(loadEventData,
                        maybeUpdateMediaLoadData(mediaLoadData));
            }
        }

        @Override
        public void onLoadError(int windowIndex, @Nullable MediaPeriodId mediaPeriodId,
                LoadEventInfo loadEventData, MediaLoadData mediaLoadData, IOException error,
                boolean wasCanceled) {
            if (maybeUpdateEventDispatcher(windowIndex, mediaPeriodId)) {
                eventDispatcher.loadError(loadEventData, maybeUpdateMediaLoadData(mediaLoadData),
                        error, wasCanceled);
            }
        }
        ...
        @Override
        public void onUpstreamDiscarded(int windowIndex, @Nullable MediaPeriodId mediaPeriodId,
        ...
        public void onDownstreamFormatChanged(int windowIndex,
                @Nullable MediaPeriodId mediaPeriodId, MediaLoadData mediaLoadData) {
            if (maybeUpdateEventDispatcher(windowIndex, mediaPeriodId)) {
        ...
        private boolean maybeUpdateEventDispatcher(int childWindowIndex,
                @Nullable MediaPeriodId childMediaPeriodId) {
            ...
            if (eventDispatcher.windowIndex != windowIndex
                    || !Utils.areEqual(eventDispatcher.mediaPeriodId, mediaPeriodId)) {
                eventDispatcher = createEventDispatcher(windowIndex, mediaPeriodId,
                        /* mediaTimeOffsetMs= */ 0);
            }
            return true;
        }
        ...
        private MediaLoadData maybeUpdateMediaLoadData(MediaLoadData mediaLoadData) {
            long mediaStartTimeMs =
                    getMediaTimeForChildMediaTime(id, mediaLoadData.mediaStartTimeMs);
            ...
            return new MediaLoadData(mediaLoadData.dataType, mediaLoadData.trackType,
