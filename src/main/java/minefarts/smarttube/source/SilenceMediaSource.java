...
public final class SilenceMediaSource extends BaseMediaSource {
  ...
  private static final byte[] SILENCE_SAMPLE =
      new byte[Utils.getPcmFrameSize(ENCODING, CHANNEL_COUNT) * 1024];
  ...
  private static final class SilenceMediaPeriod implements MediaPeriod {
    ...
    private long constrainSeekPosition(long positionUs) {
      return Utils.constrainValue(positionUs, 0, durationUs);
    }
  }
  ...
  private static final class SilenceSampleStream implements SampleStream {
    ...
    public void seekTo(long positionUs) {
      positionBytes = Utils.constrainValue(getAudioByteCount(positionUs), 0, durationBytes);
    }
  ...
  private static long getAudioByteCount(long durationUs) {
    ...
    return Utils.getPcmFrameSize(ENCODING, CHANNEL_COUNT) * audioSampleCount;
  }

  private static long getAudioPositionUs(long bytes) {
    long audioSampleCount = bytes / Utils.getPcmFrameSize(ENCODING, CHANNEL_COUNT);
