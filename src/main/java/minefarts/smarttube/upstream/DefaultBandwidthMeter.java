...
import minefarts.smarttube.utils.SlidingPercentile;
 ...
 *
 * <p>
 * The bandwidth estimate is calculated using a {@link SlidingPercentile} and is updated each time a
 * transfer ends. The initial estimate is based on the current operator's network country code or
 * the locale of the user, as well as the network connection type. This can be configured in the
 * {@link Builder}.
 */
public final class DefaultBandwidthMeter implements BandwidthMeter, TransferListener {

    /**
     * Country groups used to determine the default initial bitrate estimate. The group assignment
     * for each country is an array of group indices for [Wifi, 2G, 3G, 4G].
     */
    ...
    /**
     * Default initial bitrate estimate used when the device is offline or the network type cannot
     * be determined, in bits per second.
     */
    ...
    public static final class Builder {
        ...
        public Builder(Context context) {
            ...
            initialBitrateEstimates =
                    getInitialBitrateEstimatesForCountry(Utils.getCountryCode(context));
        ...
        /**
         * Sets the initial bitrate estimate in bits per second that should be assumed when a
        ...
        /**
         * Sets the initial bitrate estimate in bits per second that should be assumed when a
         * bandwidth estimate is unavailable and the current network connection is of the specified
         * type.
         *
         ...
         * @return This builder.
         */
        public Builder setInitialBitrateEstimate(@C.NetworkType int networkType,
         ...
         *
         * @param countryCode The ISO 3166-1 alpha-2 country code of the country whose default
         *        bitrate estimates should be used.
         * @return This builder.
         */
        public Builder setInitialBitrateEstimate(String countryCode) {
            initialBitrateEstimates =
                    getInitialBitrateEstimatesForCountry(Utils.toUpperInvariant(countryCode));
            return this;
        }

        /**
         * Sets the clock used to estimate bandwidth from data transfers. Should only be set for
         * testing purposes.
         *
        ...
        public DefaultBandwidthMeter build() {
            return new DefaultBandwidthMeter(context, initialBitrateEstimates,
                    slidingWindowMaxWeight, clock, resetOnNetworkTypeChange);
        }
        ...
        private static SparseArray<Long> getInitialBitrateEstimatesForCountry(String countryCode) {
            ...
            result.append(C.NETWORK_TYPE_WIFI,
                    DEFAULT_INITIAL_BITRATE_ESTIMATES_WIFI[groupIndices[0]]);
            ...
            // Assume default Wifi bitrate for Ethernet to prevent using the slower fallback
            // bitrate.
            result.append(C.NETWORK_TYPE_ETHERNET,
                    DEFAULT_INITIAL_BITRATE_ESTIMATES_WIFI[groupIndices[0]]);
            return result;
        }
    ...
    public DefaultBandwidthMeter() {
        this(/* context= */ null, /* initialBitrateEstimates= */ new SparseArray<>(),
                DEFAULT_SLIDING_WINDOW_MAX_WEIGHT, Clock.DEFAULT,
                /* resetOnNetworkTypeChange= */ false);
    }

    private DefaultBandwidthMeter(@Nullable Context context,
            SparseArray<Long> initialBitrateEstimates, int maxWeight, Clock clock,
        ...
        networkType = context == null ? C.NETWORK_TYPE_UNKNOWN : Utils.getNetworkType(context);
    ...
    /**
     * Overrides the network type. Handled in the same way as if the meter had detected a change
     * from the current network type to the specified network type internally.
     *
     * <p>
    ...
    public synchronized void onTransferEnd(DataSource source, DataSpec dataSpec,
        ...
        if (sampleElapsedTimeMs > 0) {
            ...
            maybeNotifyBandwidthSample(sampleElapsedTimeMs, sampleBytesTransferred,
                    bitrateEstimate);
    ...
    /*
     * Note: This class only holds a weak reference to DefaultBandwidthMeter instances. It should
     * not be made non-static, since doing so adds a strong reference (i.e.
    ...
    private static class ConnectivityActionReceiver extends BroadcastReceiver {
        ...
        public synchronized void register(DefaultBandwidthMeter bandwidthMeter) {
            ...
            // Simulate an initial update on the main thread (like the sticky broadcast we'd receive
            // if
        ...
        public synchronized void onReceive(Context context, Intent intent) {
            ...
            for (int i = 0; i < bandwidthMeters.size(); i++) {
                WeakReference<DefaultBandwidthMeter> bandwidthMeterReference =
                        bandwidthMeters.get(i);
