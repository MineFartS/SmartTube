...
import minefarts.smarttube.hls.playlist.HlsMasterPlaylist;
...
import minefarts.smarttube.hls.playlist.HlsPlaylistTracker;
...
public final class HlsMediaPeriod implements MediaPeriod, HlsSampleStreamWrapper.Callback,
  ...
  private void buildAndPrepareMainSampleStreamWrapper(
    ...
    for (int i = 0; i < masterPlaylist.variants.size(); i++) {
      ...
      if (format.height > 0 || Utils.getCodecsOfType(format.codecs, C.TRACK_TYPE_VIDEO) != null) {
        variantTypes[i] = C.TRACK_TYPE_VIDEO;
        videoVariantCount++;
      } else if (Utils.getCodecsOfType(format.codecs, C.TRACK_TYPE_AUDIO) != null) {
    ...
    if (allowChunklessPreparation && codecs != null) {
      boolean variantsContainVideoCodecs = Utils.getCodecsOfType(codecs, C.TRACK_TYPE_VIDEO) != null;
      boolean variantsContainAudioCodecs = Utils.getCodecsOfType(codecs, C.TRACK_TYPE_AUDIO) != null;
  ...
  private void buildAndPrepareAudioSampleStreamWrappers(
    ...
    for (int renditionByNameIndex = 0;
      ...
      for (int renditionIndex = 0; renditionIndex < audioRenditions.size(); renditionIndex++) {
        if (Utils.areEqual(name, audioRenditions.get(renditionIndex).name)) {
      ...
      HlsSampleStreamWrapper sampleStreamWrapper =
          buildSampleStreamWrapper(
              ...
              overridingDrmInitData,
              positionUs);
      manifestUrlsIndicesPerWrapper.add(Utils.toArray(scratchIndicesList));
      sampleStreamWrappers.add(sampleStreamWrapper);
  ...
  private static Format deriveVideoFormat(Format variantFormat) {
    String codecs = Utils.getCodecsOfType(variantFormat.codecs, C.TRACK_TYPE_VIDEO);
  ...
  private static Format deriveAudioFormat(
    ...
    if (mediaTagFormat != null) {
      ...
      codecs = Utils.getCodecsOfType(variantFormat.codecs, C.TRACK_TYPE_AUDIO);
