...
import minefarts.smarttube.source.UnrecognizedInputFormatException;
import minefarts.smarttube.hls.HlsTrackMetadataEntry;
import minefarts.smarttube.hls.HlsTrackMetadataEntry.VariantInfo;
...
public final class HlsPlaylistParser implements ParsingLoadable.Parser<HlsPlaylist> {
  ...
  private static boolean checkPlaylistHeader(BufferedReader reader) throws IOException {
    ...
    return Utils.isLinebreak(last);
  }

  private static int skipIgnorableWhitespace(BufferedReader reader, boolean skipLinebreaks, int c)
      throws IOException {
    while (c != -1 && Character.isWhitespace(c) && (skipLinebreaks || !Utils.isLinebreak(c))) {
      c = reader.read();
    }
    return c;
  }
      ...
      switch (parseStringAttr(line, REGEX_TYPE, variableDefinitions)) {
        case TYPE_VIDEO:
          ...
          if (variant != null) {
            ...
            codecs = Utils.getCodecsOfType(variantFormat.codecs, C.TRACK_TYPE_VIDEO);
        ...
        case TYPE_AUDIO:
          ...
          codecs =
              variant != null
                  ? Utils.getCodecsOfType(variant.format.codecs, C.TRACK_TYPE_AUDIO)
                  : null;
      ...
      if (line.startsWith(TAG_PLAYLIST_TYPE)) {
        ...
        if (playlistStartTimeUs == 0) {
          long programDatetimeUs =
              C.msToUs(Utils.parseXsDateTime(line.substring(line.indexOf(':') + 1)));
  ...
  private static int parseRoleFlags(String line, Map<String, String> variableDefinitions) {
    ...
    String[] characteristics = Utils.split(concatenatedCharacteristics, ",");
    @C.RoleFlags int roleFlags = 0;
    if (Utils.contains(characteristics, "public.accessibility.describes-video")) {
      roleFlags |= C.ROLE_FLAG_DESCRIBES_VIDEO;
    }
    if (Utils.contains(characteristics, "public.accessibility.transcribes-spoken-dialog")) {
      roleFlags |= C.ROLE_FLAG_TRANSCRIBES_DIALOG;
    }
    if (Utils.contains(characteristics, "public.accessibility.describes-music-and-sound")) {
      roleFlags |= C.ROLE_FLAG_DESCRIBES_MUSIC_AND_SOUND;
    }
    if (Utils.contains(characteristics, "public.easy-to-read")) {
  ...
  private static int parseChannelsAttribute(String line, Map<String, String> variableDefinitions) {
    ...
    return channelsString != null
        ? Integer.parseInt(Utils.splitAtFirst(channelsString, "/")[0])
        : Format.NO_VALUE;
  }
  ...
  private static SchemeData parseDrmSchemeData(
    ...
    if (KEYFORMAT_WIDEVINE_PSSH_BINARY.equals(keyFormat)) {
      ...
      return new SchemeData(C.WIDEVINE_UUID, "hls", Utils.getUtf8Bytes(line));
