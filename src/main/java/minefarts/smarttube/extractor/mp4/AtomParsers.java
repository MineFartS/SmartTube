...
import static minefarts.smarttube.utils.MimeTypes.getMimeTypeFromMp4ObjectType;
...
import minefarts.smarttube.audio.Ac3Util;
...
import minefarts.smarttube.video.AvcConfig;
import minefarts.smarttube.video.DolbyVisionConfig;
import minefarts.smarttube.video.HevcConfig;
...
/* package */ final class AtomParsers {
  ...
  private static final int TYPE_vide = Utils.getIntegerCodeForString("vide");
  private static final int TYPE_soun = Utils.getIntegerCodeForString("soun");
  private static final int TYPE_text = Utils.getIntegerCodeForString("text");
  private static final int TYPE_sbtl = Utils.getIntegerCodeForString("sbtl");
  private static final int TYPE_subt = Utils.getIntegerCodeForString("subt");
  private static final int TYPE_clcp = Utils.getIntegerCodeForString("clcp");
  private static final int TYPE_meta = Utils.getIntegerCodeForString("meta");
  private static final int TYPE_mdta = Utils.getIntegerCodeForString("mdta");
  ...
  private static final byte[] opusMagic = Utils.getUtf8Bytes("OpusHead");
  ...
  public static Track parseTrak(Atom.ContainerAtom trak, Atom.LeafAtom mvhd, long duration,
    ...
    if (duration == C.TIME_UNSET) {
      ...
      durationUs = Utils.scaleLargeTimestamp(duration, C.MICROS_PER_SECOND, movieTimescale);
    }
  ...
  public static TrackSampleTable parseStbl(
    ...
    if (!isFixedSampleSizeRawAudio) {
      ...
      int fixedSampleSize =
          Utils.getPcmFrameSize(track.format.pcmEncoding, track.format.channelCount);
    ...
    long durationUs = Utils.scaleLargeTimestamp(duration, C.MICROS_PER_SECOND, track.timescale);

    if (track.editListDurations == null || gaplessInfoHolder.hasGaplessInfo()) {
      // There is no edit list, or we are ignoring it as we already have gapless metadata to apply.
      // This implementation does not support applying both gapless metadata and an edit list.
      Utils.scaleLargeTimestampsInPlace(timestamps, C.MICROS_PER_SECOND, track.timescale);
      return new TrackSampleTable(
    ...
    if (track.editListDurations.length == 1
      ...
      long editEndTime = editStartTime + Utils.scaleLargeTimestamp(track.editListDurations[0],
      ...
      if (canApplyEditWithGaplessInfo(timestamps, duration, editStartTime, editEndTime)) {
        ...
        long encoderDelay = Utils.scaleLargeTimestamp(editStartTime - timestamps[0],
            track.format.sampleRate, track.timescale);
        long encoderPadding = Utils.scaleLargeTimestamp(paddingTimeUnits,
            track.format.sampleRate, track.timescale);
    ...
    if (track.editListDurations.length == 1 && track.editListDurations[0] == 0) {
      ...
      durationUs =
          Utils.scaleLargeTimestamp(duration - editStartTime, C.MICROS_PER_SECOND, track.timescale);
      return new TrackSampleTable(
          track, offsets, sizes, maximumSize, timestamps, flags, durationUs);
    }
    ...
    for (int i = 0; i < track.editListDurations.length; i++) {
      ...
      if (editMediaTime != -1) {
        ...
        startIndices[i] = Utils.binarySearchCeil(timestamps, editMediaTime, true, true);
        endIndices[i] =
            Utils.binarySearchCeil(
    ...
    for (int i = 0; i < track.editListDurations.length; i++) {
      ...
      for (int j = startIndex; j < endIndex; j++) {
        long ptsUs = Utils.scaleLargeTimestamp(pts, C.MICROS_PER_SECOND, track.movieTimescale);
    ...
    long editedDurationUs =
        Utils.scaleLargeTimestamp(pts, C.MICROS_PER_SECOND, track.movieTimescale);
    return new TrackSampleTable(
        track,
        editedOffsets,
        editedSizes,
  ...
  private static boolean canApplyEditWithGaplessInfo(
    ...
    int latestDelayIndex = Utils.constrainValue(MAX_GAPLESS_TRIM_SIZE_SAMPLES, 0, lastIndex);
    int earliestPaddingIndex =
        Utils.constrainValue(timestamps.length - MAX_GAPLESS_TRIM_SIZE_SAMPLES, 0, lastIndex);
