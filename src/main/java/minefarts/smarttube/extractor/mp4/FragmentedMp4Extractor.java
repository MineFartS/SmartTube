...
import minefarts.smarttube.extractor.mp4.Atom.ContainerAtom;
import minefarts.smarttube.extractor.mp4.Atom.LeafAtom;
...
public class FragmentedMp4Extractor implements Extractor {
  ...
  private static final int SAMPLE_GROUP_TYPE_seig = Utils.getIntegerCodeForString("seig");
    ...
    switch (version) {
      case 0:
        ...
        presentationTimeDeltaUs =
            Utils.scaleLargeTimestamp(atom.readUnsignedInt(), C.MICROS_PER_SECOND, timescale);
        if (segmentIndexEarliestPresentationTimeUs != C.TIME_UNSET) {
          sampleTimeUs = segmentIndexEarliestPresentationTimeUs + presentationTimeDeltaUs;
        }
        durationMs =
            Utils.scaleLargeTimestamp(atom.readUnsignedInt(), C.MILLIS_PER_SECOND, timescale);
        id = atom.readUnsignedInt();
        break;
      case 1:
        timescale = atom.readUnsignedInt();
        sampleTimeUs =
            Utils.scaleLargeTimestamp(atom.readUnsignedLongToLong(), C.MICROS_PER_SECOND, timescale);
        durationMs =
            Utils.scaleLargeTimestamp(atom.readUnsignedInt(), C.MILLIS_PER_SECOND, timescale);
        id = atom.readUnsignedInt();
        schemeIdUri = Assertions.checkNotNull(atom.readNullTerminatedString());
  ...
  private static int parseTrun(TrackBundle trackBundle, int index, long decodeTime,
    ...
    for (int i = trackRunStart; i < trackRunEnd; i++) {
      ...
      sampleDecodingTimeTable[i] =
          Utils.scaleLargeTimestamp(cumulativeTime, C.MILLIS_PER_SECOND, timescale) - edtsOffset;
  ...
  private static Pair<Long, ChunkIndex> parseSidx(ParsableByteArray atom, long inputPosition)
    ...
    long earliestPresentationTimeUs = Utils.scaleLargeTimestamp(earliestPresentationTime,
    ...
    for (int i = 0; i < referenceCount; i++) {
      ...
      timeUs = Utils.scaleLargeTimestamp(time, C.MICROS_PER_SECOND, timescale);
