...
public final class WebvttCueParser {
    ...
    /**
     * Parses the next valid WebVTT cue in a parsable array, including timestamps, settings and
     * text.
     *
    ...
    /**
     * Parses the text payload of a WebVTT Cue and applies modifications on
     * {@link WebvttCue.Builder}.
     *
                ...
                case CHAR_LESS_THAN:
                    ...
                    if (isClosingTag) {
                        ...
                        do {
                            ...
                            applySpansForTag(id, startTag, spannedText, styles,
                                    scratchStyleMatches);
                        } while (!startTag.name.equals(tagName));
                    } else if (!isVoidTag) {
                        startTagStack.push(
                                StartTag.buildStartTag(fullTagExpression, spannedText.length()));
                    }
                    break;
                case CHAR_AMPERSAND:
    ...
    private static boolean parseCue(String id, Matcher cueHeaderMatcher,
            ParsableByteArray webvttData, WebvttCue.Builder builder, StringBuilder textBuilder,
            List<WebvttCssStyle> styles) {
        try {
    ...
    private static void parseLineAttribute(String s, WebvttCue.Builder builder)
        ...
        if (s.endsWith("%")) {
            builder.setLine(WebvttParserUtil.parsePercentage(s))
                    .setLineType(Cue.LINE_TYPE_FRACTION);
        } else {
            int lineNumber = Integer.parseInt(s);
            if (lineNumber < 0) {
                // WebVTT defines line -1 as last visible row when lineAnchor is ANCHOR_TYPE_START,
                // where-as
    ...
    /**
     * Find end of tag (&gt;). The position returned is the position of the &gt; plus one
     * (exclusive).
     *
    ...
    private static void applySpansForTag(String cueId, StartTag startTag,
            SpannableStringBuilder text, List<WebvttCssStyle> styles,
    ...
    private static void applyStyleToText(SpannableStringBuilder spannedText, WebvttCssStyle style,
        ...
        if (style.isLinethrough()) {
            spannedText.setSpan(new StrikethroughSpan(), start, end,
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        switch (style.getFontSizeUnit()) {
            case WebvttCssStyle.FONT_SIZE_UNIT_PIXEL:
                spannedText.setSpan(new AbsoluteSizeSpan((int) style.getFontSize(), true), start,
    ...
    private static String getTagName(String tagExpression) {
        ...
        return Utils.splitAtFirst(tagExpression, "[ \\.]")[0];
    }
    ...
    private static final class StartTag {
        ...
        public static StartTag buildStartTag(String fullTagExpression, int position) {
            ...
            String[] nameAndClasses = Utils.split(fullTagExpression, "\\.");
