...
import minefarts.smarttube.C.Encoding;
...
public final class SonicAudioProcessor implements AudioProcessor {
  ...
  public float setSpeed(float speed) {
    speed = Utils.constrainValue(speed, MINIMUM_SPEED, MAXIMUM_SPEED);
    if (this.speed != speed) {
      this.speed = speed;
  ...
  public long scaleDurationForSpeedup(long duration) {
    if (outputBytes >= MIN_BYTES_FOR_SPEEDUP_CALCULATION) {
      return outputSampleRateHz == sampleRateHz
          ? Utils.scaleLargeTimestamp(duration, inputBytes, outputBytes)
          : Utils.scaleLargeTimestamp(duration, inputBytes * outputSampleRateHz,
