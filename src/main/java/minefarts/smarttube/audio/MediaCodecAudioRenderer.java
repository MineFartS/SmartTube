...
import minefarts.smarttube.audio.AudioRendererEventListener.EventDispatcher;
...
import minefarts.smarttube.mediacodec.MediaCodecInfo;
import minefarts.smarttube.mediacodec.MediaCodecRenderer;
import minefarts.smarttube.mediacodec.MediaCodecSelector;
...
import minefarts.smarttube.utils.MediaClock;
 ...
 *       message payload should be an {@link minefarts.smarttube.audio.AudioAttributes}
...
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
  ...
  protected int supportsFormat(MediaCodecSelector mediaCodecSelector,
    ...
    int tunnelingSupport = Utils.SDK_INT >= 21 ? TUNNELING_SUPPORTED : TUNNELING_NOT_SUPPORTED;
  ...
  private int getCodecMaxInputSize(MediaCodecInfo codecInfo, Format format) {
    if ("OMX.google.raw.decoder".equals(codecInfo.name)) {
      ...
      if (Utils.SDK_INT < 24 && !(Utils.SDK_INT == 23 && Utils.isTv(context))) {
  ...
  protected boolean areCodecConfigurationCompatible(Format oldFormat, Format newFormat) {
    return Utils.areEqual(oldFormat.sampleMimeType, newFormat.sampleMimeType)
  ...
  protected MediaFormat getMediaFormat(
    ...
    if (Utils.SDK_INT <= 28 && MimeTypes.AUDIO_AC4.equals(format.sampleMimeType)) {
  ...
  private static boolean deviceDoesntSupportOperatingRate() {
    return Utils.SDK_INT == 23
        && ("ZTE B2017G".equals(Utils.MODEL) || "AXON 7 mini".equals(Utils.MODEL));
  }
  ...
  private static boolean codecNeedsDiscardChannelsWorkaround(String codecName) {
    ...
    return Utils.SDK_INT < 24 && "OMX.SEC.aac.dec".equals(codecName)
        && "samsung".equals(Utils.MANUFACTURER)
        && (Utils.DEVICE.startsWith("zeroflte") || Utils.DEVICE.startsWith("herolte")
        || Utils.DEVICE.startsWith("heroqlte"));
  }
  ...
  private static boolean codecNeedsEosBufferTimestampWorkaround(String codecName) {
    return Utils.SDK_INT < 21
        && "OMX.SEC.mp3.dec".equals(codecName)
        && "samsung".equals(Utils.MANUFACTURER)
        && (Utils.DEVICE.startsWith("baffin")
            || Utils.DEVICE.startsWith("grand")
            || Utils.DEVICE.startsWith("fortuna")
            || Utils.DEVICE.startsWith("gprimelte")
            || Utils.DEVICE.startsWith("j2y18lte")
            || Utils.DEVICE.startsWith("ms01"));
  }
