...
public final class DefaultAudioSink implements AudioSink {
     ...
     * applying playback parameters (if supported). Because applying playback parameters can skip
     * and stretch/compress audio, the sink will query the chain for information on how to transform
     * its output position to map it onto a media position, via {@link #getMediaDuration(long)} and
     * {@link #getSkippedOutputFrameCount()}.
     */
    public interface AudioProcessorChain {

        /**
         * Returns the fixed chain of audio processors that will process audio. This method is
         * called once during initialization, but audio processors may change state to become
         * active/inactive during playback.
         */
        AudioProcessor[] getAudioProcessors();

        /**
         * Configures audio processors to apply the specified playback parameters immediately,
         * returning the new parameters, which may differ from those passed in. Only called when
        ...
        /**
         * Scales the specified playout duration to take into account speedup due to audio
         * processing, returning an input media duration, in arbitrary units.
         */
    ...
    /**
     * The default audio processor chain, which applies a (possibly empty) chain of user-defined
     * audio processors followed by {@link SilenceSkippingAudioProcessor} and
    ...
    public static class DefaultAudioProcessorChain implements AudioProcessorChain {
        ...
        public DefaultAudioProcessorChain(AudioProcessor... audioProcessors) {
            // The passed-in type may be more specialized than AudioProcessor[], so allocate a new
            // array
            // rather than using Arrays.copyOf.
            this.audioProcessors = new AudioProcessor[audioProcessors.length + 2];
            System.arraycopy(/* src= */ audioProcessors, /* srcPos= */ 0,
                    /* dest= */ this.audioProcessors, /* destPos= */ 0,
     ...
     *
     * @param audioCapabilities The audio capabilities for playback on this device. May be null if
     ...
     *        integer PCM to 32-bit float for output, if possible. Functionality that uses 16-bit
     *        integer audio processing (for example, speed and pitch adjustment) will not be
     *        available when float output is in use.
     */
    public DefaultAudioSink(@Nullable AudioCapabilities audioCapabilities,
            AudioProcessor[] audioProcessors, boolean enableConvertHighResIntPcmToFloat) {
    ...
    public DefaultAudioSink(
        ...
        Collections.addAll(toIntPcmAudioProcessors, new ResamplingAudioProcessor(),
    ...
    public boolean supportsOutput(int channelCount, @C.Encoding int encoding) {
        if (Utils.isEncodingLinearPcm(encoding)) {
            // AudioTrack supports 16-bit integer PCM output in all platform API versions, and float
            // output from platform API version 21 only. Other integer PCM encodings are resampled
            // by this
            ...
            return encoding != C.ENCODING_PCM_FLOAT || Utils.SDK_INT >= 21;
        } else {
    ...
    public void configure(@C.Encoding int inputEncoding, int inputChannelCount, int inputSampleRate,
        ...
        if (Utils.SDK_INT < 21 && inputChannelCount == 8 && outputChannels == null) {
        ...
        boolean isInputPcm = Utils.isEncodingLinearPcm(inputEncoding);
        boolean processingEnabled = isInputPcm && inputEncoding != C.ENCODING_PCM_FLOAT;
        int sampleRate = inputSampleRate;
        int channelCount = inputChannelCount;
        @C.Encoding
        int encoding = inputEncoding;
        boolean shouldConvertHighResIntPcmToFloat = enableConvertHighResIntPcmToFloat
                && supportsOutput(inputChannelCount, C.ENCODING_PCM_FLOAT)
                && Utils.isEncodingHighResolutionIntegerPcm(inputEncoding);
        AudioProcessor[] availableAudioProcessors =
                shouldConvertHighResIntPcmToFloat ? toFloatPcmAvailableAudioProcessors
        ...
        int inputPcmFrameSize = isInputPcm ? Utils.getPcmFrameSize(inputEncoding, inputChannelCount)
                : C.LENGTH_UNSET;
        int outputPcmFrameSize =
                isInputPcm ? Utils.getPcmFrameSize(encoding, channelCount) : C.LENGTH_UNSET;
        boolean canApplyPlaybackParameters =
                processingEnabled && !shouldConvertHighResIntPcmToFloat;
        Configuration pendingConfiguration = new Configuration(isInputPcm, inputPcmFrameSize,
                inputSampleRate, outputPcmFrameSize, sampleRate, outputChannelConfig, encoding,
                specifiedBufferSize, processingEnabled, canApplyPlaybackParameters,
                availableAudioProcessors);
        // If we have a pending configuration already, we always drain audio processors as the
        // preceding
        ...
        if (isInitialized() && (!pendingConfiguration.canReuseAudioTrack(configuration)
                || drainAudioProcessors)) {
    ...
    private void initialize(long presentationTimeUs) throws InitializationException {
        ...
        // track instances. Without this guarantee it would be possible, in extreme cases, to
        // exhaust
        ...
        audioTrackPositionTracker.setAudioTrack(audioTrack, configuration.outputEncoding,
                configuration.outputPcmFrameSize, configuration.bufferSize,
        ...
        if (inputBuffer == null) {
            ...
            if (!configuration.isInputPcm && framesPerEncodedSample == 0) {
                ...
                if (framesPerEncodedSample == 0) {
                    ...
                    // For TrueHD this can occur after some seek operations, as not every sample
                    // starts with
                    // a syncframe header. If we chunked samples together so the extracted samples
                    // always
            ...
            if (startMediaTimeState == START_NOT_SET) {
                ...
                long expectedPresentationTimeUs = startMediaTimeUs
                        + configuration.inputFramesToDurationUs(getSubmittedFrames()
                                - trimmingAudioProcessor.getTrimmedFrameCount());
                if (startMediaTimeState == START_IN_SYNC
                        && Math.abs(expectedPresentationTimeUs - presentationTimeUs) > 200000) {
                    log.w("Discontinuity detected [expected " + expectedPresentationTimeUs
                            + ", got " + presentationTimeUs + "]");
                    startMediaTimeState = START_NEED_SYNC;
                }
                if (startMediaTimeState == START_NEED_SYNC) {
                    // Adjust startMediaTimeUs to be consistent with the current buffer's start time
                    // and the
    ...
    @SuppressWarnings("ReferenceEquality")
    private void writeBuffer(ByteBuffer buffer, long avSyncPresentationTimeUs)
        ...
        if (outputBuffer != null) {
            ...
            if (Utils.SDK_INT < 21 || applyDolbyPassthroughQuirk()) { // AMZN_CHANGE_ONELINE
        ...
        if (applyDolbyPassthroughQuirk()) {
            ...
            bytesWritten =
                    audioTrack.write(preV21OutputBuffer, preV21OutputBufferOffset, bytesRemaining);
            if (bytesWritten > 0) {
                preV21OutputBufferOffset += bytesWritten;
                buffer.position(buffer.position() + bytesWritten);
            }
        } else if (Utils.SDK_INT < 21) { // isInputPcm == true
            // AMZN_CHANGE_END
    ...
    /**
     * Plays out remaining audio. {@link #isEnded()} will return {@code true} when playback has
     * ended.
     *
    ...
    public boolean isEnded() {
        return !isInitialized()
                || (handledEndOfStream && (applyDolbyPassthroughQuirk() || !hasPendingData()));// AMZN_CHANGE_ONELINE
    }
    ...
    public PlaybackParameters setPlaybackParameters(PlaybackParameters playbackParameters) {
        ...
        PlaybackParameters lastSetPlaybackParameters =
                afterDrainPlaybackParameters != null ? afterDrainPlaybackParameters
        ...
        if (!playbackParameters.equals(lastSetPlaybackParameters)) {
            if (isInitialized()) {
                // Drain the audio processors so we can determine the frame position at which the
                // new
                // parameters apply.
                afterDrainPlaybackParameters = playbackParameters;
            } else {
                // Update the playback parameters now. They will be applied to the audio processors
                // during
                // initialization.
    ...
    public void enableTunnelingV21(int tunnelingAudioSessionId) {
        ...
        Assertions.checkState(Utils.SDK_INT >= 21);
    ...
    private void applyPlaybackParameters(PlaybackParameters playbackParameters,
            long presentationTimeUs) {
        PlaybackParameters newPlaybackParameters = configuration.canApplyPlaybackParameters
    ...
    private long applySpeedup(long positionUs) {
        ...
        if (checkpoint != null) {
            // We are playing (or about to play) media with the new playback parameters, so update
            // them.
        ...
        if (playbackParametersCheckpoints.isEmpty()) {
            return playbackParametersOffsetUs + audioProcessorChain
                    .getMediaDuration(positionUs - playbackParametersPositionUs);
        }

        // We are playing data at a previous playback speed, so fall back to multiplying by the
        // speed.
        return playbackParametersOffsetUs + Utils.getMediaDurationForPlayoutDuration(
                positionUs - playbackParametersPositionUs, playbackParameters.speed);
    }

    private long applySkipping(long positionUs) {
        return positionUs + configuration
                .framesToDurationUs(audioProcessorChain.getSkippedOutputFrameCount());
    }
    ...
    private long getSubmittedFrames() {
        return configuration.isInputPcm ? (submittedPcmBytes / configuration.inputPcmFrameSize)
                : submittedEncodedFrames;
    }

    private long getWrittenFrames() {
        return configuration.isInputPcm ? (writtenPcmBytes / configuration.outputPcmFrameSize)
    ...
    private static int getChannelConfig(int channelCount, boolean isInputPcm) {
        if (Utils.SDK_INT <= 28 && !isInputPcm) {
        ...
        if (Utils.SDK_INT <= 26 && "fugu".equals(Utils.DEVICE) && !isInputPcm && channelCount == 1) {
            channelCount = 2;
        }

        return Utils.getAudioTrackChannelConfig(channelCount);
    }
    ...
    private static int getMaximumEncodedRateBytesPerSecond(@C.Encoding int encoding) {
        switch (encoding) {
            ...
            case C.ENCODING_AC4:
                return 2688 * 1000 / 8;
            case C.ENCODING_DTS:
                // DTS allows an 'open' bitrate, but we assume the maximum listed value: 1536
                // kbit/s.
                return 1536 * 1000 / 8;
    ...
    private static int getFramesPerEncodedSample(@C.Encoding int encoding, ByteBuffer buffer) {
        if (encoding == C.ENCODING_DTS || encoding == C.ENCODING_DTS_HD) {
            ...
            return syncframeOffset == C.INDEX_UNSET ? 0
    ...
    private int writeNonBlockingWithAvSyncV21(AudioTrack audioTrack, ByteBuffer buffer, int size,
        ...
        if (avSyncHeaderBytesRemaining > 0) {
            int result =
                    audioTrack.write(avSyncHeader, avSyncHeaderBytesRemaining, WRITE_NON_BLOCKING);
            if (result < 0) {
                bytesUntilNextAvSync = 0;
                return result;
            }
    ...
    private static final class PlaybackParametersCheckpoint {
        ...
        private PlaybackParametersCheckpoint(PlaybackParameters playbackParameters,
                long mediaTimeUs, long positionUs) {
            this.playbackParameters = playbackParameters;
    ...
    private final class PositionTrackerListener implements AudioTrackPositionTracker.Listener {
        ...
        @Override
        public void onPositionFramesMismatch(long audioTimestampPositionFrames,
                long audioTimestampSystemTimeUs, long systemTimeUs, long playbackPositionUs) {
            String message = "Spurious audio timestamp (frame position mismatch): "
                    + audioTimestampPositionFrames + ", " + audioTimestampSystemTimeUs + ", "
                    + systemTimeUs + ", " + playbackPositionUs + ", " + getSubmittedFrames() + ", "
                    + getWrittenFrames();
        ...
        @Override
        public void onSystemTimeUsMismatch(long audioTimestampPositionFrames,
                long audioTimestampSystemTimeUs, long systemTimeUs, long playbackPositionUs) {
    ...
    private static final class Configuration {
        ...
        public Configuration(boolean isInputPcm, int inputPcmFrameSize, int inputSampleRate,
                int outputPcmFrameSize, int outputSampleRate, int outputChannelConfig,
                int outputEncoding, int specifiedBufferSize, boolean processingEnabled,
                boolean canApplyPlaybackParameters, AudioProcessor[] availableAudioProcessors) {
            ...
            this.bufferSize =
                    specifiedBufferSize != 0 ? specifiedBufferSize : getDefaultBufferSize();
        ...
        public AudioTrack buildAudioTrack(boolean tunneling, AudioAttributes audioAttributes,
            ...
            if (Utils.SDK_INT >= 21) {
                ...
                // AMZN_CHANGE_BEGIN
                int streamType = Utils.getStreamTypeForAudioUsage(audioAttributes.usage);
            ...
            if (state != STATE_INITIALIZED) {
                try {
                    ...
                    // The track has already failed to initialize, so it wouldn't be that surprising
                    // if
                    // release were to fail too. Swallow the exception.
                }
                throw new InitializationException(state, outputSampleRate, outputChannelConfig,
                        bufferSize);
            }
            return audioTrack;
        }
        ...
        @TargetApi(21)
        private AudioTrack createAudioTrackV21(boolean tunneling, AudioAttributes audioAttributes,
                int audioSessionId) {
            ...
            AudioFormat format = new AudioFormat.Builder().setChannelMask(outputChannelConfig)
                    .setEncoding(outputEncoding).setSampleRate(outputSampleRate).build();
        ...
        private int getDefaultBufferSize() {
            if (isInputPcm) {
                ...
                return Utils.constrainValue(multipliedBufferSize, minAppBufferSize,
                        maxAppBufferSize);
            } else {
