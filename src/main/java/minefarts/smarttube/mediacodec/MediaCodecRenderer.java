...
import minefarts.smarttube.decoder.DecoderCounters;
import minefarts.smarttube.decoder.DecoderInputBuffer;
import minefarts.smarttube.drm.DrmSession;
import minefarts.smarttube.drm.DrmSession.DrmSessionException;
...
public abstract class MediaCodecRenderer extends BaseRenderer {
    ...
    public static class DecoderInitializationException extends Exception {
        ...
        /**
         * If the decoder failed to initialize and another decoder being used as a fallback also
         * failed to initialize, the {@link DecoderInitializationException} for the fallback
         * decoder. Null if there was no fallback decoder or no suitable decoders were found.
         */
        ...
        public DecoderInitializationException(Format format, Throwable cause,
            ...
            this("Decoder init failed: [" + errorCode + "], " + format, cause,
                    format.sampleMimeType, secureDecoderRequired, /* decoderName= */ null,
        ...
        public DecoderInitializationException(Format format, Throwable cause,
            ...
            this("Decoder init failed: " + decoderName + ", " + format, cause,
                    format.sampleMimeType, secureDecoderRequired, decoderName,
                    Utils.SDK_INT >= 21 ? getDiagnosticInfoV21(cause) : null,
                    /* fallbackDecoderInitializationException= */ null);
        }

        private DecoderInitializationException(String message, Throwable cause, String mimeType,
                boolean secureDecoderRequired, @Nullable String decoderName,
        ...
        private DecoderInitializationException copyWithFallbackException(
            ...
            return new DecoderInitializationException(getMessage(), getCause(), mimeType,
                    secureDecoderRequired, decoderName, diagnosticInfo, fallbackException);
        }
        ...
        private static String buildCustomDiagnosticInfo(int errorCode) {
            ...
            return "com.google.android.exoplayer.MediaCodecTrackRenderer_" + sign
                    + Math.abs(errorCode);
        }
    ...
    /**
     * If the {@link MediaCodec} is hotswapped (i.e. replaced during playback), this is the period
     * of time during which {@link #isReady()} will report true regardless of whether the new codec
     * has output frames that are ready to be rendered.
     * <p>
     * This allows codec hotswapping to be performed seamlessly, without interrupting the playback
     ...
     * {@link #canKeepCodec(MediaCodec, MediaCodecInfo, Format, Format)}.
     */
    @Documented
    @Retention(RetentionPolicy.SOURCE)
    @IntDef({KEEP_CODEC_RESULT_NO, KEEP_CODEC_RESULT_YES_WITH_FLUSH,
    ...
    /**
     * Codec configuration data has been written into the next buffer, but that buffer still needs
     * to be returned to the codec.
     */
    ...
    @Documented
    @Retention(RetentionPolicy.SOURCE)
    @IntDef({DRAIN_ACTION_NONE, DRAIN_ACTION_FLUSH, DRAIN_ACTION_UPDATE_DRM_SESSION,
     ...
     */
    private static final byte[] ADAPTATION_WORKAROUND_BUFFER = new byte[] {0, 0, 1, 103, 66, -64,
            11, -38, 37, -112, 0, 0, 1, 104, -50, 15, 19, 32, 0, 0, 1, 101, -120, -124, 13, -50,
            113, 24, -96, 0, 47, -65, 28, 49, -61, 39, 93, 120};
     ...
     *        initialization fails. This may result in using a decoder that is less efficient or
     *        slower than the primary decoder.
     * @param assumedMinimumCodecOperatingRate A codec operating rate that all codecs instantiated
     *        by this renderer are assumed to meet implicitly (i.e. without the operating rate being
     *        set explicitly using {@link MediaFormat#KEY_OPERATING_RATE}).
     */
     ...
     *
     * @param renderTimeLimitMs The render time limit in milliseconds, or {@link C#TIME_UNSET} for
     *        no limit.
     */
     ...
     */
    protected abstract void configureCodec(MediaCodecInfo codecInfo, MediaCodec codec,
            Format format, MediaCrypto crypto, float codecOperatingRate);
        ...
        if (codecDrmSession != null) {
            if (mediaCrypto == null) {
                ...
                if (sessionMediaCrypto == null) {
                    ...
                    if (drmError != null) {
                        // Continue for now. We may be able to avoid failure if the session
                        // recovers, or if a
                    ...
                    try {
                        mediaCrypto = new MediaCrypto(sessionMediaCrypto.uuid,
                                sessionMediaCrypto.sessionId);
            ...
            if (AmazonQuirks.isFireTVGen1Family()) {
                ...
                if (drmSessionState == DrmSession.STATE_ERROR) {
                    throw ExoPlaybackException.createForRenderer(codecDrmSession.getError(),
                            getIndex());
     ...
     * it is removed and returned. Otherwise returns {@code null}. Subclasses should only call this
    ...
    public void render(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException {
        ...
        if (log.allowVerbose()) {
            log.v("render: positionUs = " + positionUs + ", elapsedRealtimeUs = "
                    + elapsedRealtimeUs);
        }
        ...
        if (codec != null) {
            ...
            // We need to read any format changes despite not having a codec so that drmSession can
            // be
            // updated, and so that we have the most recent format should the codec be initialized.
            // We may
    ...
    /**
     * Flushes the codec. If flushing is not possible, the codec will be released and
     * re-instantiated. This method is a no-op if the codec is {@code null}.
     *
     * <p>
     * The implementation of this method calls {@link #flushOrReleaseCodec()}, and
     * {@link #maybeInitCodec()} if the codec needs to be re-instantiated.
     *
    ...
    protected boolean flushOrReleaseCodec() {
        ...
        if (codecDrainAction == DRAIN_ACTION_REINITIALIZE || codecNeedsFlushWorkaround
        ...
        codecReconfigurationState = codecReconfigured ? RECONFIGURATION_STATE_WRITE_PENDING
    ...
    private void maybeInitCodecWithFallback(MediaCrypto crypto,
            boolean mediaCryptoRequiresSecureDecoder) throws DecoderInitializationException {
        if (availableCodecInfos == null) {
            try {
                ...
                throw new DecoderInitializationException(inputFormat, e,
                        mediaCryptoRequiresSecureDecoder,
        ...
        if (availableCodecInfos.isEmpty()) {
            throw new DecoderInitializationException(inputFormat, /* cause= */ null,
                    mediaCryptoRequiresSecureDecoder,
        ...
        while (codec == null) {
            ...
            try {
                ...
                // This codec failed to initialize, so fall back to the next codec in the list (if
                // any). We
                // won't try to use this codec again unless there's a format change or the renderer
                // is
                ...
                if (preferredDecoderInitializationException == null) {
                    ...
                    preferredDecoderInitializationException =
                            preferredDecoderInitializationException
                                    .copyWithFallbackException(exception);
                }
                if (availableCodecInfos.isEmpty()) {
    ...
    private List<MediaCodecInfo> getAvailableCodecInfos(boolean mediaCryptoRequiresSecureDecoder)
        ...
        if (codecInfos.isEmpty() && mediaCryptoRequiresSecureDecoder) {
            ...
            // have one. Assuming that supportsFormat indicated support for the media being played,
            // we
            ...
            codecInfos = getDecoderInfos(mediaCodecSelector, inputFormat,
                    /* requiresSecureDecoder= */ false);
            if (!codecInfos.isEmpty()) {
                Log.w(TAG,
                        "Drm session requires secure decoder for " + inputFormat.sampleMimeType
    ...
    private void initCodec(MediaCodecInfo codecInfo, MediaCrypto crypto) throws Exception {
        ...
        float codecOperatingRate = Utils.SDK_INT < 23 ? CODEC_OPERATING_RATE_UNSET
    ...
    private boolean feedInputBuffer() throws ExoPlaybackException {
        if (codec == null || codecDrainState == DRAIN_STATE_WAIT_END_OF_STREAM
                || inputStreamEnded) {
            return false;
        }
        ...
        if (codecDrainState == DRAIN_STATE_SIGNAL_END_OF_STREAM) {
            // We need to re-initialize the codec. Send an end of stream signal to the existing
            // codec so
        ...
        if (waitingForKeys) {
            ...
            // For adaptive reconfiguration OMX decoders expect all reconfiguration data to be
            // supplied
        ...
        if (result == C.RESULT_FORMAT_READ) {
            ...
            if (codecReconfigurationState == RECONFIGURATION_STATE_QUEUE_PENDING) {
                // We received two formats in a row. Clear the current buffer of any reconfiguration
                // data
        ...
        if (buffer.isEndOfStream()) {
            ...
            if (codecReconfigurationState == RECONFIGURATION_STATE_QUEUE_PENDING) {
                // We received a new format immediately before the end of the stream. We need to
                // clear
                // the corresponding reconfiguration data from the current buffer, but re-write it
                // into
        ...
        if (waitingForFirstSyncSample && !buffer.isKeyFrame()) {
            buffer.clear();
            if (codecReconfigurationState == RECONFIGURATION_STATE_QUEUE_PENDING) {
                // The buffer we just cleared contained reconfiguration data. We need to re-write
                // this
        ...
        try {
            ...
            if (bufferEncrypted) {
                MediaCodec.CryptoInfo cryptoInfo =
                        getFrameworkCryptoInfo(buffer, adaptiveReconfigurationBytes);
                if (log.allowDebug()) {
                ...
                if (log.allowDebug()) {
                    log.d("queueInputBuffer: inputIndex = " + inputIndex + ", bufferSize = "
                            + buffer.data.limit() + ", presentationTimeUs = " + presentationTimeUs);
                }
    ...
    protected void onInputFormatChanged(Format newFormat) throws ExoPlaybackException {
        ...
        boolean drmInitDataChanged = !Utils.areEqual(newFormat.drmInitData,
                oldFormat == null ? null : oldFormat.drmInitData);
        if (drmInitDataChanged) {
            if (newFormat.drmInitData != null) {
                if (drmSessionManager == null) {
                    throw ExoPlaybackException.createForRenderer(
                            new IllegalStateException("Media requires a DrmSessionManager"),
                            getIndex());
                }
                DrmSession<FrameworkMediaCrypto> session =
                        drmSessionManager.acquireSession(Looper.myLooper(), newFormat.drmInitData);
                if (session == sourceDrmSession || session == codecDrmSession) {
                    // We already had this session. The manager must be reference counting, so
                    // release it once
        ...
        // We have an existing codec that we may need to reconfigure or re-initialize. If the
        // existing
        ...
        if ((sourceDrmSession == null && codecDrmSession != null)
                ...
                || (Utils.SDK_INT < 23 && sourceDrmSession != codecDrmSession)) {
            // We might need to switch between the clear and protected output paths, or we're using
            // DRM
            // prior to API level 23 where the codec needs to be re-initialized to switch to the new
            // DRM
            // session.
    ...
    /**
     * Determines whether the existing {@link MediaCodec} can be kept for a new format, and if it
     * can whether it requires reconfiguration.
     *
     * <p>
    ...
    public boolean isReady() {
        return inputFormat != null && !waitingForKeys
                && (isSourceReady() || hasOutputBuffer() || (codecHotswapDeadlineMs != C.TIME_UNSET
     ...
     * @return The codec operating rate, or {@link #CODEC_OPERATING_RATE_UNSET} if no codec
    ...
    private void updateCodecOperatingRate() throws ExoPlaybackException {
        ...
        if (codecOperatingRate == newCodecOperatingRate) {
            ...
            // We need to set the operating rate, either because we've set it previously or because
            // it's
    ...
    /**
     * Starts draining the codec for re-initialization. Re-initialization may occur immediately if
     * no buffers have been queued to the codec.
     *
    ...
    private boolean drainOutputBuffer(long positionUs, long elapsedRealtimeUs)
            throws ExoPlaybackException {
        if (log.allowVerbose()) {
            log.v("drainOutputBuffer: positionUs = " + positionUs + ", elapsedRealtimeUs = "
                    + elapsedRealtimeUs);
        }
        if (!hasOutputBuffer()) {
            int outputIndex;
            ...
            if (outputIndex < 0) {
                ...
                if (codecNeedsEosPropagation && (inputStreamEnded
                        || codecDrainState == DRAIN_STATE_WAIT_END_OF_STREAM)) {
                    processEndOfStream();
                }
                return false;
        ...
        if (codecNeedsEosOutputExceptionWorkaround && codecReceivedEos) {
            try {
                processedOutputBuffer = processOutputBuffer(positionUs, elapsedRealtimeUs, codec,
                        outputBuffer, outputIndex, outputBufferInfo.flags,
                        outputBufferInfo.presentationTimeUs, isDecodeOnlyOutputBuffer,
                        isLastOutputBuffer, outputFormat);
            } catch (IllegalStateException e) {
                processEndOfStream();
                if (outputStreamEnded) {
                    // Release the codec, as it's in an error state.
    ...
    private void processOutputFormat() throws ExoPlaybackException {
        ...
        if (codecAdaptationWorkaroundMode != ADAPTATION_WORKAROUND_MODE_NEVER
                && format.getInteger(
                        MediaFormat.KEY_WIDTH) == ADAPTATION_WORKAROUND_SLICE_WIDTH_HEIGHT
                && format.getInteger(
                        MediaFormat.KEY_HEIGHT) == ADAPTATION_WORKAROUND_SLICE_WIDTH_HEIGHT) {
     ...
     * If false is returned then the same buffer will be passed to the next call. An implementation
     * of this method is free to modify the buffer and can assume that the buffer will not be
     * externally modified between successive calls. Hence an implementation can, for example,
     * modify the buffer's position to keep track of how much of the data it has processed.
     *
     * <p>
     ...
     * @param isDecodeOnlyBuffer Whether the buffer was marked with
     *        {@link C#BUFFER_FLAG_DECODE_ONLY} by the source.
    ...
    protected abstract boolean processOutputBuffer(long positionUs, long elapsedRealtimeUs,
            ...
            long bufferPresentationTimeUs, boolean isDecodeOnlyBuffer, boolean isLastBuffer,
    ...
    private void updateDrmSessionOrReinitializeCodecV23() throws ExoPlaybackException {
        ...
        if (sessionMediaCrypto == null) {
            // We'd only expect this to happen if the CDM from which the pending session is obtained
            // needs
            // provisioning. This is unlikely to happen (it probably requires a switch from one DRM
            // scheme
            // to another, where the new CDM hasn't been used before and needs provisioning). It
            // would be
            // possible to handle this case more efficiently (i.e. with a new renderer state that
            // waits
            // for provisioning to finish and then calls mediaCrypto.setMediaDrmSession), but the
            // extra
        ...
        if (flushOrReinitializeCodec()) {
            // The codec was reinitialized. The new codec will be using the new DRM session, so
            // there's
            // nothing more to do.
            return;
        }
    ...
    private boolean isDecodeOnlyBuffer(long presentationTimeUs) {
        // We avoid using decodeOnlyPresentationTimestamps.remove(presentationTimeUs) because it
        // would
    ...
    private static boolean codecNeedsFlushWorkaround(String name) {
        return Utils.SDK_INT < 18 || (Utils.SDK_INT == 18
                && ("OMX.SEC.avc.dec".equals(name) || "OMX.SEC.avc.dec.secure".equals(name)))
                || (Utils.SDK_INT == 19 && Utils.MODEL.startsWith("SM-G800")
                        && ("OMX.Exynos.avc.dec".equals(name)
     ...
     * <a href="https://github.com/google/ExoPlayer/issues/3257">GitHub issue #3257</a>.
     *
     * @param name The name of the decoder.
    ...
    private @AdaptationWorkaroundMode int codecAdaptationWorkaroundMode(String name) {
        if (Utils.SDK_INT <= 25 && "OMX.Exynos.avc.dec.secure".equals(name)
                && (Utils.MODEL.startsWith("SM-T585") || Utils.MODEL.startsWith("SM-A510")
                        || Utils.MODEL.startsWith("SM-A520") || Utils.MODEL.startsWith("SM-J700"))) {
            return ADAPTATION_WORKAROUND_MODE_ALWAYS;
        } else if (Utils.SDK_INT < 24
                && ("OMX.Nvidia.h264.decode".equals(name)
                        || "OMX.Nvidia.h264.decode.secure".equals(name))
                && ("flounder".equals(Utils.DEVICE) || "flounder_lte".equals(Utils.DEVICE)
                        || "grouper".equals(Utils.DEVICE) || "tilapia".equals(Utils.DEVICE))) {
     ...
     *
     * <p>
     * When enabled, the workaround will always release and recreate the decoder, rather than
    ...
    private static boolean codecNeedsReconfigureWorkaround(String name) {
        return Utils.MODEL.startsWith("SM-T230")
                && "OMX.MARVELL.VIDEO.HW.CODA7542DECODER".equals(name);
    }
     ...
     * <p>
     * If true is returned, the renderer will work around the issue by discarding data up to the
     * SPS.
     *
     * @param name The name of the decoder.
    ...
    private static boolean codecNeedsDiscardToSpsWorkaround(String name, Format format) {
        return Utils.SDK_INT < 21 && format.initializationData.isEmpty()
    ...
    private static boolean codecNeedsEosPropagationWorkaround(MediaCodecInfo codecInfo) {
        ...
        return (Utils.SDK_INT <= 25 && "OMX.rk.video_decoder.avc".equals(name))
                || (Utils.SDK_INT <= 17 && "OMX.allwinner.video.decoder.avc".equals(name))
                || ("Amazon".equals(Utils.MANUFACTURER) && "AFTS".equals(Utils.MODEL)
                        && codecInfo.secure);
    }

    /**
     * Returns whether the decoder is known to behave incorrectly if flushed after receiving an
     * input buffer with {@link MediaCodec#BUFFER_FLAG_END_OF_STREAM} set.
     * <p>
     ...
     *
     * @param name The name of the decoder.
     * @return True if the decoder is known to behave incorrectly if flushed after receiving an
     *         input buffer with {@link MediaCodec#BUFFER_FLAG_END_OF_STREAM} set. False otherwise.
     */
    private static boolean codecNeedsEosFlushWorkaround(String name) {
        return (Utils.SDK_INT <= 23 && "OMX.google.vorbis.decoder".equals(name))
                || (Utils.SDK_INT <= 19
                        && ("hb2000".equals(Utils.DEVICE) || "stvm8".equals(Utils.DEVICE))
     ...
     * {@link MediaCodec#releaseOutputBuffer(int, boolean)} after receiving an input buffer with
     * {@link MediaCodec#BUFFER_FLAG_END_OF_STREAM} set.
     * <p>
    ...
    private static boolean codecNeedsEosOutputExceptionWorkaround(String name) {
        return Utils.SDK_INT == 21 && "OMX.google.aac.decoder".equals(name);
    }
    ...
    private static boolean codecNeedsMonoChannelCountWorkaround(String name, Format format) {
        return Utils.SDK_INT <= 18 && format.channelCount == 1
