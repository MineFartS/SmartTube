...
public class AppUpdatePresenter extends BasePresenter<Void> implements AppUpdateCheckerListener {
    ...
    if (mIsForceCheck) {
      ...
      if (AppUpdateCheckerListener.LATEST_VERSION.equals(error.getMessage())) {
        ...
        MessageHelpers.showMessage(
            getContext(),
            String.format(
                "%s: %s", getContext().getString(R.string.update_error), error.getMessage()));
      }
    }
  ...
  private void showUpdateDialog(String versionName, List<String> changelog, String apkPath) {
    ...
    mSettingsPresenter.appendStringsCategory(
        getContext().getString(R.string.update_changelog), createChangelogOptions(changelog));

    mSettingsPresenter.showDialog(
        String.format("%s %s", getContext().getString(R.string.app_name), versionName),
  ...
  private void pinUpdateSection(String versionName, List<String> changelog, String apkPath) {
    ...
    BrowsePresenter.instance(getContext())
        .pinItem(
            getContext().getString(R.string.update_found),
            R.drawable.action_info,
            new ErrorFragmentData() {
              @Override
              ...
              public String getMessage() {
                return String.format(
                        "%s %s", getContext().getString(R.string.app_name), versionName)
                    + " "
