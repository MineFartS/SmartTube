...
public class VideoTrack extends MediaTrack {
  ...
  public int inBounds(MediaTrack track2) {
    ...
    if (isPreset) {
      ...
      boolean respectPresetsFps =
          sizeEquals(format.height, TrackSelectorUtil.getOriginHeight(track2.format.height));

      return compare(
          track2,
          (isMultiFpsFormat || respectPresetsFps)
              ? COMPARE_TYPE_IN_BOUNDS_PRESET
              : COMPARE_TYPE_IN_BOUNDS_PRESET_NO_FPS);

    } else {

      return compare(
          track2, isMultiFpsFormat ? COMPARE_TYPE_IN_BOUNDS : COMPARE_TYPE_IN_BOUNDS_NO_FPS);
    }
  }
  ...
  private int compare(MediaTrack track2, int type) {
    ...
    size1 =
        TrackSelectorUtil.isWideScreen(format) || exceedHeightLimit(format)
            ? format.height
            : format.width;
    size2 =
        TrackSelectorUtil.isWideScreen(track2.format) || exceedHeightLimit(track2.format)
            ? track2.format.height
            : track2.format.width;
    ...
    if (type == COMPARE_TYPE_IN_BOUNDS) {
      result =
          inBounds(
              id1, id2, size1, size2, frameRate1, frameRate2, codecs1, codecs2, bitrate1, bitrate2);
  ...
  private int inBounds(
    ...
    if (Helpers.equals(id1, id2) && (size1 == size2)) {
      result = 0;
        ...
        && fpsLessOrEquals(frameRate2, frameRate1)) { // NOTE: Removed bitrate check to fix shorts?
      if (TrackSelectorUtil.isHdrFormat(id1, codecs1)
          == TrackSelectorUtil.isHdrFormat(id2, codecs2)) {
        result = 1;
  ...
  private int compare(
    ...
    if (TrackSelectorUtil.isHdrFormat(id1, codecs1)
        && !TrackSelectorUtil.isHdrFormat(id2, codecs2)) {
      leftScore += 3;
    } else if (TrackSelectorUtil.isHdrFormat(id2, codecs2)
        && !TrackSelectorUtil.isHdrFormat(id1, codecs1)) {
      rightScore += 3;
    }
    ...
    return result == 0
            && TrackSelectorUtil.codecNameShort(codecs1)
                .equals(TrackSelectorUtil.codecNameShort(codecs2))
        ? bitrate1 - bitrate2
        : result;
  }
