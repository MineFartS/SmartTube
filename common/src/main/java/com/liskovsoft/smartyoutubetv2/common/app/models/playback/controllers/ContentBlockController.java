...
public class ContentBlockController extends BasePlayerController {
  ...
  public void onVideoLoaded(Video item) {
    ...
    getPlayer()
        .setButtonState(
            R.id.action_content_block, enabled ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  ...
  public void onButtonClicked(int buttonId, int buttonState) {
    if (buttonId == R.id.action_content_block) {
      List<SponsorSegment> foundSegments =
          findMatchedSegments(getPlayer().getPositionMs(), mOriginalSegments, true);
  ...
  public void onButtonLongClicked(int buttonId, int buttonState) {
    if (buttonId == R.id.action_content_block) {
      ContentBlockSettingsPresenter.instance(getContext())
          .show(
              () -> {
                if (getPlayer() != null) {
  ...
  private void updateSponsorSegmentsAndWatch(Video item) {
    ...
    if (!Helpers.equals(mVideoId, item.videoId) || mCachedSegmentsAction == null) {
      // NOTE: SponsorBlock (when happened java.net.SocketTimeoutException) could block whole
      // application with Schedulers.io()
      // Because Schedulers.io() reuses blocked threads in RxJava 2:
      // https://github.com/ReactiveX/RxJava/issues/6542
      mCachedSegmentsAction =
          mMediaItemService
              .getSponsorSegmentsObserve(item.videoId, getContentBlockData().getEnabledCategories())
              .cache();
        ...
        mCachedSegmentsAction
            ...
            .subscribe(
                ...
                error ->
                    Log.d(
                        TAG,
                        "It's ok. Nothing to block in this video. Error msg: %s",
                        error.getMessage()));
  }
  ...
  private boolean isPositionInsideSegment(
      long positionMs, SponsorSegment segment, boolean fullMatch) {
    ...
    if (fullMatch) {
      ...
      return positionMs >= segment.getStartMs()
          && positionMs <= Math.min(segment.getStartMs() + windowSizeMs, segment.getEndMs());
    }
  }
  ...
  private void messageSkip(long skipPosMs, String category) {
    ...
    MessageHelpers.showMessage(
        ...
        String.format(
            ...
            getContext().getString(R.string.msg_skipping_segment, category)));
  ...
  private List<SeekBarSegment> toSeekBarSegments(List<SponsorSegment> segments) {
    ...
    for (SponsorSegment sponsorSegment : segments) {
      ...
      float startRatio =
          (float) sponsorSegment.getStartMs() / getPlayer().getDurationMs(); // Range: [0, 1]
      float endRatio =
          (float) sponsorSegment.getEndMs() / getPlayer().getDurationMs(); // Range: [0, 1]
      seekBarSegment.startProgress = startRatio;
      seekBarSegment.endProgress = endRatio;
      seekBarSegment.color =
          ContextCompat.getColor(
              getContext(), getContentBlockData().getColorRes(sponsorSegment.getCategory()));
   ...
   */
  private List<SponsorSegment> findMatchedSegments(
      long positionMs, List<SponsorSegment> segments, boolean fullMatch) {
    if (segments == null) {
      return null;
    }
    ...
    for (SponsorSegment segment : segments) {
      ...
      if (foundSegment == null) {
        ...
        if (isSkipAction
            && isPositionInsideSegment(lastSegment.getEndMs() + 3_000, segment, fullMatch)) {
          foundSegment.add(segment);
        }
      }
    }
  ...
  private void applyActions(List<SponsorSegment> foundSegments) {
    ...
    long skipDurationMs =
        Math.min(skipPosMs, getPlayer().getDurationMs()) - getPlayer().getPositionMs();
    boolean stayQuiet =
        skipDurationMs < 10_000 && PlayerTweaksData.instance(getContext()).isTextureViewEnabled();
