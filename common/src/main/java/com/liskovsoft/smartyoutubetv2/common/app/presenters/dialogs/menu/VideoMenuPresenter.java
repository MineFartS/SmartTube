...
public class VideoMenuPresenter extends BaseMenuPresenter {
  ...
  private void prepareAndShowDialogSigned() {
    ...
    if (!mDialogPresenter.isEmpty()) {
      ...
      // No need to add author because: 1) This could be a channel card. 2) This info isn't so
      // important.
    ...
    getDialogPresenter()
        .appendSingleButton(
            UiOptionItem.from(
                ...
                optionItem ->
                    AppDialogUtil.showAddToPlaylistDialog(getContext(), mVideo, mCallback)));
  }
  ...
  private void appendNotInterestedButton() {
    ...
    mDialogPresenter.appendSingleButton(
        UiOptionItem.from(
            ...
            optionItem -> {
              mNotInterestedAction =
                  mMediaItemService
                      .markAsNotInterestedObserve(mVideo.mediaItem.getFeedbackToken())
                      .subscribe(
                          var -> {},
                          error ->
                              Log.e(TAG, "Mark as 'not interested' error: %s", error.getMessage()),
                          () -> {
                            if (mCallback != null) {
                ...
                mNotInterestedAction =
                    mMediaItemService
                        ...
                        .subscribe(
                            var -> {},
                            error ->
                                Log.e(TAG, "Remove from history error: %s", error.getMessage()),
              ...
              mNotInterestedAction =
                  mMediaItemService
                      ...
                      .subscribe(
                          var -> {},
                          error ->
                              Log.e(TAG, "Remove from subscriptions error: %s", error.getMessage()),
                          () -> {
                            if (mCallback != null) {
                              mCallback.onItemAction(mVideo, VideoMenuCallback.ACTION_REMOVE);
                            }
        ...
        UiOptionItem.from(
            ...
            optionItem -> {
              ...
              VideoStateService.instance(getContext())
                  .save(new State(mVideo, mVideo.getDurationMs()));
  ...
  private void appendPlaylistOrderButton() {
    ...
    mDialogPresenter.appendSingleButton(
        UiOptionItem.from(
            ...
            optionItem ->
                AppDialogUtil.showPlaylistOrderDialog(
  ...
  private void addRemoveFromPlaylist(String playlistId, String playlistTitle, boolean add) {
    ...
    if (add) {
      ...
      Observable<Void> editObserve =
          mMediaItemService.removeFromPlaylistObserve(playlistId, mVideo.videoId);
  ...
  protected void updateEnabledMenuItems() {
    ...
    mIsAddToRecentPlaylistButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_RECENT_PLAYLIST);
    mIsAddToPlaybackQueueButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_ADD_TO_QUEUE);
    mIsPlayNextButtonEnabled = mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_PLAY_NEXT);
    mIsAddToPlaylistButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_ADD_TO_PLAYLIST);
    ...
    mIsShareEmbedLinkButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_SHARE_EMBED_LINK);
    mIsNotInterestedButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_NOT_INTERESTED);
    mIsNotRecommendChannelEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_NOT_RECOMMEND_CHANNEL);
    mIsRemoveFromHistoryButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_REMOVE_FROM_HISTORY);
    mIsRemoveFromSubscriptionsButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_REMOVE_FROM_SUBSCRIPTIONS);
    mIsOpenDescriptionButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_OPEN_DESCRIPTION);
    ...
    mIsShowPlaybackQueueButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_SHOW_QUEUE);
    mIsPlaylistOrderButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_PLAYLIST_ORDER);
    mIsMarkAsWatchedButtonEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_MARK_AS_WATCHED);
