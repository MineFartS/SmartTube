...
public class AutoFrameRateController extends BasePlayerController implements AutoFrameRateListener {
  ...
  public void onModeError(Mode newMode) {
    ...
    String msg =
        getContext()
            .getString(
                R.string.msg_mode_switch_error,
                newMode != null ? UhdHelper.toResolution(newMode) : null);
    Log.e(TAG, msg);
  ...
  private void onResolutionSwitchClick() {
    mAutoFrameRateHelper.setResolutionSwitchEnabled(
        getPlayerData().isAfrResSwitchEnabled(), getPlayerData().isAfrEnabled());
  }
  ...
  private void applyAfr(FormatItem videoFormat, boolean force) {
    if (videoFormat != null) {
      String msg =
          String.format(
              "Applying afr... fps: %s, resolution: %sx%s, activity: %s",
  ...
  private void savePlayback() {
    if (!skipAfr()
        && mAutoFrameRateHelper.isSupported()
        && getPlayerData().isAfrEnabled()
        && getPlayerData().getAfrPauseMs() > 0) {
  ...
  // Avoid nested dialogs. They have problems with timings. So player controls may hide without user
  // interaction.
  private void addUiOptions() {
    if (mAutoFrameRateHelper.isSupported() && getContext() != null) {
      OptionCategory afrCategory =
          createAutoFrameRateCategory(
              getContext(),
              PlayerData.instance(getContext()),
              () -> {},
              this::onResolutionSwitchClick,
              this::onFpsCorrectionClick,
              this::onDoubleRefreshRateClick,
              this::onSkip24RateClick);

      OptionCategory afrPauseCategory =
          createAutoFrameRatePauseCategory(getContext(), PlayerData.instance(getContext()));
      ...
      mHQDialogController.addCategory(
          OptionCategory.from(
              AUTO_FRAME_RATE_ID,
              OptionCategory.TYPE_STRING_LIST,
              getContext().getString(R.string.auto_frame_rate),
              options));
  ...
  public static OptionCategory createAutoFrameRateCategory(Context context, PlayerData playerData) {
    return createAutoFrameRateCategory(
        context, playerData, () -> {}, () -> {}, () -> {}, () -> {}, () -> {});
  }

  private static OptionCategory createAutoFrameRateCategory(
      Context context,
      PlayerData playerData,
      Runnable onAfrCallback,
      Runnable onResolutionCallback,
      Runnable onFpsCorrectionCallback,
      Runnable onDoubleRefreshRateCallback,
      Runnable onSkip24RateCallback) {
    ...
    String fpsCorrection =
        context.getString(R.string.frame_rate_correction, "24->23.97, 30->29.97, 60->59.94");
    ...
    OptionItem afrFpsCorrectionOption =
        UiOptionItem.from(
            fpsCorrection,
            optionItem -> {
    ...
    OptionItem doubleRefreshRateOption =
        UiOptionItem.from(
            doubleRefreshRate,
            optionItem -> {
    ...
    return OptionCategory.from(
        AUTO_FRAME_RATE_ID, OptionCategory.TYPE_CHECKBOX_LIST, afrEnable, options);
  }

  public static OptionCategory createAutoFrameRatePauseCategory(
      Context context, PlayerData playerData) {
    ...
    for (int pauseMs : Helpers.range(0, 15_000, 250)) {
      ...
      String optionTitle =
          pauseMs == 0
              ? context.getString(R.string.option_never)
              : context.getString(R.string.auto_frame_rate_sec, pauseMs / 1_000f);
      options.add(
          UiOptionItem.from(
              optionTitle,
              optionItem -> {
    ...
    return OptionCategory.from(
        AUTO_FRAME_RATE_VIDEO_PAUSE_ID, OptionCategory.TYPE_RADIO_LIST, title, options);
  }
  ...
  public static OptionCategory createAutoFrameRateModesCategory(Context context) {
    ...
    return OptionCategory.from(
        AUTO_FRAME_RATE_MODES_ID,
        OptionCategory.TYPE_LONG_TEXT,
        title,
        UiOptionItem.from(result.toString()));
  }
  ...
  private boolean skipAfr() {
    ...
    // NOTE: Avoid detecting shorts by Video.isShorts. Because this is working only in certain
    // places (e.g. Shorts section).
    return isEmbedPlayer()
        || getPlayer().getDurationMs() <= SHORTS_DURATION_MIN_MS
        || isSkipShortsPrefs();
  }

  private boolean isSkipShortsPrefs() {
    return getPlayerData().isSkipShortsEnabled()
        && (getPlayer().getVideo().isShorts
            || getPlayer().getDurationMs() <= SHORTS_DURATION_MAX_MS);
  }
}
