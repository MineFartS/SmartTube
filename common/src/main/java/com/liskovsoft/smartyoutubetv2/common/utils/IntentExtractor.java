...
public class IntentExtractor {
  ...
  private static final String SUBSCRIPTIONS_URL =
      "https://www.youtube.com/tv#/zylon-surface?c=FEsubscriptions"; // last 'resume' param isn't
                                                                     // parsed by intent and should
                                                                     // be removed
  private static final String HISTORY_URL =
      "https://www.youtube.com/tv#/zylon-surface?c=FEmy_youtube"; // last 'resume' param isn't
                                                                  // parsed by intent and should be
                                                                  // removed
  private static final String RECOMMENDED_URL =
      "https://www.youtube.com/tv#/zylon-surface?c=default"; // last 'resume' param isn't parsed by
                                                             // intent and should be removed
  ...
  public static String extractChannelId(Intent intent) {
    ...
    if (channelId == null) {
      ...
      // NOTE: can't distinguish a share video link (https://youtu.be/LpNVf8sczqU) from a non-prefix
      // channel link (https://youtu.be/builditbasement)
  ...
  public static boolean isATVChannelUrl(Intent intent) {
    return intent != null
        ...
        && Helpers.startsWithAny(
            extractUri(intent).toString(), SUBSCRIPTIONS_URL, HISTORY_URL, RECOMMENDED_URL);
  }
  ...
  public static boolean isRootUrl(Intent intent) {
    return intent != null
        && extractUri(intent) != null
        && Helpers.endsWithAny(
            extractUri(intent).toString(), ".com/", ".com", ".com/launch=remote");
  }

  public static boolean isStartVoiceCommand(Intent intent) {
    return intent != null
        && extractUri(intent) != null
        && extractUri(intent).toString().contains("launch=voice");
  }
  ...
  public static boolean isInstantPlayCommand(Intent intent) {
    return intent != null
        && extractUri(intent) != null
        && extractUri(intent).toString().contains("method=play");
  }
