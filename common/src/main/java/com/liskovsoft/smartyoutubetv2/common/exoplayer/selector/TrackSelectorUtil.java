...
public class TrackSelectorUtil {
          ...
          joinWithSeparator(
              joinWithSeparator(
                  joinWithSeparator(
                      joinWithSeparator(
                          joinWithSeparator(
                              buildResolutionShortString(format), buildFPSString(format)),
                          buildBitrateString(format)),
                      extractCodec(format)),
                  buildHDRString(format)),
              buildHighBitrateMark(format));
  ...
  private static String joinWithSeparator(String first, String second) {
    return first.length() == 0
        ? second
        : (second.length() == 0 ? first : first + SEPARATOR + second);
  }
  ...
  private static boolean isHdrCodec(String codec) {
    ...
    return codec.equals(CODEC_PREFIX_VP9_HDR)
        || Helpers.endsWithAny(
            codec, CODEC_SUFFIX_AV1_HDR, CODEC_SUFFIX_AV1_HDR2, HDR_PROFILE_ENDING);
  }
  ...
  public static int getRealHeight(Format format) {
    ...
    int originHeight =
        isUltraWide ? getHeightByWidth(width) : getOriginHeight(Math.min(height, width));
