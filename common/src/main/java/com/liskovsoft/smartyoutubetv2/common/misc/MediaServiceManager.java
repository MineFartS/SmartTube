...
public class MediaServiceManager implements OnAccountChange {
  ...
  public void loadMetadata(Video video, OnMetadata onMetadata) {
    ...
    if (video.mediaItem != null) {
      ...
      // Simply load
      observable =
          mItemService.getMetadataObserve(
  ...
  /**
   * NOTE: Load suggestions from MediaItem isn't robust. Because playlistId may be initialized from
  ...
  public void loadChannelRows(Video item, OnMediaGroupList onMediaGroupList, OnError onError) {
    ...
    Observable<List<MediaGroup>> observable =
        item.mediaItem != null
  ...
  public void updateHistory(Video video, long positionMs) {
    ...
    if (video.mediaItem != null) {
      historyObservable =
          mItemService.updateHistoryPositionObserve(video.mediaItem, positionMs / 1_000f);
    } else { // video launched form ATV channels
      historyObservable =
          mItemService.updateHistoryPositionObserve(video.videoId, positionMs / 1_000f);
    }
                ...
                videoPlaylistInfos -> {
                  ...
                  if (watchLater.isSelected()) {
                    Observable<Void> editObserve =
                        mItemService.removeFromPlaylistObserve(
   ...
   * Channels could be of two types: regular (usr channel) and playlist channel (contains single
        ...
        .loadChannelRows(
            ...
            groups -> {
              ...
              if (type == MediaGroup.TYPE_CHANNEL_UPLOADS) {
                ...
                // NOTE: Crashes RecycleView IndexOutOfBoundsException when doing add immediately
                // after clear
                Utils.postDelayed(
