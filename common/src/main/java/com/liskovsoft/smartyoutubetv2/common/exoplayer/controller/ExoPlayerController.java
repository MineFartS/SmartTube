...
public class ExoPlayerController implements Player.EventListener {
  ...
  private void applyShield720pFix() {
    ...
    mTrackSelectorManager.selectTrack(
        FormatItem.toMediaTrack(playerData.getFormat(FormatItem.TYPE_VIDEO)));
    mTrackSelectorManager.selectTrack(
        FormatItem.toMediaTrack(playerData.getFormat(FormatItem.TYPE_AUDIO)));
    mTrackSelectorManager.selectTrack(
        FormatItem.toMediaTrack(playerData.getFormat(FormatItem.TYPE_SUBTITLE)));
  }
  ...
  /**
   * NOTE: Pos gathered from content block data may slightly exceed video duration (e.g. 302200 when
   * duration is 302000).
   */
  ...
  public void onTracksChanged(TrackGroupArray trackGroups, TrackSelectionArray trackSelections) {
    ...
    if (trackGroups.length == 0) {
      Log.i(
          TAG,
          "onTracksChanged: Hmm. Strange. Received empty groups, no selections. Why is this happens"
              + " only on next/prev videos?");
      return;
    }
  ...
  private void notifyOnVideoLoad() {
    if (mOnSourceChanged) {
      ...
      // Attempt to read from field 'java.util.TreeMap$Node java.util.TreeMap$Node.left' on a null
      // object reference
  ...
  public void setPitch(float pitch) {
    if (mPlayer != null && pitch > 0 && !Helpers.floatEquals(pitch, getPitch())) {
      mPlayer.setPlaybackParameters(
          new PlaybackParameters(mPlayer.getPlaybackParameters().speed, pitch));
    }
  }
