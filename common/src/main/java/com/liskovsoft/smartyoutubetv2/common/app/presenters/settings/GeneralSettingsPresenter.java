...
public class GeneralSettingsPresenter extends BasePresenter<Void> {
          ...
          UiOptionItem.from(
              ...
              optionItem -> {
                BrowsePresenter.instance(getContext())
                    .enableSection(sectionId, optionItem.isSelected());
              },
              mSidebarService.isSectionPinned(sectionId)));
    }

    settingsPresenter.appendCheckedCategory(
        getContext().getString(R.string.side_panel_sections), options);
  }
  ...
  private void showMenuItemOrderDialog(Long menuItem) {
    ...
    dialog.appendRadioCategory(
        getContext().getString(R.string.item_postion) + " " + itemName, options);
  ...
  private void appendBackgroundPlaybackCategory(AppDialogPresenter settingsPresenter) {
    ...
    OptionCategory category =
        AppDialogUtil.createBackgroundPlaybackCategory(getContext(), mPlayerData, mGeneralData);
  ...
  private Map<Long, Integer> getMenuNames() {
    ...
    menuNames.put(
        MainUIData.MENU_ITEM_REMOVE_FROM_SUBSCRIPTIONS, R.string.remove_from_subscriptions);
