...
public class DelayMediaCodecAudioRenderer extends MediaCodecAudioRenderer {
  ...
  protected boolean processOutputBuffer(
    ...
    // Disable the use of AudioTrack.getTimestamp and force ExoPlayer to go through the legacy path
    // of using
    // AudioTrack.getPlaybackHeadPosition instead, which might help if the first one drifts but the
    // second one doesn't.
    if (mIsAudioSyncFixEnabled && mIsAudioSyncFixChanged) {
      ...
      if (audioSink != null) {
        ...
        if (audioTrackPositionTracker != null) {
          Object audioTimestampPoller =
              Helpers.getField(audioTrackPositionTracker, "audioTimestampPoller");
