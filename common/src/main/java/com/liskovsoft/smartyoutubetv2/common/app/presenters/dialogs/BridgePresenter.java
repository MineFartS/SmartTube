...
abstract class BridgePresenter extends BasePresenter<Void> implements MotherActivity.OnResult {
  ...
  private void startDialog() {
    ...
    settingsPresenter.appendSingleSwitch(
        UiOptionItem.from(
            getContext().getString(R.string.show_again),
  ...
  private boolean isOldApkInstalled() {
    ...
    return info != null
        && !Helpers.equalsAny(
            info.signingInfo.getApkContentsSigners()[0].hashCode(), getPackageSignatureHash());
  }
  ...
  public void onResult(int requestCode, int resultCode, Intent data) {
    if (requestCode == Helpers.REMOVE_PACKAGE_CODE && !isOldApkInstalled()) {
      ...
      MessageHelpers.showMessage(
          getContext(), "The package " + getPackageName() + " cannot be uninstalled!");
    }
  }
