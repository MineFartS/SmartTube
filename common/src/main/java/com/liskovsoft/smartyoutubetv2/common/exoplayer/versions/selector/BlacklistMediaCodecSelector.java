...
public class BlacklistMediaCodecSelector implements MediaCodecSelector {
  ...
  static final String[] ALL_DECODERS = {
  ...
  static final String[] HW_DECODERS = {
    "OMX.amlogic", "OMX.MTK", "OMX.Nvidia", "OMX.qcom", "OMX.rk", "OMX.allwinner"
  };
  ...
  public List<MediaCodecInfo> getDecoderInfos(
    ...
    for (MediaCodecInfo codecInfo : codecInfos) {
      ...
      for (String blacklistedDecoder : HW_DECODERS) {
        if (codecInfo != null
            && codecInfo.name.toLowerCase().startsWith(blacklistedDecoder.toLowerCase())) {
