...
public class SidebarService implements ProfileChangeListener {
  ...
  public void enableSection(int sectionId, boolean enabled) {
    ...
    if (enabled) {
      ...
      Video section =
          Helpers.findFirst(mPinnedItems, item -> item != null && item.sectionId == sectionId);
  ...
  private void initPinnedItems() {
    for (int sectionId : mDefaultSections.values()) {
      ...
      enableSection(
          ...
          !Helpers.equalsAny(
              sectionId,
              new int[] {MediaGroup.TYPE_NOTIFICATIONS, MediaGroup.TYPE_PLAYBACK_QUEUE}));
    }
  }
  ...
  public void persistState() {
    mPrefs.setSidebarData(Helpers.mergeData(mPinnedItems));
  }
