...
public class CommentsController extends BasePlayerController {
  ...
  public void onMetadata(MediaItemMetadata metadata) {
    mLiveChatKey =
        metadata != null && metadata.getLiveChatKey() != null ? metadata.getLiveChatKey() : null;
    mCommentsKey =
        metadata != null && metadata.getCommentsKey() != null ? metadata.getCommentsKey() : null;
  ...
  private void openCommentsDialog() {
    ...
    String title =
        getPlayer() != null && getPlayer().getVideo() != null
            ? getPlayer().getVideo().getTitleFull()
            : mTitle;

    CommentsReceiver commentsReceiver =
        new AbstractCommentsReceiver(getContext()) {
          @Override
          ...
          public void onCommentClicked(CommentItem commentItem) {
            ...
            CommentsReceiver nestedReceiver =
                new AbstractCommentsReceiver(getContext()) {
                  @Override
                  public void onLoadMore(CommentGroup commentGroup) {
                    loadComments(this, commentGroup.getNextCommentsKey());
  ...
  private void toggleLike(CommentsReceiver receiver, CommentItem commentItem) {
    ...
    RxHelper.execute(
        getCommentsService().toggleLikeObserve(commentItem.getNestedCommentsKey()),
  ...
  private static final class MyCommentItem implements CommentItem {
    ...
    public static MyCommentItem from(CommentItem commentItem) {
      return new MyCommentItem(
          ...
          commentItem.getAuthorPhoto(),
          commentItem.getPublishedDate(),
          commentItem.getNestedCommentsKey(),
          commentItem.isLiked(),
