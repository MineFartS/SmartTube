...
public class VideoGroup {
  ...
  public static VideoGroup from(MediaGroup mediaGroup, BrowseSection section, int groupPosition) {
    ...
    videoGroup.mTitle =
        mediaGroup != null && mediaGroup.getTitle() != null
            ? mediaGroup.getTitle()
            : section != null ? section.getTitle() : null;
    // Fix duplicated rows e.g. Shorts
    // videoGroup.mId = !TextUtils.isEmpty(videoGroup.mTitle) ? videoGroup.mTitle.hashCode() :
    // videoGroup.hashCode();
  ...
  public boolean isShorts() {
    ...
    for (int i = 0; i < Math.min(8, mVideos.size()); i++) {
      if (!mVideos.get(i).isShorts) return false;
    }
  ...
  public int getType() {
    return mType != -1
        ? mType
        : getMediaGroup() != null
            ? getMediaGroup().getType()
            : mSection != null ? mSection.getId() : -1;
  }
  ...
  public void remove(Video video) {
    ...
    try {
      ...
      mVideos.remove(video);
        ...
        | ConcurrentModificationException e) { // read only collection
      e.printStackTrace();
    }
  }
