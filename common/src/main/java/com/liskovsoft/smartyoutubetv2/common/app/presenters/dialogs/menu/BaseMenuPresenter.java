...
public abstract class BaseMenuPresenter extends BasePresenter<Void> {
  ...
  protected void appendTogglePinVideoToSidebarButton() {
    ...
    if (original == null
        || (!original.hasPlaylist()
            && !original.hasVideo()
            && !original.hasReloadPageKey()
            && !original.hasChannel())) {
      return;
    }
    ...
    boolean isPlaylist =
        original.hasPlaylist()
            || original.isPlaylistAsChannel()
            || (original.hasNestedItems() && original.belongsToUserPlaylists());
    getDialogPresenter()
        .appendSingleButton(
            UiOptionItem.from(
  ...
  private void togglePinToSidebar(Video section) {
    ...
    if (isItemPinned && section.getGroup() == null) { // allow deletion only from the Sidebar
      ...
      MessageHelpers.showMessage(
          getContext(), getContext().getString(R.string.unpinned_from_sidebar));
    } else {
  ...
  private static Video createPinnedPlaylist(Video video) {
    ...
    section.videoId =
        section.channelId = section.reloadPageKey = null; // reset to proper comparison
  ...
  private static Video createPinnedChannel(Video video) {
    ...
    Video section = Video.from(video);
    section.videoId =
        section.playlistId = section.playlistParams = null; // reset to proper comparison
  ...
  protected void appendUnpinVideoFromSidebarButton() {
    ...
    if (video == null
        || (!video.hasPlaylist() && !video.hasReloadPageKey() && !video.hasChannel())) {
      return;
    }
    ...
    getDialogPresenter()
        .appendSingleButton(
            UiOptionItem.from(
                getContext().getString(R.string.unpin_from_sidebar),
                optionItem -> {
  ...
  protected void appendSaveRemovePlaylistButton() {
    ...
    if (original == null
        || (!original.hasPlaylist()
            && !original.isPlaylistAsChannel()
            && !original.belongsToUserPlaylists())) {
      return;
    }
                ...
                optionItem -> {
                  if (original.hasPlaylist()) {
                    ...
                    mServiceManager.loadChannelUploads(
                        ...
                        mediaGroup -> {
                          ...
                          if (playlistId != null) {
                            ...
                            mServiceManager.getPlaylistInfos(
                                playlistInfos -> {
                    ...
                    mServiceManager.loadChannelPlaylist(
                        original,
                        mediaGroup ->
                            syncToggleSaveRemovePlaylist(original, getFirstPlaylistId(mediaGroup)));
                  }
                }));
  }
  ...
  private void toggleSaveRemovePlaylist(Video video) {
    mServiceManager.loadPlaylists(
        video,
        group -> {
          if (group.getMediaItems() == null) return;
          ...
          if (isSaved
              && video.belongsToUserPlaylists()) { // allow deletion only from the Playlists section
            if (video.playlistId == null) {
              ...
              AppDialogUtil.showConfirmationDialog(
                  getContext(),
                  getContext().getString(R.string.remove_playlist_fmt, video.title),
                  () -> {
  ...
  private void removePlaylist(Video video) {
    ...
    RxHelper.execute(
        ...
        () -> {
          ...
          MessageHelpers.showMessage(
              getContext(), getContext().getString(R.string.removed_from_playlists));
        });
  }
  ...
  private void savePlaylist(Video video) {
    ...
    Observable<Void> action =
        video.mediaItem != null && video.mediaItem.getPlaylistId() != null
            ? manager.savePlaylistObserve(video.mediaItem)
            : manager.savePlaylistObserve(video.playlistId);
    RxHelper.execute(
        action,
        (error) -> MessageHelpers.showMessage(getContext(), error.getLocalizedMessage()),
        () ->
            MessageHelpers.showMessage(
                getContext(), getContext().getString(R.string.saved_to_playlists)));
  }
        ...
        newValue -> {
          ...
          Observable<Void> action =
              video.mediaItem != null
                  ? manager.createPlaylistObserve(
                      newValue, video.hasVideo() ? video.mediaItem : null)
                  : manager.createPlaylistObserve(
                      newValue, video.hasVideo() ? video.videoId : null);
          RxHelper.execute(
              action,
              (error) -> MessageHelpers.showMessage(getContext(), error.getLocalizedMessage()),
              () -> {
    ...
    getDialogPresenter()
        .appendSingleButton(
            AppDialogUtil.createExcludeFromContentBlockButton(
                getContext(), original, mServiceManager, this::closeDialog));
  }
  ...
  protected void updateEnabledMenuItems() {
    ...
    mIsSavePlaylistEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_SAVE_REMOVE_PLAYLIST);
    ...
    mIsAddToNewPlaylistEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_ADD_TO_NEW_PLAYLIST);
    ...
    mIsExcludeFromContentBlockEnabled =
        mainUIData.isMenuItemEnabled(MainUIData.MENU_ITEM_EXCLUDE_FROM_CONTENT_BLOCK);
  }
}
