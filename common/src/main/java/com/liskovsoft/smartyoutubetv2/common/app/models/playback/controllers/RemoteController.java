...
public class RemoteController extends BasePlayerController implements OnDataChange {
  ...
  private void postStartPlaying(
    ...
    mPostStartPlayAction =
        RxHelper.execute(
            mRemoteControlService.postStartPlayingObserve(
                videoId, positionMs, durationMs, isPlaying));
  }
  ...
  private void postVolumeChange(int volume) {
    ...
    mPostVolumeAction = RxHelper.execute(mRemoteControlService.postVolumeChangeObserve(volume));
  }
  ...
  private void stopListening() {
    RxHelper.disposeActions(
        mListeningAction, mPostStartPlayAction, mPostStateAction, mPostVolumeAction);
          ...
          if (subs != null) {
            ...
            for (FormatItem item : subs) {
              ...
              if (item.getLanguage() != null
                  && item.getLanguage().toLowerCase().contains(currentLocale)) {
                selected = item;
                break;
              }
            }
      ...
      case Command.TYPE_DISCONNECTED:
        ...
        if (getContext() != null && mRemoteControlData.isFinishOnDisconnectEnabled()) {
          ...
          MessageHelpers.showLongMessage(
              getContext(),
              getContext().getString(R.string.device_disconnected, command.getDeviceName()));
  ...
  private void openNewVideo(Video newVideo) {
    if (Video.equals(getVideo(), newVideo)
        && getViewManager().isPlayerInForeground()) { // same video already playing
  ...
  private void movePlayerToForeground() {
    ...
    if (getPlayer() == null || !Utils.checkActivity(getActivity())) {
      new Handler(Looper.myLooper())
          .postDelayed(() -> getViewManager().movePlayerToForeground(), 5_000);
    }
  }
  ...
  private void registerVolumeObserver() {
    ...
    mVolumeObserver =
        new ContentObserver(Utils.sHandler) {
          @Override
