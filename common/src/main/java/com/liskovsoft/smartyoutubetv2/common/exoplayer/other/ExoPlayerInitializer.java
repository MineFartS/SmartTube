...
public class ExoPlayerInitializer {
  ...
  public ExoPlayerInitializer(Context context) {
    ...
    // use 196MB as that can only happens if device has more than 17GB of RAM, so 196 is enough and
    // safe
  ...
  public SimpleExoPlayer createPlayer(
    ...
    SimpleExoPlayer player =
        ExoPlayerFactory.newSimpleInstance(context, renderersFactory, trackSelector, loadControl);
  ...
  private void setupVolumeBoost(SimpleExoPlayer player) {
    ...
    float volume =
        mPlayerTweaksData.isPlayerAutoVolumeEnabled() ? 2.0f : mPlayerData.getPlayerVolume();
