...
public final class DebugInfoManager implements Runnable, Player.EventListener {
  ...
  private void updateVideoInfo() {
    ...
    mVideoInfo.add(
        new Pair<>(
            "Video/Audio bitrate",
            String.format(
                "%s/%s", toHumanReadable(video.bitrate), toHumanReadable(audio.bitrate))));
    ...
    mVideoInfo.add(
        new Pair<>(
            ...
            String.valueOf(DeviceHelpers.isHardwareAccelerated(videoCodecName))));
  }
  ...
  private void appendRuntimeInfo() {
    ...
    appendRow(
        "Dropped/Rendered frames",
        counters.droppedBufferCount + "/" + counters.renderedOutputBufferCount);
    appendRow(
        "Buffer size (seconds)",
  ...
  private void updateDisplayModeId() {
    ...
    mDisplayModeId.add(
        new Pair<>("UI resolution", currentResolution != null ? currentResolution : NOT_AVAILABLE));
    mDisplayModeId.add(
        new Pair<>("Boot resolution", bootResolution != null ? bootResolution : NOT_AVAILABLE));
  ...
  private void appendVersion() {
    ...
    appendRow(
        "ExoPlayer engine",
        PlayerTweaksData.instance(mContext).getPlayerDataSource()
  ...
  private void appendVideoInfoVersion() {
    AppInfo appInfo =
        Helpers.firstNonNull(
            MediaServiceData.instance().getFailedAppInfo(),
    ...
    if (playerUrl != null) {
      ...
      appendRow(
          "Video info version", isFailed ? Utils.color(playerVersion, Color.RED) : playerVersion);
      appendRow(
          "Video info url", isFailed ? Utils.color(shortPlayerUrl, Color.RED) : shortPlayerUrl);
    }
  }
