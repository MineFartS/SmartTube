...
public class TrackSelectorManager implements TrackSelectorCallback {
  ...
  private static final String DEFAULT_LANGUAGE =
      "original"; // original, descriptive, dubbed, secondary
   ...
   *     One of the {@link #RENDERER_INDEX_VIDEO}, {@link #RENDERER_INDEX_AUDIO}, {@link
   *     #RENDERER_INDEX_SUBTITLE}
   */
  ...
  private void initRenderer(int rendererIndex) {
    ...
    initTrackGroups(
        rendererIndex, mTrackSelector.getCurrentMappedTrackInfo(), mTrackSelector.getParameters());
  ...
  private void initTrackGroups(
      int rendererIndex, MappedTrackInfo trackInfo, Parameters parameters) {
    if (trackInfo == null) {
  ...
  private void initTrackGroups(
      int rendererIndex, TrackGroupArray trackGroups, Parameters parameters) {
  ...
  private void initMediaTracks(int rendererIndex) {
    ...
    // Won't help: renderer.sortedTracks = Collections.synchronizedSortedSet(new TreeSet<>(new
    // MediaTrackFormatComparator()));
    ...
    for (int groupIndex = 0; groupIndex < renderer.mediaTracks.length; groupIndex++) {
      ...
      for (int trackIndex = 0; trackIndex < trackGroup.length; trackIndex++) {
        ...
        mediaTrack.isSelected =
            groupIndex == trackGroupIndex
                && Helpers.equalsAny(trackIndex, trackIndexes)
                && !renderer.isDisabled;
  ...
  private Pair<Definition, MediaTrack> createSelection(
      TrackGroupArray groups, MediaTrack selectedTrack) {
    ...
    if (matchedTrack.groupIndex != -1) {
      Definition definition =
          new Definition(groups.get(matchedTrack.groupIndex), matchedTrack.trackIndex);
  ...
  private Pair<Definition, MediaTrack> createRendererSelection(
      int rendererIndex, TrackGroupArray groups, Parameters params) {
    if (mSelectedTracks[rendererIndex] == null
        || params.hasSelectionOverride(rendererIndex, groups)) {
      return null;
    }
  ...
  private void updateRendererSelection(
      int rendererIndex, TrackGroupArray groups, Parameters params, Definition definition) {
    initRenderer(rendererIndex, groups, params);
  ...
  private Definition getOverride(
      int rendererIndex,
  ...
  @Override
  public Pair<Definition, MediaTrack> onSelectVideoTrack(
      TrackGroupArray groups, Parameters params) {
    return createRendererSelection(RENDERER_INDEX_VIDEO, groups, params);
  }

  @Override
  public Pair<Definition, MediaTrack> onSelectAudioTrack(
      TrackGroupArray groups, Parameters params) {
    return createRendererSelection(RENDERER_INDEX_AUDIO, groups, params);
  }

  @Override
  public Pair<Definition, MediaTrack> onSelectSubtitleTrack(
      TrackGroupArray groups, Parameters params) {
    return createRendererSelection(RENDERER_INDEX_SUBTITLE, groups, params);
  }

  @Override
  public void updateVideoTrackSelection(
      TrackGroupArray groups, Parameters params, Definition definition) {
    updateRendererSelection(RENDERER_INDEX_VIDEO, groups, params, definition);
  }

  @Override
  public void updateAudioTrackSelection(
      TrackGroupArray groups, Parameters params, Definition definition) {
    updateRendererSelection(RENDERER_INDEX_AUDIO, groups, params, definition);
  }

  @Override
  public void updateSubtitleTrackSelection(
      TrackGroupArray groups, Parameters params, Definition definition) {
  ...
  public void selectTrack(MediaTrack track) {
    ...
    if (mRenderers[rendererIndex] == null
        || mRenderers[rendererIndex].mediaTracks == null
  ...
  private MediaTrack findBestMatch(MediaTrack originTrack) {
    ...
    if (originTrack.format != null) { // not auto selection
      ...
      if ((result instanceof AudioTrack || result instanceof VideoTrack)
          && result.isEmpty()
          && tmpResult != null) {
        result = tmpResult;
      }
  ...
  private void applyOverride(int rendererIndex) {
    ...
    mTrackSelector.setParameters(
        mTrackSelector
            .buildUponParameters()
            .setRendererDisabled(rendererIndex, renderer.isDisabled));
    ...
    if (selectedTrack != null && selectedTrack.groupIndex != -1) {
      Log.d(
          TAG,
          "Setting override for renderer %s, group %s, track %s...",
          rendererIndex,
      ...
      mTrackSelector.setParameters(
          mTrackSelector
              .buildUponParameters()
              .clearSelectionOverrides(rendererIndex)); // Auto quality button selected
    }
  }
  ...
  private MediaTrack[][] filterByLanguage(MediaTrack[][] trackGroupList, MediaTrack originTrack) {
    if (!(originTrack instanceof AudioTrack)
        || trackGroupList.length <= 2) { // non-translated list has max 2 groups
    ...
    if (originTrack.isSaved) {
      resultLanguage =
          TextUtils.isEmpty(audioLanguage) ? null : audioLanguage; // None/Default selection ("")
      ...
      if (trackGroup != null && trackGroup.length >= 1) {
        ...
        if (mediaTrack != null && mediaTrack.format != null) {
          if (Helpers.startsWith(
              mediaTrack.format.language, resultLanguage)) { // format language: en-us
          ...
          } else if (Helpers.contains(
              mediaTrack.format.language,
              DEFAULT_LANGUAGE)) { // original, descriptive, dubbed, secondary
  ...
  private static class MediaTrackFormatComparator implements Comparator<MediaTrack> {
    ...
    public int compare(MediaTrack mediaTrack1, MediaTrack mediaTrack2) {
      ...
      int leftVal =
          format2.width + (int) format2.frameRate + MediaTrack.getCodecWeight(format2.codecs);
      int rightVal =
          format1.width + (int) format1.frameRate + MediaTrack.getCodecWeight(format1.codecs);
