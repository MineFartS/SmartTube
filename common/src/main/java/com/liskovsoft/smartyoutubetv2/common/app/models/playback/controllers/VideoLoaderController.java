...
public class VideoLoaderController extends BasePlayerController {
  ...
  public void onPlayEnd() {
    ...
    if (getAppDialogPresenter().isDialogShown()
        && !getAppDialogPresenter().isOverlay()
  ...
  public void onSuggestionItemClicked(Video item) {
    ...
    if (getPlayer() != null) getPlayer().showControls(false);
  }
  ...
  private void waitMetadataSync(Video current, boolean showLoadingMsg) {
    ...
    if (current.nextMediaItem != null) {
      ...
      boolean isEnded =
          getPlayer() != null
              && Math.abs(getPlayer().getDurationMs() - getPlayer().getPositionMs()) < 100;
      if (isEnded) {
  ...
  private void processFormatInfo(MediaItemFormatInfo formatInfo) {
    ...
    if (formatInfo.isUnplayable()) {
      ...
      Log.d(
          TAG,
          "Empty format info received. Seems future live translation. No video data to pass to the"
              + " player.");
      player.setTitle(formatInfo.getPlayabilityStatus());
      player.showProgressBar(false);
      mSuggestionsController.loadSuggestions(getVideo());
  ...
  private void disposeActions() {
    ...
    Utils.removeCallbacks(
        mReloadVideo, mLoadNext, mRestartEngine, mMetadataSync, mOnLongBuffering, mRebootApp);
  }
  ...
  private void runFormatErrorAction(Throwable error) {
    ...
    String fullMsg =
        String.format(
            "loadFormatInfo error: %s: %s", className, Utils.getStackTraceAsString(error));
    Log.e(TAG, fullMsg);
    ...
    if (Helpers.containsAny(message, "Unexpected token", "Syntax error", "invalid argument")
        || // temporal fix
  ...
  private boolean applyEngineErrorAction(int type, int rendererIndex, Throwable error) {
    ...
    if (Helpers.startsWithAny(errorContent, "Unable to connect to")) {
      ...
      // restartEngine = false;
        ...
        || (error != null && error.getCause() instanceof OutOfMemoryError)) {
      if (getPlayerTweaksData().getPlayerDataSource()
    ...
    } else if (Helpers.containsAny(
        errorContent, "Exception in CronetUrlRequest", "Response code: 503")) {
      if (getVideo() != null
          && !getVideo().isLive) { // Finished live stream may provoke errors in Cronet
        ...
        && rendererIndex == PlayerEventListener.RENDERER_INDEX_UNKNOWN) {
      ...
      if (Helpers.startsWithAny(errorContent, "Response code: 403")) {
        YouTubeServiceManager.instance().applyNoPlaybackFix();
          ...
          && !FormatItem.SUBTITLE_NONE.equals(getPlayer().getSubtitleFormat())) {
      ...
      restartEngine = false;
      showMessage = false;
    } else if (type == PlayerEventListener.ERROR_TYPE_RENDERER
      ...
      case PlayerConstants.PLAYBACK_MODE_CLOSE:
        ...
        if (mPlaylist.getNext() != null) {
          ...
          if (!getPlayer().isSuggestionsShown()
              && (!dialog.isDialogShown() || dialog.isOverlay())) {
            dialog.closeDialog();
    ...
    if (getVideo().playlistInfo.getSize() != -1) {
      ...
      video.playlistIndex =
          Utils.getRandomIndex(
              getVideo().playlistInfo.getCurrentIndex(), getVideo().playlistInfo.getSize());
  ...
  private int getPlaybackMode() {
    ...
    if (video != null && video.finishOnEnded) {
      playbackMode = PlayerConstants.PLAYBACK_MODE_CLOSE;
        ...
        && video.belongsToShortsGroup()
        && getPlayerTweaksData().isLoopShortsEnabled()) {
      playbackMode = PlayerConstants.PLAYBACK_MODE_ONE;
    }
    return playbackMode;
  }
