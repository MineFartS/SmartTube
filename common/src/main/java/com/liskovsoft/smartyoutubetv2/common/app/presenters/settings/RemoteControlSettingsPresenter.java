...
public class RemoteControlSettingsPresenter extends BasePresenter<Void> {
  ...
  private void createAndShowDialog() {
    ...
    settingsPresenter.showDialog(
        getContext().getString(R.string.settings_remote_control), this::unhold);
  }

  private void appendDeviceLinkSwitch(AppDialogPresenter settingsPresenter) {
    settingsPresenter.appendSingleSwitch(
        UiOptionItem.from(
            getContext().getString(R.string.settings_remote_control),
            optionItem -> {
              ...
              if (optionItem.isSelected()
                  && !PermissionHelpers.hasOverlayPermissions(getContext())) {
                AppDialogUtil.showConfirmationDialog(
                    getContext(),
                    getContext().getString(R.string.remote_control_permission),
  ...
  private void appendAddDeviceButton(AppDialogPresenter settingsPresenter) {
    settingsPresenter.appendSingleButton(
        UiOptionItem.from(
            getContext().getString(R.string.dialog_add_device),
            option -> {
  ...
  private void appendRemoveAllDevicesButton(AppDialogPresenter settingsPresenter) {
    ...
    settingsPresenter.appendStringsCategory(
        getContext().getString(R.string.dialog_remove_all_devices), options);
  }
  ...
  private void appendMiscCategory(AppDialogPresenter settingsPresenter) {
    ...
    options.add(
        UiOptionItem.from(
            getContext().getString(R.string.finish_on_disconnect),
    ...
    options.add(
        UiOptionItem.from(
            getContext().getString(R.string.disable_remote_history),
