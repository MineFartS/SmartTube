...
public class DisplaySyncHelper implements UhdHelperListener {
  ...
  private List<Mode> filterSameResolutionModes(Mode[] oldModes, Mode currentMode) {
    ...
    for (int i = 0; i < oldModesLen; ++i) {
      ...
      if (mode.getPhysicalHeight() == currentMode.getPhysicalHeight()
  ...
  private HashMap<Integer, int[]> getRateMapping() {
    HashMap<Integer, int[]> rateMapping =
        mIsDoubleRefreshRateEnabled ? getDoubleRateMapping() : getSingleRateMapping();
  ...
  public void onModeChanged(Mode mode) {
    ...
    if (mode == null && currentMode == null) {
      ...
      if (currentMode.getModeId() != mNewMode.getModeId()) {
        ...
        Log.e(
            TAG,
            "Mode change failure. Current mode id is %s. Expected mode id is %s",
            currentMode.getModeId(),
            mNewMode.getModeId());
   ...
   * @return
   */
  public boolean syncDisplayMode(
      Window window, int videoWidth, float videoFramerate, boolean force) {
    if (supportsDisplayModeChange() && videoWidth >= 10) {
      ...
      boolean skipFps =
          mIsSkip24RateEnabled
              && videoFramerate >= 23.96
              && videoFramerate <= 24.98
              && currentMode != null;
      Mode closerMode =
          findCloserMode(resultModes, skipFps ? currentMode.getRefreshRate() : videoFramerate);
  ...
  private boolean restoreState(Window window, int state, boolean force) {
    ...
    getUhdHelper().setPreferredDisplayModeId(window, modeTmp.getModeId(), true);
