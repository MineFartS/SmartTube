...
public class SuggestionsController extends BasePlayerController {
  ...
  public void onInit() {
    mBrowseProcessor =
        new BrowseProcessorManager(
            getContext(), PlaybackPresenter.instance(getContext())::syncItem);
  ...
  private void continueGroup(VideoGroup group, OnVideoGroup callback, boolean showLoading) {
    ...
    Disposable continueAction =
        mContentService
            .continueGroupObserve(mediaGroup)
            .subscribe(
                continueMediaGroup -> {
  ...
  private void syncCurrentVideo(MediaItemMetadata mediaItemMetadata, Video video) {
    ...
    // NOTE: Skip upcoming or unplayable (no media) because default title more informative (e.g. has
    // scheduled time).
  ...
  private void appendSuggestions(Video video, MediaItemMetadata mediaItemMetadata) {
    ...
    for (MediaGroup group : suggestions) {
      ...
      if (groupIndex == 0
          && video.isSectionPlaylistEnabled(getContext())
          && video.belongsToSamePlaylistGroup()) {
        continue;
      }
  ...
  /**
   * Merge remote queue with player's queue (when phone cast just started or user clicked on
   * playlist item)
   */
  ...
  private void appendChapterSuggestionsIfNeeded() {
    ...
    VideoGroup videoGroup =
        VideoGroup.fromChapters(mChapters, getContext().getString(R.string.chapters));
  ...
  private void startChapterNotificationServiceIfNeeded() {
    if (getPlayerTweaksData().isChapterNotificationEnabled()) {
      Utils.postDelayed(
          mChapterHandler, 1_000); // small delay to give a chance to complete dialog transitions
    }
  }
  ...
  private List<SeekBarSegment> toSeekBarSegments(List<ChapterItem> chapters) {
    ...
    for (ChapterItem chapter : chapters) {
      ...
      float startRatio =
          (float) chapter.getStartTimeMs() / getPlayer().getDurationMs(); // Range: [0, 1]
      float endRatio =
          (float) (chapter.getStartTimeMs() + markLengthMs)
              / getPlayer().getDurationMs(); // Range: [0, 1]
  ...
  private void findNextSectionVideo(Video video) {
    ...
    if (mNextRetryCount > 0) {
      ...
      continueGroup(
          group,
          continuation -> findNextSectionVideoIfNeeded(video),
          getPlayer().isSuggestionsShown());
      mNextRetryCount++;
    }
  }
  ...
  private void showChapterDialog(ChapterItem chapter) {
    ...
    if (chapter == null
        || getPlayer() == null
        || getPlayer().isOverlayShown()
        || getPlayer().isInPIPMode()) {
      return;
    }

    OptionItem acceptOption =
        UiOptionItem.from(
            chapter.getTitle(),
            option -> {
  ...
  private void appendDislikes(Video video) {
    ...
    Observable<DislikeData> dislikeDataObserve =
        mMediaItemService.getDislikeDataObserve(video.videoId);

    Disposable dislikeAction =
        dislikeDataObserve.subscribe(
            dislikeData -> {
