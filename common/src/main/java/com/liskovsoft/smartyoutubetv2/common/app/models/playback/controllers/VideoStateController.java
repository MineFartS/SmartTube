...
public class VideoStateController extends BasePlayerController {
  ...
  private static final long SHORT_LIVE_BUFFER_MS =
      0; // Note, on buffer lower than the 60sec you'll notice segment skip
  ...
  public void onNewVideo(Video item) {
    ...
    if (getPlayer() != null && getPlayer().containsMedia()) {
      ...
      // NOTE: even for the same videos it's good to save state (switch from embed, video reload
      // etc)
  ...
  public boolean onNextClicked() {
    ...
    if (getVideo() != null
        && getPlayer() != null
        && getVideo().isLive
        && (getPlayer().getDurationMs() - getPlayer().getPositionMs() > getLiveThreshold())) {
  ...
  public void onVideoLoaded(Video item) {
    ...
    // Attempt to read from field 'java.util.TreeMap$TreeMapEntry
    // java.util.TreeMap$TreeMapEntry.left' on a null object reference
    // (TrackSelectorManager.java:181)
    // restoreFormats();
  ...
  private void onSpeedClicked(boolean enabled) {
    ...
    if (state != null && getPlayerData().isSpeedPerVideoEnabled()) {
      ...
      getStateService()
          .save(
              new State(
                  state.video, state.positionMs, state.durationMs, enabled ? 1.0f : lastSpeed));
    }
  ...
  private void onSpeedLongClicked(boolean enabled) {
    ...
    settingsPresenter.showDialog(
        ...
        () -> {
          State state =
              getStateService().getByVideoId(getVideo() != null ? getVideo().videoId : null);
          if (state != null && getPlayerData().isSpeedPerVideoEnabled()) {
            getStateService()
                .save(
                    new State(
                        ...
                        state.positionMs,
                        state.durationMs,
                        getPlayerData().getSpeed(getVideo().channelId)));
          }
        });
  }
  ...
  private void savePosition() {
    ...
    if (isPositionActual && !isLiveBroken) { // partially viewed
      ...
      // Mark video as fully viewed. This could help to restore proper progress marker on the video
      // card later.
    ...
    if (item.isLive
        && (state == null
            || state.durationMs - state.positionMs
                < Math.max(RESTORE_LIVE_BUFFER_MS, getLiveThreshold()))) {
  ...
  private void updateHistory() {
    ...
    MediaServiceManager.instance()
        .updateHistory(video, Math.max(getPlayer().getPositionMs(), 3_000)); // 0 == fully watched
  }
  ...
  private void restoreSpeedAndPositionIfNeeded() {
    ...
    if (liveEnd) {
      ...
      getPlayer()
          .setSpeed(
              state != null && getPlayerData().isSpeedPerVideoEnabled()
                  ? state.speed
  ...
  private boolean isStateOutdated(State state, Video item) {
    ...
    return (posPercents2 != 0 && Math.abs(posPercents1 - posPercents2) > 3)
        && state.timestamp < item.timestamp;
  }
  ...
  private long getLiveBuffer() {
    return getPlayerTweaksData().isBufferOnStreamsDisabled()
        ? SHORT_LIVE_BUFFER_MS
        : LIVE_BUFFER_MS;
  }
  ...
  private boolean isMutedEmbed() {
    return isEmbedPlayer()
        && getPlayer() != null
        && Helpers.floatEquals(getPlayer().getVolume(), 0);
  }

  private boolean isBeginEmbed() {
    return isEmbedPlayer()
        && System.currentTimeMillis() - mNewVideoTimeMs <= EMBED_THRESHOLD_MS
        && getPlayer() != null
