...
public class GDriveBackupManager {
  ...
  private void backupInt() {
    ...
    if (RxHelper.isAnyActionRunning(mBackupAction, mRestoreAction)) {
      if (!mIsBlocking) MessageHelpers.showMessage(mContext, R.string.wait_data_loading);
      return;
    }
  ...
  private void startBackupConfirm() {
    if (!mIsBlocking) {
      AppDialogUtil.showConfirmationDialog(
          mContext, mContext.getString(R.string.app_backup), this::startBackupWrapper);
    } else {
      startBackupWrapper();
    }
  }
  ...
  private void startBackup(String backupDir, String dataDir) {
    ...
    Observable<Void> uploadFile =
        DriveService.uploadFile(zipFile, Uri.parse(String.format("%s/%s", backupDir, BACKUP_NAME)));
              ...
              .subscribe(
                  ...
                  error -> {
                    ...
                    if (Helpers.startsWith(error.getMessage(), "AuthError")) {
                      logIn(
                          this
                              ::startBackupConfirm); // auth data outdated (AuthError:
                                                     // invalid_grant)
                    }
                  },
  ...
  private void startRestoreOld(String backupDir, String dataDir, Runnable onError) {
    mRestoreAction =
        DriveService.getFileList(Uri.parse(backupDir))
            .subscribeOn(Schedulers.io())
            ...
            .subscribe(
                names -> {
                  ...
                  for (String name : names) {
                    if (checkFileName(name)) {
                      MessageHelpers.showLongMessage(
                          mContext, mContext.getString(R.string.app_restore) + "\n" + name);

                      DriveService.getFile(Uri.parse(String.format("%s/%s", backupDir, name)))
                          .blockingSubscribe(
                              inputStream ->
                                  FileHelpers.copy(
                                      inputStream, new File(dataDir, fixAltPackageName(name))));
                    }
                  }
                  ...
                  new Handler(mContext.getMainLooper())
                      .postDelayed(() -> Runtime.getRuntime().exit(0), 1_000);
                },
                error -> {
  ...
  private void startRestore(String backupDir, String dataDir, Runnable onError) {
    ...
    mRestoreAction =
        DriveService.getFile(Uri.parse(String.format("%s/%s", backupDir, BACKUP_NAME)))
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(
                inputStream -> {
  ...
  private void showRestoreChooserDialog() {
    mRestoreAction =
        DriveService.getFolderList(Uri.parse(mRootBackupDir))
            .subscribeOn(Schedulers.io())
      ...
      options.add(
          UiOptionItem.from(
              ...
              optionItem -> {
                AppDialogUtil.showConfirmationDialog(
                    mContext,
                    mContext.getString(R.string.app_restore),
                    () -> {
                      String backupDir = String.format("%s/%s", mRootBackupDir, name);
                      startRestore(
                          backupDir, mDataDir, () -> startRestoreOld(backupDir, mDataDir, null));
                    });
              }));
    }
