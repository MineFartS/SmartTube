...
public class LiveDashManifestParser extends DashManifestParser {
  ...
  private void appendManifest(DashManifest newManifest) {
    ...
    if (getFirstSegmentNum(newManifest) == 0) { // Short stream. No need to do something special.
      ...
      // Below line will be needed later (> 2 hours), when the stream no longer starts from 0
      // segment
    ...
    if (mOldManifest == null) {
      ...
      for (int i = 0; i < newPeriod.adaptationSets.size(); i++) {
        for (int j = 0; j < newPeriod.adaptationSets.get(i).representations.size(); j++) {
          MultiSegmentRepresentation representation =
              (MultiSegmentRepresentation) newPeriod.adaptationSets.get(i).representations.get(j);
          ...
          SegmentList newSegmentList =
              (SegmentList) Helpers.getField(representation, "segmentBase");
          // TODO: modified
  ...
  private static void appendRepresentation(
    ...
    MultiSegmentRepresentation oldMultiRepresentation =
        (MultiSegmentRepresentation) oldRepresentation;
    MultiSegmentRepresentation newMultiRepresentation =
        (MultiSegmentRepresentation) newRepresentation;

    // TODO: modified
    // SegmentList oldSegmentList = (SegmentList) oldRepresentation.segmentBase;
    SegmentList oldSegmentList =
        (SegmentList) Helpers.getField(oldMultiRepresentation, "segmentBase");
    // TODO: modified
    // SegmentList newSegmentList = (SegmentList) newRepresentation.segmentBase;
    SegmentList newSegmentList =
        (SegmentList) Helpers.getField(newMultiRepresentation, "segmentBase");

    // TODO: modified
    // List<RangedUri> oldMediaSegments = oldSegmentList.mediaSegments;
    List<RangedUri> oldMediaSegments =
        (List<RangedUri>) Helpers.getField(oldSegmentList, "mediaSegments");
    // TODO: modified
    // List<RangedUri> newMediaSegments = newSegmentList.mediaSegments;
    List<RangedUri> newMediaSegments =
        (List<RangedUri>) Helpers.getField(newSegmentList, "mediaSegments");

    oldMediaSegments.addAll(
        newMediaSegments.subList(
            newMediaSegments.size() - (int) segmentNumShift, newMediaSegments.size()));
    ...
    if (oldMediaSegments.size() != oldSegmentTimeline.size()) {
      ...
      for (int i = 1; i <= segmentNumShift; i++) {
        oldSegmentTimeline.add(
            new SegmentTimelineElement(
                lastTimelineStartTime + (lastTimelineDuration * i), lastTimelineDuration));
      }
    }
  }

  private static void recreateRepresentation(
      Representation oldRepresentation, long segmentCount, long minUpdatePeriodMs) {
    MultiSegmentRepresentation oldMultiRepresentation =
        (MultiSegmentRepresentation) oldRepresentation;
    ...
    for (int i = 1; i <= segmentCount; i++) {
      oldMediaSegments.add(
          0,
          new RangedUri(
              String.format("sq/%s/lmt/%s", currentSegmentNum, currentSegmentLimit),
              start,
              length));
      currentSegmentNum--;
  ...
  private static long getFirstSegmentNum(DashManifest manifest) {
    DashSegmentIndex dashSegmentIndex =
        manifest.getPeriod(0).adaptationSets.get(0).representations.get(0).getIndex();
  ...
  private static long getLastSegmentNum(DashManifest manifest) {
    DashSegmentIndex dashSegmentIndex =
        manifest.getPeriod(0).adaptationSets.get(0).representations.get(0).getIndex();
    return dashSegmentIndex.getFirstSegmentNum()
        + dashSegmentIndex.getSegmentCount(DashSegmentIndex.INDEX_UNBOUNDED)
        - 1;
  }
  ...
  private static class MultiSegmentRepresentationWrapper extends MultiSegmentRepresentation {
    ...
    public MultiSegmentRepresentationWrapper(
        MultiSegmentRepresentation origin, long segmentCount, long minUpdatePeriodMs) {
      this(
          origin.revisionId,
          origin.format,
          origin.baseUrl,
          (SegmentList) Helpers.getField(origin, "segmentBase"),
          origin.inbandEventStreams);
