...
public class PlayerUIController extends BasePlayerController {
    ...
    if (matchedFormat != null) {
      ...
      getPlayer()
          .setButtonState(
              ...
              !FormatItem.SUBTITLE_NONE.equals(matchedFormat) && !enabled
                  ? PlayerUI.BUTTON_ON
                  : PlayerUI.BUTTON_OFF);
  ...
  private void onPlaylistAddClicked() {
    ...
    if (mPlaylistInfos == null) {
      AppDialogUtil.showAddToPlaylistDialog(getContext(), getVideo(), null);
    } else {
      AppDialogUtil.showAddToPlaylistDialog(
          getContext(), getVideo(), null, mPlaylistInfos, mSetPlaylistAddButtonState);
    }
  }
  ...
  private void onDebugInfoClicked(boolean enabled) {
    ...
    getPlayer()
        .setButtonState(
            R.id.action_video_stats, mDebugViewEnabled ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  }
  ...
  public void onEngineInitialized() {
    ...
    if (getAppDialogPresenter().isDialogShown()) {
      ...
      getPlayer()
          .setButtonState(
              R.id.action_video_stats,
              mDebugViewEnabled ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
    }
  }
  ...
  public void onMetadata(MediaItemMetadata metadata) {
    ...
    getPlayer()
        .setButtonState(
            ...
            metadata.getLikeStatus() == MediaItemMetadata.LIKE_STATUS_LIKE
                ? PlayerUI.BUTTON_ON
                : PlayerUI.BUTTON_OFF);
    getPlayer()
        .setButtonState(
            R.id.action_thumbs_down,
            metadata.getLikeStatus() == MediaItemMetadata.LIKE_STATUS_DISLIKE
                ? PlayerUI.BUTTON_ON
                : PlayerUI.BUTTON_OFF);
    ...
    getPlayer()
        .setButtonState(
            R.id.action_rotate,
            getPlayerData().getRotationAngle() == 0 ? PlayerUI.BUTTON_OFF : PlayerUI.BUTTON_ON);
    getPlayer()
        .setButtonState(
            R.id.action_subscribe,
            metadata.isSubscribed() ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
    getPlayer()
        .setButtonState(
            R.id.action_afr,
            getPlayerData().isAfrEnabled() ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  }

  @Override
  public void onSuggestionItemLongClicked(Video item) {
    VideoMenuPresenter.instance(getContext())
        .showMenu(
            item,
            (videoItem, action) -> {
              if (getPlayer() == null || item.getGroup() == null) return;
  ...
  private void onDislikeClicked(boolean dislike) {
    ...
    getPlayer()
        .setButtonState(
            R.id.action_thumbs_down, !dislike ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  ...
  private void onLikeClicked(boolean like) {
    getPlayer()
        .setButtonState(R.id.action_thumbs_up, !like ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  ...
  private void onShareLink() {
    ...
    AppDialogUtil.appendShareQRLinkDialogItem(
        getContext(), dialogPresenter, getVideo(), positionSec);
    AppDialogUtil.appendShareEmbedLinkDialogItem(
        getContext(), dialogPresenter, getVideo(), positionSec);
  ...
  public void onButtonClicked(int buttonId, int buttonState) {
    if (buttonId == R.id.action_rotate) {
      ...
      AppDialogUtil.showPlaybackQueueDialog(
          getContext(), item -> getMainController().onNewVideo(item));
  ...
  public void onButtonLongClicked(int buttonId, int buttonState) {
    if (buttonId == R.id.action_subscribe || buttonId == R.id.action_channel) {
      ...
      AutoFrameRateSettingsPresenter.instance(getContext())
          .show(
              () ->
                  applyAfr(
                      getPlayerData().isAfrEnabled() ? PlayerUI.BUTTON_OFF : PlayerUI.BUTTON_ON));
    } else if (buttonId == R.id.action_repeat) {
  ...
  private void callMediaItemObservable(MediaItemObservable callable) {
    ...
    Observable<Void> observable =
        callable.call(video.mediaItem != null ? video.mediaItem : video.toMediaItem());
  ...
  private boolean handleBackKey(int keyCode) {
    if (KeyHelpers.isBackKey(keyCode)) {
      ...
      if (getVideo() != null
          && getPlayer() != null
          && (getVideo().isUnplayable || getVideo().isUpcoming)
          && getPlayer().isControlsShown()) {
        getPlayer().finish();
      }
  ...
  private void setPlaylistAddButtonStateCached() {
    ...
    Disposable playlistsInfoAction =
        YouTubeServiceManager.instance()
            .getMediaItemService()
            .getPlaylistsInfoObserve(videoId)
            .subscribe(
                videoPlaylistInfos -> {
  ...
  private void setPlaylistAddButtonState() {
    ...
    getPlayer()
        .setButtonState(
            R.id.action_playlist_add, isSelected ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  }
  ...
  private void onRotate() {
    ...
    getPlayer()
        .setButtonState(
            R.id.action_rotate, rotation == 0 ? PlayerUI.BUTTON_OFF : PlayerUI.BUTTON_ON);
  ...
  private void onFlip() {
    ...
    getPlayer()
        .setButtonState(
            R.id.action_flip, newFlipEnabled ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  ...
  private void onSubscribe(int buttonState) {
    ...
    getPlayer()
        .setButtonState(
            R.id.action_subscribe,
            buttonState == PlayerUI.BUTTON_OFF ? PlayerUI.BUTTON_ON : PlayerUI.BUTTON_OFF);
  }
  ...
  private void showPlaybackModeDialog(int buttonState) {
    OptionCategory category =
        AppDialogUtil.createPlaybackModeCategory(
            getContext(),
            () -> {
  ...
  private int getNextRepeatMode(int buttonState) {
    Integer[] modeList = {
      PlayerConstants.PLAYBACK_MODE_ALL,
      PlayerConstants.PLAYBACK_MODE_ONE,
      PlayerConstants.PLAYBACK_MODE_SHUFFLE,
      PlayerConstants.PLAYBACK_MODE_LIST,
      PlayerConstants.PLAYBACK_MODE_REVERSE_LIST,
      PlayerConstants.PLAYBACK_MODE_PAUSE,
      PlayerConstants.PLAYBACK_MODE_CLOSE
    };
  ...
  private void showNotificationsDialog(int buttonState) {
                      ...
                      .setNotificationState(
                          ...
                          error ->
                              MessageHelpers.showMessage(
                                  getContext(), error.getLocalizedMessage()));
    ...
    settingsPresenter.appendRadioCategory(
        getContext().getString(R.string.header_notifications), items);
