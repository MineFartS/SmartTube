...
public abstract class MediaTrack {
  ...
  private static boolean codecEquals(String codecs1, String codecs2) {
    ...
    return Helpers.equals(
        TrackSelectorUtil.codecNameShort(codecs1), TrackSelectorUtil.codecNameShort(codecs2));
  }
  ...
  private static boolean bitrateAlmostEquals(int bitrate1, int bitrate2) {
    return bitrate1 == bitrate2
        || Math.abs(bitrate2 - bitrate1) < (bitrate2 / 100 * BITRATE_DIFF_PERCENTS);
  }
  ...
  public static int getCodecWeight(String codec) {
    return isVP9Codec(codec)
        ? sVP9Weight
        : isAVCCodec(codec) ? sAVCWeight : isAV1Codec(codec) ? sAV1Weight : 0;
  }
