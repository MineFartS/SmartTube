...
// Source:
// https://developer.amazon.com/docs/fire-tv/4k-apis-for-hdmi-mode-switch.html#amazonextension
...
public class UhdHelper {
  ...
  public static final String MODESWITCH_OVERLAY_ENABLE =
      "com.amazon.tv.notification.modeswitch_overlay.action.ENABLE";
  public static final String MODESWITCH_OVERLAY_DISABLE =
      "com.amazon.tv.notification.modeswitch_overlay.action.DISABLE";
  public static final String MODESWITCH_OVERLAY_EXTRA_STATE =
      "com.amazon.tv.notification.modeswitch_overlay.extra.STATE";
  public static final String MODESWITCH_OVERLAY_STATE_CHANGED =
  ...
  /** Handler that handles the broadcast or timeout prcoessing and issues callbacks accordingly. */
      ...
      switch (msg.what) {
        case MODE_CHANGED_MSG:
          ...
          if (mode.getModeId() == mRequestedModeId) {
            ...
            Log.w(
                TAG,
                "Callback received but not expected mode. Mode= "
                    + mode
                    + " expected= "
                    + mRequestedModeId);
          }
        ...
        case INTERSTITIAL_TIMEOUT_MSG:
          if (!isInterstitialFadeReceived) {
            Log.w(
                TAG,
                "Didn't received any broadcast for interstitial text fade till time out, starting"
                    + " the mode change.");
  ...
  /**
   * Private class for receiving the {@link
   * com.amazon.tv.notification.modeswitch_overlay.extra.STATE STATE} events.
   */
   ...
   *
   * @return {@link Mode Mode} that is currently set on the system or NULL if an error occurred.
   */
  ...
  /**
   * Utility function to parse android.view.Display,Mode to {@link Mode mode}
   *
   * @param systemMode mode
   * @return {@link Mode Mode} object or NULL if an error occurred.
   */
  private Mode convertReturnedModeToInternalMode(Object systemMode) {
    ...
    try {
      ...
      int height =
          (int) modeClass.getDeclaredMethod(sGetPhysicalHeightMethodName).invoke(systemMode);
      float refreshRate =
          (float) modeClass.getDeclaredMethod(sGetRefreshRateMethodName).invoke(systemMode);
   ...
   *
   * @return An array of {@link Mode Mode} objects or NULL if an error occurred.
   */
  ...
  /**
   * Returns current {@link Display Display} object. Assumes that the 1st display is the actual
   * display.
   *
   ...
   *
   * <p>Note that you must register a {@link UhdHelperListener listener} using {@link
   * UhdHelper#registerModeChangeListener(UhdHelperListener) registerModeChangeListener} to receive
   * the callback for success or failure. Also, note that this method need to be called from Main UI
   * thread.
   *
   * <p>The method will not attempt a mode switch and fail immediately with callback if 1) Device
   * SDK is less than Android L 2) Device is Android L but not Amazon AFT* devices.
   *
   * @param targetWindow {@link Window Window} to use for setting the display and call parameters
   * @param modeId The desired mode to switch to. Must be a valid mode supported by the platform.
  ...
  public void setPreferredDisplayModeId(
      Window targetWindow, int modeId, boolean allowOverlayDisplay) {
    ...
    if (!supportedDevice) {
      ...
      mWorkHandler.sendMessage(
          mWorkHandler.obtainMessage(SEND_CALLBACK_WITH_SUPPLIED_RESULT, 1, 1, null));
      return;
    ...
    if (mIsSetModeInProgress.get()) {
      Log.e(
          TAG,
          "setPreferredDisplayModeId is already in progress! "
              + "Cannot set another while it is in progress");
      // Send but don't cleanup as further processing is expected.
      mWorkHandler.sendMessage(
          mWorkHandler.obtainMessage(SEND_CALLBACK_WITH_SUPPLIED_RESULT, null));
      return;
    }
    ...
    if (currentMode == null || currentMode.getModeId() == modeId) {
      ...
      mWorkHandler.sendMessage(
          mWorkHandler.obtainMessage(SEND_CALLBACK_WITH_SUPPLIED_RESULT, 1, 1, currentMode));
      return;
    }
    ...
    mContext.registerReceiver(
        overlayStateChangeReceiver, new IntentFilter(MODESWITCH_OVERLAY_STATE_CHANGED));
    mDisplayListener =
    ...
    if (showInterstitial) {
      ...
      mWorkHandler.sendMessageDelayed(
          mWorkHandler.obtainMessage(INTERSTITIAL_TIMEOUT_MSG), SHOW_INTERSTITIAL_TIMEOUT_DELAY_MS);
    } else {
  ...
  /**
   * Start the mode change by setting the preferredDisplayModeId field of {@link
  ...
  private void initModeChange(int modeId, Field attributeFlagField) {
    ...
    // We assume that the mode change is not instantaneous and will send the onDisplayChanged
    // callback.
    // Start the clock on the mode change timeout
    mWorkHandler.sendMessageDelayed(
        mWorkHandler.obtainMessage(MODE_CHANGE_TIMEOUT_MSG), SET_MODE_TIMEOUT_DELAY_MS);
  }
  ...
  /**
   * Register a {@link UhdHelperListener listener} to be notified of result of the {@link
   * UhdHelper#setPreferredDisplayModeId(Window, int, boolean) setPreferredDisplayModeId} call.
   *
  ...
  public static String toResolution(Mode mode) {
    ...
    return String.format(
        "%sx%s@%s", mode.getPhysicalWidth(), mode.getPhysicalHeight(), mode.getRefreshRate());
  }
      ...
      WindowManager wm =
          (WindowManager)
              context
                  ...
                  .getSystemService(
                      Context
                          .WINDOW_SERVICE); // the results will be higher than using the activity
                                            // context object or the getWindowManager() shortcut
