...
/**
 * Work to synchronize the TV provider database with the desired list of channels and programs. This
 * sample app runs this once at install time to publish an initial set of channels and programs,
 * however in a real-world setting this might be run at other times to synchronize a server's
 * database with the TV provider database. This code will ensure that the channels from
 * "SampleClipApi.getDesiredPublishedChannelSet()" appear in the TV provider database, and that
 * these and all other programs are synchronized with TV provider database.
 */
public class GDriveBackupWorker extends Worker {
  ...
  public static void forceSchedule(Context context) {
    ...
    sAction =
        DriveService.uploadFile(id, Uri.parse(String.format("%s/%s", backupDir, BLOCKED_FILE_NAME)))
