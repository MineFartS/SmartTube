              ...
              SimpleEditDialog.show(
                  ...
                  newValue -> {
                    ...
                    if (Helpers.startsWith(newValue, "http")) {
                      RxHelper.execute(
                          mService.importGroupsObserve(Uri.parse(newValue)),
                          this::pinGroups,
                          error -> MessageHelpers.showLongMessage(mContext, error.getMessage()));
                    } else {
                      ItemGroup group =
                          mService.createChannelGroup(
                              newValue,
                              null,
                              Collections.singletonList(
                                  mService.createChannel(
                                      item.getAuthor(), item.cardImageUrl, item.channelId)));
                      mService.addChannelGroup(group);
                      BrowsePresenter.instance(mContext).pinItem(Video.from(group));
                      MessageHelpers.showMessage(
                          mContext, mContext.getString(R.string.pinned_to_sidebar));
                    }
                    return true;
                  });
            },
          ...
          UiOptionItem.from(
              ...
              optionItem -> {
                ...
                if (optionItem.isSelected()) {
                  group.add(
                      mService.createChannel(item.getAuthor(), item.cardImageUrl, item.channelId));
                } else {
                  group.remove(item.channelId);
                  Object data =
                      presenter.getCurrentSection() != null
                          ? presenter.getCurrentSection().getData()
                          : null;
                  if (callback != null
                      && (data instanceof Video)
                      && Helpers.equals(((Video) data).channelGroupId, group.getId())) {
