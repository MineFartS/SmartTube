...
public class SectionMenuPresenter extends BaseMenuPresenter {
  ...
  public void showMenu(BrowseSection section) {
    ...
    MediaServiceManager.instance()
        .authCheck(this::obtainPlaylistsAndShowDialogSigned, this::prepareAndShowDialogUnsigned);
  }
  ...
  private void appendRefreshButton() {
    ...
    mDialogPresenter.appendSingleButton(
        UiOptionItem.from(
            getContext().getString(R.string.refresh_section),
            optionItem -> {
  ...
  private void appendMoveSectionButton() {
    ...
    mDialogPresenter.appendSingleButton(
        UiOptionItem.from(
            getContext().getString(R.string.move_section_up),
            optionItem -> {
              // mDialogPresenter.closeDialog();
    ...
    mDialogPresenter.appendSingleButton(
        UiOptionItem.from(
            getContext().getString(R.string.move_section_down),
            optionItem -> {
              // mDialogPresenter.closeDialog();
  ...
  private void appendRenameSectionButton() {
    ...
    if (mSection == null
        || mSection.isDefault()
        || getVideo() == null
        || (!getVideo().hasPlaylist()
            && !getVideo().hasReloadPageKey()
            && !getVideo().hasChannel())) {
      return;
    }
  ...
  private void appendMarkAllChannelsWatchedButton() {
    ...
    mDialogPresenter.appendSingleButton(
        UiOptionItem.from(
            getContext().getString(R.string.mark_all_channels_watched),
            optionItem -> {
              mDialogPresenter.closeDialog();
              ...
              serviceManager.loadSubscribedChannels(
                  group -> {
  ...
  private void processNextChannel(
      MediaServiceManager serviceManager, Iterator<MediaItem> iterator) {
    if (iterator.hasNext()) {
      ...
      serviceManager.loadChannelUploads(
          next, (groupTmp) -> processNextChannel(serviceManager, iterator));
    } else {
