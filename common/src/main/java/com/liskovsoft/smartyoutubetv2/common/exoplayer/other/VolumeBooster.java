...
public class VolumeBooster implements AudioListener {
  ...
  public void onAudioSessionId(int audioSessionId) {
    ...
    if (mPlayer != null
        && mPlayer.getAudioFormat() != null
        && mPlayer.getAudioFormat().channelCount > 2) {
      return;
    }

    Log.d(
        TAG,
        "Audio session id is %s, supported gain %s",
        audioSessionId,
    ...
    try {
      ...
      mIsSupported = true;
        ...
        | NoSuchFieldError e) { // Cannot initialize effect engine
      e.printStackTrace();
      mIsSupported = false;
    }
  }
