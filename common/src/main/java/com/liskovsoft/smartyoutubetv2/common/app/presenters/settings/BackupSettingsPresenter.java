...
public class BackupSettingsPresenter extends BasePresenter<Void> {
  ...
  private void appendGDriveRestoreSettings(AppDialogPresenter settingsPresenter) {
    settingsPresenter.appendSingleButton(
        UiOptionItem.from(
            ...
            optionItem -> mGDriveBackupManager.restore()));
  }
  ...
  private void appendGDriveBackupSettings(AppDialogPresenter settingsPresenter) {
    settingsPresenter.appendSingleButton(
        UiOptionItem.from(
            ...
            optionItem -> mGDriveBackupManager.backup()));
  }

  private void appendLocalDriveAutoBackupOption(List<OptionItem> options) {
    options.add(
        UiOptionItem.from(
            getContext().getString(R.string.auto_backup_category),
            option -> {
              AppDialogPresenter settingsPresenter2 = AppDialogPresenter.instance(getContext());
              ...
              settingsPresenter2.appendRadioCategory(
                  getContext().getString(R.string.auto_backup_category), options2);
    ...
    settingsPresenter.appendSingleButton(
        UiOptionItem.from(
            ...
            option -> {
              ...
              settingsPresenter2.appendRadioCategory(
                  getContext().getString(R.string.auto_backup_category), options);
              settingsPresenter2.showDialog();
            }));
  }
  ...
  private void appendGDriveMiscButtons(AppDialogPresenter settingsPresenter) {
    ...
    settingsPresenter.appendSingleButton(
        UiOptionItem.from(
            ...
            option2 -> GoogleSignInPresenter.instance(getContext()).start()));
  }
        ...
        UiOptionItem.from(
            ...
            option -> {
              AppDialogUtil.showConfirmationDialog(
                  ...
                  getContext().getString(R.string.app_backup),
                  () -> {
                    mSidebarService.enableSection(
                        MediaGroup.TYPE_SETTINGS, true); // prevent Settings lock
  ...
  private void appendSubscriptionsBackupButton(AppDialogPresenter settingsPresenter) {
    settingsPresenter.appendSingleButton(
        AppDialogUtil.createSubscriptionsBackupButton(getContext()));
  }
  ...
  private void showLocalRestoreSelectorDialog(
      List<String> backups, BackupAndRestoreManager backupManager) {
